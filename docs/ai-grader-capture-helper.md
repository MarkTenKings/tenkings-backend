...
The AI Grader capture helper is the future local process boundary for rig/device control. The default implementation remains simulator/mock-only. The only opt-in real-hardware-adjacent paths are Arduino auxiliary LED readiness, GRBL/OpenBuilds stage status readiness, manual Leimac IDMU-P Ethernet readiness/status/guarded low-duty trigger-profile/polarity commands, manual Dino-Lite DNVideoX commands, and manual Basler pylon GigE readiness/list/Line2/still capture commands; each requires explicit CLI/config input. Default health, readiness, manifests, transport, and tests never open cameras, microscopes, Leimac controllers, XY stages, arm interlocks, external or controller sockets, uploads, or database connections.

The production local-station bridge constructor defaults to the real lighting write/ack boundary and bridge state-outcome projection plus the real orphan-preview scan, cleanup, start, and stop implementations only when the explicitly enabled real bridge is constructed. Software tests construct the bridge with inert injected lighting outcomes and preview-process lifecycle implementations before any service lifecycle begins. Tests do not patch process-wide prototypes, and a zero-real-access guard fails if a test reaches an external, controller, or hardware network boundary, camera, lighting, process scan/cleanup, process spawn, or hardware-discovery code. The composed Station integration intentionally opens only an in-process mock bridge HTTP socket bound to `127.0.0.1`; it does not cross an external, controller, or hardware boundary. Mock results establish software behavior only and never establish physical success.
...
The bridge exposes public local `GET /health`, pairing `POST /pair`, token-gated `GET /status`, `GET /latest-report`, `GET /session-manifest`, and token-gated `POST /actions/<action>`. Actions include `start-session`, `confirm-light-idle-off`, `confirm-fixture-rulers`, `launch-preview`, `accept-profile`, `capture-front`, `capture-back`, `run-diagnostics`, `export-report-bundle`, `safe-off`, and `end-session`. An older `confirm-flip` action may remain available for compatibility, but the v0.9 Station capture buttons do not use it to create or preserve capture authorization. Both `capture-front` and `capture-back` are authenticated, idempotency-keyed, bridge-authoritative lifecycle mutations rather than browser-composed preview/light/camera sequences. The bridge writes station session manifests outside the repo and fails closed if required confirmations, side outputs, report outputs, or truthful safe-off cleanup are missing.
...
The station page has been redesigned into a cockpit-style local workflow. The first screen is a large camera workspace with a placement guide and right-side control sidebar. The sidebar surfaces Start New Card, the explicit fixture/ruler confirmation, accepted lighting-profile controls, Start Grading, report readiness, Safe Off, local paths, preview status, and command timing. Start Grading is a capture action only: it is disabled until refreshed bridge state proves the exact session/report/front epoch has controller-acknowledged initial light-idle evidence, an explicit fixture/ruler confirmation, a durable bridge-observed accepted profile, and the one authoritative transition to `capture_front`. It sends one authenticated, idempotency-keyed `capture-front` mutation and does not separately release preview or request lighting safe-off. After front capture the page displays a red flip-card scrim; once the operator confirms the back is seated, the Back button sends one authenticated, idempotency-keyed `capture-back` mutation. The bridge owns both complete preview/light/camera handoffs before it generates diagnostics, exports the local report bundle, and refreshes history.

PR #57 adds the embedded browser preview foundation. The Dell loopback bridge now exposes token-gated, local-only `GET /preview/status` and `GET /preview/stream` endpoints. The stream is an MJPEG fetch stream from `127.0.0.1:47652`, requires the browser-local station token, and is allowed only from the configured station origin such as `https://collect.tenkings.co`. In real mode the Basler bridge opens a continuous pylon grab stream, rotates frames for portrait viewing, encodes JPEG frames in memory, and writes multipart frames to the browser. The MJPEG camera process itself does not command Leimac lighting, save persistent Basler/Leimac settings, use the production AI Grader service-account token, or appear through public report routes. The loopback bridge separately owns the bounded positioning-light lifecycle described below. In mock mode the same stream contract emits synthetic local frames for tests and UI development only.

Preview and capture use an explicit camera ownership handoff. Live browser preview is for positioning, focus, lighting/exposure tuning, and advisory geometry detection. For each side, one bridge-serialized capture operation marks an intentional transition, stops and drains preview, verifies acknowledged light safe-off, and transfers camera ownership; the browser does not compose stop, safe-off, and capture calls. After the front raw package is safely captured, the bridge releases the forensic hold and permits a fresh preview for flip/back positioning while front artifact processing continues. The hold stays active through the back capture and synchronous report path, or is detached with the captured session when rapid mode safely queues background finalization. Preview never runs concurrently with a Basler capture action. Full forensic capture is not reduced: both sides still use dark control, all-on, accepted profile, channels `1-8`, ROI/display artifacts, Surface Intelligence, Vision Lab, and unified report generation.

#### Bridge-Owned Atomic Side Capture, Back Positioning Light, and Fresh Preview Lifecycle

Back positioning uses a bridge-owned illumination operation rather than browser-supplied hardware settings. Inside atomic Front Capture, the bridge intentionally stops and drains preview, then enters `safe_off_pending`, sends the bounded safe-off frames, and permits forensic capture only after the controller acknowledgements establish `safe_off_verified`. The bridge may restore positioning illumination only after the front action succeeded, the front raw evidence package is safely persisted, the capture lock is released, `prompt_flip_card` is active, and the full-forensic preview hold is released. Restore uses only the session's durable accepted station profile: its already accepted allowlisted channels and accepted duty, still capped at `5%`. The browser cannot supply duty, channels, controller host or port, paths, profile source, or any other hardware value to capture or restore.

Physical light state is deliberately ACK-truthful. `safe_off_pending` means the safe-off write is in flight and never means the controller is off. `safe_off_verified` is recorded only after every required controller acknowledgement is received and validated. A failed write, timeout, missing acknowledgement, or invalid acknowledgement produces `physical_state_unknown`; the bridge never projects `applied.enabled=false`, physical off, or capture readiness from an attempted write alone. `physical_state_unknown` blocks capture, positioning restore, and automated lifecycle transitions until one explicit bounded safe-off recovery reaches `safe_off_verified`. Restore plus cleanup failure remains visible as redacted actionable state rather than being collapsed into a successful cleanup.

The session manifest records positioning-light restore start, success, and failure with only bounded, non-secret accepted-profile identity. A restore failure does not invalidate or delete the completed front evidence. The bridge stays honestly in back positioning, records a redacted actionable failure, performs the same ACK-truthful safe-off transition, and reports `positioningLightReady=false`; Back Capture remains disabled until physical state is known, the accepted-profile light is restored, and a fresh bound back frame is ready. Restore failure, preview loss, watchdog expiry, session or queue replacement, cancellation, session end, page disconnect, capture or processing failure, shutdown, and bridge close all use this same physical-state contract. Cleanup failures are persisted and surfaced; they are not swallowed.

The bridge-authoritative Front Capture command is one serialized lifecycle operation. It first reserves serialized lifecycle and capture ownership, acquires the existing capture lock, and validates an idempotency key. It then validates the exact active session and report, front-capture workflow step, current front side and side epoch, a fresh authoritative frame, and either current authoritative detected-Ready geometry or an explicit current manual mode. Browser session, report, side, epoch, frame, and mode values are match assertions only; browser geometry, lighting values, profile values, paths, and hardware settings are never authority. The bridge snapshots the authoritative frame and detected geometry or manual decision internally, validates the durable accepted profile and physical state, and records honest capture/manual audit. Only then does it mark preview shutdown as an intentional capture transition, stop and drain preview, enter `safe_off_pending`, and require `safe_off_verified`. Immediately before camera ownership and the first grab it revalidates and consumes that authorization exactly once, then performs one forensic capture. Only after immutable front raw evidence is safely persisted may the bridge release capture ownership and proceed to the accepted-profile back-positioning restore boundary. The same idempotency key returns the same result and never captures twice; conflicting, crossed, or stale assertions fail closed before capture.

Fixture/ruler confirmation is an explicit, idempotent, assertion-only bridge mutation bound to the current session, report, front side, and side epoch; Start Grading never manufactures it. Auto Capture and Start Grading both require the separate `Confirm Fixture & Rulers` action to have completed durably; neither control records, synthesizes, or relabels that confirmation. An `auto` front trigger is accepted only after that durable evidence and the rest of the authoritative Front workflow are ready, and it remains audited as an automatic trigger rather than a direct operator click. Manual geometry remains operator-only and is never selected by the automatic path.

Front workflow authority is persisted in the station session manifest under schema `ten-kings-ai-grader-front-workflow-authority-v1`. Its light-idle, fixture/ruler, accepted-profile, and transition records all carry the exact session/report/front epoch binding. Accepted-profile authority additionally carries a SHA-256 digest over the complete bridge-held profile, an acceptance timestamp, and a binding-specific identity; browser profile values are rejected. Each controller-verified profile application also receives an opaque bridge-issued candidate identity bound to the exact session/report/front epoch, full profile digest, acknowledgement time, and a fresh nonce. The browser may return only that opaque identity as a match assertion, so a delayed acceptance for an earlier application cannot accept a later profile. Material retunes persist consumed-key tombstones without eviction up to a bounded per-session limit, then fail closed and require a new session.

Profile and fixture mutations reserve a bridge-owned Front-workflow transaction before changing authority. Matching retries coalesce, conflicting requests fail closed, and `frontCaptureReadiness` remains pending until the atomic session-manifest write completes. A failed write rolls capture-authorizing state back in memory. Heartbeats reject during the transaction, while watchdog, preview-loss, operator-stop, shutdown, and other terminal manifest writers reserve their lane and wait for the commit or rollback before cleanup writes. Refresh and reconnect therefore reconstruct the same decision from bridge truth. Retuning, a new session/profile/epoch, capture/cleanup/safe-off work, or an ambiguous request invalidates or blocks readiness rather than reusing browser state.

Workflow rejection and safety cleanup are separate error channels. The Station preserves the first actionable transition/capture rejection while cleanup still drains preview and attempts safe-off. A later cleanup message cannot replace that rejection. An unacknowledged safe-off remains separately visible as a physical-state safety fault, never sets final-light-off truth, and blocks every further hardware operation except the explicit bounded safe-off recovery.

The bridge-authoritative Back Capture command uses that same side-generic serialized lifecycle and ordering: reserve lifecycle/capture ownership and lock; validate the idempotency key, exact active session and report, `prompt_flip_card`, current back side and side epoch, fresh authoritative frame, and detected-Ready or explicit manual mode; snapshot the authoritative decision and durable profile; record honest operator flip/capture audit; mark the transition intentional; drain preview; require `safe_off_verified`; and finally revalidate and consume authorization exactly once before one camera capture. Browser session, report, side, epoch, frame, and mode values remain match assertions only; browser geometry and hardware/profile values are never capture authority. Back Capture retains its existing idempotent success replay and fail-closed stale/conflict behavior.

The Station therefore does not normally call separate `confirm-flip`, `POST /preview/stop`, or `POST /lighting/safe-off` operations before either side's atomic capture mutation. React busy-state abort, intentional EOF, and disconnect cleanup cannot clear the bridge-owned snapshot or start a second safe-off race. The intentional lifecycle is side-generic and bound to the exact session, side, side epoch, and frame. While that exact submitted local atomic intent remains outstanding, browser EOF, reader error, effect cleanup, disconnect, or an inconclusive preview-status read cannot run generic preview-loss safe-off or reconnect. A received non-success HTTP response from the atomic capture POST may use an exact stable `live`, `starting`, `stopped`, or `error` preview status to confirm that the transition did not start. A capture POST transport rejection before any HTTP `Response` is ambiguous because a follow-up status request can race ahead of a queued POST. The Station preserves that submitted intent for a bounded 15-second ambiguity window. If the bridge then reports the exact transition active, the Station performs at most 20 sequential status checks at two-second intervals, never overlaps checks, and never issues competing safe-off or reconnect. Exact completion or failure triggers a token-gated full `GET /status` reconciliation. If the bridge still owns capture at the hard deadline, the Station surfaces a do-not-retry state and leaves the bridge authoritative; if no exact bridge authority exists, it clears the expired intent and runs the normal safe-off plus bounded reconnect recovery. A confirmed pre-transition failure starts one bounded reader recovery when the same binding remains eligible; an intentional capture-transition EOF does not reconnect, while an unexpected EOF with no atomic intent remains eligible for the bounded recovery policy. The existing forensic runner, evidence roles, hashes, normalization, grading, report, and downstream contracts are unchanged.

Positioning illumination must reach `safe_off_verified` before back forensic capture. A terminal or cleanup path may report the light off only if it reaches `safe_off_verified`; otherwise it remains visibly `physical_state_unknown`. The first back frame has a bounded grace window. After that grace, a heartbeat may renew illumination only when the bridge has received a recent live frame for the same session, back side, and side epoch. A dead, ended, stale, or replaced preview cannot keep the light on indefinitely.

`GET /preview/status` binds preview state through path-free, non-secret `sessionId`, `activeSide`, `sideEpoch`, `latestFrameId`, and `positioningLightReady` fields. Each MJPEG part carries matching `X-AI-Grader-Session-Id`, `X-AI-Grader-Preview-Side`, `X-AI-Grader-Preview-Epoch`, and `X-AI-Grader-Frame-Id` headers, in addition to the existing frame-index and captured-at headers. When front capture transitions to `prompt_flip_card`, the bridge advances a new back side epoch, clears prior back readiness, and invalidates queued or in-flight detector work from an older session, side, epoch, or frame. Front geometry remains historical evidence only and is never accepted as back geometry.

The station browser aborts the old front reader, revokes and clears its object URL and image dimensions, clears detected and manual Ready eligibility, and starts the new session-bound back epoch. The camera area shows only `Starting Back Preview` until a real frame arrives with the current session, side, and epoch binding. Late front frames, prior geometry, and detector results for another session, side, or epoch cannot make the back preview live or capture-ready. If authoritative status replaces the session, side, or side epoch without changing the workflow step, reconciliation clears and binds the new identity first and then advances the local reader-start generation exactly once. A repeated status for that same new binding is a no-op, preventing both a missing reader and a restart loop.

Within one session/side/epoch, the browser keeps a bounded image-snapshot buffer keyed by `{sessionId, side, sideEpoch, frameId}`. It retains at most eight object URLs and never retains a snapshot beyond the existing two-second freshness limit. A frame requires a finite bridge capture timestamp no later than browser receipt and no more than two seconds older than receipt; missing, future, or already ancient frames are rejected and their object URLs are revoked. Retained snapshots are pruned against both capture time and receive time. Geometry can pair only with the exact retained image frame. The display presents that matched image and geometry together and never draws geometry on a newer unmatched image. A coherent fresh pair is not discarded merely because a newer image arrives before its geometry. Missing, expired, future, wrong-session, wrong-side, wrong-epoch, or unretained-frame geometry is rejected. Session, side, or epoch change clears every snapshot and Ready state immediately. Eviction, expiry, replacement, retry, and unmount revoke every discarded object URL, keeping memory bounded.

The displayed matched pair remains advisory. Detected capture for either side requires a fresh exact image/geometry pair. Explicit Manual Overlay capture instead requires a fresh exact authoritative frame observation with internally retained image dimensions, plus deliberate current manual mode. In both cases the bridge independently reauthorizes the current frame and its authoritative detected geometry or manual decision inside the atomic command. Explicit manual mode remains available and is audited distinctly; browser-provided rectangle coordinates are not authority. This lifecycle change does not add a capture countdown or a new automatic trigger.

The browser stream lifecycle distinguishes opened, frame, clean EOF, local abort, intentional capture transition, authoritative state, and actual error. It never emits an incomplete final multipart frame. Clean unexpected EOF immediately leaves live state. While preview remains eligible, reconnect attempts are sequential and use a finite five-attempt bounded backoff; readers and streams never overlap, a real frame is the only event that resets backoff, and the loop cannot spin tightly or retry forever. Reconnect is disabled during a capture lock or action, a non-back-positioning forensic hold, rapid queue review, Safe Off or cancellation, effect cleanup, page unmount, and intentional capture transition. An authoritative capture/hold `409` is rendered as bridge state rather than a generic network error. Preview-status and geometry polling preserve authoritative stopped, paused, released, and error states; a local stale `live` value cannot override them. Before requesting preview-loss safe-off, the client pessimistically removes cached verified/off claims and projects `safe_off_pending` with capture readiness disabled. Only the acknowledged bridge response may replace that projection with verified state. A missing response, timeout, missing acknowledgement, disconnected bridge, or request failure projects `physical_state_unknown`, clears cached `enabled` and verification timestamps, retains no inferred write count, and keeps capture disabled; the UI therefore cannot retain a prior verified positioning-light state after cleanup failure.

The flip-screen Retry action is a complete generation-bound positioning-preview recovery, not a light-only retry. It first rejects an obsolete session, side, or epoch, aborts and discards stale reader/controller state and timers, resets exhausted reconnect counters, advances one local preview-start generation, and clears the browser snapshot, geometry, and manual caches plus the bridge's retained observation and Ready ledgers for that binding. It then starts exactly one reader for the current binding and invokes one authenticated accepted-profile restore through the token-gated, loopback-only, empty-body `POST /lighting/retry-back-positioning` operation. The browser rechecks the exact current session, side, and epoch after the HTTP response and before any response-driven cache or UI mutation, so an obsolete response cannot clear a replacement epoch. That bridge operation is idempotent, accepts no profile or hardware parameters, returns only bounded safe status, and is valid only for the active session at `prompt_flip_card` when no capture lock is held and no rapid queue-review item is active. Frames or geometry received before accepted-profile acknowledgement remain startup-only and cannot authorize capture; post-Retry capture readiness requires a fresh exact matched image/geometry pair, or a fresh authoritative manual frame observation, captured after the bridge's restore-verification boundary. When that exact coherent pair and verified light are already ready, Retry is disabled and a repeated invocation is a no-op; loss of either truth resets the UI to a retryable waiting state. There is no hidden light retry. The watchdog remains active and drives the ACK-truthful safe-off path if no fresh current frame arrives.

#### Detector Policy and Captured-Evidence Worker Boundary

Every card-geometry detector entry requires an explicit typed policy; there is no compile-time or runtime default. `live_preview_fast` is reserved for live MJPEG positioning and can execute only the existing bounded fast detector path. It must never invoke `perimeter_gradient_rectangle_v3`. Preview keeps latest-frame queue semantics, and a completion is advisory only while its exact `{sessionId, side, sideEpoch, frameId}` remains current and within the existing freshness window. A stale, replaced, wrong-side, wrong-epoch, or wrong-session completion is discarded and can never authorize capture.

`captured_evidence_full` is reserved for immutable full-resolution captured roles. It preserves the captured-evidence `all_on` primary authority, accepted-profile/directional consensus and pairwise-disagreement rejection, raw hashes, lossless `1200x1680` normalization, and fail-closed behavior. Its CPU-heavy `perimeter_gradient_rectangle_v3` work runs only in a dedicated compiled Node worker thread loaded from the helper's built `dist` output; camera, Leimac, preview, safe-off, and browser ownership remain on the bridge. The controller permits at most one active full-resolution geometry job and one queued job.

The versioned worker request binds request, package, station session, side, every geometry-authority captured role, the complete source-set digest, source image/frame identities, SHA-256, byte size, dimensions, MIME type, and timestamp. It carries only bounded metadata and local relative file references created from the bridge-owned immutable side package. Both sides of the worker boundary resolve canonical real paths beneath the trusted constructor-configured output root, require the exact package/side layout, and revalidate the exact main-process consumer inputs before one-time authority consumption. The worker response contains bounded geometry/evidence authority metadata only: no local path, image bytes, base64, data URL, or other hosted/public image body may cross that boundary.

Authority acceptance is two-phase. The parent first validates the exact response schema, request identity, source identities, inspected roles, and consensus. It then asks the same worker to revalidate the immutable source hashes, sizes, dimensions, roles, side, and request identity; only the matching acknowledgement can complete the job. Timeout, worker crash, malformed or extra output, wrong identity or source metadata, containment failure, failed revalidation, cancellation, terminal session failure, or shutdown ends that processing attempt. There is no in-process full-detector fallback, cold-capture fallback, synthetic geometry, hidden retry, or dropped normalization/finding gate.

Explicit manual geometry remains a separate operator-confirmed, honestly audited path. It is worker-free, does not run either automatic detector, is never selected automatically, and never substitutes for detector failure. Front artifact processing may overlap the flip/back-positioning window only because captured-evidence geometry is isolated from the bridge event loop; preview heartbeat, first-frame grace, watchdog, EOF/reconnect, disconnect cleanup, and ACK-truthful safe-off remain independently enforceable. Bridge protocol `ai-grader-local-station-bridge-v0.9` now makes Front and Back Capture share the same atomic intentional-transition contract; report-producer contract `ai-grader-report-producer-v0.2` is unchanged. This boundary preserves the PR #89 accepted-profile lighting, verified safe-off, exact preview/frame binding, and atomic Front/Back Capture contracts together with the PR #92 captured-evidence authority, normalization, worker isolation, and public-response sanitization contracts.
...
- Person 1 uses `/ai-grader/station` on the Dell for capture, grade review, Confirm Card, publication, and immediately starting the next grade. Confirm Card uses finalized/unpublished semantics and a current producer-v0.2 package: it fail-closed validates the authoritative release, actual final grade, finding extraction, and exactly one normalized front/back image before atomically creating or resolving one pending `UPLOADING` CardAsset, one Item and ownership, draft report/session linkage, and the existing durable KingsReview/SerpAPI eBay sold-comps handoff. Confirm leaves the CardBatch `UPLOADING` with `processedCount=0`; it never creates inventory QR/PackLabel artifacts, assigns a physical grading-label sheet slot, projects published sidecars, or invents label/QR state. Neither label readiness nor OCR success blocks fully manual Confirm Card. Only verified Publish promotes the CardAsset and Item to their hosted images, marks the CardAsset and CardBatch ready, and idempotently assigns the physical grading-label sheet slot from the durably published release. The station shows that slot only after Publish succeeds; downstream review controls live elsewhere.
...
- Person 2 or Mark uses `/ai-grader/labels/sheets` for the physical label queue. Sheets are 8.5in x 11in portrait with 16 slots in a 2-column by 8-row grid; each label is 2.73in x 0.83in. A partial sheet can be sealed and printed at any time. Mark Sheet Printed updates every label on that sheet atomically. The legacy per-label print-state mutation is retired; inventory readiness cannot bypass the sheet-level print transition.

Label sheet assignment, seal, slot, print audit, and confirmed card identity are stored in the existing `AiGraderLabel.payload` JSON. No schema change or migration is required. Tenant and report advisory locks serialize Confirm, publish, sheet printing, comp lifecycle, comp selection, and inventory transitions. A failed publish cannot leave a slot assignment; if publication commits but the post-publish batch/slot transaction is interrupted, Publish retry validates the same durable identities and repairs the missing assignment without selecting a second slot. Printable-content revisions cover grade, cert, QR/report URLs, and confirmed identity; changing printed content invalidates its printed status and requires a sheet reprint.

Confirm Card also writes `ai-grader-confirm-publish-authority-v1` into the existing `CardAsset.classificationSourcesJson.aiGraderPublishAuthority` JSON and mirrors the exact record at `CardAsset.aiGradingJson.publishAuthority`. Its canonical SHA-256 projection seals the report/session and producer contracts; normalized asset identities, hashes, dimensions, sides, and evidence roles; findings and finding validation; the complete final grade; gates, evidence references, warnings, and limitations; operator finalization; and immutable label/certificate fields. Publish-init checks the seal before planning or presigning, Publish-finalize rechecks it before storage verification, and the locked database and physical-label transitions re-read it before mutation. Card identity and CardAsset/Item linkage remain separately bound to the durable Confirm rows. The projection excludes only workstation paths or embedded bodies, server-generated hosted URLs and storage/upload state, and genuinely downstream comps, slab, label-sheet, batch, valuation, and inventory runtime state. Missing, malformed, contradictory, or hash-mismatched authority fails closed; Publish never reconstructs it from a legacy row or accepts browser replacements for its sealed fields.
...
Slab front/back photos continue to use authenticated upload initialization followed by direct browser-to-storage `PUT` and a small finalize manifest. Vercel never receives image bodies. Add To Inventory runs in one transaction and remains the only operation that creates or resolves the inventory QR/PackLabel pair and moves the CardAsset to `INVENTORY_READY_FOR_SALE`. It requires a published report, sheet-printed grading-label status, persisted front and back slab photos, and a positive completed selected-comp valuation.
...
Detection failure does not silently use a configured fixture boundary and does not generate a normalized geometry artifact. Explicit operator-confirmed manual mode remains recorded as `manual_capture` / `manual_override`, with `detectionUsed=false`, `confidence=0`, and a non-Ready placement state, but the bridge snapshots the authoritative current-frame manual decision inside the atomic capture operation; it does not trust browser geometry. The side manifest and analysis record front/back corners, bounding box, rotation/skew, confidence, source frame/image ID, timestamp, geometry source, normalized artifact hash/dimensions/resampling, coordinate-frame registration, and raw-source preservation evidence. The unified local report and report bundle include normalized artifact references while retaining all original evidence.

Bridge protocol `ai-grader-local-station-bridge-v0.9` and report-producer contract `ai-grader-report-producer-v0.2` make this contract explicit. The v0.9 handoff change is limited to bridge-owned atomic Front Capture plus the preserved atomic Back Capture lifecycle; it does not change report-package production. The production page rejects an older in-memory helper with an actionable update/restart message instead of sending an unknown action. Normal bridge startup rebuilds the checked-out capture helper unless the developer explicitly selects `-SkipBuild`, and a failed build stops startup rather than running stale compiled code.
...
The warm runner is the production capture execution path. If it fails, capture stops and the bridge performs the same ACK-truthful safe-off transition before releasing camera/lighting ownership and locks. Only acknowledged `safe_off_verified` may be reported as off; failure, timeout, or missing acknowledgement remains `physical_state_unknown`, is persisted/displayed, and blocks further automated work until explicit bounded recovery succeeds. There is no automatic warm-to-cold path switch. The cold command implementation can run only when an explicit developer/debug configuration disables the warm runner before the session; that mode rejects `production_fast` and can never contribute to production timing acceptance or five-seconds-per-side proof.
...
Forward-only Confirm Card OCR accepts only report-producer contract `ai-grader-report-producer-v0.2` packages and exactly one verified front plus back `normalized_card` PNG at `1200x1680`. The browser uploads those normalized artifacts directly to storage with checksum-bound presigned PUTs and submits only small init/finalize metadata; Vercel never receives image bodies, base64 data, local paths, or caller-selected image URLs. After provider-native checksum, byte-size, MIME, and decoded-dimension verification, Google Vision runs URL-only `DOCUMENT_TEXT_DETECTION` with exact response-count, per-side error, and timeout enforcement. Its bounded text/token/side/confidence/box evidence and the two verified storage image URLs then feed one strict OpenAI Responses API Structured Outputs request with `store:false`, bounded output, both images at original detail, no JSON-mode or alternate-model fallback, and an overall provider budget below the route timeout. The actual model identifier returned by OpenAI is preserved as safe bounded OCR provenance in the station/public report package. Provider timing remains in internal server diagnostics only. Provider request/response IDs and timing internals are never returned publicly.

The strict extraction covers category, player/card name, year, manufacturer, sport/game, product set, card number, insert, parallel, serial numbering, autograph, and memorabilia with explicit `supported`, `unknown`, and `disagreement` states. Shared `extractCardAttributes` heuristics may contribute positive autograph or memorabilia hints but never convert missing keywords into false evidence. Supported false requires explicit image/OCR evidence. Ten Kings set/card/insert/parallel catalog validation canonicalizes supported identities and turns missing, multiple, unsupported, or conflicting matches into review states rather than guesses. Typed, redacted Google, OpenAI, and catalog failures remain retryable in the station while fully manual Confirm Card stays available. Human confirmation is always required; OCR never finalizes a release, creates inventory, publishes, or chooses comps.
...
The browser checksum header is a signed, required part of every AI Grader OCR, publish, and slab-photo direct PUT. DigitalOcean Spaces CORS must be updated before code that requires this header is merged or deployed. The update is additive: preserve every existing CORS rule and append exactly this production-origin rule; do not replace the bucket configuration. A successful CORS preflight is necessary but not sufficient proof that Spaces persists and returns the native checksum.
...
Current execution-surface constraint: the checked-in repository has no one-shot checksum-probe endpoint or operator script that can safely exercise the PR-head browser adapter. A Vercel preview runs the PR head but is not the allowed production origin, while the production Vercel runtime has the existing protected Spaces credentials and exact `https://collect.tenkings.co` origin but does not run the unmerged PR head. The Dell checkout is not a verified holder of the production storage credentials, and pulling or copying those credentials is prohibited. Therefore the native-checksum gate is not currently executable before merge under ordinary PR-preview access, and it must not be reported as passed.

The executable path requires separate Mark authorization for a narrowly scoped production-origin canary or a separately reviewed probe-only precursor. That authorized execution must run the pinned PR head in the existing Vercel production runtime, use its already-configured protected storage credentials without reading or printing them, and expose only an authenticated one-shot probe action to a browser loaded from exactly `https://collect.tenkings.co`. The browser must generate a small non-card byte payload, calculate its SHA-256, request a checksum-bound upload plan, and send the direct PUT with the exact signed `Content-Type`, configured `x-amz-acl` when present, and `x-amz-checksum-sha256` header. The server must then call the same production `HeadObject` adapter with `ChecksumMode=ENABLED` and compare native SHA-256, exact byte size, and content type. The action may return only redacted booleans; the object key, signed URL, headers, credential material, and provider response identifiers must remain in memory and out of logs. Probe-object cleanup is a production storage mutation and must be included in the same separate authorization. Without that canary/precursor and authorization, stop before merge.
...
5. Obtain Mark's separate explicit approval for the production-origin canary or probe-only precursor described above and for its temporary non-card object create/delete. Pin it to the reviewed PR head; do not substitute a preview-origin request, current-main adapter, server-only PUT, or copied credential set.
6. From the exact allowed production origin, perform the checksum-header-signed direct PUT, then use the same canary runtime's `HeadObject` adapter with `ChecksumMode=ENABLED`; require the provider-returned native SHA-256 to match the exact test-byte digest, and require exact byte size plus compatible content type. Record only redacted pass/fail booleans.
7. Do not accept ETag, mutable metadata, request echo, a server-only PUT, or a successful PUT/preflight as checksum proof. If the native checksum is absent or mismatched, stop; do not merge or deploy. The synthetic probe and canary are not authorized by this documentation change.
8. Only after the CORS checks and production-origin synthetic provider round trip both pass, remove or disable the one-shot probe surface, verify that result, then merge normally and allow the automatic production deployment.
9. After the automatic deployment is green, Mark may authorize one newly captured card smoke through OCR Prefill, manual Confirm Card, Publish/Label, and the existing comps handoff. Capture performance, ring-light restoration, edge accuracy, and startup delay remain a separate later PR.
...
#### Public Defect Findings V1

Surface Intelligence candidates carry `analysisGeometry` in the canonical card plane: `coordinateFrame=normalized_card`, `units=fraction`, with either a bounded box or polygon. For the PR #81 path, Surface Intelligence runs directly on the canonical `1200x1680` normalized card image, so candidate pixel rectangles are divided by those exact artifact dimensions once; they are not inverse-transformed into raw Basler pixels or rotated/deskewed a second time. The legacy source-display projector remains available only for older non-normalized evidence and explicitly identifies that coordinate path. Candidate geometry carries private raw-source and exact normalized-artifact SHA-256 fingerprints; the direct path additionally requires the analysis image hash/dimensions to match that normalized artifact. Report packaging recomputes the normalized file checksum and emits a public finding only when both fingerprints match the side's current evidence. Whole-frame `displayRect` and raw sensor rectangles remain private diagnostic data and are never guessed into public card coordinates.

The report bundle keeps its compatible top-level `ai-grader-report-bundle-v0.1` version and adds optional `visionLab.defectFindings[]`. Each finding is independently versioned as `ai-grader-defect-finding-v1` and includes a stable ID, side, allowlisted category, detector/version, severity, confidence, review status, normalized geometry, safe explanation, and logical evidence asset IDs. The stable ID hashes side, category, detector identity, and canonical normalized geometry; polygon start point and winding, transient candidate order, confidence, severity, and private evidence paths do not change it. Candidate joins are side-scoped. Grade-impact candidates and final grade-impact reasons may link to findings through `findingIds[]` only when that exact scoped candidate exists.

The overlay base is always the exact normalized card image named by `evidence.trueViewAssetId`. Public asset IDs are stable logical bundle IDs such as `report/back/back-normalized-card.png`; storage object names and database artifact IDs are separate. Publication requires browser-native PNG/JPEG/WebP MIME types and exact checksum/byte-size metadata, and rejects unsafe or case-insensitively duplicated image IDs, dangling finding references, side/role mismatches, invalid geometry, and grade-impact references to unknown findings. The capture helper Sharp-decodes package images and rejects embedded EXIF/XMP/IPTC/ICC/PNG-text/Photoshop metadata; the Dell browser also checks raster structure, rejects metadata/trailing payloads, and decodes each report image before its direct browser-to-storage PUT. TIFF remains local raw evidence and is not a public report-image type. Image bodies still never pass through Vercel. The public report read path reconstructs URLs with the configured storage helper, binds keys to the current report's asset prefix, and sanitizes/revalidates the stored bundle again. Candidate `rawRect`, `displayRect`, source/artifact fingerprints, local paths, detector internals, station/bridge data, embedded image bodies, signed URLs, channel file paths, ROI paths, and hardware controls are not copied into the public finding.

The public report viewer renders boxes and polygons over the exact object-contained image footprint in a fixed `2.5 / 3.5` canvas, supports keyboard/click selection with visible focus and selected mode/side semantics, shows severity/confidence/review state, and uses the wording `AI-detected provisional ... finding`. Markers are withheld until the exact evidence image successfully loads; a missing or failed image shows an unavailable state instead of an unrelated image or floating markers. Publication/readback forces detector output to `review.status=unreviewed` until a future audited human-review write path exists, so caller-supplied confirmed/rejected claims cannot become public. Existing reports without `defectFindings` remain valid and show no fabricated overlay. A software-only fixture is available at `/ai-grader/reports/sample-defect-v1`.

No migration is required. Findings live in the existing report-bundle JSON, `AiGraderReport.visionLabArtifacts`, and managed storage artifacts. This change does not modify env vars, auth, hardware controls, direct-to-storage image upload, or the station/finish production split.

Post-deploy browser smoke for Mark:

1. Open `/ai-grader/reports/sample-defect-v1`, choose Back, and confirm one red high-severity rectangle remains attached to the same card region at desktop and narrow/mobile widths. Select the rectangle and its finding row with mouse and keyboard.
2. Grade a non-sensitive test card through the normal Dell flow. Confirm/publish it normally, then open its public report URL from a normal computer. Confirm the report loads without the Dell bridge and True View/Evidence Replay shows each finding only on the exact front/back normalized image.
3. Compare at least three visible findings against the local Surface Intelligence report. Confirm side, region, category, severity, confidence, and linked heatmap/Surface Vision references agree; confirm the browser response contains no local path, bridge URL, station token, `data:image`, presigned URL, or hardware control.
4. Check an older published report and confirm it still renders without an overlay or error. Check a card with no emitted candidates and confirm the report shows no fabricated defect boxes.
5. Do not treat this transport/display work as detector-accuracy proof. Build a versioned, blinded gold set of annotated cards spanning scratches, creases, edge/corner wear, print defects, glare, dust, sleeves, dark/light borders, and clean controls. Track per-category precision/recall, false positives per clean card, IoU/localization, calibration by confidence band, and repeated-capture stability. Promote detector/category thresholds only after Mark approves target metrics and a held-out evaluation; keep review status `unreviewed` until a human confirms or rejects each finding.
