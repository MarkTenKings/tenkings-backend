...
The existing PowerShell/pylon path remains the production default. Native selection requires a later explicit integration and Mark-approved Dell validation after Agent 2's preview/back-light lifecycle work. If a future native attempt fails, it is terminal for that attempt: safe-off, stop the worker, show the bounded error, and require a new explicit session. Never cold-fallback automatically. Rapid countdown/trigger work remains a later, separate change.
