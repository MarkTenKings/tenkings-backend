...
## Native Camera Worker Foundation (Inactive)

The standalone x64 .NET/OpenCvSharp native camera worker, protocol, TypeScript client seam, fake/replay backends, Pylon-only host, replay evaluator, packaging, and safety gates are documented in [AI Grader Native Camera Worker](./ai-grader-native-camera-worker.md). This foundation is disabled by default and is not wired into `aiGraderLocalStationBridge.ts` or the station page.

The existing PowerShell/pylon path remains the production default. Native configuration is host-owned: protocol initialization may name only an expected trusted-rig ID/digest, preview commands carry no tuning, and the Windows host must verify a protected trusted-writer-only configuration/calibration/orientation plus exact camera settings/readback before preview. Forensic evidence is committed only as one complete immutable 11-role side package after single-current-frame all-on geometry and injected safe-off pass; an exclusive staging lease prevents a concurrent retry from reconciling a live capture, staging-only owner/lease files are removed and the exact manifest-plus-11-role package is verified before the sole atomic rename commit point, partial staging is never final, and public protocol data contains no paths or device identity. Nonlinear lens-assisted detection remains display-only because a single homography cannot normalize interior raw pixels; authoritative capture currently requires `lens: null` and rejects `unsupported_lens_transform`.

Native selection requires a later explicit integration and Mark-approved Dell validation after Agent 2's preview/back-light lifecycle work. If a future native attempt fails, it is terminal for that attempt: safe-off, stop the worker, show the bounded error, and require a new explicit session. Never cold-fallback automatically. Normal build/test/health/capabilities/serve scripts cannot enable or execute Pylon. Rapid countdown/trigger work remains a later, separate change.
