...
Admins import one controlled PNG or JPEG per exact front/back identity through `/admin/ai-grader-design-references`. The browser computes a preliminary SHA-256 and requests an opaque identity/version-scoped, checksum-bound private PUT; draft creation then rereads the stored bytes and independently derives the supported raster type, dimensions, byte bounds, and SHA-256. The imported version remains inactive until a separate explicit APPROVED transition. Drafts and approved references are revoked/retired, and approval of a newer exact side supersedes the prior active version; neither the API nor UI exposes overwrite or delete authority.
