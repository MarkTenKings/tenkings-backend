...
Preview and capture use an explicit camera ownership handoff. If a capture action starts while the browser preview owns the camera, the bridge stops/releases the preview stream, marks preview status as paused for capture, runs the existing full forensic capture command, then the station reconnects preview when the workflow returns to idle. Full forensic capture was not reduced: front/back evidence packages still use dark control, all-on, accepted profile, channels `1-8`, ROI/display artifacts, Surface Intelligence, Vision Lab, and unified report generation.
