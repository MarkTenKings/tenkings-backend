...
The bridge exposes public local `GET /health`, pairing `POST /pair`, token-gated `GET /status`, `GET /latest-report`, `GET /session-manifest`, and token-gated `POST /actions/<action>`. Actions include `start-session`, `confirm-light-idle-off`, `confirm-fixture-rulers`, `launch-preview`, `accept-profile`, `capture-front`, `capture-back`, `run-diagnostics`, `export-report-bundle`, `safe-off`, and `end-session`. An older `confirm-flip` action may remain available for compatibility, but the v0.8 Station Back button does not call it and it cannot create or preserve the authorization used by Back Capture. The bridge writes station session manifests outside the repo and fails closed if required confirmations, side outputs, report outputs, or truthful safe-off cleanup are missing.
...
Preview and capture use an explicit camera ownership handoff. Live browser preview is for positioning, focus, lighting/exposure tuning, and advisory geometry detection. The bridge stops/releases the preview process before each full-forensic side capture so only one owner can access Basler. After the front raw package is safely captured, it releases the forensic hold and permits a fresh preview for flip/back positioning while front artifact processing continues. Back capture is one bridge-serialized operation that intentionally drains preview, verifies acknowledged light safe-off, and transfers camera ownership without a browser-owned stop/safe-off sequence. The hold stays active through the back capture and synchronous report path, or is detached with the captured session when rapid mode safely queues background finalization. Preview never runs concurrently with a Basler capture action. Full forensic capture is not reduced: both sides still use dark control, all-on, accepted profile, channels `1-8`, ROI/display artifacts, Surface Intelligence, Vision Lab, and unified report generation.

#### Bridge-Owned Back Positioning Light and Fresh Preview Lifecycle

Back positioning uses a bridge-owned illumination operation rather than browser-supplied hardware settings. Before front forensic ownership, the bridge intentionally stops and drains preview, then enters `safe_off_pending`, sends the bounded safe-off frames, and permits capture only after the controller acknowledgements establish `safe_off_verified`. The bridge may restore positioning illumination only after the front action succeeded, the front raw evidence package is safely persisted, the capture lock is released, `prompt_flip_card` is active, and the full-forensic preview hold is released. Restore uses only the session's durable accepted station profile: its already accepted allowlisted channels and accepted duty, still capped at `5%`. The browser cannot supply duty, channels, controller host or port, paths, profile source, or any other hardware value to this restore.
...
The bridge-authoritative Back Capture command is one serialized lifecycle operation. It first acquires capture ownership and the existing capture lock and validates an idempotency key. It then validates the exact active session and report, `prompt_flip_card`, current back side and side epoch, a fresh authoritative frame, and either current authoritative detected-Ready geometry or an explicit current manual mode. Browser session, report, side, epoch, frame, and mode values are match assertions only; browser geometry is never capture authority. The bridge snapshots the authoritative frame and geometry or manual decision internally, validates the accepted profile and physical state, records honest operator flip/capture audit, marks preview shutdown as an intentional capture transition, stops and drains preview, enters `safe_off_pending`, and requires `safe_off_verified`. Immediately before camera ownership and grab it revalidates and consumes that authorization exactly once. The same idempotency key returns the same result and never captures twice; a conflicting or stale request fails closed before capture.

The Station therefore does not normally call separate `confirm-flip`, `POST /preview/stop`, or `POST /lighting/safe-off` operations before `capture-back`. React busy-state abort, intentional EOF, and disconnect cleanup cannot clear the bridge-owned snapshot or start a second safe-off race. While the exact local atomic intent remains outstanding, browser EOF, reader error, or an inconclusive preview-status read cannot run generic preview-loss safe-off or reconnect. A received non-success HTTP response from the atomic capture POST may use an exact stable `live`, `starting`, `stopped`, or `error` preview status to confirm that the transition did not start. A capture POST transport rejection before any HTTP `Response` is ambiguous because the follow-up status request can race ahead of a queued POST; stable status alone must preserve the local intent in that case. Only an exact, bounded bridge `transition_failed` marker carrying the matching session, back side, side epoch, and frame may clear the intent after an ambiguous transport rejection. A confirmed pre-transition failure starts one bounded reader recovery when the same binding remains eligible; an intentional capture-transition EOF does not reconnect, while an unexpected EOF with no atomic intent remains eligible for the bounded recovery policy. The existing forensic runner, evidence roles, hashes, normalization, grading, report, and downstream contracts are unchanged.
