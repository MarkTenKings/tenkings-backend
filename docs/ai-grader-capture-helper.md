...
Forward-only Confirm Card OCR accepts only report-producer contract `ai-grader-report-producer-v0.2` packages and exactly one verified front plus back `normalized_card` PNG at `1200x1680`. The browser uploads those normalized artifacts directly to storage with checksum-bound presigned PUTs and submits only small init/finalize metadata; Vercel never receives image bodies, base64 data, local paths, or caller-selected image URLs. After provider-native checksum, byte-size, MIME, and decoded-dimension verification, Google Vision runs URL-only `DOCUMENT_TEXT_DETECTION` with exact response-count, per-side error, and timeout enforcement. Its bounded text/token/side/confidence/box evidence and the two verified storage image URLs then feed one strict OpenAI Responses API Structured Outputs request with `store:false`, bounded output, both images at original detail, no JSON-mode or alternate-model fallback, and an overall provider budget below the route timeout. The actual model returned by OpenAI and bounded provider timing are recorded only in server diagnostics; provider IDs and timing internals are not returned to the station or public report.
