...
The native immutable Basler all-on TIFF is the sole geometry source. The launcher and bridge expose no provisional warped-image input, derivative replacement, alternate geometry artifact, or rollback geometry path. Derived display images cannot replace the source bytes used to detect and seal the dense physical contour.
