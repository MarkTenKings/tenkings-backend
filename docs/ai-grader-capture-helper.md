...
Live front/back preview now carries path-free card geometry alongside the token-gated MJPEG stream. The bridge detects four ordered outer corners and reports one simple placement state: `not_detected`, `adjust_card`, or `ready`. `ready` accepts approximately 0.5 inch of center offset and 10 degrees of rotation/skew at the configured confidence threshold. The existing fixed overlay remains visible as an operator aid. If automatic detection is not Ready, an operator may deliberately choose Manual Capture and confirm the displayed rectangle; auto capture cannot take this path. Preview geometry is advisory and contains only image-pixel coordinates, confidence, side, safe frame ID, and timestamp.

Each captured side keeps every original raw forensic frame and also writes a lossless normalized card PNG when valid geometry exists. Normalization uses the detected quadrilateral to rotate/deskew and crop the fixed overhead view into a grading-resolution upright rectangle. Detection failure does not silently use a configured fixture boundary and does not generate a normalized geometry artifact. An explicit, operator-confirmed manual rectangle is recorded as `manual_capture` / `manual_override`, with `detectionUsed=false`, `confidence=0`, and a non-Ready placement state. The side manifest and analysis record front/back corners, bounding box, rotation/skew, confidence, source frame/image ID, timestamp, geometry source, normalized artifact hash/dimensions, and raw-source preservation evidence. The unified local report and report bundle include normalized artifact references while retaining all original evidence.
