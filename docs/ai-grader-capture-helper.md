...
Live front/back preview now carries path-free card geometry alongside the token-gated MJPEG stream. The bridge detects four ordered outer corners and reports one simple placement state: `not_detected`, `adjust_card`, or `ready`. `ready` accepts approximately 0.5 inch of center offset and 10 degrees of rotation/skew at the configured confidence threshold; the existing fixed overlay and manual card-boundary fallback remain available and manual capture is never blocked by detection. Preview geometry is advisory and contains only image-pixel coordinates, confidence, side, safe frame ID, and timestamp.

Each captured side keeps every original raw forensic frame and also writes a lossless normalized card PNG. Normalization uses the detected quadrilateral to rotate/deskew and crop the fixed overhead view into a grading-resolution upright rectangle. If detection is unavailable, the configured manual boundary may be used and is explicitly recorded as `manual_fallback`. The side manifest and analysis record front/back corners, bounding box, rotation/skew, confidence, source frame/image ID, timestamp, detection-versus-fallback source, normalized artifact hash/dimensions, and raw-source preservation evidence. The unified local report and report bundle include the normalized artifact references while retaining all original evidence.

Two station capture profiles are available:

- `full_forensic` is the default/fallback and keeps the established lossless PNG raw evidence path.
- `production_fast` is an explicit station setting. It keeps the same 11 required raw roles per side (`dark_control`, `all_on`, `accepted_profile`, and channels `1-8`) but writes lossless TIFF raw frames to avoid expensive PNG encoding. It does not count as a five-second success until a supervised Dell comparison proves the target without evidence or grading-quality loss.
...
4. For every side in both reports, verify raw roles are exactly dark control, all-on, accepted profile, and channels `1-8`; all 8 channel displays and expected ROI crops exist; raw evidence hashes/byte sizes are recorded; the raw format is PNG for the control and TIFF for the fast run; a lossless normalized PNG exists; geometry identifies front/back and detected/manual source; and neither report claims certified/final evidence beyond the existing V0 rules.
