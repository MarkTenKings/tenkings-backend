...
Preview and capture use an explicit camera ownership handoff. Live browser preview is for positioning, focus, and lighting/exposure tuning before grading. When full forensic grading starts, the bridge stops/releases the preview stream before front capture, holds preview stopped through the flip prompt, back capture, and report generation, and rejects new preview stream requests while that full-forensic hold is active. The station keeps the last displayed frame/progress overlay visible instead of reacquiring Basler during the flip window. Preview resumes only after the session is ended, cancelled, or safe-off/failure cleanup releases the hold. Full forensic capture was not reduced: front/back evidence packages still use dark control, all-on, accepted profile, channels `1-8`, ROI/display artifacts, Surface Intelligence, Vision Lab, and unified report generation.
