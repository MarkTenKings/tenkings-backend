...
Local Dell smoke on 2026-06-09 wrote package folder `C:\TenKings\capture-data\dinolite-demo\dinolite-card-demo-001-20260609T195221805Z` outside git. The package contains `manifest.json`, `preview-report.html`, and one real normal still JPG. The normal still was `normal-still.jpg`, `sha256=318794724829f9d2a94ee67f41bd6df2f61090b35dd0487ec716972fe0b31a37`, `byteSize=67150`, `mimeType=image/jpeg`. Device ID was present and is redacted from docs except USB VID/PID `vid_a168&pid_0990`.

The same smoke reported config bitfield `198`, decoded `amr=true`, `axi=true`, `edof=false`, `led=false`, and `flc=false`; AMR was `1`. Because this device/config did not report LED or FLC support, the requested lighting sweep was recorded as `LIGHTING_SWEEP_UNSUPPORTED` instead of running lighting recipes. `SaveEDR` was attempted because EDR was explicitly requested, but DNVideoX produced no non-empty output, so the manifest records `SaveEDR_NO_OUTPUT`. EDOF was not called because `GetConfig` did not report EDOF support; the manifest records `EDOF_UNSUPPORTED`. Cleanup reported `previewStopped=true`, `disconnected=true`, `hostDisposed=true`, no cleanup errors, and the final safe FLC restore recipe as unavailable due `FLC_UNSUPPORTED`.
