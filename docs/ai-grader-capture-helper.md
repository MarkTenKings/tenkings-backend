...
Back positioning uses a bridge-owned illumination operation rather than browser-supplied hardware settings. Front capture still safe-offs browser positioning light and releases the preview camera before the forensic runner begins frame capture under the existing lock. The bridge may restore positioning illumination only after the front action succeeded, the front raw evidence package is safely persisted, the capture lock is released, `prompt_flip_card` is active, and the full-forensic preview hold is released. Restore uses only the session's durable accepted station profile: its already accepted allowlisted channels and accepted duty, still capped at `5%`. The browser cannot supply duty, channels, controller host or port, paths, profile source, or any other hardware value to this restore.

The session manifest records positioning-light restore start, success, and failure with only bounded, non-secret accepted-profile identity. A restore failure does not invalidate or delete the completed front evidence. The bridge stays honestly in back positioning, records a redacted actionable failure, attempts safe-off, and reports `positioningLightReady=false`; Back Capture remains disabled until both the accepted-profile light and a fresh bound back frame are ready. The dedicated retry is token-gated, loopback-only `POST /lighting/retry-back-positioning` with an empty body. It is idempotent, accepts no profile or hardware parameters, returns only bounded safe status, and is valid only for the active session at `prompt_flip_card` when no capture lock is held and no rapid queue-review item is active. It performs one explicit attempt and has no hidden retry loop.

Positioning illumination is safe-offed before back capture and on restore failure, capture failure, cancellation, session end, bridge shutdown, page disconnect, watchdog expiry, or preview loss. The first back frame has a bounded grace window. After that grace, a heartbeat may renew illumination only when the bridge has received a recent live frame for the same session, back side, and side epoch. A dead, ended, stale, or replaced preview cannot keep the light on indefinitely.
...
The station browser aborts the old front reader, revokes and clears its object URL and image dimensions, clears detected and manual Ready eligibility, and starts the new session-bound back epoch. The camera area shows only `Starting Back Preview` until a real frame arrives with the current session, side, and epoch binding. Only that frame may move the local display to live. Late front frames, stale back frames, prior geometry, and detector results for a different session, side, epoch, or frame cannot enable detected or Manual Overlay Back Capture.

This lifecycle change does not add a capture countdown or a new automatic trigger. Existing operator-triggered and explicit Manual Overlay capture paths retain their trigger and audit metadata; the back handoff now only adds the required preview release, positioning-light safe-off, and current frame/profile authorization before the forensic runner takes ownership.

The browser stream lifecycle distinguishes opened, frame, clean EOF, local abort, and actual error. It never emits an incomplete final multipart frame. Clean EOF immediately leaves live state. While preview remains eligible, reconnect attempts are sequential and use a finite bounded backoff; readers and streams never overlap, a real frame is the only event that resets backoff, and the loop cannot spin tightly or retry forever. Reconnect is disabled during a capture lock or action, a non-back-positioning forensic hold, rapid queue review, Safe Off or cancellation, effect cleanup, and page unmount. An authoritative capture/hold `409` is rendered as bridge state rather than a generic network error. Preview-status and geometry polling preserve authoritative stopped, paused, released, and error states; a local stale `live` value cannot override them.
