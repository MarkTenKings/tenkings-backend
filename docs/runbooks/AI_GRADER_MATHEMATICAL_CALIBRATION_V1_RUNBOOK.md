...
Session creation/resume rereads the exact five-member rig source. The local 'analyze' action has no result payload: it rereads the four active checkerboard bytes and all 72 active photometric bytes, reruns exact-still geometry, decodes fixed 8-by-8 dark/flat/pattern grids, and locally derives every quick numeric result and artifact. A valid source hash attached to a browser-authored number is never accepted as measurement authority. Reopening a completed session reruns the same byte-derived analysis and requires the exact stored analysis bytes/hashes before the outer bundle and all 12 members are canonical-loader verified.

Production helper construction must provide trusted local runtime and rig-source providers, one exact-still checkerboard capture adapter, and a persistent batch-controller factory. The durable authority owns session/operation IDs and the exact next step; the browser invokes that step using only the server revision. Do not put protected context, source paths, analysis values, or hardware identities in a route request. Adapter construction alone must not open Pylon or Leimac.
...
Under ten minutes is an acceptance target, not a present claim. Measure wall time on the Dell from session creation through ready-for-explicit-activation, including operator moves, controller acknowledgements, image transfer/checkpointing, four OpenCV checkerboard reruns, 72 full-resolution Sharp decodes/grid reductions, analysis, finalization, and safe-off. Also verify on real frames that the local-neighborhood 11-by-16 residual and illumination-centroid coordinate convention are physically valid after the immutable lens transform. Never lower pose, residual, U95, repeatability, or threshold-manifest gates to meet the target.
