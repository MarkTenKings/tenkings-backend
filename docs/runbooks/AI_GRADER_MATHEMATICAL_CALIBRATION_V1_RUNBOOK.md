...
The installed 'ai-grader-station-bridge' CLI now has one inert Production construction path. Configure all six values together or none; partial configuration is a hard error:
...
- 'AI_GRADER_MATHEMATICAL_CALIBRATION_V1_2_RIG_SOURCE_MEMBER_DIR'; and
- 'AI_GRADER_MATHEMATICAL_CALIBRATION_V1_2_OPERATOR_ID'.

The exact canonical runtime file must bind the loaded content-addressed geometry/photometric manifests. The rig source must contain the exact five ordered member bytes, including the controller unit-information identity, channel wiring, stage-to-undistorted-sensor transform, target/lens authority, and algorithm hashes. The helper reads these files locally; routes never return their paths. Construction and read-only list/status do not open hardware. Start/resume probes live camera/controller context. Checkerboard capture and the 72-frame sweep open only through the serialized Basler/Leimac seam; the sweep keeps one camera and controller connection open, validates protocol-recognized ACKs, and safe-offs before/after every frame plus final close.
