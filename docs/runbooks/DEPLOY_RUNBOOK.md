...
Use the exact-source release identities and qualified workflow in [main-site release](../plans/2026-09-16-main-only-release.md) and [Inventory delivery](../plans/2026-09-21-inventory-takeover/README.md). Root promoted exact `bc92b1023640ab21499506fb2d4fb5e2bdb188df` / `dpl_FXLDCapouMhB7WtULnCTb45QXG31` at **2026-09-22 01:17:39 UTC** (September 21 local time), immutable host `tenkings-backend-nextjs-drrqyvmk1-ten-kings.vercel.app`. Root independently verified all five Production aliases on it: apex, www, collect and both inherited platform aliases. Stable main Preview serves the same source through `dpl_Hie34Nkq8rhtp1MGjURUAsY7J8yH` (`hi22vdspr`). The earlier `9a12cd35` / `dpl_2XUG…` release remains historical evidence. This release did not restart or deploy ATLAS.

Both hosted artifacts passed exact-source build, 79-page, native Linux Sharp 0.34.5, Fluid 300-second and unsigned private-API checks. Independent post-promotion proof passed 28 route/privacy checks and six assets, with build ID `BXAmDdh8X-Ikg6B_GxbR1`. The sole cron now belongs to the promoted artifact; one naturally scheduled request returned HTTP 200 at 2026-09-22 01:18:37.171 UTC, without exposing a job outcome. Optional catalog, sale-detail, full-resolution, contribution and diagnostic execution flags remain off. The separate Inventory review migration passed with **98 active and 13 historical rolled-back ledger entries**, prior entries preserved and a second deploy confirmed no-op. Vercel builds retained migration-disabled behavior.
...
- The released list/review UI does not create missing estimates. The retained aggregate covers 92 historical complete V2 jobs, all unknown with zero references and unresolved identity; four other jobs failed and one was superseded. Prioritize genuine catalog/printing reference coverage, then qualified current-input research and source-evidence qualification. These counts are not unique active cards or measured accuracy.
