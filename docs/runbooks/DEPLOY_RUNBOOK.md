...
- Before Production rollout, prove the signed main-branch digest on isolated one-GPU A40 and RTX 4090 proof Pods without using any saved session or Production endpoint. Each GPU proof must use the exact committed Cubone fixture hashes, no Memory, the current rounded-card contract, and three independent empty TorchInductor/Triton caches. It must exercise exactly eight geometric prompts per side, produce compiler cache artifacts after the first Front prompt, and require byte-identical stable `{detectorVersion, defects, detectorEvidence, deterministic instrumentation counts}` hashes across all three repetitions. Compare A40 versus RTX 4090 hashes without rounding, tolerance, field deletion, fallback, or configuration changes; any mismatch or missing compiler/cache evidence stops the Production rollout.
