...
Current ATLAS release checkpoint — September 24 UTC / September 23 Pacific, 2026: application source `1b880d0690dd0b3a64a76103557815cf40675029` is live on private image `2039ae10…` / container `f7949d01…`, staff `dpl_27Dj9ngpLzn7yqw271e6Bo2KyhPJ` and public `dpl_DjGX2yStocGFYPf8PP1kgALuJtx2`. Batch, presentation and SoldComps are enabled; physical station remains disabled. Controls25/23/3, staff47/public112, unchanged histories/grants and public aliases are verified. Mark’s internal reviewer permission is active for 90 days through2026-12-23T01:32:51.855Z; genuine human acceptance remains separate. Application CI35939784180 and documentation CI35943986135 both passed all14 jobs. The preceding release and activation intents are consumed. See [current completion evidence](../atlas/audits/2026-09-24/completion-followup.md).

Current owner work — September 24 UTC: while Mark tests, three fresh Astra Extra High specialists implement native NFC locking, complete market/dealer acceptance, and integrate Inventory’s newer shared research/catalog knowledge. All NFC operations must stay directly on the Mac without a phone or GoToTags step; old PC code is not imported. Qualified source and real physical/publication/partner evidence remain distinct. New source work does not itself change the currently serving release above.

Previous ATLAS release checkpoint — September 23, 2026: workflow source `c0a42a24be387f4c8501649dce21ddf88fa30a02`, private image `sha256:3babc8548e5471cacc97eea2e7dfa93f6ba9c032adcbc1fcbeffa39a78395200`, container `4f11d48f9209970737ce30b41554ef20900bfdbf3a23e88457e7fa4d8fb2c425` started23:01:06UTC. Staff47 and exact additive grants, Staff24/STAFF SMS22/PublicReader2, direct TLS8/8 and all four public aliases are verified. Private configuration is `/opt/atlas/manual-workflow-20260923-09df/private.env`; batch/presentation=true, market/station=false. Caddy is unchanged. Use the [connected manual runbook](../atlas/runbooks/CONNECTED_MANUAL_RELEASE.md) and [current audit](../atlas/audits/2026-09-23/workflow-completion.md). All current stop/start/schema/control/promotion intents are consumed. Do not replay them or automatically restart a predecessor against schema47. Earlier dated checkpoints are historical.

> Historical connected manual candidate — September 12, 2026: source implementation and local Node20/Linux qualification are recorded in [the connected manual audit](../atlas/audits/2026-09-12/connected-manual-workflow.md). Use the [concrete candidate release procedure](../atlas/runbooks/CONNECTED_MANUAL_RELEASE.md) for separate manual schemas, private CPU/web bindings, storage acceptance and fresh-card checks. This candidate has not migrated or activated production; historical serving checkpoints below remain historical.

> Historical I acceptance outcome (2026-09-11, observed through 19:05 UTC): source `89c55d916130` remains deployed, but the owner’s fresh-card test failed. Automatic first-pair enrollment succeeded; the first run stopped in IDENTITY after database timeouts, and the next card is blocked by the one-distinct-card pilot allowance. A settled FAILED run also lacks manual takeover. The cohort is no longer empty. See the [architecture dissection and repair recommendation](../atlas/audits/2026-09-11/README.md). No application or production-state change was made by this audit; old-card recovery remains canceled.
...
The completed H sequence binds exact merged source/all14 main checks, additive readiness700/maintenance800, unchanged canonical role access, the closed native artifact, constructor-derived controls, same-transaction settled Ja maintenance adoption, private eight-field health and all four public aliases. The immutable maintenance audit is a deployment action with NULL actor, not a synthetic HUMAN/RECOVER command. Original intent/result/readback files must remain; completed migration, adoption, routing or promotion actions must not be replayed. Exact receipt hashes are in the [historical H release record](../atlas/WEBSITE_RELEASE.md#historical-h-deployment-and-evidence).

Native generation and packaging each ran once. The required permission-only seal set the 24 data files to0444, the two executables and12 directories to0555 while preserving all26 file hashes/inodes. The initial local nested-spawn timeout and bounded diagnostics remain recorded; the unchanged verifier subsequently passed within its original timeout, without database/provider clients or source modification. Do not attribute the local timeout to application code or conflate it with the original live runner stop. GET acceptance, actual signed-in fresh-card processing and later grading progress require their own observed results.
...
- The immutable worker image must retain `CUBLAS_WORKSPACE_CONFIG=:4096:8` and the current host-compiler contract: `CC=/usr/bin/gcc-14`, `gcc-14=14.2.0-19`, `libc6-dev=2.41-12+deb13u3`, GCC `14.2.0`, and target `x86_64-linux-gnu`. Exact-image CI must verify the matching OCI environment/labels and SBOM, run the normal entrypoint's pre-Python shared-object probe, directly exercise Triton's `_build`, reject a CuBLAS or CC override with exit `78`, and require the complete worker suite. Every new worker `/health` identity must include the same compiler path, packages, version, target, executable SHA-256, and validation provenance.
- Before changing RunPod, require zero jobs in progress and zero jobs waiting and record the prior exact digest for audit only. Change only the exact image/digest and GPU eligibility/order required for the approved homogeneous RTX-4090 pool; preserve ports, model/checkpoint, secrets, disk, scaler, capacity, and every unrelated setting. Keep at least one old healthy worker serving until current-image RTX 4090 capacity passes the complete health and known-corpus gates. Recheck the zero-job gate before permanently removing every old-release worker. No old image or non-RTX-4090 GPU may remain eligible as fallback.
- The worker startup policy is `speedster-rtx4090-only-v1`: after the existing CuBLAS and compiler validations and before checkpoint download or model construction, observe CUDA directly through the runtime and require exactly one visible GPU named `NVIDIA GeForce RTX 4090` with compute capability `8.9`. No optional environment declaration is GPU identity authority. Missing CUDA, zero or multiple visible GPUs, A40, and every other name/capability fail closed before readiness. Every serving `/health` identity must include that policy plus the same observed name, capability, and count.
- The owner-approved 2026-08-21 homogeneous-GPU release decision supersedes the obsolete A40-versus-RTX-4090 equality gate. The completed exact-digest RTX 4090 three-cold-cache Cubone proof remains standing evidence and must not be rerun for a guard-only image delta. Require the new signed main-branch image to pass the complete exact-image suite, including focused accepted/rejected GPU-policy tests, and then require current-image RTX 4090 health, repeated `/ping`, and the runbook-safe known-corpus verification. Never round, tolerate, delete evidence fields, select A40, or fall back to an old image/worker.
...
## Legacy Speedster Preparation Cutover
- Keep the legacy project's exact `main:false` Git deployment hold through source publication and CPU admission. The hold prevents a new Git deployment; it does not pause the currently serving website or Speedster requests.
- Before reopening fresh capture on the prepared-evidence web release, install `20260908010000_speedster_prepared_evidence_authority`, admit the reviewed signed CPU identity, and configure server-only `AI_GRADER_SPEEDSTER_PREPARATION_SERVICE_URL=https://prepare.atlasgrading.com` plus `AI_GRADER_SPEEDSTER_PREPARATION_SERVICE_API_KEY`. The key must match the CPU service's dedicated `ATLAS_PREPARATION_API_KEY`; it must not reuse the detector key. The origin must be exact, with no path, credentials, query, fragment or trailing slash, and preparation requests reject redirects. Missing or invalid CPU transport fails before source reads, dispatch claims or provider calls; valid settings cannot bypass the code-reviewed preparation identity.
- Preserve `AI_GRADER_SPEEDSTER_SERVICE_URL` and `AI_GRADER_SPEEDSTER_SERVICE_API_KEY` for detection, geometry, Color, map registration and trace work. The CPU host serves only preparation and health; it cannot replace the detector endpoint. Close new Speedster work, drain old web requests and provider jobs, preserve unfinished drafts/checkpoints, and verify both exact identities before the final web cutover. Restore automatic main deployment only for the reviewed, fully configured release after these gates pass.

## Historical ATLAS isolated workspace deployment — September 10, 2026

- Current ATLAS state and immutable bindings are recorded in [WEBSITE_RELEASE.md](../atlas/WEBSITE_RELEASE.md). Merged C `89478881` passed all14 PR/main checks and39 private tests. Staff/public C are READY with26/6 accepted environment bindings; signed private C is running with health and15/15 private HTTPS/denial checks passed. CPU A2 and95 public/22 staff migrations plus three scoped roles remain accepted. Preserve stopped B under its archival name with all startup/route intents, diagnostic receipts and archived bindings; do not rerun B or C one-shot startup actions. StaffControl7/workspace1/source1/STAFF SMS5 are committed and verified by read-only reconciliation. All four public aliases and the production target are verified on C; public/admin/account HTTPS checks and the www redirect pass.
- Preserve the original pilot, customer/legacy bindings and legacy `main:false` hold. C source, signed private artifact, matched staff/public bindings and private HTTPS acceptance are recorded. Preserve the committed intake intent/result and do not replay its SQL. The historical STAFF SMS timestamp hold was resolved by read-only reconciliation and all five canonical privilege checks. Public promotion is complete; retain its intent/result and do not replay the action. The unlocked Mac and authenticated C staff portal were verified; complete real-photo workflow acceptance remains pending. The September 10 owner correction permits the first admitted verified photograph pair to start after exact runtime and fresh cost/hold checks; the explicit workspace pilot roster may contain one through ten real cards. Intake remains open up to ten cards, while initial processing remains one distinct card; the owner's reviewer certification remains null, and Mac physical/NFC acceptance remains separate.
- Caddy is manually attached to `atlas-workspace-private-20260910` with gateway priority `-1`; shared infra Compose is unchanged. Recreating Caddy can lose this attachment. After any recreation, inspect the actual container and its existing networks/default route, reconnect this exact network only if absent, then recheck the default route and ATLAS HTTPS identity/denials before returning traffic. A graceful reload of the existing container preserves the attachment. Do not replace CPU A2 or unrelated services to repair it.
