...
- Current ATLAS state and immutable bindings are recorded in [WEBSITE_RELEASE.md](../atlas/WEBSITE_RELEASE.md). The signed CPU A2 service and additive database/role changes are accepted. Private B stopped before listening; preserve its stopped container, startup/route intents and diagnostic receipts when releasing its successor. Do not rerun the B one-shot actions. Staff/public B are staged; intake and public promotion remain pending.
- Preserve the original pilot, customer/legacy bindings and legacy `main:false` hold. Require the actual main source, signed private artifact, matched staff/public bindings and private HTTPS acceptance before the guarded intake transaction and public promotion. Keep paid dispatch closed until the ten real photograph pairs, exact card identities and current cost holds are reconciled.
