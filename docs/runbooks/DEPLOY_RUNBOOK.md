...
Use the exact-source identities in [main-site release](../plans/2026-09-16-main-only-release.md), [Inventory delivery](../plans/2026-09-21-inventory-takeover/README.md) and [automatic recovery](../plans/2026-09-21-inventory-research-recovery.md). Current Production is `9e8a9c1d99cce0b4b8a4a5dbde241fc916cdb439` / `dpl_6GK1WtpAY19dBQh5csQbZCR6qP8B`, immutable `tenkings-backend-nextjs-ewysxk0rw-ten-kings.vercel.app`; all five aliases / project target / sole cron verified September 22 03:33:39 UTC (September 21 local). Stable main Preview serves the same source through `dpl_3BwdAS3wHjkGD17M7ZY6r2DJvBpv` (`ocp9wiuex`). Prior bc92b102/FXL/Hie34 are historical comp-review release evidence. This release did not restart or deploy ATLAS.

Both exact-source 79-page / Linux Sharp 0.34.5 / Fluid 300 builds passed. Production 28 route / six asset postflight passed at build `b9EpiUYH5zneUEWK0NAfB`; Preview six staged routes / four assets and three stable-alias checks passed. Natural cron requests on the final 6GK artifact returned HTTP 200 at 03:34:37.681 and 03:35:37.269 UTC; the initial recovery release also completed the 98-card first pass. Separate read-only canonical outcomes establish missing-job discovery and initial saved recovery assessments; request logs alone do not prove card results. Only `STAFF_INVENTORY_RESEARCH_RECOVERY_ENABLED=true` is enabled in Production. Optional catalog, sale-detail, full-resolution, contribution and diagnostic execution remain off. The separately qualified recovery migration passed with **99 active and 13 historical rolled-back entries**, exact old ledger/schema/research evidence preserved, and a no-op second deploy. Vercel builds skipped migrations.

Recovery reuses the existing schedule and job table. Do not add a cron, reset retry budgets or invoke a blind bulk rerun. The flag can pause automatic checks and already queued automatic attempts; changing it requires a new qualified serving artifact. Preserve the additive schema/history during any application rollback. Source candidate links are unreviewed; they do not authorize catalog publication or card prices.
...
- Keep `RUN_DB_MIGRATIONS=false` unless a separately qualified migration is intended. The original September 21 domain cutover required no migration; the later comp-review and recovery releases used separately qualified additive migrations. Preserve collect permanent links, mail/TXT/service records and original-origin pending commands. After a saved staff review exists, recovery must retain response-version-2 reviewed-value semantics rather than restoring an older reader that ignores decisions.
- Phone workflow and new-website acceptance are complete on Mark's September 21 confirmations. Do not reopen those owner acceptance requests. Exact Preview database availability is branch-specific: successful builds and environment pulls do not establish signed-in data access.
- The released list/review UI does not create missing estimates. The preactivation canonical replay verified 92 distinct current unknown individual cards, four failed and two missing current jobs. Automatic recovery now checks those and future cards. Prioritize genuine catalog/printing reference coverage and qualified sold evidence; activation does not establish 92 prices or measured accuracy.
