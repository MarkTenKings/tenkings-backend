...
## Inventory/main release addendum — September 22, 2026

Current Production is exact `db465c1c57659233d4841f8679cca0aac3ecdd8f` / `dpl_CyDxyASkDk2HGGRwjDtCGqovtLQX`, immutable `tenkings-backend-nextjs-41olp9f3x-ten-kings.vercel.app`. At 15:35:59 UTC, direct reads verify all five aliases, project Production target and sole enabled minute cron on this artifact. The 15:36:22 UTC postflight passed 28 routes and referenced assets, build `HdwiGQMJw0yDYUgMuhh83`. Stable main Preview remains execution-disabled `dpl_2VJgRGbJA1TUm9iikGZQvEX6WtLy` (`p6s7ca002`) at the same source. Earlier 6GK/3Bwd identities and outcomes remain historical evidence in the [recovery record](../plans/2026-09-21-inventory-research-recovery.md) and [main release](../plans/2026-09-16-main-only-release.md).

Exact-source hosted builds passed 79-page generation, native Linux x64 Sharp 0.34.5 and Fluid 300 checks; Production explicitly skipped migrations. Production recovery and `STAFF_INVENTORY_RESEARCH_SALE_DETAILS` are true. Catalog/admin-consumer, full-resolution, contribution and diagnostic execution remain off. This release made no new schema migration, host restart or ATLAS deployment; the previously verified ledger remains 99 active plus 13 retained rolled-back entries. Root released the Inventory coordination hold after the first canonical preservation snapshot. ATLAS retains its own release authority.

V6 separates candidate retrieval from match/value qualification. Missing internal catalog coverage or verified originals cannot block a sensible query from saved facts. Original-photo, exact-printing, grade/condition, affirmative sale, known price, distinct-image and arithmetic checks still govern selected comps and values. Recovery uses the existing schedule/job table, keeps prior reviewed decisions and all histories, and preserves finite budgets and intake headroom. Do not add a cron, reset retries, fabricate edits or trigger a blind bulk rerun. Optional ordinary-sale details retain at most two item IDs across three searches, six seconds each, cached failure and no detail retry; an unknown accepted-offer amount cannot become the advertised price.

**Compatibility boundary — already crossed:** the 15:48:10.922 UTC snapshot contains 36 v2 recovery states and nine v6 results. A v2 claim can persist `behavior_revision` before paid research or an estimate, making legacy recovery readers unsafe. Do not route reads or cron back to 6GK or another legacy artifact. Roll forward with v6-compatible code; preserve additive schema and every input/result/attempt/retry/recovery/review record. A newly qualified artifact with `STAFF_INVENTORY_RESEARCH_RECOVERY_ENABLED=false` pauses recovery checks and queued automatic refresh attempts. It is not a global ordinary-research stop switch. Environment metadata edits do not alter an already serving artifact.

The first canonical snapshot at 15:36:24.434 UTC retains all 99 baseline jobs and original result/history bytes: 98 eligible individual cards, 11 ineligible shared-bulk units, 92 legacy unknown results, four failed and two queued jobs. No v6 result or appended research attempt existed yet. This is preservation proof, not backlog completion. Use natural cron observations and separate bounded canonical snapshots for actual outcomes; HTTP success alone proves neither a processed card nor an estimate. The final Snivy diagnostic remains unknown with zero matched/selected comps and a retained P2 optional duplicate-observation failure. Source/local/hosted qualification does not establish all-card accuracy or hosted load performance.
...
Private evidence under the TenKingsInventory investigations root:

- `20260922-v6-timing-production-preparation/postpromotion-metadata-153601.json`, SHA `6797af97d5c192112d49bf080ae82039b273c58b2d0d0979b03520ccf4a75e5c`; sites receipt `postpromotion-sites-153624.json`, SHA `e7c51549fa101dac3a9337b43a0f219827124be6e9c75e829bbe6d54d276be12`.
- `20260922-v6-timing-postpromotion-outcome-preparation/runs/first-outcome/outcome.json`, SHA `d08fc7d71dc27c1c1acf7a1222766d13a4628ec714836c28a83d9bbb455d53e2`, compared against baseline `c7b81c33fa5f819d2d6279be87187f8f00f03cc74e34f84610bd91218311bde3`. The sealed runner uses one fixed-fingerprint RepeatableRead READ ONLY snapshot, no providers/host files/manual cron, and no automatic retry. It separately counts all current eligible cards, the original 98 and historical 92, retaining engine-unknown null-result attempts as unknown.
...
- Build from the intended Git commit and its exact Production/Preview environment. The sparse main worktree and older checkout holding the Vercel project link are not upload sources.
- Before/after staging and promotion, verify actual `/v4/aliases/{hostname}` destinations, project target and sole cron owner. Staging with `autoAssignCustomDomains:false` has previously moved generated aliases; project/deployment alias arrays can be stale.
- Qualify and explicitly assign the intended stable Preview if its actual alias stays on an older artifact. Diagnostic execution is closed only once that host serves the qualified disabled artifact; preserve terminal recovery through a normal signed-in read.
- Keep `RUN_DB_MIGRATIONS=false` unless a separately qualified migration is intended. Preserve collect permanent links, DNS/mail/service records, pending commands and staff review semantics.
- Model time is capped at 120 seconds and remaining 150-second engine time minus 10 seconds finalization. The optional round remains 84 seconds; the diagnostic invocation remains 170 seconds and worker lease 180 seconds. These limits and request caps do not guarantee a price or verified billed cost.
- Phone and website acceptance remain complete. Do not reopen those owner requests. A successful build does not establish authenticated database access, and a diagnostic never updates the saved research job.
