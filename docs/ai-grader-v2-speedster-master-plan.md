...
## 2026-08-18 evidence-bound Production hardening supplement

- Physical geometry has one explicit authority chain. The current outline engine displays its best complete structurally valid four-corner proposal for human confirmation; mat-color coverage, per-side percentages, contrast, frame coverage, and runner-up similarity are diagnostics only and cannot hide that proposal. If the current engine finds no complete outline, the operator places four corners on the visibly rendered exact source. The shared browser/API/Python validator still rejects out-of-range, crossed, concave, collapsed, or reordered quads. No generic contour, fixture rectangle, coordinate clamp, hidden transform, retired Color engine result, or stale URL may become current geometry authority.
- Printed-frame centering uses only an accepted current-engine proposal or four explicit human points on the visibly rendered prepared image. It never invokes the retired edge finder and never substitutes a default `5 mm` border for a missing side.
- Exact-source URLs and Card Map lookup are bounded requests with visible retry. A loaded map requires valid independent Front and Back registration to its exact immutable revision/hash/scope. Registration failure or lookup failure cannot silently become mapless authority, but the operator may make a separate durable `HUMAN_REVIEW_WITHOUT_MAP` decision after an exact recorded `LOOKUP_FAILED` or `REGISTRATION_BLOCKED` event. That decision preserves the failure/revision/side evidence, removes map registrations and projected zones, survives reload, and permits normal human centering; it never bypasses an integrity error. A genuine authoritative `NO_MAP` result remains distinct from both failure and explicit human continuation.
- Retired runtime paths are not current-grade fallbacks. New grades reject legacy fixed-key iPhone pairs, synthesized registration-orchestration metadata, retired Color Geometry output, contour-only detector findings, and Legacy Memory V1 decisions. Historical Color, Card Map, and completed-grade evidence remains readable and immutable; eligible persisted Card Map schema history remains authoritative data, not a substitute detection engine.
- Detector and Memory evidence is complete before zone filtering. Raw masks/candidates, Memory decisions, Smart Marks, removal dispositions, exact source/prepared hashes, request trace, model/release identity, and filtered results remain separately auditable. Exact and Family maps never merge.
- Initial originals, iPhone pairs, prepared evidence, and completed slab photos are private. New original/slab keys are content-addressed; iPhone PLAN and COMPLETE bind exact Front/Back SHA-256 and byte sizes. Finalization verifies content type, length, and either provider-native SHA-256 or the same bounded server-streamed bytes before readiness or persistence.
- Detection is recoverable per side without weakening authority. Each Front/Back checkpoint binds the session revision, complete six-asset hash set, one Memory snapshot/hash, operation/request identity, exact detector result/hash, detector release/model identity, and an HMAC receipt. A saved Front may be reused after Back fails only when every binding revalidates; the pair commits under the original session revision CAS.
- The sole SAM 3 release is immutable: source commit `96914d2425f90a64f45ca977c2b5165418099543`, official model repository revision `3c879f39826c281e95690f02c7821c4de09afae7`, reviewed checkpoint SHA-256 `9999e2341ceef5e136daa386eecb55cb414446a00ac2b55eb2dfd2f7c3cf8c9e`, and pinned Python base digest `sha256:2c941e860699f878900b0edc2403613c234d4b32eda3cc9fa7036991a2a63c4a`. Production startup must download from the official gated source and verify the exact bytes; a mismatch is a blocker, never a substitute-model path.
- Release CI uses Node 20, pnpm `9.12.0`, a frozen lockfile, commit-pinned actions, the complete protected Speedster frontend/adversarial suite, the full service suite inside the exact image, and the disposable migration chain before publication. The tested run-specific image is signed, SBOM-attested, provenance-attested, verified, and deployed only by OCI digest.
- Audit evidence tables reject UPDATE, DELETE, and TRUNCATE. General admin routes are bearer-session only. Static operator authority is server-only and capability-scoped to the five Set Ops batch-import dependencies; it is never browser configuration or a general admin bypass.
- Production acceptance requires the exact web commit and exact signed OCI digest, the additive migrations, official-checkpoint GPU startup, identical `/health` identity on at least two workers, repeated known-corpus detection, explicit fail-closed mismatch probes, and a human natural-card acceptance pass. Synthetic/unit evidence must never be described as calibrated accuracy or final live-card proof.
