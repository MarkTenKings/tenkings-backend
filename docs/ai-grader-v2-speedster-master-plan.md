...
- Physical geometry has one explicit authority chain. Color Geometry may provide either an accepted editable proposal or a clearly labelled rejected diagnostic draft; otherwise the operator places four corners on the visibly rendered exact source. The shared browser/API/Python validator rejects out-of-range, crossed, concave, collapsed, or reordered quads. No generic contour, fixture rectangle, coordinate clamp, hidden transform, or stale URL may become geometry authority.
- Exact-source URLs and Card Map lookup are bounded requests with visible retry. A loaded map requires valid independent Front and Back registration to its exact immutable revision/hash/scope. Registration failure, integrity error, or lookup failure cannot silently continue under mapless authority. A genuine authoritative `NO_MAP` result remains distinct from a failed lookup.
