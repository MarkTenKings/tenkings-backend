...
- PR/deploy status:
  - PR #49 permanent auth merged at `2026-07-03T08:55:08Z`, merge commit `e786e400343e658f8a5bad739df7e686827eca24`, and Vercel Production deployment succeeded.
  - Mark configured the permanent-auth Vercel Production env vars and redeployed; runtime status then reported human operator/admin auth configured, service account configured, and `serviceAccountScopeCount=5`.
  - PR #50 public-output security fix merged at `2026-07-03T21:20:02Z`, merge commit `aee7ac9cd6c28a0a5474fe1297c70ad9ad0288e3`, with Vercel Production deployment success updated `2026-07-03T21:21:03Z`.
- Production status endpoint on `collect.tenkings.co` returned publish enabled, public DB reads enabled, live comps enabled, human operator/admin configured, service account configured, five service scopes configured, and `noHardwareControls=true`.
- Fresh hosted production smoke through the permanent service-account auth path succeeded:
...
- Storage/public report smoke:
  - Hosted publish returned `200`, `publicationStatus=published`, `uploadedAssetCount=7`, and `evidenceAssetCount=7`.
  - Expected storage artifacts were uploaded and then read back through the public API: `report-bundle.json`, `production-release.json`, `label-data.json`, `publication-manifest.json`, `integration-contract.json`, `asset-manifest.json`, and `label-preview.html`.
  - Public report API returned `200`, matched report ID/cert ID, exposed read-only output, and showed label/QR generated from persisted data.
- DB persistence smoke:
  - Verified through hosted production write/read API surfaces rather than direct SQL: publish wrote the AI Grader session/report/evidence/grade/label/publication surfaces, slab uploads added evidence rows, comps persisted valuation status, and history/latest read the persisted smoke report.
  - No direct production SQL row-count query was run in this final pass and no `DATABASE_URL` value was printed.
- Slabbed photo smoke:
  - Safe front/back fixture PNG uploads returned `200`.
  - Public report merged two `slabbed_photo` assets with sides `front` and `back`; Basler monochrome evidence was not overwritten.
- Card/item linkage:
  - Used safe manual draft identity: `1990 SkyBox Michael Jordan #41 AI Grader Smoke Draft`.
  - No `CardAsset` or `Item` production inventory row was modified; update counts were `0`.
- eBay/SerpAPI comps:
  ...
  - The selected query returned zero comp refs/no valuation amount; public report still merged valuation status as `completed`.
- Operator station/history:
  - Production station route remained reachable.
  - Authenticated history returned `200`; the fresh report was found at index `0`, and latest report ID matched `ai-grader-prod-smoke-20260703T212555`.
...
- Public output security:
  - Post-PR #50 scan of public JSON/HTML found no `DATABASE_URL`, storage key/secret markers, SerpAPI key markers, service token, token hash, Dell local paths, bridge URLs/tokens, or hardware controls.
- Validation after rollout/security fix:
  ...
  - `git diff --check` -> pass.
- Result: AI Grader is production-live for hosted publish, storage-backed public report/label/QR, slabbed photo uploads, manual identity, live comps execution path, history/latest, and security checks under the permanent auth model.
- Remaining follow-up:
  - DigitalOcean managed Postgres credential rotation remains deferred by Mark and still needs to be completed after rollout.
  - If a non-empty eBay valuation is required as a business acceptance proof, run one additional controlled comps smoke with an approved card/query likely to return sold comps.
- Guardrails held: no secret value was printed, no shell tracing was used, no destructive DB operation was run, no hardware/capture command was run, no temporary auth bypass was added, and no production untracked file was deleted or modified.
...
- Local runbooks do not contain a Ten Kings-specific DigitalOcean managed database password rotation command. Official DigitalOcean PostgreSQL docs identify the approved manual path as resetting the affected database user's password from the database cluster `Users & Databases` tab, or using the documented database user API with an authenticated DigitalOcean token.
- Stop condition remains active until Mark rotates the exposed production Postgres credential for the user referenced by the current production `DATABASE_URL` and provides/installs the rotated connection string through approved runtime env management. Do not paste the value into chat or logs.
- Redacted droplet runtime checklist from key-only inspection:
  - `env/auth-service.env` -> `DATABASE_URL`
  - `env/bytebot-lite-service.env` -> `DATABASE_URL`; `SERPAPI_KEY` present
  - `env/ingestion-service.env` -> `DATABASE_URL`
  - `env/marketplace-service.env` -> `DATABASE_URL`
  - `env/pack-service.env` -> `DATABASE_URL`
  - `env/pricing-service.env` -> `DATABASE_URL`
  - `env/vault-service.env` -> `DATABASE_URL`
  - `env/vending-gw.env` -> `DATABASE_URL`
  - `env/wallet-service.env` -> `DATABASE_URL`
  - `env/nextjs-app.env` -> `CARD_STORAGE_MODE`; no `DATABASE_URL` key found in this file
- Running compose service checklist from key-only inspection:
  - `auth-service`, `bytebot-lite-service`, `ingestion-service`, `marketplace-service`, `pack-service`, `pricing-service`, `vault-service`, `vending-gw`, and `wallet-service` currently expose `DATABASE_URL` in the container environment.
  - `bytebot-lite-service` currently exposes `SERPAPI_KEY`.
  - `caddy`, `postgres`, and `ocr-service` did not expose the checked DB/AI-Grader/storage/SerpAPI keys.
- AI Grader gates were not found in checked droplet env files/running service envs:
  - `AI_GRADER_PRODUCTION_PUBLISH_ENABLED`
  - `AI_GRADER_PUBLIC_REPORT_DB_ENABLED`
  - `AI_GRADER_PUBLIC_REPORT_BASE_URL`
  - `AI_GRADER_PRODUCTION_TENANT_ID`
  - `AI_GRADER_EBAY_COMPS_ENABLED`
- Local and droplet `vercel` CLI/project linkage was not available. Vercel production env updates, if required for DB-backed public report reads, must be done from the approved Ten Kings Vercel project target; do not guess the project/env.
- Local and droplet `doctl` were not available. DigitalOcean password reset must be completed by Mark through the dashboard or an approved authenticated API path outside this transcript.
...
## Session Update (2026-06-09, AI Grader Dino-Lite demo capture package)
- Branch `feature/ai-grader-dinolite-capture-package` was created from latest `origin/main`.
...
- Added capture-helper manual CLI commands:
  - `dinolite-capture-package --bridge-exe <path> --adapter dnvideox --device-index 0 --output-dir C:\TenKings\capture-data\dinolite-packages --label card-demo-001 --include-lighting-sweep --include-edr --include-edof`
  - `dinolite-capture-demo-package --bridge-exe <path> --adapter dnvideox --device-index 0 --output-dir C:\TenKings\capture-data\dinolite-demo --label card-demo-001`
- Default readiness, health, server/admin paths, and tests still do not instantiate DNVideoX or spawn the real adapter.
- Capture package outputs are required to be outside the git repo and include `manifest.json`, `preview-report.html`, image metadata, checksums, cleanup status, and the limitation text `Dino-Lite capture package preview -- not a certified grade.`
- Manual real DNVideoX demo package smoke was run after automated validation:
  - output folder outside git: `C:\TenKings\capture-data\dinolite-demo\dinolite-card-demo-001-20260609T215851704Z`
  - device `Dino-Lite Edge`; device ID redacted except USB VID/PID `vid_a168&pid_0990`
  - OCX version `3, 0, 56, 6`
  - config bitfield `198`; decoded using SDK bit layout as `edof=true`, `amr=true`, `ledMode=1`, `led=true`, `flc=true`, `axi=false`
  - AMR `1`
  - normal still `normal-still.jpg`, `sha256=9aa3a6577a3426a97955648aedefc0495c759426e93e49a09b2b7639c4c60e06`, `byteSize=67582`, `mimeType=image/jpeg`
  - lighting sweep succeeded with `all-leds-on-normal`, `flc-all-level-3`, `flc-quadrant-1-level-4`, and `flc-quadrant-2-level-4` JPGs
  - EDR succeeded after file-read polling: `edr.jpg`, `sha256=63654191144d854e853959f6a4f4fe9d029d5a64e2472d2deffb26ad2d3ba71c`, `byteSize=513420`
  - EDOF remains unavailable: `SaveEDOF(0, 3, path)` returned SDK result `1`, but no `edof.jpg` appeared after 15 seconds of polling; recorded `SaveEDOF_NO_OUTPUT_TIMEOUT`
  - runtime dependency diagnostics showed `enfuse.exe`, `SMIUtility.dll`, and `d3dx9_31.dll` absent from the bridge executable directory and current working directory; SDK inventory lists these helper files outside git, and they were not copied into the repo
  - cleanup: `previewStopped=true`, `disconnected=true`, `hostDisposed=true`, no cleanup errors; final FLC restore succeeded via `SetFLCLevel(0,3)` and `SetFLCSwitch(0,15)`
- Validation passed:
  - `dotnet build packages\ai-grader-dinolite-bridge\DinoLiteBridge.sln -p:Platform=x86 -p:Configuration=Release` -> pass, 0 warnings, 0 errors
  ...
  - `pnpm --filter @tenkings/nextjs-app build` -> pass with existing `<img>` warnings
  - `git diff --check` -> pass with line-ending warnings only
- Guardrails held so far: no production/staging migration, no `RUN_DB_MIGRATIONS=true`, no manual deploy, no runtime DB operation, no `regsvr32`, no SDK binary/OCX/DLL/vendor SDK commit, no captured image commit, no `GetRGB`/`GetEDRRGB`/`GetEDOFRGB`, no lens/focus/exposure-setting/DPQ method, and no certified grade/report claim.
...
- Added manual-only DNVideoX enumeration through the Windows .NET Framework 4.8 x86 STA bridge:
  - JSONL command `dinolite.enumerateDevices`
  - fake adapter deterministic enumeration payload
  - real adapter opt-in only with `--adapter dnvideox --manual-enumerate`
  - COM ProgID `VIDEOCAPX.VideoCapXCtrl.1`
  - allowed calls only: `GetVideoDeviceCount`, `GetVideoDeviceName`, optional `GetVideoDeviceDesc`, optional `GetDeviceID`
- Added capture-helper manual CLI command:
  - `dinolite-enumerate --bridge-exe <path> --adapter fake|dnvideox`
  - missing bridge exe/config fails before spawning
  - `dnvideox` spawn is restricted to manual enumeration only
- Default readiness, health, local transport, admin UI paths, and tests do not instantiate DNVideoX or spawn the real adapter.
- Manual fake enumeration smoke passed with `comActiveXInstantiated=false`, `connected=false`, `preview=false`, `deviceCount=1`, and fake AF7915MZTL-like device metadata.
- Manual real DNVideoX enumeration smoke was run once on the Dell Windows capture node after automated validation:
  - command used explicit bridge exe path, `--adapter dnvideox`, and `--bridge-timeout-ms 10000`
  - DNVideoX ActiveX instantiated: `comActiveXInstantiated=true`
  - OCX version reported: `3, 0, 56, 6`
  - result: `SDK_NOT_READY`, `deviceCount=0`, `devices=[]`
  - error code: `DNVIDEOX_ENUMERATION_FAILED`
  - error message observed before error-unwrapping cleanup: `Exception has been thrown by the target of an invocation.`
  - `connected=false`, `preview=false`, `forbiddenOperationsInvoked=false`
- Follow-up diagnostic on PR #27 found the root cause: DNVideoX enumeration requires an ActiveX host/control site. The SDK C#, VB6, HTML, and C++ samples host DNVideoX as a UI ActiveX control; plain COM activation can instantiate the OCX and read version info but does not initialize enumeration correctly on this Dell PC.
- Windows read-only device inspection sees the microscope:
  - `Dino-Lite Edge`
  - class `Camera`
  - status `OK`
  - USB VID/PID `VID_A168&PID_0990`
- SDK sample app was not run because the sample source includes normal UI paths that set `Connected=True`, `Preview=True`, light, and capture/control methods. Source inspection confirmed that its read-only initial enumeration happens from a hosted WinForms ActiveX control in `Form1_Load`.
- Bridge fix: real manual enumeration now creates DNVideoX inside a hidden offscreen WinForms `AxHost`, then calls only the allowed read-only enumeration methods.
...
- Validation passed so far:
  ...
  - `pnpm --filter @tenkings/ai-grader-simulator test` -> pass, 6 tests
  - `pnpm --filter @tenkings/nextjs-app build` -> pass with existing `<img>` warnings
- Guardrails held: no `Connected=True`, no `Preview=True`, no capture method, no LED/FLC/lens/focus/exposure/EDR/EDOF/DPQ/control command, no `regsvr32`, no production/staging migration, no `RUN_DB_MIGRATIONS=true`, no manual deploy, no runtime DB operation, and no SDK binaries/OCX/DLL/vendor SDK files were committed.
...
- Added a shared serial-line transport abstraction reused by Arduino LED readiness and GRBL stage readiness; `serialport` remains dynamically imported only after an explicit port-backed readiness path is invoked.
- Added manual CLI support: `tk-ai-grader-capture-helper stage-health --port <serial-port> --baud 115200`.
- Extended readiness so `driverSet=real` plus `stage=grbl` plus an explicit port can include GRBL stage health; missing port fails closed without opening serial.
- Added fake-serial tests for GRBL status success, timeout failure, malformed response failure, default no-open readiness, missing-port fail-closed behavior, and no emitted motion/enabling command strings.
- Validation passed locally:
  ...
  - `pnpm --filter @tenkings/ai-grader-simulator test` -> 6 tests
  - `pnpm --filter @tenkings/ai-grader-capture-helper build`
  - `pnpm --filter @tenkings/ai-grader-capture-helper test` -> 46 tests
...
- No real serial or hardware command was run; all GRBL checks in this implementation used fake serial.
- No GRBL homing, motion, jogging, unlock, reset, spindle, coolant, `$H`, `G0`, `G1`, camera/microscope access, image acquisition, uploads, grading math, auth algorithm, report, or PDF work was run or added.
...
## Session Update (2026-05-29, AI Grader admin API scaffold PR #13 merged)
- PR #13 merged into `main`: `https://github.com/MarkTenKings/tenkings-backend/pull/13`.
- Merge commit/final post-merge `origin/main` HEAD before this handoff docs update: `c932a37308b9fd852b5ab33845d00eb5699b0ac7`.
- GitHub Actions post-merge run `26630385563` succeeded, including Install & Build and Docker image jobs for frontend, wallet, vault, marketplace, pricing, pack, ingestion, and vending-gw.
- Vercel production deploy completed successfully: `https://vercel.com/ten-kings/tenkings-backend-nextjs-app/5yVYkcywydX4xWNSPftG9oVwEDVW`.
- Migrations remained skipped by the Vercel build gate because `RUN_DB_MIGRATIONS` was not set to `true`.
- No migrations, `RUN_DB_MIGRATIONS=true`, manual deploy command, runtime DB operation, deploy restart, or next AI Grader phase work was run during merge verification.
- Next approved slice is the feature-gated AI Grader admin UI shell; continue to avoid hardware/capture, image-processing/grading math, CMYK/auth algorithms, reports, PDFs, migrations, runtime DB operations, unsafe public routes, and DB-backed UI calls unless the existing API feature gate allows them.

## Session Update (2026-05-29, AI Grader admin UI shell)
- Part A completed before this branch:
  - PR #13 merged with merge commit `c932a37308b9fd852b5ab33845d00eb5699b0ac7`.
  - PR #13 post-merge GitHub Actions run `26630385563` and Vercel production deploy `https://vercel.com/ten-kings/tenkings-backend-nextjs-app/5yVYkcywydX4xWNSPftG9oVwEDVW` succeeded.
  - handoff docs follow-up commit `7b00cebfa83ec3a0fee478d6389008d2cf52b409` was pushed to `main`; its GitHub Actions run `26630731433` and Vercel production deploy `https://vercel.com/ten-kings/tenkings-backend-nextjs-app/GssxhKsgZNPon4czJDrRaPNhJpfU` succeeded.
  - migrations stayed skipped by the Vercel gate because `RUN_DB_MIGRATIONS=true` was not set.
- Branch `feature/ai-grader-admin-ui-shell` was created from `origin/main` at `7b00cebfa83ec3a0fee478d6389008d2cf52b409`.
- Added admin-only shell page `/admin/ai-grader` and linked it from the admin console Monitoring section.
- The page uses existing admin session/admin-id/admin-phone gate helpers before loading AI Grader status.
- The UI automatically calls only `/api/admin/ai-grader/status`; create/update operation cards remain disabled unless the feature-gated API returns `enabled: true`.
- Added thin frontend client/helpers for status loading, operation POST paths, disabled response handling, validation error mapping, and admin gate state.
- Added mocked client/gate tests covering disabled status, enabled status, validation errors, and admin gate behavior.
- Files changed:
...
## Session Update (2026-05-30, AI Grader simulated session workflow PR #18 merged)
...
- PR #18 added the no-DB/no-hardware simulated STANDARD admin workflow behind the existing AI Grader API and simulator gates.
...
## Session Update (2026-06-16 UTC / 2026-06-15 ET, AI Grader Dino-Lite report diagnostics PR #33 indoor supervised run)
- Continued PR #33 on branch `feature/ai-grader-dinolite-report-diagnostics`; PR remains unmerged.
- Mark confirmed the current Dell USB hub/dock Dino-Lite connection is acceptable for this PR #33 supervised indoor workflow because prior Dino-Lite preview/capture workflows worked through the same connection. The direct-to-Dell USB precondition was waived for this run.
...
- Verified output files:
  ...
  - `preview-report.html` exists
  - 12 normal JPG artifacts exist
...
- Mark template confirmation status:
...
- Added manual-only Leimac IDMU-P Ethernet read-only readiness/status support to `@tenkings/ai-grader-capture-helper`:
  ...
  - Results include raw request frame, raw response text, defensive parsed fields only when confident, controller host/port/timeout/unit metadata, and safety flags `writesAllowed=false`, `lightsCommanded=false`, `outputSettingsChanged=false`, and `triggerSettingsChanged=false`.
  - Follow-up audit corrected the initial command-frame assumption: Leimac ASCII order is header + command number + target/unit + data, not header + unit + command.
  - Exact-frame follow-up now sends the manual ASCII frame itself with no implicit CR/LF terminator.
...
- One read-only hardware smoke was attempted against Mark-approved candidate `169.254.191.156:1000` after command-order audit and before the final exact-frame/no-terminator patch. The CLI attempted status read `requestAscii=R0801`; that build still wrote the internal CR/LF-terminated frame. The controller returned raw response `WR00NAK`, parsed as NAK/incorrect header or command, readiness `FAIL`. No retry and no other IP/port probing was run.
- Mock protocol coverage after command-order audit uses read frames `R0801`, `R0800`, `R1601`, `R47`, `R8001`, and `R83`; no write frames are generated by runtime paths.
- Test-only manual write example coverage confirms `W 01 01 0001` serializes as `W01010001`, while runtime/hardware paths still reject `W` commands.
...
## Session Update (2026-06-23 UTC, AI Grader Leimac IDMU-P PR #35 final readiness)
- Continued PR #35 branch `feature/ai-grader-leimac-idmu-readiness`.
- Added safe manual diagnostic CLI command `leimac-idmu-read-frame --host <ip> --port 1000 --frame <R...> --timeout-ms <ms>`:
  - requires explicit host and frame
  - uses no implicit CR/LF terminator
  - rejects frames not starting with `R`
  - rejects any frame containing `W`
  - rejects non-uppercase-ASCII-alphanumeric frames
  - rejects frames longer than `32` characters
  - rejects command numbers outside read allowlist `08`, `16`, `47`, `80`, `83`
  - performs no scan and no automatic retry
- Added dry-run-only trigger-sync plan command `leimac-idmu-trigger-sync-plan --mode basler-exposure-active-to-trg-in1`:
  ...
  - required cable `CEBR119 or CEBR120`
  - required trigger supply `5-24 VDC`
  - safety flags `dryRun=true`, `writesApplied=false`, `lightsCommanded=false`, `baslerSettingsChanged=false`, `leimacSettingsChanged=false`
- Vendor PDFs requested under `C:\TenKings\vendor-docs\leimac` were not present on the Dell node. The same PDFs were present and reviewed from `C:\Users\Mark\Downloads`; they were not copied into the repo or committed.
- Final code/tests now confirm command-before-target/unit framing and no implicit terminator. Unit information framing was updated from ambiguous `R83` to hardware-confirmed `R830000`.
- Final expected/tested Leimac frames:
  - unit status: `R0801`
  - system status: `R0800`
...
- Validation:
  - `pnpm --filter @tenkings/ai-grader-capture-helper build` -> pass.
  - `pnpm --filter @tenkings/ai-grader-capture-helper test` -> pass, 102 tests.
  - `pnpm --filter @tenkings/shared test` -> pass, 105 tests.
  - `pnpm --filter @tenkings/ai-grader-simulator test` -> pass, 6 tests.
  - `pnpm --filter @tenkings/nextjs-app build` -> pass with existing `<img>` warnings.
  - `git diff --check` -> pass with line-ending warnings only.
- Passive Windows inventory only:
  - `Ethernet 2` / `Realtek USB GbE Family Controller #2` is `Up` at `1 Gbps`, MAC `00-05-1B-DF-02-39`, IPv4 `169.254.215.165/16`.
  - ARP/neighbor table on `Ethernet 2` shows Basler `169.254.68.71` / `00-30-53-38-7B-E2`.
  - ARP/neighbor table on `Ethernet 2` shows `169.254.191.156` / `AC-BD-0B-00-5E-E2`.
  - Wi-Fi is up at `192.168.2.20/24`; default route remains via `192.168.2.254`.
- Final read-only hardware smoke against only `169.254.191.156:1000`, timeout `2000 ms`:
...
- PR #35 is merge-ready from the capture-node implementation side after this commit and PR checks, but it remains unmerged pending Mark approval.
- PR #36 acceptance gates:
  - `CEBR119` or `CEBR120` cable confirmed.
  - EXT I/O wiring confirmed with power off.
  - Leimac IP/readiness confirmed.
  - Basler Line 2 can be configured under explicit operator approval.
...
## Session Update (2026-06-26 UTC, AI Grader Basler/Leimac sync PR #36 supervised smoke aborted)
- Continued PR #36 on branch `feature/ai-grader-basler-leimac-sync`, HEAD before this docs update `eda72c5`.
- Mark confirmed current hardware state before the supervised run:
  - Mark present.
  - Basler `CCB-M8IO` / Leimac EXT I/O wiring complete.
  - Unused Basler I/O wires insulated.
  - Leimac STATUS light green.
  - Ring light initially on.
- Targeted validation before hardware:
  - `pnpm --filter @tenkings/ai-grader-capture-helper build` -> pass.
  - `pnpm --filter @tenkings/ai-grader-capture-helper test` -> pass, 111 tests.
- Passive inventory before hardware:
  - `Ethernet 2` / Realtek USB GbE Family Controller #2 is `Up` at `1 Gbps`, IPv4 `169.254.215.165/16`.
  - `Wi-Fi` is `Up` at `192.168.2.20/24`.
...
- Preflight hardware checks:
  - Leimac `169.254.191.156:1000`, request `R830000` -> raw `R83000100000008`; parsed `totalUnits=1`, `dimmingMethod=0000` / PWM, `lightingOutputChannels=8`.
  - Basler readiness/list found pylon `26.05.0.18278`, one camera `a2A2448-23gmBAS`, transport `GEV`, device IP `169.254.185.171`, interface IP `169.254.215.165`, status `reachable`.
- Safe-off verification:
  - Because the ring light was initially on, ran `leimac-idmu-safe-off --apply`.
  - Frames and raw responses: `W8601010000020000030000040000050000060000070000080000` -> `W86ACK0`; `W8501010000020000030000040000050000060000070000080000` -> `W85ACK0`; `W1101010000020000030000040000050000060000070000080000` -> `W11ACK0`.
  - Mark confirmed the ring light turned off.
- Dry-run/apply sequence before abort:
  - Basler dry-run reported `applied=false`, no camera opened.
  - Leimac trigger-profile dry-run reported low-duty 5% planned frames only, no TCP write, no persistent save.
  - Basler transient Line2 apply succeeded and read back `LineSelector=Line2`, `LineMode=Output`, `LineSource=ExposureActive`, `LineInverter=false`, `persistentSaved=false`; no image capture and no User Set save.
  - Mark confirmed the ring light stayed off after Basler Line2 apply.
- Leimac low-duty profile apply:
  - Unit-info precheck: `R830000` -> `R83000100000008`.
  - Safe-off-before-profile frames ACKed: `W86...0000` -> `W86ACK0`; `W85...0000` -> `W85ACK0`; `W11...0000` -> `W11ACK0`.
  - Trigger/profile frames ACKed:
    - `W0901010002020002030002040002050002060002070002080002` -> `W09ACK0`.
    - `W6501010000020000030000040000050000060000070000080000` -> `W65ACK0`.
    - `W8401010000020000030000040000050000060000070000080000` -> `W84ACK0`.
    - `W1301010000020000030000040000050000060000070000080000` -> `W13ACK0`.
    - `W1101010050020050030050040050050050060050070050080050` -> `W11ACK0`.
    - `W8501010000020000030000040000050000060000070000080000` -> `W85ACK0`.
    - `W8601010001020001030001040001050001060001070001080001` -> `W86ACK0`.
  - Mark then confirmed the ring light was on continuously. This is the required abort condition.
- Smoke outcome:
  - Synchronized capture was not run.
  - No PNG was captured and no output artifact was created.
  - Ran safe-off immediately after the continuous-light confirmation; `W86...0000`, `W85...0000`, and `W11...0000` all ACKed again.
  - Mark confirmed final ring-light state was off.
  - Safe-off works, but the current trigger-profile sequence is not accepted because the output-enable step leaves the ring light continuously on on this rig.
  - PR #36 is not merge-ready for synchronized-lighting hardware acceptance until the trigger-only Leimac output behavior is corrected and a supervised retry keeps the light off between exposures.
...
- Guardrails held: no migration, no `RUN_DB_MIGRATIONS=true`, no deploy, no runtime DB operation, no `regsvr32`, no Dino-Lite command, no Arduino command, no stage/motor command, no Windows network setting change, no persistent Basler User Set save, no persistent Leimac User Set save, no Basler capture, no synchronized image capture, no Leimac SYSTEM RESET or FACTORY DEFAULT, no high-duty lighting, no SDK/vendor binary commit, no captured image commit, no calibrated macro evidence claim, no final AI grade claim, no certificate claim, and no certified grading claim.

## Session Update (2026-06-26 UTC, AI Grader Basler/Leimac sync PR #36 polarity diagnostics)
- Continued PR #36 on `feature/ai-grader-basler-leimac-sync`, PR `https://github.com/MarkTenKings/tenkings-backend/pull/36`.
- Implemented the supervised polarity diagnostic layer after the previous `LineInverter=false` / Leimac `LevelLow` attempt caused continuous idle-on and was safely aborted.
- Basler Line2 support now accepts explicit `--line-inverter true|false` and reads/reports Line2 status after configuration when supported:
  - `LineSelector`
  - `LineMode`
  - `LineSource`
  - `LineInverter`
  - `LineStatus`
  - `LineStatusAll`
  Unsupported status fields are reported as unsupported instead of fabricated.
- Leimac trigger-profile support now accepts only explicit supervised trigger activation values `LevelLow` or `LevelHigh`.
- Added `basler-leimac-polarity-smoke`:
  - dry-run by default
  - requires `--apply`, exact confirmation text, `--mark-present`, `--wiring-confirmed`, and `--leimac-status-green` before hardware
  - defaults diagnostic duty to `1%` and rejects duty above `5%`
  - applies exactly one candidate and then stops for Mark visual idle-light confirmation
  - capture is a separate explicit `--capture-confirmed` path after Mark confirms idle-off
  - runs safe-off before each candidate and provides an `--operator-reported-idle-on` path to run safe-off after an idle-on failure
  - rejects output paths inside the repo before any capture
- Supported polarity candidates are:
  ...
  - `line2-inverter-level-high`: final supported combination
- Exact new tested Leimac frames:
...
- Tests added/updated for Basler Line2 status readback, configurable Basler LineInverter bridge arguments, Leimac LevelHigh trigger activation, polarity candidate order/manifest metadata, diagnostic duty default `1%`, duty `>5%` rejection, repo-output rejection before capture, and hardware-closed default paths.
- Validation so far:
  - `pnpm --filter @tenkings/ai-grader-capture-helper build` -> pass.
  - `pnpm --filter @tenkings/ai-grader-capture-helper test` -> pass, 115 tests.
- Hardware polarity retry has not run yet in this continuation. Mark last confirmed the ring light was off after the previous safe-off.
- Guardrails held so far in this continuation: no migration, no `RUN_DB_MIGRATIONS=true`, no deploy, no runtime DB operation, no `regsvr32`, no Dino-Lite command, no Arduino command, no stage/motor command, no Windows network setting change, no persistent Basler User Set save, no persistent Leimac User Set save, no Basler capture, no synchronized image capture, no Leimac SYSTEM RESET or FACTORY DEFAULT, no high-duty lighting, no SDK/vendor binary commit, no captured image commit, no calibrated macro evidence claim, no final AI grade claim, no certificate claim, and no certified grading claim.

## Session Update (2026-06-29 UTC, AI Grader Basler/Leimac sync PR #36 accepted pulse and image-stat smoke)
- Continued PR #36 on `feature/ai-grader-basler-leimac-sync`, PR `https://github.com/MarkTenKings/tenkings-backend/pull/36`.
...
- Manual pulse result:
  - Command used `LineInverter=true`, idle `UserOutputValue=false`, pulse `UserOutputValue=true`, pulse `500 ms`.
  - Leimac profile was `TriggerActivation=LevelLow`, duty `3%`, trigger source `TRG IN1`, synchronous, async output OFF.
  - Basler readback changed `LineStatus=false`, `LineStatusAll=4` before pulse to `LineStatus=true`, `LineStatusAll=6` during pulse, then back to `LineStatus=false`, `LineStatusAll=4`.
  - Mark visually confirmed the ring light turned on during the pulse.
  - Safe-off ran before and after and ACKed.
- Accepted synchronized image-stat smoke:
  - Selected polarity: Basler `LineInverter=true` + Leimac `TriggerActivation=LevelLow`.
  - Exposure `50000 us`, gain `0`, Leimac duty `3%` / `30` steps, `isCalibrated=false`, `evidenceClass=macro_sync_smoke_uncalibrated`.
  - Basler Line2 readback after ExposureActive apply: `LineSelector=Line2`, `LineMode=Output`, `LineSource=ExposureActive`, `LineInverter=true`, `LineStatus=false`, `LineStatusAll=4`, `persistentSaved=false`.
...
## Session Update (2026-06-29 UTC, AI Grader PR #39 active lighting/profile evidence package follow-up)
- PR #39 remains open and must not be merged yet.
- Implemented the follow-up fixes requested after the first hardware smoke:
  - Accepted operator preview lighting now persists as a local software-side active fixed-rig lighting profile, stored outside the repo as `fixed-rig-active-lighting-profile.json` near the fixed-rig capture-data root.
  - The active profile records `selectedDutyPercent`, `actualLeimacPwmStep`, `selectedChannels`, `profileSource`, `acceptedAt`, and `resetToDefault`.
  - `basler-fixed-rig-focus-assist`, `ai-grader-fixed-rig-v1-local`, and `ai-grader-fixed-rig-v1-evidence-package` read the active profile unless a CLI `--duty` override or `--reset-default-lighting-profile` is used.
  - No persistent Basler or Leimac User Set save was added.
- Report orientation/display handling was revised:
  - Raw Basler captures remain unchanged in `basler_sensor_pixels` with original hashes.
  - Reports generate derived portrait display PNGs, portrait overlay debug PNGs, and portrait ROI crops.
  - Display assets record `displayTransform` and `displayCoordinateFrame=ai_grader_card_portrait_display`.
  - Raw evidence remains clean; overlays are not baked into raw captures.
- Pixel/mm estimate logic was corrected for raw/display orientation:
  - Raw landscape card width maps to the physical long side (`88.9mm`) and raw height maps to the short side (`63.5mm`).
  - Reports/manifests include `pixelToMmOrientationUsed` and `pixelToMmConsistency`.
  - Divergent X/Y estimates warn and keep `isCalibrated=false`.
- Overlay alignment is now auditable:
  - Reports/manifests include template and detected boundary rectangles, center offset, margins, detected/expected aspect ratio, orientation used, and `overlayAlignmentStatus`.
  - Framing warnings are emitted when the detected card is near a frame edge, off-center, missing, or aspect-ratio suspicious.
- Added supervised `ai-grader-fixed-rig-v1-evidence-package` command:
  - Front/back plan captures dark control, all-on, accepted active lighting profile, and Leimac channels `1-8`.
  - Generates derived portrait display images, overlay debug images, and ROI crops for full card, corners, edges, and surfaces.
  - Uses `evidenceClass=macro_fixed_rig_v1_uncalibrated`, `isCalibrated=false`, and no final/certified grading claim.
- Focused validation after code changes:
  - `pnpm --filter @tenkings/ai-grader-capture-helper build` -> pass.
  - `pnpm --filter @tenkings/ai-grader-capture-helper test` -> pass, 134 tests.
- Pending before PR #39 hardware acceptance:
  - Run full validation matrix.
  - Relaunch operator preview and have Mark accept a changed preview duty such as `1.3%` or `1.4%`.
  - Run focus/framing assistant and prove the manifest/report used the accepted preview duty rather than default `1.2%`.
  - Verify portrait report images, overlay alignment, orientation-corrected pixel/mm estimate, and then run the fixed-rig V1 uncalibrated evidence package.
  - Mark must confirm final physical Leimac ring light state is off.
- Guardrails held so far: no migration, no `RUN_DB_MIGRATIONS=true`, no manual deploy, no runtime DB operation, no network setting change, no `regsvr32`, no Arduino/stage/motor command, no Leimac reset/default, no persistent Leimac or Basler User Set save, no high-duty lighting, no captured image/vendor binary commit, and no calibrated/final/certificate/certified claim.

## Session Update (2026-06-29 UTC, AI Grader PR #39 active profile and evidence package hardware accepted)
- PR #39 remains open and must not be merged yet.
- Mark accepted a changed live-preview lighting profile:
  - Active profile path: `C:\TenKings\capture-data\fixed-rig-active-lighting-profile.json`.
  - Selected duty `1.3%`, Leimac PWM step `13`, channels `1-8`, `profileSource=operator_preview`.
  - No persistent Basler or Leimac User Set save.
  - A back-positioning preview briefly accepted `1.2%`; the active profile was restored to `1.3%` before the back evidence package, and the back manifest confirms `1.3%`.
- Operator preview/focus:
  - First accepted profile preview: `C:\TenKings\capture-data\fixed-rig-calibration\basler-fixed-rig-operator-preview-2026-06-29T223557442Z`.
  - FPS `20.5`, frame age `2 ms`, duty `1.3%`, overlay alignment `warn`, display transform `rotate90cw`.
  - Back positioning/restored profile preview: `C:\TenKings\capture-data\fixed-rig-calibration\basler-fixed-rig-operator-preview-2026-06-29T230738709Z`.
  - FPS `19.7`, frame age `2 ms`, duty `1.3%`, overlay alignment `pass`, pixel/mm consistency `pass`, display transform `rotate90cw`.
- Focus/framing assistant proof:
  - Output folder: `C:\TenKings\capture-data\fixed-rig-calibration\basler-fixed-rig-focus-assist-2026-06-29T224331062Z`.
  - Manifest/report used accepted preview brightness: duty `1.3%`, PWM `13`, source `operator_preview`.
  - Metrics: mean `54.3913`, clipped `0.001314`, dark `0.068241`, sharpness `402.9915`, coverage `0.952434`.
  - Display transform `rotate90cw`; overlay alignment `warn`; pixel/mm consistency `warn` due near-edge framing, not calibrated.
- Fixed-rig V1 uncalibrated evidence package completed:
  - Evidence class `macro_fixed_rig_v1_uncalibrated`; `isCalibrated=false`; no final grade/certificate/certified claim.
  - Front folder: `C:\TenKings\capture-data\fixed-rig-v1\ai-grader-fixed-rig-v1-evidence-package-2026-06-29T225500347Z`.
  - Front manifest: `C:\TenKings\capture-data\fixed-rig-v1\ai-grader-fixed-rig-v1-evidence-package-2026-06-29T225500347Z\manifest.json`.
  ...
  - Front all-on raw: SHA-256 `8284b71aa6a4d4cbf377d953c58efefbf7e2ee28d3dfca812aef2431cede04e4`, `2813838` bytes, `2448x2048`.
  - Front metrics: mean `61.0675`, clipped `0.010289`, dark `0.025929`, sharpness `494.9914`, coverage `0.940953`, overlay alignment `warn`.
  - Back folder: `C:\TenKings\capture-data\fixed-rig-v1\ai-grader-fixed-rig-v1-evidence-package-2026-06-29T230849461Z`.
  - Back manifest: `C:\TenKings\capture-data\fixed-rig-v1\ai-grader-fixed-rig-v1-evidence-package-2026-06-29T230849461Z\manifest.json`.
  ...
  - Back all-on raw: SHA-256 `cbf692169c4d570ecb601afda2466f8ac2ac2487cebf9c36a37c9434d78546d5`, `2510345` bytes, `2448x2048`.
  - Back metrics: mean `120.8304`, clipped `0.013025`, dark `0.053224`, sharpness `681.029`, coverage `0.690135`, overlay alignment `pass`.
  - Each side captured dark control, all-on, accepted-profile, channels `1-8`, portrait display, overlay, and 12 ROI crops.
  - Raw captures remain in `basler_sensor_pixels`; display/overlay/ROI outputs use portrait display transform `rotate90cw`.
- Final physical Leimac ring light state: confirmed off by Mark.
- Remaining limitations:
  - Still uncalibrated; manual focus only.
  - Front framing/overlay alignment remains `warn`.
  - Channel physical mapping remains not confirmed.
  - Circular ring reflection remains an unresolved optical/glare limitation.
  - Captured outputs remain outside repo and must not be committed.
- Next required step before merge: run final validation matrix, commit/push PR #39 changes, verify PR checks. Do not merge yet.

## Session Update (2026-06-30 UTC, AI Grader PR #39 rough fixed-fixture calibration extension)
- PR #39 remains open and must not be merged yet.
- Mark updated the physical rig with fixed card positioning; treat it as an operator-built fixed-position V1 fixture.
- Implemented local/offline fixed-rig rough calibration and repeatability foundations:
  - New `fixed-rig-fixture-calibration` command captures a supervised Basler/Leimac macro package using the accepted active lighting profile and writes a rough fixture profile.
  - Rough fixture profiles support `referenceType=card_dimensions|metric_ruler|cutting_mat|measurement_board|certified_target|unknown`.
  - Non-certified references are marked `rough_reference_unvalidated`; `isCalibrated=false` remains mandatory.
  - Profiles record fixture label/id, reference physical size, raw/display coordinate frames, display transform, pixel/mm and mm/pixel estimates, X/Y consistency warnings, aspect ratio, active lighting profile, exposure/gain/duty/channels, operator acceptance, and notes.
  - Lens distortion and homography remain `not_computed`.
- Implemented `fixed-rig-repeatability-test`:
  - Uses the active preview lighting profile unless CLI duty/reset override is supplied.
  - Supports `--repeatability-phase no-touch|remove-replace`, `--capture-count`, and supervised remove/re-place gating.
  - Reports center offset variation, optional mm offsets, boundary width/height variation, pixel/mm variation, sharpness/brightness/clipping stability, overlay alignment counts, and `repeatabilityStatus=pass|warn|fail`.
  - Repeatability is diagnostic only and does not mark the rig calibrated.
- Extended fixed-rig V1 evidence-package outputs:
  - Generates per-channel portrait display images for Leimac channels `1-8`.
  - Adds `preliminary_surface_anomaly_detector_v0` schema with per-channel stats and empty candidates until a robust detector is accepted.
  - Adds preliminary diagnostic grading sections for centering, corners, edges, and surface.
  - Reports keep `finalGradeComputed=false`, `certifiedClaim=false`, `isCalibrated=false`, and no final grade/certificate/certified claim.
- Focused validation:
  - `pnpm --filter @tenkings/ai-grader-capture-helper build` -> pass.
  - `pnpm --filter @tenkings/ai-grader-capture-helper test` -> pass, 137 tests.
- Pending:
  - Run full validation matrix.
  - Supervised hardware flow: live preview, rough fixture calibration capture, no-touch repeatability, remove/re-place repeatability if Mark is available, front/back evidence package, report review, safe-off, final physical ring-light-off confirmation.
- Guardrails held so far: no migrations, no `RUN_DB_MIGRATIONS=true`, no deploy, no runtime DB ops, no network setting changes, no `regsvr32`, no Arduino/stage/motor commands, no Leimac reset/default, no persistent Basler/Leimac User Set save, no high-duty lighting, no captured image/vendor binary commit, and no calibrated/final/certificate/certified claims.

## Session Update (2026-06-30 UTC, AI Grader PR #39 rough fixture hardware smoke)
- PR #39 remains open and must not be merged yet.
- Mark confirmed hardware preflight and final physical Leimac ring light state off.
- Accepted live preview:
  - Folder: `C:\TenKings\capture-data\fixed-rig-calibration\basler-fixed-rig-operator-preview-2026-06-30T062619141Z`.
  - Report: `C:\TenKings\capture-data\fixed-rig-calibration\basler-fixed-rig-operator-preview-2026-06-30T062619141Z\preview-report.html`.
  - Measured `20.5 FPS`, frame age `0 ms`, display transform `rotate90cw`, overlay visible.
  - Accepted profile: duty `1.4%`, PWM step `14`, channels `1-8`, source `operator_preview`.
  - Readiness remained `not_ready` because the detected card boundary touched the frame edge; Mark accepted rough/unvalidated continuation.
- Rough fixture calibration:
  - Folder: `C:\TenKings\capture-data\fixed-rig-calibration\fixed-rig-fixture-calibration-2026-06-30T063338472Z`.
  - Manifest/report: `C:\TenKings\capture-data\fixed-rig-calibration\fixed-rig-fixture-calibration-2026-06-30T063338472Z\manifest.json`, `C:\TenKings\capture-data\fixed-rig-calibration\fixed-rig-fixture-calibration-2026-06-30T063338472Z\preview-report.html`.
  - Used accepted profile duty `1.4%`, PWM `14`, channels `1-8`, source `operator_preview`.
  - Result `rough_reference_unvalidated`, `isCalibrated=false`, reference `card_dimensions`, fixture `fixed-v1-l-stop`.
  - Pixel scale estimate `mmPerPixelX=0.039025`, `mmPerPixelY=0.031006`; consistency `warn`.
  - Boundary `170,0,2278,2048`, overlay `warn`, aspect ratio warning, card touches frame edge.
- No-touch repeatability:
  - Folder: `C:\TenKings\capture-data\fixed-rig-calibration\fixed-rig-repeatability-test-2026-06-30T063417135Z`.
  - Manifest/report: `C:\TenKings\capture-data\fixed-rig-calibration\fixed-rig-repeatability-test-2026-06-30T063417135Z\manifest.json`, `C:\TenKings\capture-data\fixed-rig-calibration\fixed-rig-repeatability-test-2026-06-30T063417135Z\preview-report.html`.
  - Used accepted profile duty `1.4%`, PWM `14`, channels `1-8`, source `operator_preview`.
  - Result `repeatabilityStatus=warn`, `runCount=5`, `centerOffsetMeanPx=84.6015`, `boundaryWidthVariationPx=1`, `boundaryHeightVariationPx=0`, `meanBrightnessVariation=0.8244`, `clippingMax=0.01448`.
  - Remove/re-place repeatability was skipped/pending because it requires operator re-seat pauses and Mark clarified normal evidence capture should run continuously per side.
- Fixed-rig V1 uncalibrated evidence package:
  - Front folder/report: `C:\TenKings\capture-data\fixed-rig-v1\ai-grader-fixed-rig-v1-evidence-package-2026-06-30T064206795Z`, `C:\TenKings\capture-data\fixed-rig-v1\ai-grader-fixed-rig-v1-evidence-package-2026-06-30T064206795Z\preview-report.html`.
  - Front used duty `1.4%`, PWM `14`, channels `1-8`, source `operator_preview`.
  - Front all-on raw SHA-256 `3844fffbf4e0f52181608b13be9ebf8a8af26628c5e9cd523a9e023452399a72`, `2843916` bytes, `2448x2048`.
  - Back positioning preview briefly accepted `1.2%`; Mark requested `1.4%`, so the back evidence package used `--duty 1.4` and records `profileSource=cli_override`.
  - Back folder/report: `C:\TenKings\capture-data\fixed-rig-v1\ai-grader-fixed-rig-v1-evidence-package-2026-06-30T064843000Z`, `C:\TenKings\capture-data\fixed-rig-v1\ai-grader-fixed-rig-v1-evidence-package-2026-06-30T064843000Z\preview-report.html`.
  - Back all-on raw SHA-256 `4a282671b0fba6f44505bc857a0651d3c6dcd8cdb2e431ca5255bf81613754f2`, `2680124` bytes, `2448x2048`.
  - Each side captured 11 raw images: dark control, all-on, accepted-profile, and channels `1-8`; each side also generated 8 portrait channel displays and 12 ROI crops.
  - Evidence class `macro_fixed_rig_v1_uncalibrated`; `isCalibrated=false`; raw captures remain `basler_sensor_pixels`; display/ROI outputs use `ai_grader_card_portrait_display`.
  - Diagnostic grading sections are `computed_diagnostic`; surface analysis remains `not_computed` with `preliminary_surface_anomaly_detector_v0` per-channel evidence and no accepted robust candidates.
- Remaining limitations:
  - Not production calibrated; rough reference only.
  - Framing/card boundary touches the frame edge, overlay and pixel/mm consistency remain warnings.
  - Channel physical mapping remains unconfirmed.
  - Ring reflection/glare remains unresolved.
  - No final grade, certificate, certified grading claim, captured image/vendor binary commit, migration, deploy, DB op, network setting change, Arduino/stage/motor command, Leimac reset/default, persistent User Set save, or high-duty lighting occurred.

## Session Update (2026-06-30 UTC, AI Grader PR #39 fixed-ruler production-gate update)
- PR #39 remains open and must not be merged yet.
- Continued on `feature/ai-grader-fixed-rig-calibration-preview`.
- Mark updated the physical fixture with fixed metric rulers next to the card. The implementation now treats fixed metric rulers as the primary production-candidate calibration reference:
  - `fixed-rig-fixture-calibration` accepts `--reference-type fixed_metric_rulers`.
  - Required ruler inputs are `--horizontal-span-mm`, `--horizontal-start-px x,y`, `--horizontal-end-px x,y`, `--vertical-span-mm`, `--vertical-start-px x,y`, and `--vertical-end-px x,y`.
  - Missing ruler-span coordinates fail closed instead of silently falling back to card-size calibration.
  - Fixture profiles record ruler span distances, pixel coordinates, computed pixel distances, calibration image path, `pixelsPerMmX/Y`, `mmPerPixelX/Y`, and X/Y consistency.
  - Card dimensions remain a cross-check/fallback, not the primary production-candidate scale source.
- Added strict framing and production-readiness gate metadata:
  - Framing gate records margins in px/mm, center offset px/mm, aspect-ratio error, overlay alignment status, and pass/warn/fail.
  - Card touching the image boundary is a framing `fail`.
  - Production readiness records ruler calibration, framing, overlay, repeatability, lighting-profile, and final-safe-off gates plus blockers.
  - `production_candidate` is allowed only when those gates pass; this remains diagnostic only and not a final/certificate/certified claim.
- Updated overlay/report metadata:
  - Ruler spans can be drawn on the derived overlay/debug image.
  - Raw Basler evidence remains unchanged in sensor coordinates.
- Updated repeatability:
  - Remove/re-place phase defaults to five captures/placements unless overridden.
  - Repeatability run mm offsets use ruler-derived scale when a fixture profile is supplied.
- Focused validation after code changes:
  - `pnpm --filter @tenkings/ai-grader-capture-helper build` -> pass.
  - `pnpm --filter @tenkings/ai-grader-capture-helper test` -> pass, 139 tests.
- Pending before PR #39 acceptance:
  - Full validation matrix.
  - Supervised ruler calibration hardware run using real ruler span coordinates/distances.
  - Guided remove/re-place repeatability, at least five placements.
  - Only after ruler/framing/repeatability gates pass, or after explicit operator diagnostic override, run the fixed-rig evidence package.
  - Safe-off and final physical ring-light-off confirmation.
- Guardrails held so far in this update: no migrations, no `RUN_DB_MIGRATIONS=true`, no deploy, no runtime DB ops, no network setting changes, no `regsvr32`, no Arduino/stage/motor commands, no Leimac reset/default, no persistent Basler/Leimac User Set save, no high-duty lighting, no captured image/vendor binary commit, and no calibrated/final/certificate/certified claims.

## Session Update (2026-06-30 UTC, AI Grader PR #39 fixed-ruler acceptance smoke)
- PR #39 remains open and must not be merged yet.
- Mark used the live Basler preview window to align the fixed-position fixture and rulers, then explicitly accepted the fixture positioning/re-seat behavior as passed for production-candidate fixture positioning. This is still diagnostic/uncalibrated evidence; no final grade, certificate, or certified calibration claim is made.
- Ruler calibration run:
  - Folder: `C:\TenKings\capture-data\fixed-rig-calibration\fixed-rig-fixture-calibration-2026-06-30T075916276Z`.
  - Manifest/report: `C:\TenKings\capture-data\fixed-rig-calibration\fixed-rig-fixture-calibration-2026-06-30T075916276Z\manifest.json`, `C:\TenKings\capture-data\fixed-rig-calibration\fixed-rig-fixture-calibration-2026-06-30T075916276Z\preview-report.html`.
  - Reference: `fixed_metric_rulers`; known ruler spans were entered as `50.8mm` horizontal from raw px `540,205` to `1620,205`, and `50.8mm` vertical from raw px `2295,145` to `2295,1218`.
  - Operator card boundary override: raw rect `285,349,1878,1350`; used because rulers/fixture edges can confuse automatic boundary detection.
  - Computed scale: `pixelsPerMmX=21.2599`, `pixelsPerMmY=21.122`, `mmPerPixelX=0.047037`, `mmPerPixelY=0.047344`; X/Y consistency `pass` with relative difference `0.0065`.
  - Framing gate `pass`; margins left/right `285px` (`13.406mm`), top/bottom `349px` (`16.523mm`); overlay alignment `pass`; detected card aspect `1.391111` vs expected `1.4`.
  - Lighting: duty `1.4%`, PWM step `14`, channels `1-8`, exposure `45000us`, gain `0`.
- Guided remove/re-seat repeatability run:
  - Folder/report: `C:\TenKings\capture-data\fixed-rig-calibration\fixed-rig-repeatability-test-2026-06-30T080625670Z`, `C:\TenKings\capture-data\fixed-rig-calibration\fixed-rig-repeatability-test-2026-06-30T080625670Z\preview-report.html`.
  - Phase `remove_replace`, `5` captures/placements, same ruler spans and card boundary override.
  - Geometry result accepted by Mark as passed for production fixture positioning: center offset max `0.7071px` (`0.0334mm`), boundary width variation `0px`, boundary height variation `0px`, overlay alignment `pass` for all 5 runs.
  - Manifest status remains `warn` because clipping reached `0.082032`; this is recorded as a per-card lighting/exposure tuning issue, not a fixed-fixture positioning failure.
- Fixed-rig V1 uncalibrated evidence package with fixed-ruler metadata:
  - Front folder/report: `C:\TenKings\capture-data\fixed-rig-v1\ai-grader-fixed-rig-v1-evidence-package-2026-06-30T082102851Z`, `C:\TenKings\capture-data\fixed-rig-v1\ai-grader-fixed-rig-v1-evidence-package-2026-06-30T082102851Z\preview-report.html`.
  - Back folder/report: `C:\TenKings\capture-data\fixed-rig-v1\ai-grader-fixed-rig-v1-evidence-package-2026-06-30T082749115Z`, `C:\TenKings\capture-data\fixed-rig-v1\ai-grader-fixed-rig-v1-evidence-package-2026-06-30T082749115Z\preview-report.html`.
  - Each side captured dark control, all-on, accepted-profile, and Leimac channels `1-8`; each generated 8 portrait channel display images and 12 ROI crops.
  - Front all-on raw: SHA-256 `5879d28284911cdd4e845276f3d37a9c99fee8690a4648e78e672c306be5b571`, raw `2448x2048`, portrait display `2048x2448`, mean `96.037`, clipped `0.038942`, dark `0.005755`, sharpness `426.106`, overlay `pass`.
  - Back all-on raw: SHA-256 `cf27e1ff636fb67eb911a91091077308282a3293c1970660105b8adc72e36e38`, raw `2448x2048`, portrait display `2048x2448`, mean `134.8357`, clipped `0.096152`, dark `0.005312`, sharpness `560.4625`, overlay `pass`.
  - Evidence class `macro_fixed_rig_v1_uncalibrated`; `isCalibrated=false`; raw Basler images remain `basler_sensor_pixels`; derived report/ROI assets use `ai_grader_card_portrait_display` with `displayTransform=rotate90cw`.
  - Diagnostic sections remain preliminary only: `finalGradeComputed=false`, `certifiedClaim=false`, surface candidates not accepted/robust yet.
- Final physical Leimac ring light state was confirmed off by Mark after the back evidence package safe-off.
- Remaining blockers/limitations:
  - The fixed-ruler profile is an unvalidated local fixture reference, not certified machine-vision calibration; `isCalibrated=false` remains required.
...
## Session Update (2026-07-02 UTC, AI Grader PR #47 production integration continuation)
- Continued PR #47 on branch `feature/ai-grader-production-release-v0`; PR remains open and must not be merged yet.
- Completed the remaining production-path implementation behind explicit gates:
  - Station page now supports existing `CardAsset`/`Item` lookup plus manual draft identity and carries `cardAssetId`/`itemId` into the production report bundle.
  - Admin production API now includes `card-search`, `upload-slab-photo`, and `run-comps` actions.
  - Slabbed front/back color photo upload uses existing storage helpers and persists as `AiGraderEvidenceAsset` rows with `artifactClass=slabbed_photo`; these are separate from Basler monochrome evidence.
  - Operator-triggered eBay comps use the existing KingsReview/SerpAPI sold-comps helper when `AI_GRADER_EBAY_COMPS_ENABLED=true`; tests use mocked execution only and readiness blocks on missing final grade or identity.
  - Public read-only report API can merge persisted production-release, label, slabbed-photo, and valuation/comps data; label preview can load persisted report bundles.
- Migration/runbook:
  - Review migration `20260702120000_ai_grader_production_release_v0` adds `AiGraderSession`, `AiGraderReport`, `AiGraderEvidenceAsset`, `AiGraderGrade`, `AiGraderLabel`, `AiGraderPublication`, and `AiGraderValuation`.
  - Codex did not run a production migration and did not set `RUN_DB_MIGRATIONS=true`.
  - Human rollout later must review/apply the migration through the approved database package migration command with an approved `DATABASE_URL`, then enable publication gates only after storage/auth verification.
- Production env/config needed before live use:
  - `DATABASE_URL`, `AI_GRADER_PRODUCTION_TENANT_ID`, existing admin auth/session config.
  - `AI_GRADER_PRODUCTION_PUBLISH_ENABLED=true` only after migration/storage review.
  - `AI_GRADER_PUBLIC_REPORT_DB_ENABLED=true` only after persisted public reads are ready.
  - Existing Ten Kings storage helper env for bucket/region/base URL/credentials.
  - `AI_GRADER_PUBLIC_REPORT_BASE_URL` or equivalent deployed base URL for QR/public URLs.
  - `AI_GRADER_EBAY_COMPS_ENABLED=true` plus SerpAPI env only for explicit operator-triggered comps.
- Validation:
  - `pnpm --filter @tenkings/database build` -> pass.
  ...
  - `pnpm --filter @tenkings/nextjs-app build` -> pass with existing unrelated `<img>` warnings.
  - `pnpm --filter @tenkings/ai-grader-capture-helper build` -> pass.
  - `pnpm --filter @tenkings/ai-grader-capture-helper test` -> pass, `167` tests.
  - `pnpm --filter @tenkings/shared test` -> pass, `105` tests.
  - `pnpm --filter @tenkings/ai-grader-simulator test` -> pass, `6` tests.
...
## Session Update (2026-07-03 UTC, AI Grader production rollout resumed with credential rotation deferred)
- Mark explicitly accepted the temporary risk of continuing with the currently exposed production Postgres credential and deferred DigitalOcean managed Postgres credential rotation until after the AI Grader production rollout. The credential remains exposed and must still be rotated after rollout; no rotation was attempted in this pass.
- Redacted production verification:
  - Production DB connection worked from approved droplet service env without printing the connection string.
  - Prisma migration status reported `68 migrations found` and `Database schema is up to date!`.
  - AI Grader table presence check returned `7` matching `AiGrader*` tables.
  - Droplet repo stayed on `main` at `0dda9336950023675d2c5640d15313f5bcbc1207`.
  - Known untracked production files remained present: `backend/Dockerfile`, `data/`, `frontend/nextjs-app/start.sh`, `logs/`, `packages/database/seed-progress.js`, `scripts/backfill-item-images.js`, and the existing `scripts/variant-db/*.bak` files.
- Env gate result:
  - `env/nextjs-app.env` was backed up as `env/nextjs-app.env.backup-ai-grader-20260703T040107Z`.
  - `AI_GRADER_PRODUCTION_PUBLISH_ENABLED=true`, `AI_GRADER_PUBLIC_REPORT_DB_ENABLED=true`, `AI_GRADER_PUBLIC_REPORT_BASE_URL=https://collect.tenkings.co`, and `AI_GRADER_PRODUCTION_TENANT_ID=ten-kings` were added/verified in that env file.
  - `AI_GRADER_EBAY_COMPS_ENABLED` was not added to `env/nextjs-app.env` because `SERPAPI_KEY` was not present in that same Next env source.
  - Current droplet `tenkings-kiosk.service` runs `frontend/nextjs-app/start.sh`, and that known untracked start script does not source `env/nextjs-app.env`; therefore no active droplet web runtime was confirmed to consume the added gates.
  - `collect.tenkings.co` is served by Vercel. Local/droplet Vercel CLI/project linkage was unavailable or ambiguous, so Vercel production env mutation was skipped.
- Service restart/deploy result:
  - No service restart/recreate was run because the edited droplet env file is not consumed by the active systemd service, and changing the untracked start script or guessing the Vercel project/env target would be unsafe.
- Storage/slab result:
  - Managed storage smoke was skipped. Active runtime storage config was not available: only `CARD_STORAGE_MODE` was present in `env/nextjs-app.env`; bucket/region/public-base/access/secret keys were not verified in the active web runtime.
  - Slabbed photo upload smoke was skipped for the same storage/runtime blocker.
- Public report and label/QR smoke:
  - A direct DB persisted smoke report was created without managed storage upload.
  ...
  - Hosted public report API returned `503` with `AI_GRADER_PUBLIC_REPORT_DB_DISABLED`, confirming hosted Vercel env gates are still pending.
  - Hosted report and label pages returned `200`, but they cannot be accepted as persisted/stored-data reads until Vercel env and storage are configured.
- Card/inventory linkage result:
  - Manual draft identity path was verified in the DB smoke.
  - No `CardAsset` or `Item` production inventory row was linked or modified; update counts were `0`.
- eBay/SerpAPI comps result:
  - One controlled live SerpAPI/eBay smoke was run from the existing droplet service env because `SERPAPI_KEY` was present there.
  - It required the persisted report final grade and manual card identity, persisted one valuation row, returned `status=completed`, and returned `compsCount=1`; parsed valuation amount remained `null`.
- Security verification:
  - Smoke report/related JSON scan found no local Windows paths, loopback URLs, `DATABASE_URL` markers, SerpAPI markers, or generic secret markers.
  - Hosted API/report/label scans did not show Dell paths, bridge URLs/tokens, hardware controls, or secret markers.
  - Public report API rejected `POST` with `405`; public API `GET` remains env-disabled until Vercel env is configured.
...
- Validation:
  ...
  - `pnpm --filter @tenkings/nextjs-app build` -> pass with existing unrelated `<img>`, Browserslist, and Tailwind glob warnings.
  - `pnpm --filter @tenkings/ai-grader-capture-helper build` -> pass.
  - `pnpm --filter @tenkings/ai-grader-capture-helper test` -> pass, `167` tests.
  - `pnpm --filter @tenkings/shared test` -> pass, `105` tests.
  - `pnpm --filter @tenkings/ai-grader-simulator test` -> pass, `6` tests.
  ...
  - `git diff --check` -> pass with line-ending warnings only.
- Remaining blockers:
  - Configure the Vercel production project identified in repo docs as `ten-kings/tenkings-backend-nextjs-app` for `collect.tenkings.co` with the redacted required envs: `DATABASE_URL`, `AI_GRADER_PRODUCTION_PUBLISH_ENABLED=true`, `AI_GRADER_PUBLIC_REPORT_DB_ENABLED=true`, `AI_GRADER_PUBLIC_REPORT_BASE_URL=https://collect.tenkings.co`, `AI_GRADER_PRODUCTION_TENANT_ID=ten-kings`, full Ten Kings storage helper env, and optional `SERPAPI_KEY` plus `AI_GRADER_EBAY_COMPS_ENABLED=true` if web-runtime comps are desired.
  - Redeploy/restart the confirmed Vercel production target after env updates, then re-run hosted persisted report/storage/label/slab smokes.
  - Rotate the exposed DigitalOcean managed Postgres credential after rollout and update every approved runtime `DATABASE_URL` without printing the value.

## Session Update (2026-07-03 UTC, AI Grader Vercel env verification and authenticated-smoke blocker)
- Mark reported that the required Vercel Production env vars were added for the `collect.tenkings.co` project, including the production database URL, AI Grader gates, storage config, and SerpAPI/comps config where applicable. DigitalOcean Postgres credential rotation remains deferred by Mark.
- Local repo state:
  - Branch `main`.
  - HEAD `1b992dc7776e6a8e15e806cd7059163ef70b6d25`, matching `origin/main` after fetch.
  - Worktree was not clean because previous rollout docs were already modified locally; those changes were preserved.
- Production DB verification:
  - Prisma migration status from the approved droplet service env reported `68 migrations found` and `Database schema is up to date!`.
  - AI Grader table check returned `7` matching tables.
  - Droplet repo remains at `0dda9336950023675d2c5640d15313f5bcbc1207` with only the known untracked production files present.
- Vercel deployment/env behavior:
  - GitHub deployment record for `1b992dc7776e6a8e15e806cd7059163ef70b6d25` reports Production `success`.
  - Deployment URL: `https://tenkings-backend-nextjs-lmo5jwx69-ten-kings.vercel.app`.
  - Runtime behavior confirmed the new AI Grader Vercel env gates are active without printing env values:
    - `GET /api/admin/ai-grader/production/status` returned `enabled=true`.
    - `publicReportDbReadsEnabled=true`.
    - `liveEbayCompsEnabled=true`.
    - `noHardwareControls=true`.
  - Fresh redeploy was not triggered because the hosted runtime had already picked up the new production env behavior.
- Existing persisted smoke report check:
  - `GET /api/ai-grader/reports/ai-grader-prod-smoke-20260703T040532` no longer returns `AI_GRADER_PUBLIC_REPORT_DB_DISABLED`; it now reaches the storage-backed read path but returns `500`.
  - DB row inspection showed the old direct-DB smoke row is `published` and has storage-key fields, but it was not created through the real storage-upload path. It is not a valid hosted storage-backed public report proof.
  - The public report page returned `200`, but it was still showing fixture/sample fallback content rather than persisted report content.
- Code hardening applied locally for a PR:
  - Unknown generated report IDs now render a `missing_report_data` bundle instead of rewriting fixture/sample data to the requested ID.
  - The report viewer marks unresolved generated report IDs as missing persisted/local-bridge data.
  - The public report API treats a missing report-bundle storage object as not found instead of throwing a server `500`.
  - Focused test `pnpm --filter @tenkings/nextjs-app exec tsx --test tests/aiGraderLocalStation.test.ts` passed with `18` tests.
- Authenticated write-smoke blocker:
  - Production status confirms publish/storage/comps gates are active, but publish/history/card-search/slab/comps actions remain correctly admin-gated.
  - Unauthenticated `history`, `card-search`, and `publish` attempts returned `Missing or invalid Authorization header`.
  - The only attempted non-interactive operator-key path was stopped after a malformed remote curl command may have emitted an operator-key-like value in tool output. Treat that operator key as exposed; it is intentionally not repeated here.
  - No further authenticated production write, storage, slabbed-photo, card/item linkage, or comps smoke was run with that key.
- Operator station/web verification:
  - `GET /ai-grader/station` returned `200`.
  - Production station route is reachable and still includes local bridge configuration for the Dell operator workflow.
  - Public report API `POST` remains rejected with `405`.
  - Admin routes require existing Ten Kings auth/operator auth before writes.
- Remaining blockers:
  - Rotate the exposed operator/API key used for the attempted non-interactive admin smoke, then provide a safe authenticated smoke path without printing the value.
