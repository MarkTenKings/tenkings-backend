...
Subsequent owner instructions in the CAD task explicitly select that 100-door Rev27 source for a new 72-door review design, halve the product-display height, center the controls, and prioritize three sheets using a 6-inch cassette and shallower cabinet while retaining 6 × 2.5-inch door faces. These instructions resolve the compact design's source; they do not supply a native 125-door drawing or approve physical release. The CAD task's preliminary assembled dimensions and nesting remain review work, not executable profile evidence.
...
The CAD proposal uses a right-handed frame (x right, y rear, z up), decimal millimeters and explicit per-part datums/assembly transforms. Source inches convert by exactly 25.4; 0.0625 inches is 1.5875 mm. Native mechanical outlines, mounting/obstruction envelopes and assembly transforms remain in the external evidence artifacts; the current UI rectangles must never be used to fabricate or derive wiring. Physical profile release still needs an authoritative mechanical source, exact part identities and independent mapping/capability evidence.
