...
| G-04 screen/Windows/enclosure | OPEN—responsive software tests only | One SER PC and separate ViewSonic touch/TV roles are the current direction. Exact installed identities, scaling/touch, enclosure, assigned access and final Windows package remain unqualified; older Elo references are historical. | Technician/Admin | Installed appliance acceptance |
...
- Authorized: local code, additive migration source, deterministic mocks/simulators, newly created disposable SQLite and guarded loopback/tmpfs PostgreSQL validation (including full migration chain and cleanup), documentation, commits, normal push, and an open PR with green checks.
- Not authorized: migration outside the disposable validation harness, production/staging deploy or restart, production data access/mutation, live payment, Nayax activation, real controller command, physical actuation, credential change, PR merge, pilot, or launch.
