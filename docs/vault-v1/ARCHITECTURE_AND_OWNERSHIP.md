...
- Config activation waits for a safe boundary: no active customer/payment/reconciliation, commands, staff, restock or certification session. A topology or mapping change also requires the exact pending version/digest, an empty machine, cloud acknowledgment of earlier state-bearing events and explicit Technician/Admin service confirmation. Historical snapshots and retired door history remain intact.
