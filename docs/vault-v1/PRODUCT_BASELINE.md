...
Software corrections cover runtime synchronization, payment/recovery/concurrency, scoped staff and cloud authority, configuration/reporting/certification integrity, kiosk recovery/accessibility and Windows artifact staging safety. Combined local validation passes:240tests, full Vault/Next builds,13public-route checks,17browser scenarios and the95-migration disposable PostgreSQL chain with legacy upgrade/resume, public-HTTP projections and no-op/cleanup proof. Exact-commit independent review and PR checks are the remaining delivery steps.
