...
V1.2 authority is reconstructable rather than self-attested. Session creation and every resume reread and hash-verify an exact canonical five-member one-time rig source. The trusted local analyzer reruns the checkerboard detector against each exact active checkpoint, requires the rederived pose to equal the accepted event geometry, and derives normalization/boundary residuals from the detected 11-by-16 corner lattice. It decodes every exact dark, flat, and illumination checkpoint into fixed 8-by-8 linear grids; dark subtraction, relative response, response scale, directional centroids/errors, residual patterns, and artifact bytes are then computed locally. The public analysis mutation accepts no builder values, artifact bytes, acceptance booleans, or hashes.
...
The helper constructs a durable local-session authority when given trusted local runtime/rig providers plus checkerboard-camera and persistent camera/controller adapters. That authority enumerates and resumes on-disk sessions, owns operation and expected-step identity, checks optimistic revisions, derives protected context locally for each mutation, and projects immutable pose/supersession/failure/sweep/analysis/finalization history without exposing secrets or absolute paths. Construction is inert; adapters are opened only by the explicit server-owned capture action.
