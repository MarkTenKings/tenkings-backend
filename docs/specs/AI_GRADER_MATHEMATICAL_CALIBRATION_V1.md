...
V1.2 authority is reconstructable rather than self-attested. Session creation and every resume reread and hash-verify an exact canonical five-member one-time rig source. Capture-time pose geometry is bound to the exact accepted frame and four distinct positive bounded in-frame corners. Analysis derives geometry from the active pose/event ledger, rejects caller-authored geometry and mutated one-time inputs, and reparses/rebuilds its deterministic serialization. Finalization and resume canonical-loader verify the outer bytes and all 12 checkpointed members.
