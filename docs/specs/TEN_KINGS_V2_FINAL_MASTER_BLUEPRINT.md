...
The common experience is phone→SMS verification→first-time name/address/email (complete returning profiles skip)→repeat Add card/front-camera/back-camera with automatic background identification and immediate return to Add card→Done→list review and complete checkout totals/taxes→Pay→confirmed order number and receipt delivered by email and SMS→packaging instructions and a printed package label→kiosk dropbox or mail. Kiosk QR/NFC links open the same website with configured location attribution; customers use their own mobile camera in both routes. Kiosk payment uses its associated tap/insert terminal; direct mail-in uses remote online payment. No separate customer start-identification/grading button or dealer-operated submission step is required.

Present a simple price/turnaround/transport comparison at the beginning, before account/capture steps, and a nearby authorized-kiosk map/list for the dealer option. Mark explicitly requires the advertised turnaround to appear both there and at checkout. The kiosk week begins at actual ATLAS collection: Monday customer deposit with Wednesday ATLAS pickup is due the following Wednesday. Communicate each location's real pickup/return days and times, next collection and projected return, with local time zone, relevant cutoff and schedule exceptions; do not invent route schedules or treat a scheduled time as actual custody. Mail-in shipping is quoted using the ATLAS FedEx account and included in the order total before checkout; after payment, provide the customer a FedEx label to print and tape to the box. Use the selected card-count packaging and actual required shipment inputs for the carrier quote, without inventing a rate. Whether checkout includes both mail-in shipping directions and the mail-in turnaround start event are still unspecified; return postage, dates, refunds and payout timing must not be silently invented.

Provide a location-scoped dealer portal showing usage, customer/submission counts and physical progress, and a customer tracker limited to that customer's own orders/cards. Physical deposit, pickup, ATLAS receipt, grading, dispatch and return require actual recorded events. Customer uploads are intake/identification evidence, not proof of physical custody, pre-arrival grading or human-approved grading. Preserve distinct customer, dealer and grading-staff permissions, current grading policy, payment idempotency and immutable order/profile/price/location snapshots. This authorizes recording and developing the clarified ATLAS workflow; it does not assert live checkout, terminal, receipt, label, pickup or payout acceptance. The [detailed owner workflow and implementation boundary](../atlas/plans/CUSTOMER_SUBMISSION_WORKFLOW_20260924.md) records the complete sequence and remaining operating inputs.

### Owner intake clarification — continuous automatic batch grading, September 23 Pacific / September 24 UTC, 2026

Mark requires a photo-first intake that accepts a Front and Back pair, saves it durably, queues preparation and Astra grading automatically, and immediately returns to empty photo slots for the next card. No separate start-grading action, card-details form or queue-selection action belongs in ordinary intake. The same flow must support one card or up to100 paired cards at a time; bulk files require unambiguous Front/Back pairing, while explicit side slots accept arbitrary camera filenames. Uploading and bounded processing continue while the operator adds more photos or views the queue. Preserve exact original images, recoverable intent IDs, honest attention states for ambiguous evidence, current staff authorization and the separate human review/approval of the final report. This expands intake capacity and simplifies its interaction; it does not assert100 concurrent provider requests or that real-card production acceptance has passed.

### Owner execution direction — native NFC and shared market research, September 23 Pacific / September 24 UTC, 2026

While Mark tests the live ATLAS workflow, he explicitly requests three to four fresh Astra Extra High specialists to build and complete native NFC locking and market/dealer acceptance. This includes integrating Ten Kings Inventory’s newer research/shared-learning features into ATLAS; the earlier audit’s recommendation to defer that adoption is now superseded by this implementation request. Reuse a neutral shared engine and the existing scoped catalog host boundary, preserve each application’s own card/photo/receipt authority, and retain human review before knowledge publication or public sale selection. This request does not choose a new automatic valuation formula or change numerical grading policy, Inventory financial records or immutable approved reports.

Mark explicitly requires NFC writing, verification and permanent locking directly through ATLAS on his Mac with the connected ACR1552U. No phone or GoToTags operational step is required. The old PC grader is reference-only for hardware specifications and history, not an implementation source. Preserve the current F8215/physical specification unless Mark selects a replacement. Complete the missing native locking implementation and exact-stock qualification; no unverified lock mask, fabricated physical result or machine final card approval is admitted by this direction. Real authorized dealer terms and reviewed shared catalog publications remain sourced inputs, not facts supplied by software tests.
...
Mark supplied `Atlas Grading logo.png` and directs that the ATLAS label use that exact gold fingerprint-A artwork. The front center should emphasize the player or character name in large bold type, followed only by the optional parallel/variant; omit year, manufacturer, set and card number from the printed center. Complete identity remains in the approved report. The reverse must be completely plain black, with no QR, text, logo or border. Preserve the selected physical dimensions and NFC placement unless Mark changes them. Provide a reusable, versioned template and an interactive design preview so later typography/layout iterations are straightforward. Preview edits are design drafts, not changes to a saved report, physical print intent or deployed station.

Before further NFC implementation, audit and reuse the reader/chip work already completed on Mark's MacBook. The retained September 8 Pacific test confirms native writing and full readback, with Mark's subsequent phone-scan confirmation. Do not characterize the reader or basic encoding as missing. Exact approved-report integration, station activation and permanent-lock qualification are separate remaining work; the successful diagnostic test did not attempt permanent locking. This amendment changes label presentation and records the reuse requirement without altering grades or authorizing new hardware effects.

### Owner direction — ATLAS finishing, batch automation and dealer network, September 22, 2026

After testing the released manual workflow, Mark reports that ATLAS looks good overall and directs the next build: integrated NFC; automatic label printing and a new ATLAS label design; an optional actual graded/slabbed-card photograph prominently above the report; Astra-first preparation of batches of 10–50 cards for a rapid human review queue; complete card details in reports; eBay graded sold-sale references; and an authorized-dealer network for offers, grading submissions and configured partner service terms. Use Astra Extra High implementation specialists in parallel within available runtime capacity. This authorizes implementation of these ATLAS features and supersedes the earlier discussion-only/replacement-operator deferral for this next build. It does not establish the unresolved historical incident's exact cause, change numerical grading policy, or authorize machine final approval.

Verified photo pairs should enter preparation automatically; identification, geometry and defect assistance preserve their actual source bindings and human corrections. A batch is an intake/work unit, not a promise of fifty simultaneous provider calls. Measure throughput against actual compute and provider capacity; one failed card must not block the rest. Automated findings and tentative reports retain machine provenance until the trained human reviews them. The rapid review screen should minimize navigation and offer one deliberate approval of the exact reviewed report. No machine action may publish itself as a human-reviewed learning example.

Approval should prepare the exact label and NFC handoff together. Continue the owner-selected MacBook/ACR1552U/F8215 integration and canonical label dimensions until Mark supplies a replacement physical specification. Printer/material setup, calibrated scale, native signing/pairing and real F8215 write/readback/lock qualification are implementation inputs; a browser print request is not physical print completion. Preserve separate label, NFC, assembly and welding facts and recovery from uncertain device results. Reuse the permanent ATLAS report reference without inferring Ten Kings ownership or silently issuing a new certificate namespace.

The optional slab photograph is presentation media, never replacement grading evidence. A lightweight interactive presentation of the actual photo is the scalable starting proposal; genuine multi-angle/3D capture remains optional and must not fabricate unseen card details. Historical approved report bytes and grading evidence remain immutable when ancillary presentation or market data changes.

Expose the saved card identity clearly. Market information remains independent of the grade: show sourced sold listings by grader, grade and sale date, with recent first within the selected view; do not equate graders or the most recent sale with market value. An estimate, if introduced, must identify its method, matching evidence, sample size, currency, date and uncertainty. No new automatic valuation formula is selected by this request. Dealer offers require actual authorized partner terms, exact scope, amount/currency and expiry; an invitation to request a quote is not a firm buyback. The ATLAS dealer program is distinct from the existing Ten Kings vaulted-card 80%-TKD contract.

Build a searchable authorized-dealer directory linked from the report, with buying and grading-submission capabilities and Google Maps integration when configured. Only real approved dealer locations and supplied prices/turnaround terms may be published. Dealer roster, commercial terms and printer details are pending owner inputs; this does not block independent software and design work. No customer payout, dealer message, physical chip locking, fabricated offer or invented location is implied by implementation authorization. Current release evidence remains separate from this new build direction.

Owner implementation approval — September 22, 2026 Pacific: Mark explicitly approved building the complete report/workflow/design direction now, using three Astra/xhigh specialists. The authorized scope is independent durable per-photo geometry, the interactive black/gold report and shared staff/customer brand, and connecting exact human final approval to a public versioned report with evidence deep links and print/save-PDF. This does not change grading rules, approve a card on his behalf, issue a physical slab/NFC credential, or authorize paid grading experiments. Implementation and release evidence are recorded separately below and in the session log.

Owner report, design and early-geometry direction — September 22, 2026 Pacific: after testing the Back-geometry correction, Mark reports that it works better and requests the next experience improvements. Geometry should begin automatically as uploaded photographs become usable, without waiting for the Geometry button, card identification or completed card details. Preserve the required verification/working-image step and separate human geometry confirmation. This supersedes the earlier dependency between saving complete details and starting geometry processing, while retaining recoverable saves and review navigation. Mark explicitly selects the ATLAS logo and black/gold visual identity across both staff and customer-facing experiences, with exceptional visual quality, clarity, transparency and interactive card/defect exploration as product goals. These are owner requirements, not claims that the redesign or early processing is deployed, nor approval of a particular report, new scoring policy or speculative visual feature. The [report and workflow direction](../atlas/plans/ATLAS_REPORT_EXPERIENCE_20260922.md) separates the observed approval/publication gap from proposed design details. The earlier statement that branding is deferred is superseded; an existing black/gold ATLAS brand image is present in the public app, and no replacement logo is inferred.

Owner intake and responsiveness clarification — September 21, 2026 Pacific: after testing the rebuilt ATLAS workspace, Mark requests one **Save & Review Geometry** action that saves the current card details and opens geometry, replacing the separate Save and Review buttons. Preserve validation and recoverable saved requests; a missing field or preparation failure must explain the next step. Allow Front and Back photo intake to overlap instead of disabling the opposite side during upload. Retain untouched full-resolution originals and the approved full-resolution SDR working-image policy. Reduce avoidable navigation, save and preparation delays while preserving source binding, current authorization, deterministic grading and exact human approval. Measure improvements rather than promising instant native processing. Mark reports that the interactive draft worked well but has not accepted a final report and wants to test more cards. Black-and-gold styling and the ATLAS logo are a later design pass pending the supplied brand assets. This amendment does not approve a card/report or change scoring.

Owner grading-policy clarification — September 21, 2026 Pacific: for the rebuilt ATLAS grading workflow, Mark directs that the awarded final grade use only whole or half points. Keep the existing defect measurements, multipliers, category thresholds, Front70%/Back30% weighting, equal category weighting and unrounded overall calculation. Show the calculated decimal score in report details, then round the raw overall directly to the nearest0.5 for the awarded grade:9.8→10 and9.7→9.5. Exact quarter-point ties round upward (9.75→10); the one-decimal detail is not an intermediate rounding input. Show whole awarded grades without a decimal suffix. Persist the final grade and versioned rounding policy in each new report and bind final human approval to that exact report. Historical reports retain their original grading policy and bytes; this does not silently regrade earlier approved cards or change the shared legacy Speedster engine. The full interactive draft must expose the exact findings and measurement/scoring explanation before separate final approval. See the [report-review audit](../atlas/audits/2026-09-21/report-review.md) and session handoff for implementation and qualification status.

Owner direction — September 12, 2026 Pacific: this task is focused exclusively on ATLAS Grading. Mark wants the first new defect-assistance experiment to use the `gpt-6-astra` API with ATLAS's proprietary human-reviewed defect memory. Do not integrate SAM 3 into the new workflow as a prerequisite to that experiment or require a new SAM baseline build first. Evaluate Astra's defect finding, classification and proposed outlines against human-reviewed physical-card evidence; SAM integration remains deferred until those results establish whether it is useful. Keep the existing geometry, trace-editing, deterministic measurement/scoring and separate final human report approval. Mark requests that every deliberately human-reviewed defect can contribute a lesson for the next relevant analysis. This is application-managed retrieval of reviewed examples, not automatic model-weight training or guaranteed improvement after every example. The exact UI action that publishes lessons remains a stage-design detail; the lead recommends the existing Confirm findings action without a second confirmation. This scoped detector experiment does not waive the unresolved initiating-cause prerequisite for a replacement autonomous grading operator. The revised build sequence is recorded in `docs/atlas/plans/ATLAS_SPEEDSTER_REBUILD_PLAN.md`.
...
Shared-engine direction, September 11: Mark requests reuse of the refined Ten Kings physical-inventory Google Vision + Astra identification and variation/sold-comps research in ATLAS. Use the same relevant card fields and evidence behavior; start identification automatically after photographs are available. Share reusable card identity/variation knowledge and sold-comps engines across the two companies, while preserving separate inventories, ownership, locations, financial records and grading decisions. Consult the named Financial App task and reuse its deployed engine source rather than recreating or copying an older OCR flow. The consultation identifies application `bb10f235d5bddd2ca30ba19e5389e80d43cd2fec` as the current collect release, with eight-field fast identification plus separate background variant/value research. This owner direction adds shared capability to the planned ATLAS workflow; it does not authorize company inventory merges, an immediate model replacement or weakening existing evidence meanings. Astra-assisted defect finding and map drafting are evaluation proposals. See the [shared-engine discussion](../atlas/plans/SHARED_CARD_ENGINES_DISCUSSION.md).

Planning continuation, September 11: Mark asked the agent to continue the investigation/design flow and compile the connected plan before stopping. The [consolidated ATLAS Speedster rebuild plan](../atlas/plans/ATLAS_SPEEDSTER_REBUILD_PLAN.md) now brings together the current/proposed comparison, complete manual journey, selected engine extraction, shared identity/research, detector/learning evaluation, action dependencies, lightweight recovery and acceptance sequence. It preserves the distinction between owner requirements and recommendations. Server HEIC support, detector selection, precise full-bleed/size policy, Astra auto-confirmation and grading-lesson publication are not silently approved by compilation. The initiating transaction cause remains unconfirmed beyond the observed failing subsystem; implementation and deployment have not begun.
...
Owner layout approval, September 12 UTC: after trying the sample combined geometry screen and clarifying that the Front physical-edge adjustment was only experimentation, Mark said “yes, it feels great.” The approved layout keeps Front left and Back right, with physical-edge and printed-border tools in that same workspace and one geometry confirmation for both reviewed sides. This approves the layout; it does not adopt the sample coordinates as real-card corrections, certify image detail, settle borderless grading, or authorize automatic Astra confirmation/final report approval.
...
Owner defect-review decision, September 12 UTC: after clarification using a six-finding example, Mark selected one **Confirm findings** action for the corrected displayed list after inspecting both sides. This bulk action accepts the current reviewed list, including remaining displayed findings, while preserving individual corrections. Final human grading-report approval remains a separate action. The exact knowledge-publication event and automatic Astra continuation rules remain unresolved; this choice does not publish a lesson or approve a report implicitly.

Owner build direction, September 12 Pacific: after selecting ATLAS-only, Astra-first defect evaluation and the proposed single-confirmation memory loop, Mark instructed “lets build this now” with multiple Astra Extra High agents. For this build, the existing deliberate **Confirm findings** after both sides are inspected publishes the reviewed outcomes into proprietary defect memory; its authenticated REVIEWER must have current card edit authority and must be the confirmer of that publication. Report certification is not required for this reviewed-example event; final **APPROVE_REPORT** remains a separate trained-human authority check. This resolves the earlier pending event/mapping for the bounded human-requested experiment. Save immutable source/crop/trace and reviewer/proposal provenance, acknowledge publication before treating examples as available, and retrieve current relevant lessons for each new Astra request. Test gpt-6-astra finding/classification/outlining first; SAM3 integration remains deferred until measured gaps justify it. This authorizes the build and does not assert model accuracy, deployed availability, automatic grading approval or closure of the separate replacement-operator initiating-cause prerequisite.

Subsequent implementation evidence, September 12 UTC: the [paired workspace and decoder checkpoint](../atlas/audits/2026-09-12/workspace-and-decoder-implementation.md) records the approved controlled geometry layout, checked side actions and raster decoding, with 72 new unit tests and seven synthetic browser scenarios passing after independent review. This is local implementation evidence; native HEIC, durable service wiring, remaining grading stages and real-card detail acceptance are unfinished. The owner-approved product scope, final-human-approval rule, serving application and initiating-cause prerequisite are unchanged.

Next integration evidence, September 12 UTC: the [preparation/storage/identification checkpoint](../atlas/audits/2026-09-12/preparation-storage-and-identification.md) records actual CPU preparation/current border proposals in the synthetic geometry harness, an immutable photo-storage adapter and the unchanged shared eight-field identifier behind injected effects. Eight browser scenarios pass. Native HEVC direct/grid decoding now covers a qualified subset; Exif/ICC/HDR, general iPhone and optical grading acceptance remain incomplete, and the unchanged preparation engine currently accepts opaque sRGB RGB8 only. These are code/evidence updates under the authorized staged build, not new image-quality policy, final approval authority, production wiring or closure of the operator-cause prerequisite.

Manual workflow implementation evidence, September 12 UTC: the [defect/persistence/photo metadata checkpoint](../atlas/audits/2026-09-12/defects-persistence-and-iphone-metadata.md) now verifies paired defect tracing/correction, checked CPU measurement, ordinary staff authentication, compact PostgreSQL CAS drafts and immutable full artifacts, the selected bulk findings confirmation, and a separate exact trained-human report snapshot. Nine authenticated synthetic browser scenarios and 160 local tests pass; the SQL proposal is inactive outside owned local fixtures. Bounded EXIF/ICC decoding is qualified, while retained HDR gain-map iPhone originals remain refused and the owner working-view quality choice is pending. This records implementation under the approved staged build; it does not approve HDR flattening, change grading/learning rules, deploy the app or close the initiating-cause prerequisite.
