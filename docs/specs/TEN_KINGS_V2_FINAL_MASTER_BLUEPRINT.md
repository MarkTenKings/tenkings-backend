...
### Owner direction — prepare the clean main website (2026-09-16)

Following the domain/admin investigation, Mark supports preparing tenkings.co as the clean future consumer and staff experience, with most engineering effort continuing the approved recognition/catalog/private-comps plan and one work lane preparing the main website. Mark explicitly states that the existing Wix marketing content has no required utility and may all be discarded; recreating or importing that content is not a prerequisite for the replacement.

Preparation should reuse the existing Ten Kings repository and proven Inventory components/services. It must preserve existing identities, canonical stock/catalog writers, private originals, financial history, email and required subdomains. The current collect permanent-card/NFC URLs and V1 customer rights remain unchanged by this direction; the future main website does not by itself authorize retiring all collect services or moving Atlas into the Ten Kings app.

The exact route/host implementation and hosting recommendation belong in the dated preparation plan. Vercel remains the current web host; Cloudflare hosting versus DNS is a question to resolve, not a newly locked platform decision. This amendment records preparation and disposable Wix content, not a completed domain cutover, provider-account cancellation, DNS mutation or application release.

### Owner approval — execute the five-lane delivery (2026-09-16)

Mark approved the concrete work split and hosting recommendation and instructed the team to build and finish it. Implement the private-comp repair/qualification, durable reviewed shared catalog, real sports/Pokémon source pilot, responsiveness/accuracy and Atlas integration acceptance, and the clean tenkings.co consumer/staff shell in the existing repository. Keep the application on the current Vercel foundation; no Cloudflare Workers or DNS-provider migration is part of this approval. Root coordinates the qualified releases and main-domain web cutover while preserving email, collect permanent/customer links, existing sole writers and Atlas ownership.

This approval authorizes the implementation and necessary qualified release work without another generic go-ahead. It does not manufacture catalog evidence review, missing provider facts, actual-phone acceptance or independent accuracy labels. Preserve the explicit reviewed publication boundary and record unresolved evidence honestly. Wix marketing content may be discarded; domain/account cancellation and destructive set/data operations are not required by the replacement.


### Owner addition — sold-comp value in the Inventory list (2026-09-21)

Mark requests the eBay sold-comparison value on the main staff Inventory card list and a clear explanation of how multiple comparisons and incorrect matches affect it. Show the latest applicable saved research estimate as a separately labeled eBay comp value, with its selected-sale count and access to the actual evidence and calculation. Preserve the staff-entered expected sale price, cost and financial/profit records. Unknown, stale, unavailable and unfinished research must remain explicit rather than become zero or an average of all returned listings. Reading the list must not invoke providers, start/retry research or delay capture/save. Selected verified sale prices retain the existing integer-cent arithmetic mean and exclusion rules; exact duplicated listing-image evidence must not gain extra weight. Historical research evidence remains immutable. The subsequent owner direction below authorizes durable staff review of this selection.


### Owner refinement — staff correction and visual comp review (2026-09-21)

Mark confirms that staff must be able to correct an incorrect Astra-selected comp and have the card value recalculate. Put the comps used for the value first. Provide quick side-by-side comparison of the saved Inventory card and one eBay comp, with confirm/exclude buttons and equivalent horizontal swipe gestures. Preserve front/back inspection, source price and listing evidence. Exclusion removes that comparison from the derived market value; confirmation can restore an originally selected comp. The initial control does not promote possible/rejected/unverified listings or waive existing identity, condition and sold-price requirements.

Persist each authenticated staff decision against the exact completed research input/result and current card description, with reviewer, time, revision and idempotent request identity. Keep original AI results and all candidate evidence immutable. Concurrent or stale reviews must not overwrite each other silently; uncertain saves retry the identical request. Compute the displayed/list value from the same saved selection using the existing integer-cent mean and exact-image deduplication. Fewer than two eligible distinct-image comparisons remains insufficient evidence, not a fabricated price. Partial human review must not be described as fully staff-confirmed. This review is private market evidence; it does not update acquisition cost, expected sale price, accounting, catalog authority or grading. Additive persistence and the necessary qualified migration/release are within the existing authorized Inventory delivery.


### Owner approval — automatic Inventory research recovery (2026-09-21)

After reviewing the diagnosis of 92 current completed jobs without qualified identity/reference evidence, Mark authorizes building and activating an ongoing recovery process for existing and future individual inventory cards when required details or evidence are missing. This narrowly supersedes the September 16 restriction to deliberately initiated affected-input research: the existing scheduled worker may diagnose current inputs, make bounded original-photo recognition attempts, retain private missing-field proposals, check approved catalog evidence again, and automatically queue bounded research when a meaningful new evidence digest supports it. It must also discover eligible current individual cards that lack a job. Bulk receipts remain outside per-card research.

Keep recovery on the existing durable research lifecycle and sole scheduled worker, with shared concurrency/intake headroom, exact input/result binding, cancellation on changed descriptions, finite paid-work allowances and backoff. Preserve manual retry history, prior attempt evidence and staff-reviewed results. An exhausted manual retry allowance does not authorize resetting history; separately audited automatic attempts remain within the research lifetime ceiling. Empty or unchanged evidence cannot produce recurring paid sold-comp searches.

High-confidence facts read from verified original photos may fill only absent fields in private research context, with original-photo provenance. They do not overwrite saved staff descriptions; disagreements are review needs. Actual human edits continue through the canonical Inventory writer. Existing catalog publication, source/diagnostic evidence, independent sold-image and verified-price requirements remain in force. The process must expose actionable missing fields, conflicts and evidence needs, and resume when supported evidence changes. It cannot approve an unreviewed catalog draft or make an unknown price look complete. Capture, recognition and saving do not await recovery. Qualified additive persistence, release and activation are within this approval; existing shared-host coordination and non-destructive controls remain applicable.

### Owner correction — automatic eBay research for every card (2026-09-21 evening)

Mark explicitly corrects the recovery workflow: "we should always research and fetch ebay comps for every card automatically. my staff should not have to do this." Missing internal approved catalog records must not prevent automatic eBay candidate retrieval for existing or future cards. The system owns research and evidence acquisition; staff review uncertain results and correct mistakes rather than manually researching every card or publishing a catalog entry before a search can begin. This supersedes the preceding recovery amendment wherever its catalog-readiness prerequisite would block searching.

Separate automatic candidate discovery from determining whether a value is supported. Retain and show the actual listing evidence and matching uncertainty. A search attempt is not proof that matching sold listings exist; an unverified asking price, unknown accepted offer, wrong printing, or wrong grade cannot become a qualified value. Preserve bounded paid work, exact card/input provenance, immutable history, reviewed reusable-catalog publication, and responsive intake. The fresh ASTRA ULTRA lead owns correcting this workflow and verifying real comp delivery, including recovery of the existing backlog. Mark's phone and website acceptance remain complete.

Implementation boundary for this correction: private research may compare positive, exact card and printing observations from the saved original front/back photographs with the actual sold-listing photographs without first publishing an internal catalog record. Record this as a separate, explicitly versioned photo-based research match; it does not change an unresolved catalog identity into an approved identity. A private estimate still requires supported name, year, set, card number and visible treatment, agreement with saved facts, exact photo provenance, matching raw/graded condition, verified actual sold amounts and at least two distinct listing images. Seller titles, a model's confidence, a generic “base” label, or the absence of visible foil cannot alone establish the printing. Unresolved or contradictory evidence leaves the value unknown while retaining retrieved listings for inspection. This implements the automatic private-research correction, without expanding catalog publication, public grading-comp confirmation, finance or grading authority. Historical result versions retain their original validation and bytes.
