...
A failed physical observation creates no hosted activation row/pointer and no local activation pointer/receipt. Any nonempty observation staging directory is atomically quarantined beneath the exact failed observation identity; every adapter-written evidence file is protected and recorded by relative filename, SHA-256, and byte size in an immutable failed-observation manifest. Only an actually empty staging directory is removed. The failed identity blocks a second hardware observation. A later hosted request/completion failure uses the existing append-only failure event and archives only its exact matching local PENDING pointer. If hosted has already reached ACTIVE, the exported reconciliation path reuses that exact signed ACTIVE authority and converges the local pointer without another observation or hosted request.
...
Existing content-addressed bundles are never overwritten. Successful activation observations are retained at `activation-evidence/successful/<observationId>/` with the fixed PNG, hosted OBSERVATION authority, and signed workstation observation. Every nonempty failed probe directory is retained at `activation-evidence/failed/<observationId>/` with all adapter-written files and their immutable retained-evidence manifest, including noncanonical or partial filenames written before the adapter throws. Hosted operating contexts are stored immutably per activation. Receipts are immutable at `receipts/<activationId>.json`. The only mutable local file is the atomically replaced `active-pointer-v1.json`; exact failed PENDING pointers are moved into the protected failure ledger.
