...
The reviewed operation permits exactly one URI record, one correlation identity, the exact Ten Kings public URL, Verify After Write, Permanent Lock, and one loopback HTTP integration. The operator keeps the tag off the reader, selects **Prepare F8215 NFC Job** in Ten Kings, clicks **Start Encoding** once in the opened GoToTags operation, and then places one fresh F8215 tag on the ACR1552U. GoToTags performs the encode, complete readback verification, and lock. The helper accepts only one terminal `VERIFIED` callback that also matches the correlation, exact requested and read-back URL, FEIJU/F8215/Type 2/496-byte profile, ACR1552U PCSC2 reader, approved application version, and static locked state.
...
Important enablement blocker: the supported terminal callback proves the final exact URL, post-write verification, and permanent lock, but the qualified operation emitted no independently attributable pre-write callback or blank-NDEF proof. The documented operation options do not provide a reviewed blank-only gate. Therefore this implementation cannot yet prove that an unexpected unlocked nonblank F8215 was not overwritten before locking. Keep both F8215 gates false until independent review accepts a supported pre-write blank-state mechanism. Do not substitute UI automation, clipboard injection, SendKeys, proprietary command probing, or an operator memory check.
...
F8215 jobs share that same operation gate, so a native PC/SC operation and a GoToTags job cannot overlap. `POST /prepare` validates the exact hosted attempt, challenge, public tag identity, URL, F8215 profile, expiration, and local idempotency context; writes one protected job directory; validates and launches one generated operation; and returns `awaiting_manual_start` without holding the browser request open. `GET /operation-status` returns only the sanitized phase and exact terminal evidence for that attempt. `POST /operation-ack` removes only that completed job after hosted activation. One active job is allowed, its TTL cannot exceed the hosted attempt, and an uncertain interruption retains only minimum protected recovery state so the same attempt can resume after a helper restart without allocating another public identity.
...
Use the same command with `-Disable` to turn only the local adapter gate off. The command validates the fixed version, Authenticode publisher, pinned installed template, protected paths/ACLs, empty recovery directory, stopped/disabled Certificate Propagation, and running Smart Card service. It does not change a service, start/restart the helper, rotate a token or signing key, touch the reader, or remove nonempty recovery state.
...
9. Resolve and review the F8215 pre-write blank-state blocker documented above. Only then separately approve `AI_GRADER_NFC_FEIJU_F8215_ENABLED=true`, confirm both hosted and local gates, and run a controlled production-profile smoke under separate real-tag authority.
