...
Fresh Git and authenticated Vercel metadata checks are recorded in [verified-state.json](verified-state.json). Both deployed Preview branch heads match the remote; all non-documentation main Preview blobs equal Inventory `e286ea0a`.
...
| --- | --- |
| [Comp qualification](comp-qualification.md) | Add one V5-enabled real default-cron/disposable-PostgreSQL fixture with synthetic external transports. Then qualify a bounded exact-input hosted diagnostic and actual additional background load. Existing ordinary-sale positives are auctions; fixed-price and independent visual truth remain open. |
...
The local repair gives catalog review its own explicit selected set and displays that target. Existing-set choices, selected ingestion jobs and successful prepared imports can open it. Typing, create-new selection and clearing the selected job clear the catalog target. Legacy draft/version bindings remain independent. A delayed import cannot overwrite a newer explicit catalog choice, and the unchanged preparation child retains the exact pending request through set switches. No intake, research, writer, schema, permission or publication semantics changed.
...
The fix is **local and unreleased**. The main workspace and both deployed Previews still have their verified application commits above. A qualified newer Preview is required before the repaired sports selection path can be used live. No push, build, deployment, flag change, migration or source staging occurred in this takeover session.
