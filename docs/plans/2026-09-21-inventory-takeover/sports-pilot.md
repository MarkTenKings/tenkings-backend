...
**Current follow-through, September 21:** the parent selection repair `de6fa5b6` is included in LIVE Production and main Preview `9a12cd35`; phone acceptance is closed and the admin catalog feature remains off. Seventeen focused UI checks, scoped lint and changed-file type checks pass. The fresh read-only operational preflight below verifies live schema and preserved sports identities through the normal managed-database path. Inventory Preview has no branch `DATABASE_URL` record and is not staging-ready; root must qualify the combined release and supported specialist origin. The original audit and hashes describe the handed-off `e286ea0a` application; its defect section is historical evidence, not an outstanding code task.
...
The runbook now records the **remaining origin decision**: Inventory Preview lacks a branch `DATABASE_URL`; main Preview has its database override but intentionally hides specialist SetOps; collect Production has working DB/current sports selection repair and is a possible operational origin after the qualified Pokémon combined release. Root owns that choice. No flag, domain, schema, provider, host service, source job, publication or UI state was changed. Only admin `SET_CATALOG_EVIDENCE_ENABLED` is in the eventual activation scope; research catalog/consumer, contributions, sale details/full-resolution and diagnostics stay off unless separately qualified.
