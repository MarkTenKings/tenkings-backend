...
Status: preparation-ready audit and next-action packet. The narrow Big Kahuna pilot has qualified preparation code and retained real source evidence, but no actual staging or publication receipt in this lane. It can proceed without importing the 171 unnumbered entries or replacing any of the 379 numbered card identities. The parent-page selection defect is repaired. Actual human source review remains required. Use the [source-bound staging and review runbook](sports-staging-review-runbook.md) for the current next action.

**Current follow-through, September 21:** the parent selection repair `de6fa5b6` is included in LIVE Production `9a12cd35` / `dpl_2XUGoG74E88eVeFGETVzFkQDBhRC`; phone acceptance is closed and the admin catalog feature remains off. Main Preview has advanced to the separately qualified diagnostic candidate recorded in the [current index](README.md); it still intentionally hides specialist SetOps. Seventeen focused UI checks, scoped lint and changed-file type checks pass. The fresh read-only operational preflight below verifies live schema and preserved sports identities through the normal managed-database path. Inventory Preview has no branch `DATABASE_URL` record and is not staging-ready. Real staging/publication await the qualified specialist release/flag, normal collect sign-in and genuine human source/applicability/publication review. The original audit and hashes describe the handed-off `e286ea0a` application; its defect section is historical evidence, not an outstanding code task.

Initial audit checkout: `/Users/markthomas/tenkings/codex-staff-inventory-release-20260910`, HEAD `297fd302b866286824fcea79b8f77c6d995ca3db`; application `e286ea0a190bfdc6807fe8ca58c47b34c94b55c1`. That initial audit changed only this file and performed no provider, DB, hosted API, UI, configuration, deployment, shared-host or ATLAS operation. Later bounded live reads are explicitly recorded below. Root owns SESSION_LOG and integration.
...
## Historical UI integration defect — now repaired
...
This was a source-level finding against the retained real initial state, independent of the default-off feature gate. Root completed the reviewed parent-page selection fix and regressions in `de6fa5b6`; 17 focused checks include the actual mounted catalog/preparation children with an empty pending queue and existing legacy draft context. No fake job, draft rebuild, browser-state injection or fabricated live import was needed. The seven standing PostgreSQL tests were preserved without repetition.

Implemented fix: a distinct explicit `catalogSetId` is updated by a verified existing-set dropdown choice, existing job selection and successful prepared import, and cleared on create-new selection, manual set-text edits and Clear Selected Job. That selection reaches the catalog panel without changing legacy `selectedSetId`, version or draft/editor bindings. The regressions also preserve a newer explicit catalog choice against a delayed import completion. This implementation is completed history; no further parent-page repair is pending.
...
## Fresh operational preflight — September 21

At **21:04:32 UTC**, normal existing BatchMode SSH to the documented droplet streamed a bounded Node read into `infra-bytebot-lite-service-1`. Prisma used that service's existing managed `DATABASE_URL` internally; no credential/env file was exported or printed. PostgreSQL independently returned **17.11**, `transaction_read_only=on`, `transaction_isolation=repeatable read`; queries used 5-second statement and 500 ms lock limits. No migration or business write was attempted.

- Live ledger: **110 rows; 97 active finished; 13 historical rolled back; zero unfinished**. Every one of this checkout's 96 migrations is present with an equal checksum. The one additional live migration is `20260908010000_speedster_prepared_evidence_authority`; it was neither changed nor inferred from an ATLAS report. Catalog migration `20260916233000_set_catalog_evidence` finished September 17 at `00:30:48.961Z`, one step, no logs, exact local checksum.
- All 19 inspected taxonomy/catalog/guard tables and `SetDraft.currentCatalogPublicationId` exist. All eight catalog triggers are enabled, including immutable audit history. Each new catalog table has 13 validated constraints. The follow-up read verifies all six catalog trigger-function bodies against the checked-in migration and all eight migration-created unique indexes are valid/ready. PostgreSQL truncates the observation-proposal index name to `SetCatalogObservationProposal_producer_observationId_inputRevis`; direct table-index read confirms its exact `(producer, observationId, inputRevision)` columns. The private receipt's untruncated-name lookup is therefore not an absent-index finding. These are actual live facts, not the disposable migration count.
- Sports still has **one APPROVED/unarchived draft, eight ingestion jobs (six APPROVED/two FAILED), six sources, ten programs, 379 cards and six versions**. Card/program/version ID-roster hashes match the pinned preparation plan. Version 6, its ID/hash and all three Big Kahuna card IDs/names remain unchanged. Parallels, scopes, variations, odds and publications are zero; active seed/replacement work is zero.
- Pokémon exact key `Black & White-Legendary Treasures` has zero draft/job/source/program/card/parallel/scope/variation/odds/replacement/publication rows. Bounded `legendary…treasures` / `bw11` alias checks find none in seven core set tables, legacy `CardVariant`, taxonomy bridges, reference-image set keys or published manifest text. This is a scoped absence observation, not a license to skip the fresh ordinary import preflight or invent aliases/identities.

The runbook records the **remaining specialist activation**: Inventory Preview lacks a branch `DATABASE_URL`; main Preview has its database override but intentionally hides specialist SetOps; collect Production has working DB/current sports selection repair and still needs the qualified combined release/flag and normal human sign-in before real catalog operations. Root owns that release. No flag, domain, schema, provider, host service, source job, publication or UI state was changed by this preflight. Only admin `SET_CATALOG_EVIDENCE_ENABLED` is in the eventual catalog activation scope; research catalog/consumer, contributions and sale details/full-resolution remain separate. Root's later bounded main Preview diagnostic has its own record and does not stage catalog evidence or change Production flags.

One separately root-pinned Snivy development unit was read at **21:08:02.900Z** without reading unrelated inventory inputs. Its sole completed research job, canonical input hash, current description event/hash and both managed photo keys still match retained evidence; the one-card opening receipt has no observed cancellation. Private receipt: `/Users/markthomas/Library/Application Support/TenKingsInventory/investigations/20260921-catalog-readonly-preflight/schema-and-snivy-bindings-210805.json`, SHA-256 `7cd36f2129007d383a5fb896b88e1f90c68b7dcc2370fc9d34dc0a4a98699c3f`. No original photo bytes or provider/model calls were fetched. This input remains development evidence excluded from the independent holdout; bindings are not factual/visual truth or diagnostic execution approval. The narrow read is not a full workflow replay.

## September 21 continuation audit and delivery

The current compiler, source-fact-use V2 input, preparation API/receipt and genuine taxonomy exporter support the narrow packet without another sports application change. [The new runnable runbook](sports-staging-review-runbook.md) gives the exact export route, real receipt-array mappings and create-new compiler commands. [The selection input template](sports-selection.pending-mapping.template.json) contains only the four preserved existing IDs; eight unstaged IDs remain null. It is intentionally not a finished selection or publication packet.

Rehashed all three retained source PDFs, including the compiler's unchanged Pokémon prerequisite; all exact bytes match the pins. Visually inspected complete checklist page 10 and Round 2 odds page 2. TBK-1/2/3 and the three program rows agree with the proposed recipe. No source approval, per-card applicability or image permission follows from this check. Private byte-check receipt and derived QA renders are under `/private/tmp/tenkings-sports-review-preparation-20260921-01`.

Independent read-only review of the new V5 default-cron fixture and runner found no actionable correctness or containment issue. The reviewed prepared-02 launch seal `3e47594574ad695a812f54835667fed929496ef261dc5f54ba85b87db9632565` and all 415 sealed source files matched at audit, with no added sealed-root paths. Root subsequently reports its freshly resealed run passed three claims/completions and cleanup. That run is root's execution evidence, not a sports-lane run or performance/accuracy claim. This continuation changes only this document and the two linked sports handoff artifacts; root owns integration and SESSION_LOG.
