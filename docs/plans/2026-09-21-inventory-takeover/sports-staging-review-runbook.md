...
Use `/admin/set-ops-review` on the root-qualified specialist origin containing selection repair `de6fa5b6` and any combined Pokémon release required for this operation. Production remains `9a12cd35` / `dpl_2XUGoG74E88eVeFGETVzFkQDBhRC`; main Preview has advanced to the separate diagnostic candidate in the [current index](README.md). The main website and configured main Preview intentionally hide this specialist route. **Inventory Preview is not staging-ready: root's fresh metadata shows no branch `DATABASE_URL` record.** Collect Production has working DB/current sports selection repair; the qualified specialist release/flag and normal collect sign-in remain pending before real staging. Ordinary human authority is required: `reviewer` for preparation and `approver` for publication. No token copying, static operator key or service identity substitutes for that session.
...
Root chooses and qualifies the stable specialist origin and exact combined artifact, verifies normal human DB reads there, and only then applies the scoped **admin catalog** flag/build change. Keep `STAFF_INVENTORY_RESEARCH_CATALOG_EVIDENCE`, `STAFF_INVENTORY_CATALOG_CONTRIBUTIONS_ENABLED`, sale details/full-resolution and Production diagnostic execution disabled. Root's later, separately bounded main Preview diagnostic does not authorize catalog activation. Keep `RUN_DB_MIGRATIONS=false`, existing Production/Preview settings outside the chosen delta, and the sole cron owner. The current read does not turn a service credential into a human publication session.

Catalog mutations left undone: specialist combined release/flag activation, source uploads, preparation writes, Pokémon import, draft approval and catalog publication. The dated read-only preflight above made no environment or deployment change; later Preview diagnostic work is recorded separately in the current index. Preparation remains unreviewed until the actual human factual/applicability/publication decision; image reuse permission remains separately required. Repeat the service's own bounded plan/snapshot immediately before the authorized preparation, since this observation does not reserve the tables.
