...
Current status: main implementation frozen at `945812ca`, included with the catalog follow-up in qualified Vercel Preview `62c0479b`; production web/domain activation pending. The original preparation packet below is retained as the release checklist. The owner subsequently approved full execution. Exact-host routing, the main/staff shell, server-checked access, and auth hostname support now exist; the auth-only change is live. Initial 328 integrated checks, migration-disabled production build and Chrome/WebKit layout/route checks pass; the expanded follow-up suite passes344 checks. Exact branch Preview hostname passes hosted home/staff/API/privacy checks on deployment `dpl_B8VHHRN4R72fKJ44h6P8UBZL4gqx`, with its Git-connected build and native Linux Sharp verification passing. Cloudflare widget/staff sign-in, phone acceptance, the qualified production application release, and apex/www DNS cutover remain pending. See `docs/handoffs/SESSION_LOG.md` for current evidence.
