...
- The versioned v2 content/filter contract is implemented behind a fail-closed activation gate: content type is descriptive, filter authority defaults text/logo/border On and artwork/foil/other Off, each zone is adjustable, and the proposed filter area is a deterministic `0.6 mm` physical-card dilation of the unchanged human content polygon under strict full-contour containment. Historical v1 maps and decisions remain byte-compatible. The frozen 50-card audit is `INCONCLUSIVE` (corpus SHA `255e3b81…`, report SHA `71806fab…`): it has 2,292 human outcomes but no compatible 2023 MEW EN Reverse Holo card, no compatible map registration/revision binding, and no exact evidence hashes. V2 save/activation therefore remains blocked; the review diagnostic may explain current v1 containment without changing decisions.
