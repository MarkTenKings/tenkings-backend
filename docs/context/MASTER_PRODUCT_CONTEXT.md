...
- Mathematical Calibration V1 is an explicit, fail-closed machine-grading contract. Normal mathematical acceptance remains controlled by the centralized formulas and thresholds: an ordinary rejected or incomplete run cannot produce or use a finalized/trusted bundle. A normal accepted profile has `status=finalized`, `isCalibrated=true`, and the exact 12-member V1.2 bundle. It never falls back automatically to V0, a manual score, another calibration, another camera, or another image. An authenticated admin may separately adjudicate a persisted failed report by explicitly confirming all four scores; that produces a visibly human-reviewed completion while preserving the immutable machine failure and never changes calibration authority.
- One exact incident-bound session, `math-cal-v1-20260722-4cfa410c-01`, has a separately versioned product-owner operational policy path. Its mathematical profile remains `status=rejected` and `isCalibrated=false`; all measurements and the complete 36-issue rejection ledger remain unchanged and visible. Its transparent 13-member bundle is operationally eligible only after exact loader and registry verification plus a fresh-human-admin ECDSA-signed `ACTIVE` calibration activation authority over the exact bundle manifest, member ledger, runtime context, rig, and operating context. The owner record is content-addressed decision metadata; the signed activation authenticates and authorizes its Production use. No browser boolean, threshold-pass claim, fallback, cross-session reuse, newest/closest/LKG selection, or automatic rollback is allowed. Owner-accepted reports require that exact activation and visibly disclose `REJECTED`, `isCalibrated=false`, Mark / Ten Kings, the decision reason and timestamp, all 36 exceptions, and the activation signature/bundle provenance. Revocation or supersession remains explicit and append-only. This policy does not modify normal V1.2 12-member mathematical acceptance or authorize any other rejected calibration.
