...
The main website and staff Inventory are live at `https://tenkings.co`; www redirects to the main origin and collect retains specialist/customer/card/NFC links. Current Production is exact `9e8a9c1d99cce0b4b8a4a5dbde241fc916cdb439`, deployment `dpl_6GK1WtpAY19dBQh5csQbZCR6qP8B` (`ewysxk0rw`), verified on all five Production aliases and the sole minute research cron at September 22 03:33:39 UTC (September 21 local). Postflight passed 28 route/privacy checks and six assets, build `b9EpiUYH5zneUEWK0NAfB`. Stable main Preview is the same source through `dpl_3BwdAS3wHjkGD17M7ZY6r2DJvBpv` (`ocp9wiuex`), with its exact artifact build/private routes/assets verified. Mark has accepted his phone test and new website; do not reopen those acceptance requests.

The Inventory lead owns `codex-staff-inventory-release-20260910`; the Git-connected main release uses `codex-main-site-release-20260916`. ATLAS remains separately owned. The current source retains the earlier Pokémon preparation, V4/V5 tooling, disabled diagnostics, list eBay values and durable side-by-side comp review, and now adds ongoing automatic recovery. Only `STAFF_INVENTORY_RESEARCH_RECOVERY_ENABLED=true` was enabled in Production; optional catalog/admin-consumer, sale-detail, full-resolution, contribution and diagnostic execution remains off. The additive recovery migration passed exact old ledger/schema/job/history preservation and a no-op second deploy: 99 active entries plus 13 retained rolled-back entries. Vercel builds skipped migrations.

### Automatic research recovery — live

**Owner correction, September 21 evening:** the current recovery implementation is incomplete: its catalog-readiness check can prevent automatic eBay searches, including on new cards. Mark requires automatic research and eBay comp retrieval for every card without staff doing per-card research or catalog preparation. A fresh ASTRA ULTRA lead owns this correction; see the [short current handoff](../handoffs/2026-09-21-inventory-automatic-comps-lead-handoff.md). The deployed recovery counts below describe checks performed, not recovered comp coverage. No correction release is claimed by this handoff.

Mark approved ongoing recovery for existing and future incomplete individual cards. The existing cron now performs bounded original-photo enrichment, reviewed-evidence checks, changed-evidence research refreshes, historical missing-job discovery and staff-facing blockers/source suggestions. It preserves nonnull staff facts, manual retries, prior attempts and comp review decisions. Source search links remain unreviewed and cannot authorize identity or pricing. Local qualification passed 423 staff tests and 21 real disposable PostgreSQL cases; see [recovery implementation and runtime evidence](../plans/2026-09-21-inventory-research-recovery.md).

The initial automatic pass is complete across all 98 eligible individual cards: 80 wait for reviewed catalog evidence and 18 need staff review. Within the original 92 unknown cards, 76 wait for catalog evidence and 16 need details/conflict review; all 18 missing-anchor photo analyses completed once. The four failed and two previously missing jobs were also assessed. The 03:30:01 UTC canonical audit proves all 97 original inputs/manual-retry/attempt prefixes remain intact, with no missing jobs, stale bindings, due checks or live/expired recovery leases at that snapshot. No new sold-research attempt or qualified estimate was produced. These are real recovery outcomes, not evidence of complete catalog coverage or market accuracy. The source-search correction also fixes a redirecting fallback endpoint and rejects provider challenges/malformed payloads; 15 focused tests and 54 source/worker integration checks pass. Existing receipts and budgets are preserved. Current records are indexed in [Inventory delivery](../plans/2026-09-21-inventory-takeover/README.md) and [main release](../plans/2026-09-16-main-only-release.md).
