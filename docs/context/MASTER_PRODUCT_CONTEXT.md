...
The consumer/staff main website is live at `https://tenkings.co`; www redirects to the main origin. Collect retains specialist tools and existing customer/card/NFC links. Exact Production source is `bc92b1023640ab21499506fb2d4fb5e2bdb188df`, deployment `dpl_FXLDCapouMhB7WtULnCTb45QXG31`. The sole inventory research schedule remains on that deployment. DNS/TLS passed the earlier main-site cutover; this release passed 28 routing/private-header checks, six assets and a natural cron HTTP 200 request on its exact deployment. Mark has confirmed both his previous phone test and the new website working; those owner acceptance items are complete as of September 21.

The Inventory lead uses `codex-staff-inventory-release-20260910`; main release uses `codex-main-site-release-20260916`. ATLAS grading remains separately owned. The qualified Production source now includes Pokémon preparation, V4/V5 accuracy tooling, the disabled private exact-input diagnostic, Inventory-list eBay values and durable side-by-side staff comp review. Optional catalog/enrichment/diagnostic execution remains off. The additive review migration passed exact-ledger preservation and second-deploy no-op; 98 active migrations and 13 retained rolled-back rows are recorded. One real Snivy diagnostic completed with retained private evidence and an unchanged saved research record; identity/value remained unresolved. Its execution flag remains false in the current qualified main Preview `dpl_Hie34Nkq8rhtp1MGjURUAsY7J8yH` from the same bc92b102 application source; the current delivery index records the evidence and remaining browser check. Optional catalog/research enrichment stays off pending its own qualified release and evidence. A bounded saved-result aggregate found all 92 completed jobs are legacy V2 unknowns with unresolved identity and zero references; none has a saved estimate or selected comp. The new UI exposes evidence and recalculates eligible selections but cannot supply missing catalog/reference evidence. The follow-up canonical audit verifies these are 92 distinct current cards: 18 lack required lookup fields; the other 74 have zero exact legacy set-alias hits. Ninety have rerun budget; two have exhausted the explicit retry allowance. New jobs use V3 with the same catalog gate while the shared-catalog consumer is off. See [research recovery](../plans/2026-09-21-inventory-research-recovery.md) for the precise remedy and current-result-versus-attempt-history distinction. Current receipts and remaining work are indexed in [Inventory delivery](../plans/2026-09-21-inventory-takeover/README.md) and [main release](../plans/2026-09-16-main-only-release.md). The older general verification date and architecture history below are retained with their original scope.

### Automatic research recovery candidate — September 21

Mark additionally approved ongoing recovery for both existing and future incomplete individual cards. The local candidate implements bounded original-photo enrichment, approved-evidence rechecks, exact-digest automatic research, historical missing-job discovery and staff-facing blockers/source suggestions on the existing sole cron. Saved staff facts, prices and review history remain authoritative. Local staff tests and real disposable PostgreSQL checks pass; see [recovery implementation](../plans/2026-09-21-inventory-research-recovery.md). This candidate is not yet deployed or activated: Production remains bc92b102/FXL, and the separately owned ATLAS shared-host/DB mutation hold is still in effect.
