...
- Keep one Basler/Pylon capture implementation, one bridge-owned watchdog, one capture lock, and one serialized hardware operation. Rapid Capture is a retained throughput queue: after the operator explicitly captures both sides, one serialized background worker finishes that card's report while the next card may begin. It is not automatic capture or a fallback. There is no production cold/debug path, manual capture/geometry path, browser Safe Off system, or compatibility fallback.
