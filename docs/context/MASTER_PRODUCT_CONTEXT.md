...
- Production Card Maps now contain valid append-only revisions authored from the recovered 2023 Pokémon MEW EN Reverse Holo Squirtle draft: FAMILY v5 `cmspjh5h50003lddtgtdl9o5q` and EXACT v1 `cmspjh5sc0005lddthf23tvt6`. The source session and both side registrations pin EXACT; a different matching card identity resolves FAMILY; the invalid historical FAMILY v1-v4 rows remain immutable evidence. The numeric persistence fix is deployed in merge `a1813ea282c1e9e539e3b36b02d9716c975d0c24` and normalizes new immutable payload numbers before both hashing and JSON persistence without changing legacy revision hash verification.
