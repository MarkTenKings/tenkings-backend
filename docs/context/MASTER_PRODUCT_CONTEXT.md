...
verified_by: Codex from protected main code at 4b48a386e79718ac4e1d88e431d29304b7d7a685, PR #98-#104 handoff evidence, and the one-road Rapid correction
...
- Unpublished cards remain in the existing persisted local **Finish / Review Queue**. The fixed-rig side worker keeps active concurrency at one while the helper's serialized finalization conveyor prepares exact-card grade, report, release, and Label V1 data; separately, the station browser orchestrates the existing hosted Google Vision OCR lifecycle after both exact normalized PNGs are verified. Every stage remains bound to the same `queueItemId`, `gradingSessionId`, and `reportId`. OCR persists one safe result or terminal failure, never retries automatically, and never confirms, publishes, or mutates inventory.
- The hosted Finish queue remains post-publication only. **Approve & Publish** requires the activated exact local queue/session/report identity and cannot authorize another capture or publish another card.
