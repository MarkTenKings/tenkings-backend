...
## Inventory and main website verification — September 22, 2026

The main website and staff Inventory are live at `https://tenkings.co`; www redirects to the main origin and collect retains specialist/customer/card/NFC links. Production is exact `db465c1c57659233d4841f8679cca0aac3ecdd8f`, deployment `dpl_CyDxyASkDk2HGGRwjDtCGqovtLQX` (`41olp9f3x`). Direct readback at 15:35:59 UTC verifies all five Production aliases, project target and the sole minute research cron. The 15:36:22 UTC postflight passed 28 route/privacy checks and referenced assets, build `HdwiGQMJw0yDYUgMuhh83`. Stable main Preview is the same source on execution-disabled `dpl_2VJgRGbJA1TUm9iikGZQvEX6WtLy` (`p6s7ca002`). Mark's phone and website acceptance remain complete.

The Inventory lead owns `codex-staff-inventory-release-20260910`; the Git-connected main release uses `codex-main-site-release-20260916`. Production has `STAFF_INVENTORY_RESEARCH_RECOVERY_ENABLED=true` and `STAFF_INVENTORY_RESEARCH_SALE_DETAILS=true`; catalog/admin-consumer, full-resolution, contribution and diagnostic execution remain off. This release required no new migration or shared-host restart. Vercel skipped migrations; the previously verified ledger remains 99 active and 13 retained rolled-back entries. ATLAS remains separately owned; root released the Inventory coordination hold after promotion and the first preservation snapshot.

### Automatic research recovery — v6 live, outcomes still being observed

The owner's automatic-research correction is deployed: usable saved descriptions can trigger eBay candidate retrieval without an internal catalog record or verified originals. Missing photos, unresolved printing and conflicting details remain visible needs and cannot supply match/value authority. If saved identity is unusable, bounded recognition may propose missing facts; research otherwise records the missing identity without inventing a query. Known individual cards remain eligible after loading. A bulk-origin unit qualifies only through its exact one-unit description and distinct individual photos; inherited group photos cannot authorize per-card research. No new units, physical-possession claims or sales are invented.

V6 permits a private photo-supported identity without claiming catalog approval. A value still requires supported exact identity/treatment, compatible raw/grader/grade condition and at least two verified sold comparisons with known amounts and independent listing images. Unknown Best Offer amounts remain unknown. Nonnull staff facts, exact revisions, prior results, manual retries, attempts, recovery histories and review decisions are preserved. One behavior-version recheck makes the old backlog due without a six-hour wait; unchanged or lost catalog evidence cannot create recurring paid refreshes. Finite lifetime budgets and intake concurrency controls remain in force.

The first postpromotion canonical snapshot at **15:36:24.434 UTC** preserves all 99 baseline jobs, all 93 original nonnull result payloads, manual retry/attempt prefixes and all four recovery histories. It finds 98 eligible individual cards and 11 ineligible shared-bulk units: 92 legacy V2 unknowns, four failed jobs and two queued jobs, with 1,500 historical candidates across 87 cards, zero selected/value and **zero v6 results or new attempts yet**. This is initial preservation evidence, not completed recovery or measured coverage. Subsequent natural-worker outcomes belong in the [recovery record](../plans/2026-09-21-inventory-research-recovery.md), with the original 98-card and historical 92-card cohorts kept distinct.

**Latest canonical checkpoint — September 22, 15:48:10.922 UTC:** 98 eligible cards include nine current v6 results with 77 candidates, 50 assessed comparisons bound to retained image hashes, two matched assessments, zero selected comps and zero estimates. Eight of the nine results retain comparisons; two record an initial comparison failure and one an optional comparison failure. Failure markers can overlap retained comparisons: a completed job means a result was persisted, not that every model assessment succeeded. Thirty-six jobs now have the v2 recovery revision and 36 automatic refresh entries have been appended. Current statuses are 28 queued, one running, four pre-existing failed and 65 complete, including legacy results. All 99 original inputs and history prefixes and all 93 original result payloads remain preserved (84 current, nine in attempt history), with zero stale bindings or integrity failures. This is a dated, incomplete checkpoint; matched labels alone do not establish selected value or measured accuracy. See the [recovery record](../plans/2026-09-21-inventory-research-recovery.md) for the expanded receipt and prior checkpoints. The legacy-reader rollback boundary has now been crossed by persisted v2 recovery state and v6 results; serving and cron code must remain v6-compatible.

Final source qualification passed 520 staff tests and 133 focused tests; the sealed recovery prerequisite passed 26 disposable PostgreSQL cases with no-op migration replay and cleanup. That local receipt explicitly makes no whole-release or hosted-performance conclusion. The real Snivy timing diagnostic completed two model responses beyond the old 45-second cutoff, but delivered 24 possible candidates, zero matched/selected comps and unknown value. A duplicate manufacturer observation caused the optional second assessment to fail strict validation; the first valid findings were preserved. This retained P2 availability limitation does not establish useful matched-comp coverage. See the [current handoff](../handoffs/2026-09-21-inventory-automatic-comps-lead-handoff.md) and [deployment boundary](../runbooks/DEPLOY_RUNBOOK.md).
