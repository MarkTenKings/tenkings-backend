...
verified_by: Codex from protected-main evidence, the Mathematical Calibration V1 implementation/offline validation branch, and protected PRs #106-#107 one-road Rapid/OCR and launcher corrections through d75998837f5e9d6fb994f5efa09decc5ef03035d
...
- Mathematical Calibration V1 is an explicit, fail-closed machine-grading contract. It may be selected only when the bridge has verified one finalized physical-calibration bundle and the hosted readiness boundary has independently trusted that exact bundle. It never falls back automatically to V0, a manual score, another camera, or another image. Until the supervised physical calibration is accepted and independently reviewed, V1 machine grading remains unavailable rather than claiming calibrated measurements. An authenticated admin may separately adjudicate a persisted failed report by explicitly confirming all four scores; that produces a visibly human-reviewed completion while preserving the immutable machine failure and never changes calibration authority.
