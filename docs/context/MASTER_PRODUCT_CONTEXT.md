...
## ATLAS implementation and live website checkpoint — September 9, 2026

ATLAS has implemented components for durable phone access/assignments, supervised initialization and bounded Astra review, human report/proposal/identity corrections, exact public approvals, scoped label/NFC/physical finishing, operational recovery and separate trusted-learning decisions pending bank application. The complete new-photo-to-grade workspace is not implemented or live: staff currently enter the human review queue, new-photo capture is not connected, and the initial operator tools cover report/crop/proposal review. Mark explicitly requires the full manual workflow and the ability to watch Astra operate it. His ten-card pilot now uses brand-new Front/Back photos for every card, superseding the earlier saved-capture selection. See [the grading workspace plan](../atlas/GRADING_WORKSPACE.md). The current activation lead task is `01a0873d-cbbe-7781-90cd-3433787751d7` in worktree `b487`, branch `codex/atlas-activation-lead-20260909`; the earlier `f4fe` and `9545` checkpoints are historical.

The original core checkpoint passed128 PostgreSQL scenarios,55 paired-web/restart checks and70 built-production denials. The latest website/database candidate passes155 PostgreSQL scenarios on93 public plus16 additive ATLAS migrations and no-op replay, including customer ownership, current approval/finishing/shipment and SMS-budget regressions. Three Next15.5.25 applications build; the connected synthetic website passes13 HTTP test groups including process restart. All39 configured-disabled hosted checks passed before login activation. Original committed migration bytes and grading arithmetic are retained. Detailed evidence and remaining inputs are in [ATLAS completion](../atlas/COMPLETION.md), [local acceptance](../atlas/LOCAL_ACCEPTANCE.md) and the [website release](../atlas/WEBSITE_RELEASE.md).
...
The live website and customer SMS acceptance are distinct from grading acceptance. The preparation/worker release remains unadmitted; no live Astra real-card evaluation, graded-card NFC finishing, real print, assembly or weld acceptance has run. The same ten-card supervised grading cohort and separate physical finishing acceptance remain. Runtime/DB evidence governs deployed state.

`https://atlasgrading.com/admin` and `/account` are deployed with distinct sessions/CSRF, additive database policy and signed forwarding. The selected c5ba35fb release restores the original gold/black homepage and accepts ordinary ten-digit U.S. input, adding+1 through the provider handoff. Login controls are enabled/revision6 and SMS policies enabled/revision4. DNS, managed TLS,16 migrations and separate serving credentials are configured. Original Site version21 is preserved. Exact bindings and current live verification are in [website release status](../atlas/WEBSITE_RELEASE.md). Worker/Astra and production NFC remain disabled; the79 operator-package tests used synthetic responses.
