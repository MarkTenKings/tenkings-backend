...
## Current Priorities
- Keep the station workflow at four actions: Start New Card, Capture Front, Capture Back, Approve & Publish.
- Keep one Basler/Pylon capture implementation, one bridge-owned watchdog, one capture lock, and one serialized hardware operation. There is no production cold/debug path, rapid queue, manual capture/geometry path, browser Safe Off system, or compatibility fallback.
