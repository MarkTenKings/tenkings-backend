...
On September9 Mark selected `https://atlasgrading.com/admin` for private grading and `/account` for customer accounts. Both are now implemented locally with distinct sessions/CSRF, additive database policy and signed per-zone forwarding behind the public apex. Production NFC remains disabled pending its separate new native protocol; historical Windows receipts are preserved. The public app has pilot entry points; the owner's separate ATLAS marketing Site remains unchanged. See [website release plan](../atlas/WEBSITE_RELEASE.md) and [deployment runbook](../atlas/WEBSITE_DEPLOYMENT.md). Astra's79 operator-package tests used synthetic responses; no live model card-quality evaluation has run.
