...
## Native removal and re-presentation verified

On September 9 at 04:41:08 UTC (September 8, 21:41 PDT), Mark confirmed the tag was removed. The unchanged native probe returned `no_tag` at `interface_resolution`, with two candidate interfaces and `fixedReadAttempted: false` (expected exit 2, 0.021 seconds). After Mark confirmed the same tag was back, the probe returned `header_observed` at 04:42:04 UTC (exit 0, 0.104 seconds). Its nonidentifying summary matched the earlier observation: manufacturer code `1D`, CC `E1103E00`, 496 advertised bytes and candidate static-lock bytes `0000`. No UID comparison was made; same-tag continuity is operator-reported. This proves the observed present → absent → present read-only sequence, not continuous monitoring, encoding throughput or permanent-lock qualification.

Both checks used source commit `b0dd2f092f6539955622f5783a1258a5f96aaf40` and the previously tested binary SHA-256 `5cf7fb3dd5c6d2f260a4653da55eb12442623de87f9a9209dae32e2a55805495`. No code/build/reader-setting change or tag mutation occurred. Private evidence: `MAC_NFC_NATIVE_REMOVAL_20260909T044108Z.json` and `MAC_NFC_NATIVE_REPRESENT_20260909T044204Z.json`. Direct Mac reader access and basic placement/removal checks are complete; F8215 write/lock qualification and the integrated native finishing workflow remain pending.

## First controlled Mac write — September 8 evening / September 9 UTC

Following owner authorization, the separate fixed-URI diagnostic successfully encoded the designated unused tag at 2026-09-09T05:32:53Z. Its exclusive connection completed 14 writes (pages 5–17 then 4), 38 reads and full 56-byte NDEF/padding verification with unchanged header/trailing bytes in 0.38 seconds, exit 0. The mode-0600 durable journal contains all 14 ordered intents and matching verified readbacks, plus terminal verification. Evidence: private `MAC_NFC_DIAGNOSTIC_PREPARED_20260909.json`, `MAC_NFC_DIAGNOSTIC_WRITE_RESULT_20260909.json`, `MAC_NFC_DIAGNOSTIC_WRITE_JOURNAL_20260909.jsonl` and its review receipt. The original probe binary remains unchanged. No lock, further driver/reader-setting change or production-card operation occurred. The test tag now contains a marked diagnostic URL and must remain separate from graded-card inventory; Mark confirmed the requested phone scan worked on September 9. This is operator-reported phone reading, not a deployed grading-report check.
