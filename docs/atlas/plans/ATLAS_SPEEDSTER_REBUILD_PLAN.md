...
Latest owner direction, September 12 Pacific: focus exclusively on ATLAS Grading. Test Astra defect finding with proprietary human-reviewed memory before integrating SAM 3. The revised sequence below starts manual testing early and brings the minimum defect-memory loop ahead of that experiment. Later shared market research is not a prerequisite. The old SAM baseline recommendations are superseded; existing SAM observations remain historical evidence. The manual candidate is locally qualified but not deployed; the separately authorized storage CORS correction is applied, and its one authorized object canary now failed safely: Spaces accepted an incorrect checksum; its65-byte object was deleted and verified absent. Storage qualification remains a release blocker.
...
The prior [defects/persistence/photo metadata checkpoint](../audits/2026-09-12/defects-persistence-and-iphone-metadata.md) connects actual staff authentication, PostgreSQL drafts and private artifact storage to the paired defect screen and existing CPU measurement. Saved candidates support retry/discard; one findings confirmation and separate exact report approval survive reload. Nine authenticated synthetic browser scenarios pass, including physical re-preparation and preservation of earlier approval history. Qualified EXIF/ICC decoding is implemented, while real HDR iPhone photos, provider/serving app intake and optical acceptance remain open. Detector/Astra, learning, shared research and finishing are still separate work; serving I and the initiating-cause prerequisite are unchanged.

The current [connected manual checkpoint](../audits/2026-09-12/connected-manual-workflow.md) implements Mark's approved untouched-HDR-original/full-resolution-SDR policy, actual staff native intake, shared identification and durable paired manual review/report integration. The dedicated Vercel staff app proxies signed ordinary-staff requests to a private Linux CPU service; full-resolution images use private signed object reads and browser byte verification. Node20/Linux qualification and native PostgreSQL fixtures are recorded there. These source-level gaps are now closed; live provider/target-host activation and fresh sports/Pokémon optical acceptance remain. The earlier checkpoints describe their historical stages and do not claim a current deployment.
...
The clean intake now includes an explicit **server HEIC decoder** and owner-approved SDR working conversion. The previous serving `decode_preparation_source` accepts only JPEG, PNG and WebP, and serving browser intake converts HEIC to PNG. The connected candidate handles qualified direct/grid HEVC with EXIF/ICC provenance and explicit Apple HDR-primary admission. It retains untouched HDR originals and derives full-resolution SDR working images, while other unqualified HDR/format cases remain refused. Production activation is still pending. Verify orientation, color handling, bit depth and fine-defect visibility on the actual uploaded files. Future camera RAW/DNG handling depends on the selected camera/SDK; a common intake interface alone does not implement that decoder.
...
Build the minimum loop before the Astra experiment: **human review → save each reviewed lesson and its source → publish a new knowledge version → retrieve relevant lessons for the next applicable action**. Retain the exact crop/trace, confirmed type, accepted/corrected/rejected disposition, missed defects added by the human, card/design/material context and reviewer provenance. Preserve revisions and permit deliberate supersession of a mistaken lesson. Do not teach from merely opening a finding, temporary edits or unreviewed model guesses. Mark accepted this build: the existing bulk Confirm findings action publishes its deliberately reviewed outcomes without a second confirmation. The authenticated original confirmer must be a REVIEWER with current edit authority on the card; report certification remains a separate final-approval requirement. Publication failures preserve the committed review and expose same-confirmation retry.

Publish corrections directly from the action/result, rather than rediscovering them by scanning old completed sessions under the certificate/label lock. If company data and shared knowledge live in different databases, publication needs a small idempotent pending-publication record; it is not an atomic cross-database transaction by assumption. A card correction can save successfully while shared publication is pending. Start publication automatically after confirmation, show its acknowledged or pending state, and retry only the same confirmation on explicit retry or command replay.
...
4. **Learning publication:** Mark now requests lessons after each deliberately reviewed defect. The accepted build uses the existing bulk **Confirm findings** event with authenticated original REVIEWER confirmer and current card edit authority; final report certification stays separate. The event and authority mapping are resolved for this build.
