...
**Still required before the replacement operator:** obtain the existing database slow-statement/lock logs for September 11, 15:34–15:43 and 18:30–18:32 UTC, or capture an instrumented representative failure and its correction. Mark chose to relay a prepared message to DigitalOcean's AI assistant and return its answer; no outside support request has been sent by this task. The app's original log omitted exception detail. If historical logs cannot settle the cause, use a bounded stub-provider reproduction with per-query/pool/lock/CPU/commit and logging-volume measurements. Include production-equivalent slow-log settings and representative runtime resources; compare bounded versus unlimited parameters without lowering the real threshold merely to force a failure. PostgreSQL calculates displayed statement duration before its own parameter-log formatting/emission, so capture client and lock-lifetime timing too. A known gap is not a passed prerequisite. Forced timeouts can validate recovery; merely extending a timeout or obtaining another passing local run cannot establish the historical cause.
