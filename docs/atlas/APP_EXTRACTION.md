...
Current route target, September 9: Mark reserved the root website for customers and selected `/admin` for private grading. The [website release plan](WEBSITE_RELEASE.md) and canonical blueprint supersede the staff-subdomain design below. Existing runtime still pins that subdomain; routing, authentication and policy migration remain required. This document retains the earlier extraction design as historical context.
