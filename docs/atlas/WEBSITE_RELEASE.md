...
The public and staff builds and serving roles remain separate behind one hostname. A checked path router sends only `/admin` and its descendants, including staff APIs and assets, to the staff application. All other customer/report routes stay with the public application. `/administrator` and similar prefixes are not staff routes. Unknown routes do not receive a private-app fallback. The browser remains at `/admin`; a redirect to the old staff subdomain does not implement this target.
