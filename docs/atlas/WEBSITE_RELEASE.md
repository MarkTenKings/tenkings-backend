...
Updated September 9, 2026 after the first hosted repair. DNS and managed SSL are configured, and the exact 1e6a16fd public build is assigned to the apex. Twilio services/keys, additive database migrations and disabled login controls are installed. The native Prisma engine packaging repair is proven by real database-disabled responses. Hosted acceptance passes 38 of 39 checks; customer root Next data navigation and missing root nonce middleware coverage are being repaired. Signup/sign-in and live SMS remain disabled and unaccepted. Grading quality and physical finishing remain separate acceptance steps.
...
The selected releases report READY, production target, CLI source and automatic Git metadata for `1e6a16fd468c7cc0fec07f9c34817ce472a6d74a`. Uploads came from a clean detached checkout; each of the 1,988 selected files matched the committed bytes, with no environment, generated or untracked paths. The projects have separate roots, Node 20.x build settings, no Git connection and production-only environment payloads: 18 staff settings, 10 customer settings and six public routing settings. No branch push occurred. Earlier unconfigured CE70 builds and dirty/manual-metadata D10 preparation builds are not the selected release.

| Area | Protected deployment | Provider deployment ID |
| --- | --- | --- |
| Public entry and gateway | [Configured public build](https://atlas-grading-public-o18k835x6-ten-kings.vercel.app) | `dpl_4VqXz7X8QadBGYoAxUy7hv2MTEYB` |
| Staff `/admin` | [Configured staff build](https://atlas-grading-staff-aqaa72jzw-ten-kings.vercel.app/admin) | `dpl_BSJTSL71Y53mevwmEsZbsAcbF5bL` |
| Customer `/account` | [Configured customer build](https://atlas-grading-customer-6rxau6jwa-ten-kings.vercel.app/account) | `dpl_5Wex5o7SotEWKVGjEzKwQw2kCEsc` |

These immutable deployment URLs retain Vercel protection. Provider metadata/build/environment checks passed. The initial D10 artifact omitted the native engine; 1e6a16fd fixes packaging and returns the correct database-disabled responses. Its hosted run passed 38 of 39 GETs, then stopped at the customer root index.json HTML404. Signed direct/apex profile and submit JSON pass. A compiled middleware matcher check now catches the missing canonical /account match; the staged normalization-only 3ef8e377 candidate failed the nonce check and was not selected. The earlier CE70 release's six anonymous and sixteen authenticated GET observations remain historical evidence only. The two current private projects each retain one distinct server-only gateway bypass; no bypass is exposed to the browser.

Namecheap now serves apex A `76.76.21.21` and www CNAME `cname.vercel-dns.com.`, with the old parking redirect removed and Google verification, DKIM and MX preserved. Both authoritative nameservers and Vercel report correct DNS. Vercel issued auto-renewing managed certificate `cert_9W8RGIPVjhVxlfm9BA8xWBMs` for apex/www, and promoted the exact configured public deployment above; alias readback and ordinary TLS verification passed. The www project-domain record retains its 308 apex redirect. Replace the remaining customer root-navigation defect with the repaired exact-source release and repeat the retained 39-GET disabled-host acceptance with fresh controls. The separate marketing Site remains unchanged.

The reviewed managed database now contains all 16 ATLAS migrations; migration replay reports no pending changes and the legacy public ledger is preserved. Separate staff/customer serving logins pass their source-defined privilege checks. Only the rotated credentials were installed in hosting. StaffControl and CustomerControl are disabled at revision 2, bound to the immutable hosts above and source 1e6a16fd. Their configuration hashes are respectively `611af54486f26d7bf6b67334c2ae933277b28cc8fabc0f25743ef4b6c843f2bb` and `a16194cda97d081b2b878bb59343f68ba30e6e56085dfdaa064c32f7e6ea1563`.
