...
| Object storage | Existing private bucket responds to scoped HEAD; observed unversioned; exact-origin GET/HEAD preflights pass but candidate PUT preflight returns 403 | Obtain the current CORS rule for a reviewed additive change, then qualify actual conditional writes, checksums, private reads and cleanup through the separately authorized canary |
