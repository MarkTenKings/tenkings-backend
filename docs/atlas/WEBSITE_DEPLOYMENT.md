...
Latest ATLAS runtime evidence — September 11, 02:16 UTC (September 10, 19:16 Pacific): merged recovery/workspace source `dff540c4` is live on the corrected staff/public deployments and private E. All four public aliases, mounted staff assets, public/customer/staff entry and E health are verified. Production has 95 public and 25 staff migrations; revisions are staff11/workspace6/source5/grading2/image2/operator2/STAFF SMS9. Exact hosts and evidence are in [the current release record](WEBSITE_RELEASE.md).

Continuous Start Astra, bounded crop delivery, saved-response recovery and the live stage/activity interface with active and per-stage timing are deployed. The original Charmander remains at Identity, revision8, with run `UNKNOWN`, one applied step and one retained `RECEIVED` response; recovery has not been executed. Fresh staff sign-in and the authenticated recovery-through-human-review test remain pending. Earlier C/D deployment, disabled-processing and no-inference checkpoints below are historical; original receipts, pilot limits and human approval boundaries remain preserved.
