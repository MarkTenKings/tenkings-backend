...
Mark authorized the website pilot and supplied the approved US staff phone, provider access and $100 total SMS/image/Astra ceiling. These are existing inputs, not new permission questions. At the September 9 configured-release checkpoint, Twilio services/keys, Vercel production settings/builds, 16 additive ATLAS migrations and disabled login controls are installed. Namecheap authentication, DNS cutover, managed TLS and D10 apex assignment are complete. The first hosted acceptance exposed a missing native Prisma engine in the customer deployment; repair/redeploy and repeat hosted acceptance before activating login. Selected worker resources and actual card/capture admission remain separate prerequisites. Use [release status](WEBSITE_RELEASE.md) for exact current deployment and control bindings.
