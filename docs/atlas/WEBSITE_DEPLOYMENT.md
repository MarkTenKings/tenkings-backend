...
Mark authorized the website pilot and supplied the approved US staff phone, provider access and $100 total SMS/image/Astra ceiling. These are existing inputs, not new permission questions. Twilio services/keys, Vercel production settings/builds and 16 additive ATLAS migrations are installed. Namecheap authentication, DNS cutover, managed TLS and exact acb2e007 apex assignment are complete. All 39 configured-disabled hosted checks passed before enabling owner-only login controls at revision4 and SMS policies at revision2. Mark subsequently confirmed replacement-code delivery and successful customer sign-in after the first code expired. The 21:39:09 UTC database readback confirms one customer account/session and two SMS reservations totaling $1; the authenticated submission form is visible in the browser. Staff login subsequently succeeded; at22:02:07UTC the database confirms one staff identity/session and two staff reservations totaling$1, and the browser shows its empty Review queue. Selected worker resources and actual card/capture admission remain separate prerequisites. Use [release status](WEBSITE_RELEASE.md) for exact current deployment and control bindings.
