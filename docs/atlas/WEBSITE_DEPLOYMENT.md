...
Mark authorized the website pilot and supplied the approved US staff phone, provider access and $100 total SMS/image/Astra ceiling. These are existing inputs, not new permission questions. At the September 9 configured-release checkpoint, Twilio services/keys, Vercel production settings/builds, 16 additive ATLAS migrations and disabled login controls are installed. Namecheap authentication, DNS cutover, managed TLS and exact 1e6a16fd apex assignment are complete. The native Prisma engine defect is repaired. Hosted acceptance passes 38 of 39 checks; fix customer root middleware/data navigation and repeat hosted acceptance before activating login. Selected worker resources and actual card/capture admission remain separate prerequisites. Use [release status](WEBSITE_RELEASE.md) for exact current deployment and control bindings.
...
The September 9 `ce70c574` provider audit confirms the actual staff API is Node 20.x with a 240-second timeout and the customer API is Node 20.x with 30 seconds. Vercel's generated public/customer middleware uses its managed Node 24.x runtime with 60 seconds, despite Node 20 project/build settings. Do not describe every hosted function as Node 20 or infer that the 240-second local proxy setting proves the deployed middleware path. Signed forwarding and supervised long grading responses still require actual apex verification before activation.
