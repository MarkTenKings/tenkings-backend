...
Mark authorized the website pilot and supplied the approved US staff phone, provider access and $100 total SMS/image/Astra ceiling. Twilio, three isolated Vercel projects,22 additive ATLAS migrations, DNS and managed TLS are configured. The production ledger is95 public/22 staff,13 historical rollbacks and zero unfinished migrations; the seven new migrations and three private roles have separate actual verification. The selected c5ba35fb release restores the original homepage and supports ordinary U.S. phone entry through the strict provider handoff. Login controls are enabled/revision6 and SMS policies enabled/revision4, with the original pilot identity, expiry and every reservation retained. Customer and staff previously completed real SMS sign-in; current release revisions require fresh sessions. The original39 disabled-host checks preceded first activation; enabled-release checks cover the replacement. Exact current deployment/binding and live sign-in evidence is in [release status](WEBSITE_RELEASE.md). Actual worker/storage/operator resources and the ten real cards remain separate prerequisites.
...
For the current workspace upgrade, keep legacy main deployment held. Stage the exact successor staff/public bindings, preserve admitted CPU A2, and verify the signed successor private service and HTTPS denials. Use a fresh controls/roster/SMS/holds census, then apply only the reviewed intake transaction and verify actual rows/grants before promoting the staged public project. Independently verify all original aliases and unchanged customer/legacy bindings. Claims, preparation and Astra remain disabled. An old public alias alone cannot restore the earlier strict staff grants/control binding. The first-install procedure below is historical guidance and must not be rerun against existing controls.
