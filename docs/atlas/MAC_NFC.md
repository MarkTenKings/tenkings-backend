...
Before setup, the Mac had the generic CCID 1.5.1 bundle and no ACS driver in the inspected standard locations/receipts. Separate fixed read-only reader-radio queries were rejected by that PC/SC path; the actual radio state remained unknown. Mark installed the official ACS 1.1.13 Mac driver and restarted. Post-restart checks verified the package receipt, version, strict bundle signature and exact previously audited driver binary hash. Initial post-restart USB/PCSC checks found no attached ACR1552U; reconnection and tag detection remain the next hardware steps. [Reader setup and post-restart checks](MAC_NFC_READER_SETUP.md) record the evidence and remaining qualification.
