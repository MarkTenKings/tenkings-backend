...
The original read-only investigation proved the canonical mismatch but could not derive its first differing field from the historical SHA-256 digest alone. The subsequent atomic implementation initially still failed safely against Production, which made a rollback-only read-back comparison possible without committing a fifth revision. That comparison isolated the first exact field: Prisma's JSON transport changed `backMap.anchors[3].point.x` from `0.11073133680555555` before insertion to `0.1107313368055556` after read-back. The retained v1-v4 rows show the same final-digit normalization pattern in multiple geometry values. JSONB property ordering remains ruled out; the root cause is numeric representation at the Prisma JSON persistence boundary.
...
This matrix remains valid negative evidence for the historical rows. The later rollback-only in-transaction comparison supplied the missing pre-insert/read-back evidence directly and identified the numeric persistence-boundary difference above.

The historical v1-v4 pre-insert byte strings still cannot be reconstructed in full because they were not logged and SHA-256 is not reversible. However, the same recovered draft reproduced the failure in the corrected in-transaction path, and the rollback-only comparison proved a concrete pre-insert/read-back numeric difference. New revision payloads therefore quantize finite non-integer JSON numbers to twelve decimal places before both hashing and insertion. The legacy canonical hash algorithm is unchanged for reading existing valid revisions. A real Production rollback-only preflight then independently verified candidate FAMILY v5 and EXACT v1 hashes inside one transaction and deliberately rolled back, leaving the database at one map and four revisions.
