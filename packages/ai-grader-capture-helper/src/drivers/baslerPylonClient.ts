...
export type BaslerPylonAction =
  ...
  | "calibration-preview-mjpeg-stream"
...
export interface BaslerCalibrationPreviewMjpegStreamOptions {
  cameraIndex?: number;
  exposureUs?: number;
  refreshIntervalMs?: number;
  jpegQuality?: number;
...
export class BaslerPylonClient {
  ...
  startCalibrationPreviewMjpegStream(options: BaslerCalibrationPreviewMjpegStreamOptions = {}): ChildProcessWithoutNullStreams {
    const cameraIndex = options.cameraIndex ?? 0;
    if (!Number.isInteger(cameraIndex) || cameraIndex < 0) {
      throw new BaslerPylonClientError("BASLER_CAMERA_INDEX_INVALID", "--camera-index must be a non-negative integer.");
    }
