...
import {
  ...
  assertAiGraderHumanGeometryReportProjectionMatchesReceiptV1,
...
export interface AiGraderLocalStationBridgeActionRequest {
  ...
  operatorResolutionSubmission?: FixedRigOperatorResolutionSubmissionV1;
  operatorAuthentication?: AiGraderOperatorResolutionAuthenticationV1;
...
export interface AiGraderLocalStationBridgeDependencies {
  ...
  observeCalibrationActivationRuntime?: (
    ...
    runtimeObservation: MathematicalCalibrationRuntimeObservationV1;
    evidenceImage: {
...
function verifyOperatorResolutionAuthenticationV1(input: {
  value: unknown;
  expected: {
    queueItemId: string;
    gradingSessionId: string;
  ...
  if (!input.hmacKey || !input.hmacKeyId) {
    throw new Error("Operator-resolution authentication verification is not configured.");
  }
  const authentication =
    aiGraderOperatorResolutionAuthenticationV1Schema.parse(input.value);
  const payload = authentication.payload;
  for (const [key, expected] of Object.entries(input.expected)) {
    if (payload[key as keyof typeof payload] !== expected) {
      throw new Error("Operator-resolution authentication is stale or bound to different evidence.");
    }
  }
  if (authentication.authentication.keyId !== input.hmacKeyId) {
    throw new Error("Operator-resolution authentication key identity mismatch.");
  }
  ...
  const signature = crypto.createHmac("sha256", input.hmacKey)
    ...
    .digest("hex");
  if (
    !secretMatches(authentication.payloadSha256, payloadSha256) ||
    !secretMatches(authentication.authentication.signature, signature)
  ) {
    throw new Error("Operator-resolution authentication signature is invalid.");
  }
  ...
  if (
    ...
    expiresAtMs - issuedAtMs > OPERATOR_RESOLUTION_AUTHENTICATION_MAX_LIFETIME_MS ||
    issuedAtMs > nowMs + OPERATOR_RESOLUTION_AUTHENTICATION_CLOCK_SKEW_MS ||
    expiresAtMs < nowMs
  ) {
    throw new Error("Operator-resolution authentication is expired or has invalid timing.");
  }
...
function newLocalMathematicalV1State(
  ...
  return {
    ...
    ...(calibrationActivationAuthority ? { calibrationActivationAuthority: structuredClone(calibrationActivationAuthority) } : {}),
  };
}
...
function bridgeEndpoints() {
    ...
    { method: "POST", action: "prepare-calibration-activation", hardwareAccess: false, description: "Verify exact immutable observation and bundle bytes, then write the signed workstation receipt and local PENDING pointer." },
    { method: "POST", action: "confirm-calibration-activation", hardwareAccess: false, description: "Converge exact hosted/local ACTIVE activation agreement without hardware access." },
    { method: "POST", action: "abort-calibration-activation", hardwareAccess: false, description: "Archive only the exact local PENDING pointer after hosted failure recording." },
...
class StaleHumanGeometryWorkerResultV1 extends Error {
  constructor() {
    super(
      "A superseded Human Geometry worker result was discarded without changing the active receipt.",
    );
    this.name = "StaleHumanGeometryWorkerResultV1";
  }
}

interface HumanGeometryReceiptAuthorityV1 {
  queueItemId: string;
  receiptVersion: number;
  receiptSha256: string;
}

interface HumanGeometryWorkerClaimV1
  extends HumanGeometryReceiptAuthorityV1 {
  attempt: number;
  startedAt: string;
}
...
function isStaleHumanGeometryWorkerResult(error: unknown): boolean {
  return error instanceof StaleHumanGeometryWorkerResultV1;
}
...
function persistedMathematicalRapidQueueSummary(
  ...
  if (raw.status === "operator_resolution_required") {
    if (!exactObjectKeys(raw, ["status", "reviewRequestSha256"]) ||
        typeof raw.reviewRequestSha256 !== "string" ||
        !SHA256_LOWERCASE_RE.test(raw.reviewRequestSha256)) {
      throw new Error("Persisted Mathematical Rapid operator-resolution summary is invalid.");
    }
    return {
      status: "operator_resolution_required",
      ...(eyesCenteringSelectionState
...
function mathematicalRapidQueueSummary(
  ...
  if (execution.status === "operator_resolution_required") {
    return {
      status: execution.status,
      reviewRequestSha256: execution.request.requestSha256,
    };
  }
...
export class AiGraderLocalStationBridgeService {
  ...
  private calibrationActivationObservation?: AiGraderCalibrationWorkstationObservationV1;
  ...
  status(): AiGraderLocalStationBridgeStatus {
    ...
    return {
      ...
      calibrationActivation: {
        configured: Boolean(this.dependencies.calibrationActivationRegistry),
        state: this.calibrationActivationState,
        ...(this.calibrationActivationObservation
          ? { observation: this.calibrationActivationObservation }
          : {}),
  ...
  private async mathematicalReportBundleResponse(
    ...
    this.assertLockedHumanGeometryReportProjection(
      source.manifest,
  ...
  private humanGeometryReceiptAuthorityIsCurrent(
    manifest: AiGraderLocalStationBridgeManifest,
    authority: HumanGeometryReceiptAuthorityV1,
  ): boolean {
    const current = this.queuedManifests.get(authority.queueItemId);
    const geometry = current?.humanGeometryAssist;
    return (
      current === manifest &&
      geometry?.state === "locked" &&
      geometry.receiptVersion === authority.receiptVersion &&
      geometry.lockedReceipt?.receiptVersion === authority.receiptVersion &&
      geometry.lockedReceipt.receiptSha256 === authority.receiptSha256 &&
      geometry.gradingClaimReceiptSha256 === authority.receiptSha256
    );
  }

  private assertHumanGeometryReceiptAuthorityCurrent(
    manifest: AiGraderLocalStationBridgeManifest,
    authority: HumanGeometryReceiptAuthorityV1,
  ): void {
    if (!this.humanGeometryReceiptAuthorityIsCurrent(manifest, authority)) {
      throw new StaleHumanGeometryWorkerResultV1();
    }
  }

  private humanGeometryWorkerClaimIsCurrent(
    manifest: AiGraderLocalStationBridgeManifest,
    claim: HumanGeometryWorkerClaimV1,
  ): boolean {
    const current = this.queuedManifests.get(claim.queueItemId);
    const execution = current?.mathematicalV1?.execution;
    return (
      this.humanGeometryReceiptAuthorityIsCurrent(manifest, claim) &&
      execution?.status === "processing" &&
      execution.attempt === claim.attempt &&
      execution.startedAt === claim.startedAt
    );
  }

  private assertHumanGeometryWorkerClaimCurrent(
    manifest: AiGraderLocalStationBridgeManifest,
    claim: HumanGeometryWorkerClaimV1,
  ): void {
    if (!this.humanGeometryWorkerClaimIsCurrent(manifest, claim)) {
      throw new StaleHumanGeometryWorkerResultV1();
    }
  }

  private assertLockedHumanGeometryReportProjection(
    manifest: AiGraderLocalStationBridgeManifest,
    bundle: AiGraderReportBundleV03,
  ) {
    const geometry = manifest.humanGeometryAssist;
    if (!geometry) return;
    if (geometry.state !== "locked" || !geometry.lockedReceipt) {
      throw new Error(
        "A new Human Geometry report requires the exact currently locked receipt.",
      );
    }
    return assertAiGraderHumanGeometryReportProjectionMatchesReceiptV1(
      bundle.geometry,
  ...
  private async runMathematicalStationPackageInternal(
    ...
    const activationAuthority = manifest.mathematicalV1.calibrationActivationAuthority;
    let calibrationBundlePath = this.config.mathematicalCalibrationBundlePath;
    let calibrationBundleSha256 = this.config.mathematicalCalibrationBundleSha256;
    if (this.config.mode === "real") {
      ...
      calibrationBundleSha256 = activationAuthority.bundleManifestSha256;
    } else if (!calibrationBundlePath || !calibrationBundleSha256) {
      throw new Error(
        "Mathematical V1 mock processing requires one explicit finalized calibration fixture. " +
        "Production configured-bundle fallback remains prohibited.",
      );
    }
    const prior = manifest.mathematicalV1.execution;
    const queueItem = this.rapidQueue.items.find((candidate) =>
      candidate.sessionId === manifest.sessionId && candidate.reportId === manifest.reportId);
    if (!queueItem) {
    ...
    const workerClaim: HumanGeometryWorkerClaimV1 = {
      queueItemId: queueItem.queueItemId,
    ...
    try {
      ...
      this.assertHumanGeometryWorkerClaimCurrent(manifest, workerClaim);
      ...
      if (
        isStaleHumanGeometryWorkerResult(error) ||
        !this.humanGeometryWorkerClaimIsCurrent(manifest, workerClaim)
      ) {
        throw new StaleHumanGeometryWorkerResultV1();
      }
    ...
    return this.runRapidQueueMutation(async ({ trackManifest }) => {
      this.assertHumanGeometryWorkerClaimCurrent(manifest, workerClaim);
      this.exactMutableQueuedItem({
        queueItemId: queueItem.queueItemId,
        gradingSessionId: manifest.sessionId,
        reportId: manifest.reportId,
      });
      const mathematicalV1 = manifest.mathematicalV1;
      if (!mathematicalV1) {
        throw new StaleHumanGeometryWorkerResultV1();
      }
      ...
      delete mathematicalV1.eyesCenteringCandidateLedger;
      delete mathematicalV1.eyesCenteringCandidateAssets;
      delete mathematicalV1.eyesCenteringSelections;
      delete mathematicalV1.eyesCenteringSelectionReceipt;
      if (result.status === "completed") {
        this.operatorResolutionAnalysisCheckpoints.delete(queueItem.queueItemId);
        delete mathematicalV1.reviewAssets;
        delete mathematicalV1.operatorResolutionWorkspaceAssets;
        this.assertLockedHumanGeometryReportProjection(
          manifest,
        ...
        mathematicalV1.execution = {
          ...
          operatorResolutionRequest: structuredClone(result.operatorResolutionRequest),
        };
        manifest.progressLog.push(
        ...
        mathematicalV1.reviewAssets = reviewAssets;
        delete mathematicalV1.operatorResolutionWorkspaceAssets;
        mathematicalV1.execution = {
          status: "finding_review_required",
          completedAt,
        ...
        delete mathematicalV1.reviewAssets;
        mathematicalV1.operatorResolutionWorkspaceAssets =
          operatorWorkspaceAssets;
        mathematicalV1.execution = {
          status: "operator_resolution_required",
          completedAt,
        ...
        manifest.progressLog.push(
          completedAt + " Mathematical V1 reached the authenticated element-resolution boundary for request " +
      ...
      return {
        value: mathematicalV1.execution,
        manifests: [manifest],
      };
    });
  }
  ...
  private enqueueRapidFinalization(
    ...
    const finalization = Promise.resolve().then(async () => {
      ...
      try {
        ...
        let backgroundGeometryAuthority:
          HumanGeometryReceiptAuthorityV1 | undefined;
        try {
          ...
          backgroundGeometryAuthority = {
            queueItemId,
            receiptVersion:
              manifest.humanGeometryAssist.lockedReceipt.receiptVersion,
            receiptSha256:
          ...
          if (gradingContractFor(manifest) === "mathematical_calibration_v1") {
            ...
            this.assertHumanGeometryReceiptAuthorityCurrent(
              manifest,
              backgroundGeometryAuthority,
            );
            await this.observeRapidOcrEligibility(queueItemId);
            ...
            if (execution.status === "operator_resolution_required") {
              ...
              this.transitionRapidWorkflow(
                manifest,
                "operator_resolution_required",
                "Exact element results are ready for authenticated owner resolution bound to request " +
                execution.request.requestSha256 + ".",
              );
              await this.syncQueuedManifest(manifest);
              return;
            }
          ...
          const finalGeometryAuthority = backgroundGeometryAuthority;
          if (!finalGeometryAuthority) {
            throw new StaleHumanGeometryWorkerResultV1();
          }
          await this.runRapidQueueMutation(async ({ trackManifest }) => {
            this.assertHumanGeometryReceiptAuthorityCurrent(
              manifest,
              finalGeometryAuthority,
            );
          ...
          if (
            isMathematicalStationShutdown(error) ||
            isStaleHumanGeometryWorkerResult(error) ||
            (
              backgroundGeometryAuthority &&
              !this.humanGeometryReceiptAuthorityIsCurrent(
                manifest,
                backgroundGeometryAuthority,
              )
            )
  ...
  private async reattachExactPersistedReportReadyReview(): Promise<void> {
    ...
    if (isMathematicalReportBundle(manifest.reportBundle)) {
      this.assertLockedHumanGeometryReportProjection(
        manifest,
        manifest.reportBundle,
      );
  ...
  private persistedOperatorResolutionRequest(
    ...
    const execution = manifest.mathematicalV1?.execution;
    if (execution?.status === "operator_resolution_required") {
      if (!verifyFixedRigOperatorResolutionRequestV1(execution.request)) {
  ...
  private async submitOperatorResolutions(
    ...
    const item = this.exactQueuedItem(request);
    if (this.activeQueueItemId !== item.queueItemId) {
      throw new Error("Operator resolution requires the exact currently activated queue/session/report triple.");
    }
    const idempotencyKey = exactStationString(
      request.idempotencyKey,
      "Operator resolution idempotencyKey",
    );
    const manifest = await this.exactQueuedManifest(item);
    if (gradingContractFor(manifest) !== "mathematical_calibration_v1" ||
        !manifest.mathematicalV1) {
    ...
    const submission = parseFixedRigOperatorResolutionSubmissionV1(
      request.operatorResolutionSubmission,
      {
        width: FIXED_RIG_STANDARD_TRADING_CARD_WIDTH_MM,
        height: FIXED_RIG_STANDARD_TRADING_CARD_HEIGHT_MM,
      },
    ...
    if (existingReceipt) {
      if (
        ...
        existingReceipt.authority.operatorId !== operatorId
      ) {
  ...
  private async publishSelectedRapidQueueItem(request: AiGraderLocalStationBridgeActionRequest): Promise<void> {
    ...
    await this.runRapidQueueMutation(async ({ trackManifest }) => {
      ...
      if (isMathematicalReportBundle(mutableManifest.reportBundle)) {
        this.assertLockedHumanGeometryReportProjection(
          mutableManifest,
          mutableManifest.reportBundle,
        );
      }
  ...
  async observeMathematicalCalibrationActivationRuntime(
    ...
    if (this.dependencies.observeCalibrationActivationRuntime) {
      return this.dependencies.observeCalibrationActivationRuntime(expected, evidenceDirectory);
    }
    if (this.config.mode !== "real") {
      throw new Error(
    ...
    const runtimeObservation = {
      ...
      controller: {
        ...
        allWritesAcknowledged: true as const,
      },
    ...
    return {
      ...
      evidenceImage: {
        ...
        byteSize: captureResult.byteSize,
  ...
  private async writeProductionReleaseForManifest(
    ...
    if (gradingContractFor(manifest) === "mathematical_calibration_v1") {
      return withAiGraderReportPackageOperation(reportId, async () => {
        const lockedGeometryReceipt =
          manifest.humanGeometryAssist?.state === "locked"
            ? manifest.humanGeometryAssist.lockedReceipt
            : undefined;
        const geometryAuthority = lockedGeometryReceipt &&
          manifest.rapidCapture.queueItemId
          ? {
              queueItemId: manifest.rapidCapture.queueItemId,
              receiptVersion: lockedGeometryReceipt.receiptVersion,
              receiptSha256: lockedGeometryReceipt.receiptSha256,
            }
          : undefined;
        if (manifest.humanGeometryAssist && !geometryAuthority) {
          throw new Error(
            "A new Human Geometry release requires the exact currently locked receipt.",
          );
        }
  ...
  recoverKnownMathematicalCalibrationBlankReverseTimestampFalseStop(): Promise<FixedRigMathematicalCalibrationFalseStopRecoveryV1> {
    this.assertCalibrationSessionIsolated();
    if (!this.mathematicalCalibrationV1SessionId) {
      throw new Error("The incident-bound V1.0.1 false-stop recovery requires its exact session to be explicitly resumed and bridge-bound first.");
    }
    return this.requireMathematicalCalibrationCaptureProducer()
      .recoverKnownBlankReverseTimestampFalseStop(this.mathematicalCalibrationV1SessionId)
      .then((result) => {
        this.resetMathematicalCalibrationDisplayedFrameAuthority(
          "Incident-bound V1.0.1 false-stop recovery invalidated all displayed-frame authority.",
        );
        return result;
      });
  }
  ...
  async action(action: AiGraderLocalStationBridgeAction, request: AiGraderLocalStationBridgeActionRequest = {}): Promise<AiGraderLocalStationBridgeStatus> {
    ...
    if (action === "observe-calibration-activation") {
      ...
      if (this.manifest.sessionId || this.captureLock || this.frontCaptureTransition ||
          this.manifest.previewStatus.intentionalTransition.active ||
          this.terminalLifecyclePending > 0 || this.lightingLifecyclePending > 0) {
        throw new Error("Local helper must be exactly idle and sessionless before the one activation observation.");
      }
    ...
    if (action === "prepare-calibration-activation") {
      ...
      if (this.manifest.sessionId || this.captureLock || this.terminalLifecyclePending > 0 || this.lightingLifecyclePending > 0) {
        throw new Error("Local helper must be exactly idle and sessionless before calibration activation.");
      }
      ...
      this.calibrationActivationReceipt = receipt;
      return this.status();
    }
    if (action === "confirm-calibration-activation") {
      assertExactActionRequestKeys(request, action, ["hostedCalibrationActivationAuthority"]);
      if (!this.dependencies.calibrationActivationRegistry) {
        throw new Error("Local calibration activation registry is unavailable; no fallback is permitted.");
      }
      await this.dependencies.calibrationActivationRegistry.confirmHostedActivation(request.hostedCalibrationActivationAuthority);
      this.calibrationActivationAuthority = request.hostedCalibrationActivationAuthority;
      this.calibrationActivationState = "ACTIVE";
      return this.status();
    }
    if (action === "abort-calibration-activation") {
      assertExactActionRequestKeys(request, action, ["calibrationPendingAuthority"]);
      if (!this.dependencies.calibrationActivationRegistry) {
        throw new Error("Local calibration activation registry is unavailable; no fallback is permitted.");
      }
      await this.dependencies.calibrationActivationRegistry.abortPendingActivation(
        request.calibrationPendingAuthority,
      );
      this.calibrationActivationState = "IDLE";
      this.calibrationActivationReceipt = undefined;
      this.calibrationActivationAuthority = undefined;
      return this.status();
    }
...
export function createAiGraderLocalStationBridgeHttpServer(
  ...
  let service: AiGraderLocalStationBridgeService;
  ...
  if (!runtimeDependencies.calibrationActivationRegistry && config.mode === "real" && activationConfigurationRequested) {
    ...
    const trustedLiveOperatingContext = createMathematicalCalibrationOperatingContextRuntimeV1({
      ...
      observeRuntime: async (expected) => {
        if (!service) {
          throw new Error("Local bridge service is not initialized for calibration runtime observation.");
        }
        const result = await service.observeMathematicalCalibrationActivationRuntime(
          expected,
          helperInstanceId,
          AI_GRADER_LOCAL_STATION_BRIDGE_VERSION,
        );
        return result.runtimeObservation;
      },
    });
    runtimeDependencies.calibrationActivationRegistry = createMathematicalCalibrationActivationRegistryV1({
      ...
      observeActivationRuntime: async (expected, evidenceDirectory) => {
        ...
        trustedLiveOperatingContext.validateObservation(expected, result.runtimeObservation);
        return result;
      },
      isIdle: () => {
        if (!service) return false;
        const status = service.status();
        return !status.sessionId && status.currentStep === "start_new_card" &&
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (url.pathname === "/calibration/mathematical-v1/recover-blank-reverse-timestamp-false-stop") {
        if (req.method !== "POST") return sendJson(res, 405, { ok: false, code: "METHOD_NOT_ALLOWED", message: "POST is required for the incident-bound calibration false-stop recovery." }, origin, config);
        if (!tokenMatches(req, config)) return sendJson(res, 401, { ok: false, code: "AI_GRADER_STATION_BRIDGE_UNAUTHORIZED", message: "Station token is required." }, origin, config);
        if (url.search.length > 0) throw new Error("The incident-bound calibration false-stop recovery accepts no query or caller-authored authority.");
        const body = await readJsonBody(req);
        if (Object.keys(body).length !== 0) {
          throw new Error("The incident-bound calibration false-stop recovery accepts no browser/operator-authored recovery fields.");
        }
        return sendJson(res, 200, {
          ok: true,
