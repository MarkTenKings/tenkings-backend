...
`buildAstraBackgroundDefectRequest(input)` preserves that legacy layout with transport/evidence version `atlas-astra-defect-analysis-v2`. New connected human-requested analyses explicitly use `buildAstraContextBackgroundDefectRequest(input)` and `planDefectCrops(side, INSPECTION_CONTEXT_CROP_LAYOUT)`. Its four 739×993 crops start at `(0,0)`, `(611,0)`, `(0,865)` and `(611,865)`, retaining the actual 40-pixel photo context beyond each outer card edge. The whole inspection images, source hashes, physical-card bounds, prompt, schema and original-detail policy remain unchanged.

The context builder records `cropLayoutVersion: 'inspection-context-v1'` in V2 request evidence and proposal provenance. An absent marker means the exact legacy layout; it never acquires a default/null field. Unsupported markers, markers on V1, mismatched crop rectangles and altered retained requests are rejected. Both legacy builders retain their exact request/evidence/result bytes and hashes. Restoration uses the saved marker; it never prepares new photos, retrieves new memory or selects a new layout for an existing request. Existing V2 background custody SQL remains compatible without a migration. The new layout applies only when an authorized person requests a new analysis; reading, reconnecting or reviewing old results cannot dispatch another analysis.
