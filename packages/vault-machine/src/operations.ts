...
export class VaultOperationsService {
  ...
  async startOrResumeRestock(staffSessionId: string, requestedDoorIds?: VaultDoorId[], certificationId?: string): Promise<{ sessionId: string; expectedDoorIds: VaultDoorId[] }> {
    const actor = this.machine.staff.requireSession(staffSessionId, "RESTOCK_RUN");
    if (certificationId) this.machine.requireCertificationAuthority({ sessionId: certificationId, staffSessionId });
    ...
    if (existing) {
      if (existing.certification_session_id) this.machine.staff.requireSession(staffSessionId, "CERTIFICATION_COLLECT");
      if (existing.actor_user_id !== actor.userId && !["TECHNICIAN", "ADMIN"].includes(actor.role)) throw new VaultError("RESTOCK_SESSION_SCOPE", "Restock recovery by another staff identity requires Technician or Admin", 403);
      if (existing.actor_session_id !== staffSessionId) {
        this.machine.store.transaction(() => {
          ...
          this.machine.events.append({ type: "RESTOCK_SESSION_RESUMED", actor: actor.userId, payload: { restockSessionId: existing.session_id, previousActorUserId: existing.actor_user_id, actorRole: actor.role } });
          if (existing.certification_session_id) {
            const certification = this.machine.store.one(`SELECT cs.session_id,ss.user_id FROM certification_session cs JOIN staff_session ss ON ss.session_id=cs.actor_session_id WHERE cs.session_id=? AND cs.status='ACTIVE'`, existing.certification_session_id);
            this.machine.store.run(`UPDATE certification_session SET actor_session_id=? WHERE session_id=?`, staffSessionId, certification.session_id);
            this.machine.events.append({ type: "CERTIFICATION_SESSION_RESUMED", mode: "CERTIFICATION", actor: actor.userId, payload: { certificationSessionId: certification.session_id, previousActorUserId: certification.user_id } });
          }
    ...
    if (this.machine.store.maybeOne(`SELECT 1 FROM sale WHERE state NOT IN ('COMPLETED','PAYMENT_DECLINED','PAYMENT_CANCELLED') OR payment_state NOT IN ('NOT_REQUESTED','DECLINED','CANCELLED','SETTLED') LIMIT 1`)) throw new VaultError("RESTOCK_PREFLIGHT_ACTIVE_SALE", "Restock cannot begin during an active sale or unresolved payment", 409);
    const activeCertification = this.machine.store.maybeOne(`SELECT session_id FROM certification_session WHERE status IN ('ACTIVE','CRITICAL_STOP') LIMIT 1`);
    ...
    this.machine.store.transaction(() => {
      ...
      if (certificationId) this.machine.store.run(`UPDATE restock_session SET certification_session_id=? WHERE session_id=?`, certificationId, sessionId);
      ...
      const actorGrant = this.machine.store.one(`SELECT g.grant_version FROM staff_session ss JOIN staff_grant g ON g.grant_id=ss.grant_id WHERE ss.session_id=?`, staffSessionId);
      const profile = configMachineProfile(this.machine.config.active()!.payload);
      this.machine.events.append({ type: "RESTOCK_SESSION_STARTED", actor: actor.userId, payload: { restockSessionId: sessionId, expectedDoorIds: requested, plannedItems, configVersion: meta.active_config_version, ...(profile ? { profileDigest: machineProfileDigest(profile) } : {}), actorRole: actor.role, actorGrantVersion: Number(actorGrant.grant_version), ...(certificationId ? { certificationSessionId: certificationId } : {}) } });
      this.machine.store.bumpStateVersion();
    });
  ...
  recordRestockOutcome(staffSessionId: string, restockId: string, doorId: VaultDoorId, outcome: Exclude<VaultRestockItemState, "UNREVIEWED">, notes = "", productFitConfirmed = false): void {
    const actor = this.machine.staff.requireSession(staffSessionId, "RESTOCK_RUN");
    ...
    const profile = configMachineProfile(parseJson<VaultConfigPayload>(this.machine.store.one(`SELECT payload_json FROM config_snapshot WHERE version=?`, session.config_version).payload_json));
    if (profile && outcome === "FILLED" && !productFitConfirmed) throw new VaultError("PRODUCT_FIT_CONFIRMATION_REQUIRED", "Confirm that the actual packaged product fits the selected compartment before marking it filled", 409);
    ...
    const item = this.machine.store.one(`SELECT planned_product_id,outcome,notes FROM restock_item WHERE session_id=? AND door_id=?`, restockId, doorId);
    if (item.outcome !== "UNREVIEWED") {
      if (item.outcome === outcome && String(item.notes ?? "") === notes.slice(0, 1000)) return;
      throw new VaultError("RESTOCK_OBSERVATION_IMMUTABLE", "Recorded door observation cannot be replaced", 409);
    }
    const command = this.machine.store.maybeOne(`SELECT state,completed_at FROM command_intent WHERE restock_session_id=? AND door_id=?`, restockId, doorId);
    if (!command?.completed_at || !["ACCEPTED", "SENT_UNKNOWN", "REJECTED", "TIMEOUT"].includes(String(command.state))) {
    ...
    this.machine.store.transaction(() => {
      ...
      this.machine.events.append({ type: "RESTOCK_DOOR_REVIEWED", actor: actor.userId, payload: { restockSessionId: restockId, doorId, outcome, notes: notes.slice(0, 1000), ...(profile && outcome === "FILLED" ? { productFitConfirmed: true } : {}) } });
  ...
  async startCertification(staffSessionId: string): Promise<{ sessionId: string; scheduledDoorId: VaultDoorId; commandId: string }> {
    ...
    if (!session) {
      ...
      if (!/^[a-f0-9]{40}$/.test(String(meta.source_commit))) {
      ...
      this.machine.staff.requireSession(staffSessionId, "CERTIFICATION_COLLECT");
      if (this.machine.store.maybeOne(`SELECT 1 FROM certification_session WHERE status IN ('ACTIVE','CRITICAL_STOP') LIMIT 1`)) throw new VaultError("CERTIFICATION_SESSION_CONFLICT", "Another certification session is active or requires critical recovery", 409);
      if (!controller.ready || controller.mode === "LIVE" || payment.mode === "LIVE") throw new VaultError("CERTIFICATION_ADAPTER_UNQUALIFIED", "This build permits only ready simulator or explicitly non-live test adapters", 409);
      if (this.machine.store.maybeOne(`SELECT 1 FROM restock_session WHERE finalized_at IS NULL LIMIT 1`)) throw new VaultError("CERTIFICATION_RESTOCK_ACTIVE", "Finish the active restock before starting certification", 409);
      this.machine.store.transaction(() => {
        this.machine.store.run(
          `INSERT INTO certification_session(session_id,actor_session_id,config_version,adapter_mode,status,source_commit,app_version,schema_version,controller_identity_json,payment_identity_json,retention_policy,created_at) VALUES(?,?,?,?,'ACTIVE',?,?,?,?,?,?,?)`,
          sessionId, staffSessionId, config.payload.version, payment.mode, meta.source_commit, meta.app_version, LOCAL_SCHEMA_VERSION, json(controller), json(payment), "SERVICE_LIFE_PLUS_3_YEARS", now,
        );
        const profile = configMachineProfile(config.payload);
        this.machine.events.append({ type: "CERTIFICATION_SESSION_STARTED", mode: "CERTIFICATION", actor: actor.userId, payload: { certificationSessionId: sessionId, configVersion: config.payload.version, configDigest: config.digest, ...(profile ? { profileDigest: machineProfileDigest(profile) } : {}), sourceCommit: meta.source_commit, appVersion: meta.app_version, localSchemaVersion: LOCAL_SCHEMA_VERSION, contractVersion: 1, adapterMode: payment.mode, controllerIdentity: controller, paymentIdentity: payment, retentionPolicy: "SERVICE_LIFE_PLUS_3_YEARS" } });
        this.machine.store.bumpStateVersion();
      });
      ...
      const previousActor = this.machine.store.one(`SELECT user_id FROM staff_session WHERE session_id=?`, resumed.actor_session_id);
      this.machine.store.transaction(() => {
        this.machine.store.run(`UPDATE certification_session SET actor_session_id=? WHERE session_id=? AND status='ACTIVE'`, staffSessionId, resumed.session_id);
        this.machine.events.append({ type: "CERTIFICATION_SESSION_RESUMED", mode: "CERTIFICATION", actor: actor.userId, payload: { certificationSessionId: resumed.session_id, previousActorUserId: previousActor.user_id } });
        this.machine.store.bumpStateVersion();
      });
      session = this.machine.store.one(`SELECT * FROM certification_session WHERE session_id=?`, session.session_id);
    }
    return this.ensureNextCertificationCommand(session, actor.userId, configDoorIds(config.payload).map(id => config.payload.doorMapping.find(entry => entry.doorId === id)!));
  }

  async startCertificationCycle(staffSessionId: string, certificationId: string, cycleType: "PURCHASE" | "RESTOCK"): Promise<{ cycleType: string; saleId?: string; restockSessionId?: string; doorId: VaultDoorId }> {
    this.machine.requireCertificationAuthority({ sessionId: certificationId, staffSessionId });
    if (cycleType !== "PURCHASE" && cycleType !== "RESTOCK") throw new VaultError("CERTIFICATION_CYCLE_INVALID", "Choose a purchase or restock cycle", 400);
    if (this.machine.store.maybeOne(`SELECT 1 FROM command_intent ci LEFT JOIN certification_evidence ce ON ce.command_id=ci.command_id WHERE ci.certification_session_id=? AND ce.command_id IS NULL LIMIT 1`, certificationId)) throw new VaultError("CERTIFICATION_OBSERVATION_REQUIRED", "Record the previous command observation before starting another cycle", 409);
    if (this.machine.store.maybeOne(`SELECT 1 FROM restock_session WHERE finalized_at IS NULL LIMIT 1`) || this.machine.store.maybeOne(`SELECT 1 FROM sale WHERE presentation_done_at IS NULL AND state NOT IN ('PAYMENT_DECLINED','PAYMENT_CANCELLED') LIMIT 1`)) throw new VaultError("CERTIFICATION_CYCLE_ACTIVE", "Complete the current sale or restock before the next cycle", 409);
    const config = this.machine.config.active()!;
    const candidates = config.payload.doorMapping.filter(entry => config.payload.assignments[entry.doorId] && config.payload.products.some(product => product.active && product.id === config.payload.assignments[entry.doorId]));
    if (!candidates.length) throw new VaultError("CERTIFICATION_PRODUCTS_REQUIRED", "Certification cycles require an active planned product", 409);
    const counts = Object.fromEntries(this.machine.store.all(`SELECT ce.door_id,COUNT(*) AS count FROM certification_evidence ce JOIN command_intent ci ON ci.command_id=ce.command_id WHERE ce.session_id=? AND ce.outcome='PASS' AND ci.authority=? GROUP BY ce.door_id`, certificationId, cycleType === "PURCHASE" ? "PAID_SALE" : "RESTOCK").map(row => [String(row.door_id), Number(row.count)]));
    const doorId = nextUnderTestedDoor(counts, candidates.map(entry => entry.doorId)) as VaultDoorId;
    const session = this.machine.store.one(`SELECT inventory_snapshot_json FROM certification_session WHERE session_id=?`, certificationId);
    this.machine.store.transaction(() => {
      if (!session.inventory_snapshot_json) this.machine.store.run(`UPDATE certification_session SET inventory_snapshot_json=? WHERE session_id=?`, json(this.machine.store.all(`SELECT door_id,state,product_id,owning_sale_id,owning_restock_id FROM door WHERE active=1 ORDER BY display_order`)), certificationId);
      // This method is restricted to the non-actuating MOCK adapter. It prepares
      // test inventory only and restores the exact prior inventory on submission.
      this.machine.store.run(`UPDATE door SET state=?,product_id=?,owning_sale_id=NULL,owning_restock_id=NULL,version=version+1 WHERE door_id=?`, cycleType === "PURCHASE" ? "AVAILABLE" : "EMPTY", cycleType === "PURCHASE" ? config.payload.assignments[doorId] : null, doorId);
    });
    if (cycleType === "PURCHASE") {
      const authority = { sessionId: certificationId, staffSessionId };
      this.machine.selectCartDoor(doorId, config.payload.assignments[doorId]!, true, authority);
      const sale = (await this.machine.checkout({ idempotencyKey: randomUUID(), mode: "CERTIFICATION", configVersion: config.payload.version, doorIds: [doorId] }, authority)).sale;
      if (!sale) throw new VaultError("CERTIFICATION_CHECKOUT_CONFLICT", "Certification checkout lost its selected door", 409);
      this.machine.events.append({ type: "CERTIFICATION_CYCLE_STARTED", mode: "CERTIFICATION", payload: { certificationSessionId: certificationId, cycleType, saleId: sale.saleId } });
      await this.machine.startPayment(sale.saleId, deterministicId("cert-payment", sale.saleId));
      await this.machine.advancePayments();
      return { cycleType, saleId: sale.saleId, doorId };
    }
    const restock = await this.startOrResumeRestock(staffSessionId, [doorId], certificationId);
    this.machine.events.append({ type: "CERTIFICATION_CYCLE_STARTED", mode: "CERTIFICATION", payload: { certificationSessionId: certificationId, cycleType, restockSessionId: restock.sessionId } });
    return { cycleType, restockSessionId: restock.sessionId, doorId };
  }
  ...
  submitCertification(staffSessionId: string, certificationId: string, physicalCloseConfirmed: boolean): void {
    ...
    if (this.machine.store.maybeOne(`SELECT 1 FROM restock_session WHERE certification_session_id=? AND finalized_at IS NULL LIMIT 1`, certificationId)) throw new VaultError("CERTIFICATION_RESTOCK_INCOMPLETE", "Finalize the certification restock before submission", 409);
    if (this.machine.store.maybeOne(`SELECT 1 FROM sale WHERE certification_session_id=? AND (presentation_done_at IS NULL OR payment_state NOT IN ('DECLINED','CANCELLED','SETTLED')) LIMIT 1`, certificationId)) throw new VaultError("CERTIFICATION_PAYMENT_INCOMPLETE", "Finish the certification payment before restoring its inventory snapshot", 409);
    this.machine.store.transaction(() => {
      ...
      if (session.inventory_snapshot_json) {
        if (session.adapter_mode !== "MOCK") throw new VaultError("CERTIFICATION_INVENTORY_RESTORE_FORBIDDEN", "Only non-actuating mock test inventory may be restored", 409);
        for (const door of parseJson<Array<Record<string, unknown>>>(session.inventory_snapshot_json)) this.machine.store.run(`UPDATE door SET state=?,product_id=?,owning_sale_id=?,owning_restock_id=?,version=version+1 WHERE door_id=?`, door.state, door.product_id, door.owning_sale_id, door.owning_restock_id, door.door_id);
      }
  ...
  private async ensureNextRestockCommand(session: Record<string, unknown>, actorUserId: string): Promise<void> {
    ...
    if (!unobserved) {
      const next = this.machine.store.maybeOne(`SELECT ri.door_id FROM restock_item ri JOIN door d ON d.door_id=ri.door_id LEFT JOIN command_intent ci ON ci.restock_session_id=ri.session_id AND ci.door_id=ri.door_id WHERE ri.session_id=? AND ri.outcome='UNREVIEWED' AND ci.command_id IS NULL ORDER BY ri.planned_product_id,d.display_order LIMIT 1`, session.session_id);
      if (next) {
        const door = this.machine.store.one(`SELECT * FROM door WHERE door_id=? AND active=1`, next.door_id);
  ...
  private async ensureNextCertificationCommand(
    ...
    const door = this.machine.store.one(`SELECT * FROM door WHERE door_id=? AND active=1`, scheduledDoorId);
