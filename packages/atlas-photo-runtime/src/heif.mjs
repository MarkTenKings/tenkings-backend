...
async function encodePng(native, geometry, outputPath, maxOutputBytes) {
  ...
  async function* png() {
    ...
    if (native.nclx) yield chunk('sRGB', Buffer.from([0]));
...
export async function decodeHeif(bytes, request) {
  ...
  need(probe.propertyTypes.slice(primaryProperties.length).every(type=>probe.grid&&probe.nclx&&type==='colr'), 'PHOTO_SOURCE_MISMATCH');
  ...
  const metadata = {
    ...
    colorSpace: probe.nclx ? `NCLX:${probe.nclx.primaries}/${probe.nclx.transfer}/${probe.nclx.matrix}/${probe.nclx.fullRange}` : null,
    // Absence of a profile is unknown, not evidence of SDR or sRGB.
    dynamicRange: probe.nclx ? 'SDR' : request.existingOriginal?.metadata?.dynamicRange ?? null,
  };
  ...
  return { original, decodePlan, output,
    treatment: { decoder: 'libheif/libde265', version: probe.version,
      policyVersion: 'atlas-heif-primary-lossless-v1', channels: 3, bitDepth: output.bitDepth,
      colorSpace: probe.nclx ? 'sRGB' : null, colorTreatment: probe.nclx ? 'converted' : 'unmanaged',
