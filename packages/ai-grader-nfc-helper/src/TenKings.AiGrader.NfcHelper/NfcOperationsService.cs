...
public sealed partial class NfcOperationsService
{
    ...
    public NfcOperationsService(INfcReaderBackend backend, ISafeLogger? logger = null, int operationTimeoutMs = NfcProtocol.DefaultOperationTimeoutMs)
    {
    ...
    public async Task<NfcWriteResponse> WriteAsync(NfcWriteRequest request, string requestId, CancellationToken cancellationToken)
    {
        ...
        var entry = _idempotency.GetOrAdd(request.IdempotencyKey, _ =>
            new IdempotencyEntry(
                ...
                new Lazy<Task<NfcWriteResponse>>(
        ...
        try
        {
            return await entry.Operation.Value.WaitAsync(_operationTimeout, cancellationToken);
        }
        catch (TimeoutException)
        {
            _logger.Error("nfc_write_wait_failed", requestId, "reader_timeout");
            throw new NfcHelperException("reader_timeout", "The NFC write timed out; remove the tag and retry only after status is no longer busy.", true, 504);
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        {
            _logger.Error("nfc_write_wait_failed", requestId, "request_cancelled");
            throw new NfcHelperException("request_cancelled", "The request ended while the NFC write was still running; wait until status is no longer busy before retrying.", true, 409);
        }
        catch (NfcHelperException error) when (error.Code == "writer_busy")
        {
    ...
    private async Task<NfcWriteResponse> ExecuteWriteWithLockAsync(NfcWriteRequest request, string requestId)
    {
    ...
    private NfcWriteResponse WriteCore(NfcWriteRequest request, bool hardwareGateTest = false)
    {
        ...
        {
            return VerifiedResponse(target, snapshot.UidFingerprint, "already_programmed_exact");
        }
        ...
        return VerifiedResponse(target, snapshot.UidFingerprint, "write_verified_pcsc_readback");
    }

    private static NfcWriteResponse VerifiedResponse(EncodedNdefUrl target, string uidFingerprint, string resultCode) =>
        new(
            NfcProtocol.ProtocolVersion,
            NfcProtocol.ChipType,
            target.Url,
    ...
    private static void ValidateWriteRequest(NfcWriteRequest request)
    {
        ...
        _ = NdefCodec.ValidateProductionUrl(request.Url);
    }
    ...
    private void TrimIdempotencyCache()
    {
        ...
        foreach (var stale in _idempotency.OrderBy(item => item.Value.Sequence).Take(_idempotency.Count - 256))
            _idempotency.TryRemove(stale.Key, out _);
    }
    ...
    private sealed record IdempotencyEntry(string RequestDigest, long Sequence, Lazy<Task<NfcWriteResponse>> Operation);
}
