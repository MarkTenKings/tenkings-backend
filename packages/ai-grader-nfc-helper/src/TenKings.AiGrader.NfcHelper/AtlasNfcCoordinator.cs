...
internal sealed record AtlasPersistedOperation(
    AtlasNfcSignedJob SignedJob,
    ...
    AtlasNfcTerminalResult? TerminalResult,
    string CreatedAt,
    string UpdatedAt);

public sealed partial class AtlasNfcCoordinator
{
    internal const string StateFileName = "active-atlas-job.json";
    ...
    private readonly AtlasServerTrust _serverTrust;
    private readonly ISafeLogger _logger;
    private readonly TimeProvider _timeProvider;
    private readonly int _callbackPort;
    private readonly string? _statePath;
    private AtlasPersistedOperation? _operation;
    private bool _ownsGate;

    public AtlasNfcCoordinator(
        ...
        TimeProvider? timeProvider = null)
    {
        ...
        if (serverTrust.KeyIds.Contains(signer.WorkstationKeyId, StringComparer.Ordinal))
            throw Error("atlas_nfc_server_trust_invalid", "The ATLAS server and workstation identities must be distinct.", false, 503);
    ...
    public AtlasNfcOperationResponse Prepare(AtlasNfcPrepareRequest request, string requestId)
    {
        if (!Available) throw Error("atlas_nfc_unavailable", "ATLAS NFC is unavailable on this helper configuration.", false, 503);
        if (!request.FreshTagConfirmed) throw Error("atlas_nfc_fresh_tag_required", "Confirm one unused F8215 from controlled inventory, kept off the reader until Start Encoding.", false, 400);
        var job = request.Job ?? throw Error("atlas_nfc_context_invalid", "The ATLAS NFC signed job is required.", false, 400);
        var spki = RequireTrustedJob(job);
        try { AtlasNfcProtocol.RequireMayStart(job, UtcNow()); }
        ...
        lock (_sync)
        {
            if (_operation is not null)
            {
                if (!SecureEquals(_operation.JobEnvelopeSha256, hash))
                    throw Error("atlas_nfc_operation_conflict", "Finish the protected NFC operation before starting another card.", false, 409);
            ...
            if (!_operationGate.TryEnterAsync(CancellationToken.None).GetAwaiter().GetResult())
                throw Error("reader_busy", "Another V1 or ATLAS NFC operation already owns the workstation.", true, 409);
            _ownsGate = true;

            var now = UtcNow();
            var callbackIdentity = RandomIdentity(32);
            var correlationId = RandomIdentity(32);
            var operationFileName = $"f8215-atlas-{RandomIdentity(16)}.gototags";
            var operation = new AtlasPersistedOperation(
                job,
                hash,
                job.Url,
                correlationId,
            ...
            catch (Exception error)
            {
                ...
                {
                    Phase = "uncertain",
                    ErrorCode = error is NfcHelperException helperError ? helperError.Code : "atlas_nfc_prepare_failed",
    ...
    public AtlasNfcOperationResponse Status(AtlasNfcStatusRequest request)
    {
        ...
        lock (_sync)
        {
                ...
                UtcNow() > ParseTimestamp(operation.SignedJob.ExpiresAt))
            {
                ...
                {
                    Phase = "uncertain",
                    ErrorCode = "atlas_nfc_job_expired",
    ...
    public AtlasNfcAcknowledgeResponse AcknowledgeSuccess(
        AtlasNfcSuccessAcknowledgeRequest request,
        string requestId)
    {
        ValidateHash(request.JobEnvelopeSha256);
        if (!request.TagRemoved) throw Error("atlas_nfc_tag_removal_required", "Remove the tag before closing the operation.", false, 400);
        lock (_sync)
        {
            var operation = RequireMatching(request.JobEnvelopeSha256);
            if (operation.Phase != "completed" &&
                !(operation.Phase == "closing" && operation.TerminalResult is not null && operation.ErrorCode is null))
                throw Error("atlas_nfc_success_not_ready", "Only a completed ATLAS NFC result may be acknowledged.", false, 409);
            operation = operation with
            {
                Phase = "closing",
                ErrorCode = null,
            ...
            Persist(operation);
            Close(operation);
            _logger.Info("atlas_nfc_success_acknowledged", requestId, "protected_operation_removed");
            return new AtlasNfcAcknowledgeResponse(true);
        }
    }

    public AtlasNfcAcknowledgeResponse AcknowledgeDiscard(
        AtlasNfcDiscardAcknowledgeRequest request,
        string requestId)
    {
        ValidateHash(request.JobEnvelopeSha256);
        if (!request.TagRemoved) throw Error("atlas_nfc_tag_removal_required", "Remove and discard the uncertain tag before closing the operation.", false, 400);
        if (request.Phase is not ("failed" or "uncertain" or "completed_unrecorded") ||
            ...
            throw Error("atlas_nfc_discard_ack_invalid", "The exact tag discard acknowledgement is invalid.", false, 400);
        lock (_sync)
        {
            var operation = RequireMatching(request.JobEnvelopeSha256);
            ...
            if ((!acceptedClosing && operation.Phase != expectedActivePhase) ||
                ...
                throw Error("atlas_nfc_discard_ack_mismatch", "The discard acknowledgement does not match the exact failed operation.", false, 409);
            operation = operation with
            {
                Phase = "closing",
            ...
            Persist(operation);
            Close(operation);
            _logger.Info("atlas_nfc_discard_acknowledged", requestId, request.Phase);
            return new AtlasNfcAcknowledgeResponse(true);
        }
    }
    ...
    public void AcceptCallback(string callbackIdentity, ReadOnlyMemory<byte> body, string requestId)
    {
        ...
        lock (_sync)
        {
            ...
            try
            {
                ...
                try
                {
                    result = AtlasNfcProtocol.CreateTerminalResult(
                        operation.SignedJob,
                        spki,
                        _signer,
                ...
                _operation = operation;
                Persist(operation);
                _logger.Info("atlas_nfc_operation_completed", requestId, AtlasNfcProtocol.ReaderResultCode);
            }
    ...
    private byte[] RequireTrustedJob(AtlasNfcSignedJob job)
    {
        if (!_serverTrust.TryCopyPublicSpki(job.SigningKeyId, out var spki) ||
            !AtlasNfcProtocol.VerifyJob(job, spki))
        {
            if (spki.Length > 0) CryptographicOperations.ZeroMemory(spki);
            throw Error("atlas_nfc_job_untrusted", "The ATLAS NFC server job is not trusted.", false, 403);
        }
        return spki;
    }
    ...
    private void RecoverPersistedState()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                var operation = JsonSerializer.Deserialize<AtlasPersistedOperation>(bytes, PersistedJson) ?? throw new JsonException();
                ValidatePersisted(operation);
                if (!_operationGate.TryEnterAsync(CancellationToken.None).GetAwaiter().GetResult())
                    throw Error("atlas_nfc_recovery_conflict", "Persisted V1 and ATLAS NFC operations conflict.", false, 503);
                _ownsGate = true;
                _operation = operation;
                if (operation.Phase is "preparing" or "awaiting_manual_start")
                {
                    ...
                    {
                        Phase = "uncertain",
                        ErrorCode = "atlas_nfc_helper_restarted",
                        DiscardAcknowledgementNonce = operation.DiscardAcknowledgementNonce ?? RandomIdentity(24),
                        UpdatedAt = CanonicalUtc(UtcNow()),
                    };
                    _operation = operation;
        ...
        {
            throw Error("atlas_nfc_recovery_state_invalid", "The protected ATLAS NFC state requires review.", false, 503);
        }
    }

    private void ValidatePersisted(AtlasPersistedOperation operation)
    {
        ...
        if (operation.JobEnvelopeSha256 != AtlasNfcProtocol.JobEnvelopeSha256(operation.SignedJob) ||
        ...
        try
        {
            if (operation.TerminalResult is not null)
            {
                ...
                try
                {
                    if (!AtlasNfcProtocol.VerifyTerminalResult(operation.TerminalResult, operation.SignedJob, workstationSpki))
                        throw new JsonException();
                }
    ...
    private void AssertNoDualPersistedState()
    {
        ...
        var v2Path = ProtectedJobDirectory.ContainedFile(_options.JobRoot, TenKingsV2NfcCoordinator.StateFileName);
        if ((File.Exists(v1Path) || File.Exists(v2Path)) && File.Exists(_statePath))
            throw Error("atlas_nfc_dual_recovery_state", "Persisted V1 and ATLAS NFC states conflict; no state was changed.", false, 503);
    }
    ...
    private void AssertNoOrphanedV2TemporaryState()
    {
        try
        {
            foreach (var path in Directory.EnumerateFiles(_options.JobRoot, "atlas-state-*.tmp", SearchOption.TopDirectoryOnly))
            {
                if (TemporaryStateFilePattern().IsMatch(Path.GetFileName(path)))
                    throw Error("atlas_nfc_recovery_state_invalid", "A protected ATLAS NFC state write requires review.", false, 503);
            }
        }
        catch (NfcHelperException) { throw; }
        catch (Exception error) when (error is IOException or UnauthorizedAccessException)
        { throw Error("atlas_nfc_recovery_state_invalid", "The protected ATLAS NFC state directory requires review.", false, 503); }
    }
    ...
    private void AssertNoPersistedStateBeforeStart()
    {
        ...
        var v2Path = ProtectedJobDirectory.ContainedFile(_options.JobRoot, TenKingsV2NfcCoordinator.StateFileName);
        if (File.Exists(v1Path) || File.Exists(v2Path) || File.Exists(_statePath))
            throw Error("atlas_nfc_recovery_conflict", "A protected V1 or ATLAS NFC operation requires recovery before another card may start.", false, 503);
    }

    private void Persist(AtlasPersistedOperation operation)
    {
        if (_statePath is null) throw Error("atlas_nfc_unavailable", "ATLAS NFC protected state is unavailable.", false, 503);
        var temporary = ProtectedJobDirectory.ContainedFile(_options.JobRoot, $"atlas-state-{RandomIdentity(12)}.tmp");
        ...
        {
            throw Error("atlas_nfc_state_unavailable", "The protected ATLAS NFC state could not be saved.", false, 503);
        }
        finally
        {
    ...
    private void Close(AtlasPersistedOperation operation)
    {
    ...
    private AtlasPersistedOperation RequireMatching(string hash)
    {
        var operation = _operation ?? throw Error("atlas_nfc_job_not_found", "No protected ATLAS NFC operation matches this browser state.", false, 404);
        if (!SecureEquals(operation.JobEnvelopeSha256, hash))
            throw Error("atlas_nfc_job_mismatch", "A different NFC operation owns the workstation.", false, 409);
        return operation;
    }

    private static AtlasNfcOperationResponse Response(AtlasPersistedOperation operation) => new(
        NfcProtocol.ProtocolVersion,
        NfcProtocol.HelperVersion,
        AtlasNfcProtocol.HelperCapability,
        operation.JobEnvelopeSha256,
        operation.SignedJob.SpecimenId,
        operation.SignedJob.ApprovalId,
        operation.SignedJob.ApprovalVersion,
        operation.SignedJob.PublicHash,
    ...
    private static void ValidateHash(string value) { if (string.IsNullOrEmpty(value) || !Sha256Pattern().IsMatch(value)) throw Error("atlas_nfc_context_invalid", "The ATLAS NFC operation identity is invalid.", false, 400); }
    ...
    [GeneratedRegex("^[a-f0-9]{64}\\z", RegexOptions.CultureInvariant)] private static partial Regex Sha256Pattern();
    [GeneratedRegex("^[A-Za-z0-9_-]{43}\\z", RegexOptions.CultureInvariant)] private static partial Regex CallbackPattern();
    [GeneratedRegex("^[A-Za-z0-9_-]{32}\\z", RegexOptions.CultureInvariant)] private static partial Regex NoncePattern();
    [GeneratedRegex("^f8215-atlas-[A-Za-z0-9_-]{22}\\.gototags\\z", RegexOptions.CultureInvariant)] private static partial Regex OperationFilePattern();
    [GeneratedRegex("^atlas-state-[A-Za-z0-9_-]{16}\\.tmp\\z", RegexOptions.CultureInvariant)] private static partial Regex TemporaryStateFilePattern();
    [GeneratedRegex("^[a-z0-9_]{1,80}\\z", RegexOptions.CultureInvariant)] private static partial Regex ErrorCodePattern();
}
