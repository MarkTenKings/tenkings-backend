#!/usr/bin/env node
/** Sports preparation transaction qualification only. Reuses the owned cron
...
const fixture = 'frontend/nextjs-app/tests/setCatalogSportsPreparationPostgres.test.ts';
const runner = 'packages/database/scripts/testSetCatalogSportsPreparationDisposable.mjs';
...
async function seal() {
  ...
  const singles = [fixture, runner, 'frontend/nextjs-app/tests/setCatalogSportsPreparationFixture.json', 'docs/plans/catalog-pilot-20260916/sports-reconciliation-report.unreviewed.json', 'docs/plans/catalog-pilot-20260916/sports-publication-pilot.unsubmitted.json', 'frontend/nextjs-app/tsconfig.json', 'package.json', 'pnpm-lock.yaml',
  ...
  const activeModels = ['SetDraft', 'SetDraftVersion', 'SetIngestionJob', 'SetTaxonomySource', 'SetProgram', 'SetCard', 'SetParallel', 'SetParallelScope', 'SetAuditEvent', 'SetSeedJob', 'SetReplaceJob', 'SetCatalogEvidencePublication'];
  ...
  const packet = JSON.parse(await readFile(join(repo, 'docs/plans/catalog-pilot-20260916/sports-publication-pilot.unsubmitted.json'), 'utf8'));
  const sourceArtifacts = [];
  for (const source of packet.sourceEvidence) { const bytes = await readFile(source.localEvidencePath); sourceArtifacts.push({ sourceRef: source.sourceRef, path: await realpath(source.localEvidencePath), byteSize: bytes.length, sha256: sha(bytes) }); }
  ...
  return { schema_version: 1, scope: 'sports-additive-preparation-transaction-only', repo, node: { path: process.execPath, version: process.version, platform: platform(), arch: arch() },
    limits: { total_ms: capMs, cleanup_reserve_ms: cleanupReserveMs, disk_floor_bytes: diskFloor, launch_reserve_bytes: launchReserve, source_timeout_ms: 20_000, fixture_timeout_ms: 120_000 },
...
try {
  ...
  if (prepareOnly) {
    ...
    temp = await mkdtemp(join(tmpdir(), 'tk-sports-preparation-'));
    ...
    await limited(() => admin.query('CREATE DATABASE tenkings_sports_preparation_disposable'), 15_000, 'Create owned database');
    await limited(() => admin.end(), 3000, 'Administrative disconnect'); clients.delete(admin);
    const databaseUrl = url('tenkings_sports_preparation_disposable'), nonce = randomBytes(24).toString('hex');
    ...
    const observer = await connection('tenkings_sports_preparation_disposable');
    ...
    assert.equal(server.database, 'tenkings_sports_preparation_disposable'); assert.equal(server.host, '127.0.0.1'); assert.equal(server.encoding, 'UTF8');
    ...
    const fixtureEnv = { ...cleanEnv, DATABASE_URL: databaseUrl, TEN_KINGS_SPORTS_PREPARATION_DISPOSABLE_VALIDATION: '1',
      TEN_KINGS_SPORTS_PREPARATION_OWNERSHIP_FILE: ownershipFile, TEN_KINGS_SPORTS_PREPARATION_NONCE: nonce, SET_CATALOG_EVIDENCE_ENABLED: 'true',
      TSX_TSCONFIG_PATH: join(frontend, 'tsconfig.json') };
    await disk(diskFloor);
    fixtureStarted = true;
    const tap = await run(process.execPath, ['--import', pathToFileURL(requireFront.resolve('tsx')).href, '--test', '--test-reporter=tap', '--test-concurrency=1', join(repo, fixture)], { env: fixtureEnv, cwd: frontend, label: 'sports-preparation-functional', timeoutMs: 150_000 });
    for (const [name, count] of [['tests', 7], ['pass', 7], ['fail', 0], ['cancelled', 0], ['skipped', 0]]) assert.match(tap, new RegExp(`^# ${name} ${count}$`, 'm'), `The actual sports fixture must pass without skip: ${name}.`);
  ...
  try {
    ...
    if (!prepareOnly) await limited(() => artifact('summary.json', { complete: completed && cleanup.errors.length === 0, functional_integration_pass: completed && cleanup.errors.length === 0,
      release_pass: false, performance_pass: false, actual_sports_preparation_service_started: fixtureStarted, fixture_only_catalog_flag: true,
      real_provider_calls: 0, preserved_fixture_identity_counts_if_complete: completed ? { cards: 379, programs: 10, versions: 6 } : null,
      limitations: ['Synthetic historical payloads and human session; real pinned local PDF bytes', 'No hosted request or actual authenticated API/UI execution', 'No production database, approval, grant or publication', 'No performance conclusion', 'Direct card-only maintenance requires exclusive coordination'], elapsed_ms: Date.now() - startedAt }), remaining(3000), 'Summary receipt', false);
