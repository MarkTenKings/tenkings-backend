#!/usr/bin/env node
/** Changed-writer split comparison. Frozen driver/role logic and failed studies remain untouched. Preparation is DB-free. */
...
try {
  const preregPath = join(repo, 'docs/plans/2026-09-16-intake-history-reuse-comparison-protocol.json');
  ...
  assert.deepEqual(refs, { A: '882e40a20bac3f3a276e03c1cbe9f823785d42a1', B: '12e13992cd341eeea96dbe0955cc4dbfba67bc1d' });
  ...
  assert.deepEqual(runtimeDiff, prereg.runtime_changed_paths, 'Only the qualified writer may differ in exported runtime inputs');
