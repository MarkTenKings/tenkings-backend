#!/usr/bin/env node
/** Append-only staff research review transaction qualification only. Never a performance or release gate. */
...
const fixture = 'frontend/nextjs-app/tests/staffInventoryResearchReviewPostgres.test.ts';
const runner = 'packages/database/scripts/testStaffInventoryResearchReviewDisposable.mjs';
...
async function seal() {
  ...
  // Review persistence uses raw SQL. The only ignored generated-client delta is
  // the new reciprocal relation, unused by existing ORM calls in this fixture.
  // Every scalar field and every other active relation must still match exactly.
  const model = (text, name) => { const match = text.match(new RegExp(`model ${name} \\{([\\s\\S]*?)^\\}`, 'm')); assert.ok(match, `Missing active model ${name}.`); return match[1].replace(/\/\/[^\n]*/g, '').replace(/^\s*staffResearchReviews\s+StaffInventoryResearchReviewV2\[\]\s*$/gm, '').replace(/\s+/g, ' ').trim(); };
  ...
  return { schema_version: 1, scope: 'staff-research-review-transactions-only', repo, node: { path: process.execPath, version: process.version, platform: platform(), arch: arch() },
    limits: { total_ms: capMs, cleanup_reserve_ms: cleanupReserveMs, disk_floor_bytes: diskFloor, launch_reserve_bytes: launchReserve, fixture_timeout_ms: 120_000, real_provider_calls: 0 },
    sources, generated_client: { root: generatedRoot, files: clientFiles, active_models: activeModels, ignored_unexercised_relation: 'staffResearchReviews StaffInventoryResearchReviewV2[]', review_model_access: 'raw SQL only; no generated review delegate used' }, versions, native };
}
...
try {
  ...
  if (prepareOnly) {
    ...
    temp = await mkdtemp(join(tmpdir(), 'tk-research-review-'));
    ...
    await limited(() => admin.query('CREATE DATABASE tenkings_research_review_disposable'), 15_000, 'Create owned database');
    await limited(() => admin.end(), 3000, 'Administrative disconnect'); clients.delete(admin);
    const databaseUrl = url('tenkings_research_review_disposable'), nonce = randomBytes(24).toString('hex');
    ...
    const observer = await connection('tenkings_research_review_disposable');
    ...
    assert.equal(server.database, 'tenkings_research_review_disposable'); assert.equal(server.host, '127.0.0.1'); assert.equal(server.encoding, 'UTF8');
    ...
    const fixtureEnv = { ...cleanEnv, DATABASE_URL: databaseUrl, TEN_KINGS_RESEARCH_REVIEW_DISPOSABLE_VALIDATION: '1',
      TEN_KINGS_RESEARCH_REVIEW_OWNERSHIP_FILE: ownershipFile, TEN_KINGS_RESEARCH_REVIEW_NONCE: nonce,
      TSX_TSCONFIG_PATH: join(frontend, 'tsconfig.json') };
    await disk(diskFloor);
    fixtureStarted = true;
    const tap = await run(process.execPath, ['--import', pathToFileURL(requireFront.resolve('tsx')).href, '--test', '--test-reporter=tap', '--test-concurrency=1', join(repo, fixture)], { env: fixtureEnv, cwd: frontend, label: 'research-review-functional', timeoutMs: 150_000 });
    for (const [name, count] of [['tests', 13], ['pass', 13], ['fail', 0], ['cancelled', 0], ['skipped', 0]]) assert.match(tap, new RegExp(`^# ${name} ${count}$`, 'm'), `The actual fixture must pass without skip: ${name}.`);
  ...
  try {
    ...
    if (!prepareOnly) await limited(() => artifact('summary.json', { complete: completed && cleanup.errors.length === 0, functional_integration_pass: completed && cleanup.errors.length === 0,
      release_pass: false, performance_pass: false, actual_review_writer_started: fixtureStarted,
      real_provider_calls: 0,
      limitations: ['Synthetic inventory and research inputs in an owned loopback database', 'No hosted request, browser, live migration or publication', 'No performance conclusion'], elapsed_ms: Date.now() - startedAt }), remaining(3000), 'Summary receipt', false);
