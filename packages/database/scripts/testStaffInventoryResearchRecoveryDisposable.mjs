#!/usr/bin/env node
/** Bounded automatic research recovery transaction qualification only. Never a performance or release gate. */
...
const fixture = 'frontend/nextjs-app/tests/staffInventoryResearchRecoveryPostgres.test.ts';
const runner = 'packages/database/scripts/testStaffInventoryResearchRecoveryDisposable.mjs';
...
async function seal() {
  ...
  // Recovery uses raw SQL only; its five additive fields are ignored for ORM compatibility. Every existing field must match exactly.
  const model = (text, name) => { const match = text.match(new RegExp(`model ${name} \\{([\\s\\S]*?)^\\}`, 'm')); assert.ok(match, `Missing active model ${name}.`); return match[1].replace(/\/\/[^\n]*/g, '').replace(/^\s*staffResearchReviews\s+StaffInventoryResearchReviewV2\[\]\s*$/gm, '').replace(/^\s*recovery(?:StateHash|State|NextCheckAt|LeaseToken|LeaseExpiresAt)\s+[^\n]+$/gm, '').replace(/\s+/g, ' ').trim(); };
  ...
  return { schema_version: 1, scope: 'staff-research-recovery-transactions-only', repo, node: { path: process.execPath, version: process.version, platform: platform(), arch: arch() },
    ...
    sources, generated_client: { root: generatedRoot, files: clientFiles, active_models: activeModels, ignored_additive_raw_sql_fields: ['staffResearchReviews', 'recoveryState', 'recoveryStateHash', 'recoveryNextCheckAt', 'recoveryLeaseToken', 'recoveryLeaseExpiresAt'], review_model_access: 'raw SQL only; no generated review delegate used' }, versions, native };
}
...
try {
  ...
  if (prepareOnly) {
    ...
    temp = await mkdtemp(join(tmpdir(), 'tk-research-recovery-'));
    ...
    await limited(() => admin.query('CREATE DATABASE tenkings_research_recovery_disposable'), 15_000, 'Create owned database');
    await limited(() => admin.end(), 3000, 'Administrative disconnect'); clients.delete(admin);
    const databaseUrl = url('tenkings_research_recovery_disposable'), nonce = randomBytes(24).toString('hex');
    ...
    const observer = await connection('tenkings_research_recovery_disposable');
    ...
    assert.equal(server.database, 'tenkings_research_recovery_disposable'); assert.equal(server.host, '127.0.0.1'); assert.equal(server.encoding, 'UTF8');
    ...
    const fixtureEnv = { ...cleanEnv, DATABASE_URL: databaseUrl, TEN_KINGS_RESEARCH_RECOVERY_DISPOSABLE_VALIDATION: '1',
      TEN_KINGS_RESEARCH_RECOVERY_OWNERSHIP_FILE: ownershipFile, TEN_KINGS_RESEARCH_RECOVERY_NONCE: nonce,
  ...
  try {
    ...
    if (!prepareOnly) await limited(() => artifact('summary.json', { complete: completed && cleanup.errors.length === 0, functional_integration_pass: completed && cleanup.errors.length === 0,
      release_pass: false, performance_pass: false, actual_recovery_writer_started: fixtureStarted,
