#!/usr/bin/env node
/** Pokemon preparation transaction qualification only. Reuses the owned cron
 * supervisor safeguards; separate database, fixtures, seals and result contract.
 * Never a performance, publication or production authorization gate. */
...
assert.equal(process.version, 'v22.23.2'); assert.equal(platform(), 'darwin'); assert.equal(arch(), 'arm64');
...
const fixture = 'frontend/nextjs-app/tests/setCatalogPokemonPreparationPostgres.test.ts';
const runner = 'packages/database/scripts/testPokemonCatalogDisposable.mjs';
...
async function run(command, commandArgs, { env = cleanEnv, cwd = repo, label, timeoutMs = 120_000 } = {}) {
  ...
  try {
    ...
    return output;
  } finally { await writeFile(join(out, `${label}.log`), redact(output), { mode: 0o600, flag: 'wx' }); }
}
...
async function seal() {
  ...
  const singles = [fixture, runner, 'docs/plans/catalog-pilot-20260916/pokemon-complete-import.unreviewed.json', 'docs/plans/catalog-pilot-20260916/pokemon-complete-checklist.unreviewed.json', 'docs/plans/catalog-pilot-20260916/source-manifest.draft.json', 'frontend/nextjs-app/tsconfig.json', 'package.json', 'pnpm-lock.yaml',
  ...
  const activeModels = ['SetDraft', 'SetDraftVersion', 'SetIngestionJob', 'SetTaxonomySource', 'SetProgram', 'SetCard', 'SetParallel', 'SetParallelScope', 'SetAuditEvent', 'SetSeedJob', 'SetReplaceJob', 'SetCatalogEvidencePublication', 'SetApproval', 'SetVariation', 'SetOddsByFormat', 'SetTaxonomyConflict', 'SetTaxonomyAmbiguityQueue', 'CardVariantTaxonomyMap'];
  ...
  const manifest = JSON.parse(await readFile(join(repo, 'docs/plans/catalog-pilot-20260916/source-manifest.draft.json'), 'utf8'));
  const source = manifest.sources.find(s => s.id === 'pokemon-checklist'); assert.ok(source);
  const bytes = await readFile(source.local_evidence_path);
  assert.equal(bytes.length, 2227474); assert.equal(sha(bytes), 'd597ba707c20f1f8ec11e4809bb1f6e85bb0e502c05a93df41f36116a56dc923');
  const plan = JSON.parse(await readFile(join(repo, 'frontend/nextjs-app/lib/server/setCatalogPokemonPreparationPlan.json'), 'utf8'));
  assert.equal(plan.source.sha256, sha(bytes)); assert.equal(plan.source.byteSize, bytes.length); assert.equal(plan.cards.length, 138);
  const sourceArtifacts = [{ sourceRef: plan.source.ref, path: await realpath(source.local_evidence_path), byteSize: bytes.length, sha256: sha(bytes) }];
  ...
  return { schema_version: 1, scope: 'pokemon-additive-preparation-transaction-only', repo, node: { path: process.execPath, version: process.version, platform: platform(), arch: arch() },
    limits: { total_ms: capMs, cleanup_reserve_ms: cleanupReserveMs, disk_floor_bytes: diskFloor, launch_reserve_bytes: launchReserve, source_timeout_ms: 20_000, fixture_timeout_ms: 150_000 },
    sources, sourceArtifacts, generated_client: { root: generatedRoot, files: clientFiles, active_models: activeModels }, versions, native };
}
...
try {
  ...
  if (prepareOnly) {
    await artifact('preparation.json', { prepared_only: true, database_started: false, database_writes: 0, launch_seal_sha256: sha(sealBytes), functional_integration_pass: false, release_pass: false });
    ...
    temp = await mkdtemp(join(tmpdir(), 'tk-pokemon-preparation-'));
    ...
    await limited(() => admin.query('CREATE DATABASE tenkings_pokemon_preparation_disposable'), 15_000, 'Create owned database');
    await limited(() => admin.end(), 3000, 'Administrative disconnect'); clients.delete(admin);
    const databaseUrl = url('tenkings_pokemon_preparation_disposable'), nonce = randomBytes(24).toString('hex');
    const ownershipFile = join(temp, 'fixture-ownership.json');
    await writeFile(ownershipFile, JSON.stringify({ schema_version: 1, database_url_sha256: sha(new URL(databaseUrl).href), nonce, owner_uid: process.getuid(), postgres_pid: postgresChild.pid }), { mode: 0o600, flag: 'wx' });
    ...
    const schemaRoot = join(temp, 'schema'); await cp(join(repo, 'packages/database/prisma'), schemaRoot, { recursive: true });
    ...
    const observer = await connection('tenkings_pokemon_preparation_disposable');
    ...
    assert.equal(server.database, 'tenkings_pokemon_preparation_disposable'); assert.equal(server.host, '127.0.0.1'); assert.equal(server.encoding, 'UTF8');
    ...
    const fixtureEnv = { ...cleanEnv, DATABASE_URL: databaseUrl, TEN_KINGS_POKEMON_PREPARATION_DISPOSABLE_VALIDATION: '1',
      TEN_KINGS_POKEMON_PREPARATION_OWNERSHIP_FILE: ownershipFile, TEN_KINGS_POKEMON_PREPARATION_NONCE: nonce, SET_CATALOG_EVIDENCE_ENABLED: 'true',
      TSX_TSCONFIG_PATH: join(frontend, 'tsconfig.json') };
    await disk(diskFloor);
    fixtureStarted = true;
    const tap = await run(process.execPath, ['--import', pathToFileURL(requireFront.resolve('tsx')).href, '--test', '--test-reporter=tap', '--test-concurrency=1', join(repo, fixture)], { env: fixtureEnv, cwd: frontend, label: 'pokemon-preparation-functional', timeoutMs: 150_000 });
    for (const [name, count] of [['tests', 9], ['pass', 9], ['fail', 0], ['cancelled', 0], ['skipped', 0]]) assert.match(tap, new RegExp(`^# ${name} ${count}$`, 'm'), `The actual pokemon fixture must pass without skip: ${name}.`);
  ...
  try {
    ...
    if (!prepareOnly) await limited(() => artifact('summary.json', { complete: completed && cleanup.errors.length === 0, functional_integration_pass: completed && cleanup.errors.length === 0,
      release_pass: false, performance_pass: false, actual_pokemon_preparation_service_started: fixtureStarted, fixture_only_catalog_flag: true,
      real_provider_calls: 0, preserved_fixture_identity_counts_if_complete: completed ? { cards: 138, programs: 1, versions: 1, sources: 1, jobs: 1 } : null,
      limitations: ['Synthetic fixture row IDs and human session; real pinned local PDF and exact original 138-row transcription', 'No hosted request or actual authenticated API/UI execution', 'No production database, approval, grant or publication', 'No performance conclusion', 'All seventeen service table locks exercised; no shared-host load or live concurrency conclusion'], elapsed_ms: Date.now() - startedAt }), remaining(3000), 'Summary receipt', false);
