#!/usr/bin/env node
/** Default cron wiring qualification only. Never a performance or release gate. */
...
const fixture = 'frontend/nextjs-app/tests/staffInventoryResearchCronPostgres.test.ts';
const runner = 'packages/database/scripts/testStaffInventoryResearchCronDisposable.mjs';
...
async function seal() {
  ...
  return { schema_version: 1, scope: 'default-authenticated-cron-functional-only', repo, node: { path: process.execPath, version: process.version, platform: platform(), arch: arch() },
...
try {
  ...
  if (prepareOnly) {
    ...
    temp = await mkdtemp(join(tmpdir(), 'tk-research-cron-'));
    ...
    await limited(() => admin.query('CREATE DATABASE tenkings_research_cron_disposable'), 15_000, 'Create owned database');
    await limited(() => admin.end(), 3000, 'Administrative disconnect'); clients.delete(admin);
    const databaseUrl = url('tenkings_research_cron_disposable'), nonce = randomBytes(24).toString('hex');
    ...
    const observer = await connection('tenkings_research_cron_disposable');
    ...
    assert.equal(server.database, 'tenkings_research_cron_disposable'); assert.equal(server.host, '127.0.0.1'); assert.equal(server.encoding, 'UTF8');
    ...
    const fixtureEnv = { ...cleanEnv, DATABASE_URL: databaseUrl, TEN_KINGS_RESEARCH_CRON_DISPOSABLE_VALIDATION: '1',
      TEN_KINGS_RESEARCH_CRON_OWNERSHIP_FILE: ownershipFile, TEN_KINGS_RESEARCH_CRON_NONCE: nonce, CARD_STORAGE_MODE: 's3',
    ...
    await disk(diskFloor);
    fixtureStarted = true;
    const tap = await run(process.execPath, ['--import', pathToFileURL(requireFront.resolve('tsx')).href, '--test', '--test-reporter=tap', '--test-concurrency=1', join(repo, fixture)], { env: fixtureEnv, cwd: frontend, label: 'default-cron-functional', timeoutMs: 150_000 });
  ...
  try {
    ...
    if (!prepareOnly) await limited(() => artifact('summary.json', { complete: completed && cleanup.errors.length === 0, functional_integration_pass: completed && cleanup.errors.length === 0,
      release_pass: false, performance_pass: false, actual_default_cron_and_worker_started: fixtureStarted, optional_features: 'absent/default OFF',
      real_provider_calls: 0, real_saves_if_complete: completed ? 3 : null, result_completions_if_complete: completed ? 2 : null,
      limitations: ['Synthetic external transports and model responses', 'No hosted request, browser, camera, upload, recognition or simultaneous-device proof', 'No performance conclusion', 'Optional detail/catalog/full-resolution activation unqualified'], elapsed_ms: Date.now() - startedAt }), remaining(3000), 'Summary receipt', false);
