...
import { StaffInventoryResearchReferenceSchema, StaffInventoryResearchCatalogContextSchema } from './contract.mjs';
const fields = ['language', 'edition', 'format', 'channel'];
const sha = (value) => createHash('sha256').update(canonicalJson(value)).digest('hex');
const pinKey = (pin) => canonicalJson(pin);
...
export function retainResearchCatalogScope(previous, photos) {
    if (!previous || !photos.front || !photos.back || photos.front.sha256 === photos.back.sha256)
        return null;
    ...
    if (!parsed.success || !parsed.data.scope_receipt)
        return null;
    ...
    if (receipt.images.some(image => photos[image.side]?.sha256 !== image.transmitted_sha256)
        ...
        || evidence.some(item => photos[item.side]?.sha256 !== item.photo_sha256))
        return null;
    return { scope_evidence: evidence, scope_receipt: receipt };
}
export function researchCatalogQuery(description) {
    ...
    if (!family || !description.year || !description.set_name || !description.card_number || !description.name)
        return null;
...
export function catalogResearchReferences(lookups, target) {
    const references = [];
    for (const lookup of lookups) {
        if (lookup.authority !== 'host_authorized_setops_publication' || !lookup.publication || lookup.truncated
            ...
            || !Number.isSafeInteger(lookup.totalCandidateCount) || lookup.totalCandidateCount !== lookup.returnedCount)
            continue;
        ...
        const independentImage = (imageId) => {
            if (!target)
                return true;
            const seenSources = new Set(), seenImages = new Set();
            const sourceAllowed = (sourceId) => {
        ...
        for (const candidate of lookup.candidates) {
            if (candidate.applicability !== 'supported' || candidate.unresolvedScopeFields.length || fields.some(field => candidate.printing[field] === null))
                continue;
            ...
            for (const image of candidate.images.slice(0, 4)) {
                if (!independentImage(image.imageId))
                    continue;
...
export function createResearchCatalogAdapter(dependencies = {}) {
    if (!dependencies.host)
        throw new Error('A scoped catalog host adapter is required.');
    const host = async () => dependencies.host;
    async function load(description, photos, signal) {
        const context = { schema_version: 1, status: 'no_publication', publications: [], scope_evidence: [], scope_receipt: null };
        const previous = retainResearchCatalogScope(dependencies.previousScope, photos);
        if (previous) {
            context.scope_evidence = previous.scope_evidence;
            context.scope_receipt = previous.scope_receipt;
        }
        const query = researchCatalogQuery(description);
        if (!query || signal.aborted)
            return { context, references: [] };
        const api = await host(), pins = await api.findCurrentSetCatalogPublications({ query, consumer: dependencies.consumer ?? 'inventory' });
        if (pins.length > 8 || new Set(pins.map(pinKey)).size !== pins.length)
            throw new Error('Invalid catalog discovery.');
        if (!pins.length)
            return { context, references: [] };
        const read = (q) => Promise.all(pins.map(publication => api.lookupPublishedSetCatalogEvidence({ publication, query: q, consumer: dependencies.consumer ?? 'inventory' })));
        ...
        if (photos.front && photos.back && photos.front.sha256 !== photos.back.sha256
            ...
            if (discovery.every(lookup => !lookup.truncated && lookup.coverage.text.status !== 'truncated'
                && lookup.coverage.applicability.status !== 'truncated'))
                scopeChoices = discovery;
        }
        const choices = Object.fromEntries(fields.map(field => [field, [...new Set(scopeChoices.flatMap(l => l.candidates.map(c => c.printing[field])).filter((v) => v !== null && v !== 'not_applicable'))].slice(0, 32)]));
        if (photos.front && photos.back && photos.front.sha256 !== photos.back.sha256 && fields.some(f => choices[f].length)) {
            ...
            if (resolved) {
                ...
                if (evidence.length && !receipt || receipt?.images.some(image => photos[image.side]?.sha256 !== image.transmitted_sha256) || evidence.length > 4 || new Set(evidence.map(e => e.field)).size !== evidence.length || evidence.some(e => !choices[e.field].includes(e.value) || photos[e.side]?.sha256 !== e.photo_sha256))
                    throw new Error('Invalid observed catalog scope.');
                context.scope_evidence = evidence;
                context.scope_receipt = receipt;
                lookups = await read({ ...query, ...Object.fromEntries(evidence.map(e => [e.field, e.value])) });
            }
        }
        for (const [index, lookup] of lookups.entries()) {
            if (!lookup.publication || lookup.authority !== 'host_authorized_setops_publication')
                throw new Error('Missing catalog authority.');
            const { setApprovalId: _approval, reviewedAt: _reviewed, ...pin } = lookup.publication;
            if (pinKey(pin) !== pinKey(pins[index]))
        ...
        if (signal.aborted)
            throw new Error('Catalog lookup cancelled.');
    ...
    async function current(snapshot, signal) {
        ...
        const results = await Promise.all(snapshot.context.publications.map(entry => api.isSetCatalogPublicationCurrent(entry.publication, dependencies.consumer ?? 'inventory')));
        return !signal.aborted && results.every(Boolean);
    }
    async function image(reference, signal) {
        const binding = reference.catalog_binding;
        if (signal.aborted || !binding?.image_id || !reference.image)
            throw new Error('Unbound catalog image.');
        const verified = await (await host()).readPublishedSetCatalogImage({ publication: binding.publication, imageId: binding.image_id, consumer: dependencies.consumer ?? 'inventory' });
        if (signal.aborted || verified.sha256 !== reference.image.sha256 || verified.mimeType !== reference.image.content_type)
            throw new Error('Catalog image binding changed.');
