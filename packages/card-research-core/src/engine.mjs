...
import { z } from 'zod';
import { EBAY_SOLD_COMPS_V2_REQUEST_COUNT, parseEbaySoldCompsV2Candidate, normalizeEbaySoldCompsV2Text, } from '@tenkings/ebay-sold-comps-v2';
import { STAFF_INVENTORY_RESEARCH_MODEL, STAFF_INVENTORY_RESEARCH_ENGINE_VERSION, STAFF_INVENTORY_RESEARCH_LIMITS, STAFF_INVENTORY_RESEARCH_CATALOG_ENGINE_VERSION, StaffInventoryResearchCatalogContextSchema, StaffInventoryResearchScopeReceiptSchema, STAFF_INVENTORY_RESEARCH_PHOTO_ENGINE_VERSION, StaffInventoryResearchPhotoIdentitySchema, STAFF_INVENTORY_RESEARCH_ERROR_MESSAGES, StaffInventoryResearchInputSchema, StaffInventoryResearchResultSchema, StaffInventoryResearchDescriptionSchema, StaffInventoryResearchReferenceSchema, StaffInventoryResearchIdentitySchema, StaffInventoryResearchConditionSchema, StaffInventoryResearchComparisonSchema, isStaffInventoryResearchImageUrl, } from './contract.mjs';
import { researchImageOptions, researchSaleEvidence } from './evidence.mjs';
import { readResearchPriceEvidence } from './price.mjs';
import { resolveStaffInventoryResearchSaleDetails } from './sale-details.mjs';
import { assessStaffInventoryResearchComparison, compareStaffInventoryResearchCondition, inspectStaffInventoryResearchSale, inspectStaffInventoryResearchTitle, readStaffInventoryResearchGradeEvidence, staffInventoryResearchYearForms } from './decisions.mjs';
import { runCardCatalogScopeEffect } from './scope-effect.mjs';
const SOLDCOMPS_ENDPOINT = 'https://api.sold-comps.com/v1/scrape';
const OPENAI_ENDPOINT = 'https://api.openai.com/v1/responses';
const DEADLINES = { photos: 6000, sources: 20000, images: 6000, imageRound: 18000, model: 120000, refinement: 84000, overall: STAFF_INVENTORY_RESEARCH_LIMITS.overallTimeoutMs };
const IMAGE_BUDGETS = [6, 3, 3];
...
const safeText = (value, max) => typeof value === 'string' && value.trim().length > 0 && value.trim().length <= max && !unsafeText.test(value.trim()) ? value.trim() : null;
...
async function bounded(operation, milliseconds, parent) {
    if (parent?.aborted)
        throw new StaffInventoryResearchError('cancelled');
...
async function readResponseBytes(response, maximum, signal) {
    ...
    try {
        ...
        if (length !== null && (!encoding || encoding.toLowerCase() === 'identity') && size !== Number(length))
            throw new StaffInventoryResearchError('malformed_response');
...
async function request(url, init, deps, signal) {
    let response;
    try {
        response = await (deps.fetchImpl ?? fetch)(url, { ...init, redirect: 'error', cache: 'no-store', signal });
    }
    catch {
        throw new StaffInventoryResearchError(signal.aborted ? 'cancelled' : 'provider_error');
    }
...
async function jsonRequest(url, init, maximum, deps, signal) {
    ...
    const env = deps.env ?? {};
    if ([env.OPENAI_API_KEY, env.SOLDCOMPS_API_KEY].some(secret => secret && bytes.includes(Buffer.from(secret))))
        throw new StaffInventoryResearchError('malformed_response');
    try {
...
function wholeSpan(haystack, needle) { return needle.length > 0 && ` ${haystack} `.includes(` ${needle} `); }
function productIdentity(value, year, maker) {
    let product = normalized(value);
    for (const prefix of [normalized(year), normalized(maker)])
        if (prefix && product.startsWith(`${prefix} `))
            product = product.slice(prefix.length + 1);
    return product;
}
/** Exact explicit anchors; descriptive names may contain a complete catalog name as a token span. */
export function isExactStaffInventoryResearchReference(description, reference) {
    const identity = reference.identity;
    if (!description.name || !description.year || !description.set_name || !description.card_number || !identity.name || !identity.year || !identity.set_name || !identity.card_number)
        return false;
    if (description.category === 'Sports cards' && (!description.manufacturer || !identity.manufacturer))
        return false;
...
export function buildStaffInventoryResearchQuery(description) {
    if (!safeText(description.name, 160))
        return null;
    ...
    if (values.some(value => value !== null && !safeText(value, 400)))
        return null;
    ...
    for (const value of values) {
        ...
        for (const token of value.split(/\s+/)) {
            ...
            if (!identity || priorFields.has(identity))
                continue;
            tokens.push(token);
            seen.add(identity);
        }
...
const querySignature = (query) => [...new Set(normalized(query).split(' '))].sort().join(' ');
function yearQueryForms(year) { return staffInventoryResearchYearForms(year); }
...
export function validateRefinedStaffInventoryResearchQuery(description, query, references = [], condition) {
    if (safeText(query, 400) !== query || query.length < 3 || !/^[\p{L}\p{N}\s#&'’./-]+$/u.test(query) || /(?:^|\s)[-/]/.test(query))
        return null;
    ...
    if (!description.name || required.some(value => normalized(value).split(' ').filter(Boolean).some(token => !tokenSet.has(token))))
        return null;
    const years = yearQueryForms(description.year);
    if (years.length && !years.some(year => wholeSpan(normalized(query), normalized(year))))
        return null;
    if (description.card_number && !query.split(/\s+/).some(token => token.replace(/^#/, '').toLocaleLowerCase() === description.card_number.replace(/^#/, '').toLocaleLowerCase()))
        return null;
...
function parserInput(description, query) {
...
function titleMatches(description, candidate, reference) {
    return inspectStaffInventoryResearchTitle(description, candidate, reference).estimate_anchored;
}
function titleAnchorCoverage(description, candidate) {
...
function candidateSearchEvidence(candidate) {
    const { retrieved_at: _retrievedAt, source_response_sha256: _responseHash, image: _image, image_url: _selectedImage, source_eligible: _eligible, exclusion_reason: _reason, ordinary_sale_detail: _detail, sold_price: _priceSpelling, sold_price_cents: _derivedPrice, best_offer_accepted: _offerProjection, ...evidence } = candidate;
    return evidence;
}
...
function mayConfirmOrdinarySale(candidate, raw, description, state) {
...
function rememberSearchObservation(state, candidate, raw) {
    ...
    if (previous !== undefined && previous !== facts)
        state.conflicts.add(candidate.id);
    else
        state.facts.set(candidate.id, facts);
}
function applyOrdinarySaleDetail(state, candidate, description) {
    const original = state.originals.get(candidate), detail = state.completed.get(candidate.id.slice(5));
    if (!original || !detail || !mayConfirmOrdinarySale(candidate, original.raw, description, state))
        return;
    ...
    if (!mayConfirmOrdinarySale(candidate, original.raw, description, state))
        return;
    candidate.ordinary_sale_detail = decision.evidence;
    candidate.sold_price_cents = decision.sold_price_cents;
    candidate.source_eligible = true;
    candidate.exclusion_reason = null;
}
async function enrichOrdinarySales(state, candidates, description, apiKey, deps, signal, sourceEndsAt) {
    ...
    await Promise.all(candidates.map(async (candidate) => {
        ...
        if (!original || !mayConfirmOrdinarySale(candidate, original.raw, description, state))
            return;
        let pending = state.cache.get(id);
        if (!pending) {
            const remaining = Math.floor(sourceEndsAt - Date.now() - reserve);
            if (signal.aborted || remaining <= 0 || state.cache.size >= 2)
                return;
            const entry = { item_id: id, requested_at: now(), completed_at: now(),
            ...
            pending = bounded(async (detailSignal) => {
                const response = await jsonRequest(`https://api.sold-comps.com/v1/item/${id}?ebaySite=ebay.com`, { method: 'GET', headers: { Authorization: `Bearer ${apiKey}` } }, MAX_SOURCE_BYTES, deps, detailSignal);
                if (detailSignal.aborted)
                    throw new StaffInventoryResearchError('cancelled');
                const item = object(response.value), env = deps.env ?? {};
                if (!item || [env.OPENAI_API_KEY, env.SOLDCOMPS_API_KEY].some(secret => secret && JSON.stringify(item).includes(secret)))
                    throw new StaffInventoryResearchError('malformed_response');
                return { item, response_sha256: response.sha256, retrieved_at: now() };
            }, Math.min(6000, remaining), signal).then(receipt => {
                entry.status = 'observed';
                entry.response_sha256 = receipt.response_sha256;
                entry.error_code = null;
                entry.completed_at = receipt.retrieved_at;
                state.completed.set(id, receipt);
                return receipt;
            }).catch(error => {
                entry.completed_at = now();
                entry.error_code = error instanceof StaffInventoryResearchError
                    && ['unavailable', 'provider_error', 'malformed_response', 'timeout', 'cancelled'].includes(error.code)
                    ? error.code : 'malformed_response';
                state.completed.set(id, null);
                return null;
            });
        ...
        if (!signal.aborted)
            applyOrdinarySaleDetail(state, candidate, description);
    }));
}
async function fetchCandidates(description, query, apiKey, deps, signal, details) {
    const sourceEndsAt = details ? Date.now() + deadline(deps, 'sources') : 0;
    const params = new URLSearchParams({ keyword: query, ebaySite: 'ebay.com', count: String(deps.requestCount ?? EBAY_SOLD_COMPS_V2_REQUEST_COUNT), page: '1', sold: 'true', includeCompleteListing: 'true', exactMatch: 'true', hydrateBoa: deps.hydrateBoa === false ? 'false' : 'true' });
    ...
    if (!payload || typeof payload.keyword !== 'string' || payload.keyword.replace(/\s+/g, ' ').trim() !== query || payload.page !== 1 || !Number.isSafeInteger(payload.totalItems) || Number(payload.totalItems) < 0 || typeof payload.hasNextPage !== 'boolean' || !Array.isArray(payload.items))
        throw new StaffInventoryResearchError('malformed_response');
    const now = (deps.now ?? (() => new Date()))().toISOString(), candidates = new Map();
    for (const item of payload.items.slice(0, deps.requestCount ?? EBAY_SOLD_COMPS_V2_REQUEST_COUNT)) {
        const raw = object(item), parsed = parseEbaySoldCompsV2Candidate(item, parserInput(description, query));
        if (!raw || !parsed || !safeText(parsed.title, 500))
            continue;
        ...
        let reason = saleReason ?? (bestOffer === true && !price.accepted_offer ? 'The accepted Best Offer amount is not disclosed.' : bestOffer === null ? 'Best Offer status is unavailable.' : (price.accepted_offer?.currency ?? soldCurrency) !== 'USD' ? 'A verified USD sale price is unavailable.' : price.sold_price_cents === null ? 'The sold price is missing or unsafe.' : parsed.soldDate === null ? 'The sold date is missing or invalid.' : parsed.soldDate > now.slice(0, 10) ? 'The sold date is later than retrieval.' : unsupportedSale.test(`${parsed.title} ${parsed.condition ?? ''}`) ? 'The listing describes a lot or unsupported card product.' : raw.soldPriceMax != null || raw.currentPriceMax != null ? 'A multi-option price range does not establish the sold card and amount.' : null);
        const candidate = {
            ...
            ...researchImageOptions({ ...raw, thumbnailUrl: raw.thumbnailUrl ?? parsed.imageUrl }, (deps.env ?? {}).STAFF_INVENTORY_RESEARCH_FULL_RES_IMAGES === 'true'),
            image: null, source_eligible: reason === null, exclusion_reason: reason,
        };
        if (grade.status === 'graded') {
            candidate.grader = grade.grader;
            candidate.numeric_grade = grade.numeric_grade;
            candidate.raw = false;
        }
        if (!product.supported) {
            candidate.source_eligible = false;
            candidate.exclusion_reason = 'The listing does not establish one supported exact card sale.';
        }
        if (details)
            rememberSearchObservation(details, candidate, raw);
        ...
        if (prior) {
            ...
            if (details ? details.conflicts.has(candidate.id) || details.originals.get(prior)?.facts !== details.originals.get(candidate)?.facts
                ...
                prior.source_eligible = false;
                prior.exclusion_reason = reason;
            }
            else if (details && !prior.source_eligible && candidate.source_eligible)
    ...
    if (details) {
        for (const candidate of retained)
            if (details.conflicts.has(candidate.id)) {
                candidate.source_eligible = false;
                candidate.exclusion_reason = 'The provider returned conflicting evidence for the same listing.';
            }
...
async function verifyImage(bytes, contentType, expectedHash) {
    if (!Buffer.isBuffer(bytes) || bytes.length < 1 || bytes.length > MAX_IMAGE_BYTES || (expectedHash && sha256(bytes) !== expectedHash))
        throw new StaffInventoryResearchError('malformed_response');
    let metadata;
    try {
        metadata = await sharp(bytes, { limitInputPixels: 16_000_000, failOn: 'warning' }).metadata();
    }
    catch {
        throw new StaffInventoryResearchError('malformed_response');
    }
    const actual = metadata.format === 'jpeg' ? 'image/jpeg' : metadata.format === 'png' ? 'image/png' : metadata.format === 'webp' ? 'image/webp' : null;
    if (!actual || actual !== contentType || !metadata.width || !metadata.height || metadata.width > 8000 || metadata.height > 8000 || metadata.width * metadata.height > 16_000_000 || (metadata.pages ?? 1) !== 1)
        throw new StaffInventoryResearchError('malformed_response');
    return actual;
}
async function fetchImage(url, deps, signal, expectedHash) {
    if (!isStaffInventoryResearchImageUrl(url))
        throw new StaffInventoryResearchError('malformed_response');
...
async function mapFour(values, run) {
    let next = 0;
    await Promise.all(Array.from({ length: Math.min(4, values.length) }, async () => {
        while (next < values.length)
            await run(values[next++]);
    }));
}
const modelText = (max) => z.string().min(1).max(max).refine(value => safeText(value, max) === value);
...
function unresolvedPhotoIdentity() {
...
function photographedDescription(analysis, input, photos) {
    const evidence = analysis.photo_identity;
    if (!evidence)
        return input.description;
    if (evidence.observations.some(observation => photos[observation.side].sha256 !== observation.photo_sha256))
        throw new StaffInventoryResearchError('malformed_response');
    if (evidence.status !== 'supported')
        return input.description;
    const description = { ...input.description };
    const product = (value) => productIdentity(value, description.year, description.manufacturer)
    ...
    const cardNumber = (value) => value.replace(/^#\s*/, '').replace(/\s*([/-])\s*/g, '$1').toLowerCase();
    for (const observation of evidence.observations) {
        if (/^(?:unknown|unresolved|n\/?a|not visible|not supplied)$/i.test(observation.value))
            throw new StaffInventoryResearchError('malformed_response');
        if (observation.field === 'treatment') {
            // A saved treatment remains a constraint on private comparison evidence.
            if (input.description.variant !== null && !wholeSpan(normalized(`${observation.value} ${observation.observation}`), normalized(input.description.variant)))
                throw new StaffInventoryResearchError('malformed_response');
            continue;
        }
        ...
        if (!agrees)
            throw new StaffInventoryResearchError('malformed_response');
        if (saved === null || field === 'name')
            description[field] = observation.value;
    }
    if (input.description.manufacturer !== null && !evidence.observations.some(observation => observation.field === 'manufacturer'))
        throw new StaffInventoryResearchError('malformed_response');
...
function decodeResearchModelOutput(payload) {
    const response = object(payload);
    if (!response || response.model !== STAFF_INVENTORY_RESEARCH_MODEL || response.status !== 'completed' || response.error != null || response.incomplete_details != null || !Array.isArray(response.output))
        throw new StaffInventoryResearchError('malformed_response');
    const texts = [];
    for (const item of response.output) {
        ...
        if (row.type === 'reasoning')
            continue;
        if (row.type !== 'message' || row.role !== 'assistant' || row.status !== 'completed' || !Array.isArray(row.content))
            throw new StaffInventoryResearchError('malformed_response');
    ...
    if (texts.length !== 1 || texts[0].length > 64000)
        throw new StaffInventoryResearchError('malformed_response');
    let decoded;
    try {
...
export function parseStaffInventoryResearchOutput(payload) {
...
export function researchCatalogScopeResolver(deps = {}) {
    return async ({ choices, photos }, signal) => {
        const key = (deps.env ?? {}).OPENAI_API_KEY?.trim();
        if ((!key && !deps.catalogScopeEffects?.dispatch) || !photos.front || !photos.back || photos.front.sha256 === photos.back.sha256)
        ...
        return bounded(async (scopeSignal) => {
            const requestBody = JSON.stringify({
                ...
                input: [{ role: 'user', content: [{ type: 'input_text', text: `Permitted scope values: ${JSON.stringify(choices)}` },
                            ...['front', 'back'].flatMap(side => [{ type: 'input_text', text: `${side} transmitted image SHA-256 ${photos[side].sha256}` },
            ...
            const response = await runCardCatalogScopeEffect({ schemaVersion: 1, engineVersion: STAFF_INVENTORY_RESEARCH_CATALOG_ENGINE_VERSION, stage: 'printing_scope',
                ...
                requestBody, requestSha256: sha256(Buffer.from(requestBody)), images: ['front', 'back'].map(side => ({ side,
            ...
            if (response.httpStatus < 200 || response.httpStatus >= 300)
                throw new StaffInventoryResearchError('provider_error');
            if (!/^application\/json\b/i.test(response.contentType) || key && response.responseBytes.includes(Buffer.from(key)))
                throw new StaffInventoryResearchError('malformed_response');
            const receipt = StaffInventoryResearchScopeReceiptSchema.parse(response.receipt);
            let payload;
            try {
                payload = JSON.parse(response.responseBytes.toString('utf8'));
            }
            catch {
                throw new StaffInventoryResearchError('malformed_response');
            }
            const parsed = scopeSchema.parse(decodeResearchModelOutput(payload));
            if (new Set(parsed.evidence.map(e => e.field)).size !== parsed.evidence.length || parsed.evidence.some(e => !choices[e.field].includes(e.value) || photos[e.side]?.sha256 !== e.photo_sha256 || e.value === 'not_applicable'))
                throw new StaffInventoryResearchError('malformed_response');
            return { evidence: parsed.evidence, receipt };
        }, Math.min(15_000, deadline(deps, 'model')), signal);
    };
}
function modelRequest(input, photos, references, candidates, images, referenceImages, queries) {
    return {
        model: STAFF_INVENTORY_RESEARCH_MODEL, store: false, reasoning: { effort: 'medium' }, max_output_tokens: 6500,
        instructions: [
            ...(Object.values(photos).some(photo=>photo.sourceSha256&&photo.sourceSha256!==photo.sha256)?['The uploaded target images are research derivatives. Their hashes identify transmitted pixels; original hashes are retained lineage. Do not claim unchanged original bytes were inspected.']:[]),
            ...
            'photo_identity.observations contains at most six entries and at most one entry for each field: name, year, manufacturer, set_name, card_number, treatment. This rule also applies when status is unresolved. For each field, use the one original side with the strongest directly visible evidence and its exact SHA-256; never repeat a field to cite both sides. A supported identity must cite both originals through different fields and satisfy every required fact. If that is not possible, retain only truthful, unique partial observations with status unresolved; never invent an observation or duplicate a field to satisfy the two-side requirement.',
        ...
        input: [{ role: 'user', content: [
                    ...
                    ...['front', 'back'].flatMap(side => [
                        ...
                        { type: 'input_image', image_url: `data:${photos[side].mimeType};base64,${photos[side].bytes.toString('base64')}`, detail: 'high' },
                    ]),
...
function validateAnalysis(analysis, input, photos, references, candidates, referenceImages) {
    ...
    if (comparisons.size !== analysis.comparisons.length || comparisons.size !== candidates.length || [...comparisons.keys()].some(id => !candidateMap.has(id)) || new Set(analysis.selected_candidate_ids).size !== analysis.selected_candidate_ids.length || analysis.selected_candidate_ids.some(id => !candidateMap.has(id)))
        throw new StaffInventoryResearchError('malformed_response');
    const identity = analysis.identity;
    if (new Set(identity.reference_ids).size !== identity.reference_ids.length || identity.reference_ids.some(id => !referenceMap.has(id)))
        throw new StaffInventoryResearchError('malformed_response');
    if (identity.suggestion !== null && identity.suggestion !== input.description.variant && !references.some(reference => reference.variant_name === identity.suggestion))
        throw new StaffInventoryResearchError('malformed_response');
    for (const feature of identity.photo_features) {
        const reference = referenceMap.get(feature.reference_id);
        if (!reference || !identity.reference_ids.includes(feature.reference_id) || photos[feature.side].sha256 !== feature.photo_sha256)
            throw new StaffInventoryResearchError('malformed_response');
    ...
    else if (identity.variant_name !== null || analysis.selected_candidate_ids.length > 0 && !photoSupported)
        throw new StaffInventoryResearchError('malformed_response');
    if (analysis.target_condition.status === 'graded') {
        ...
        if (!namedGrader || conflictingBgsService || !exactNumericGrade)
            throw new StaffInventoryResearchError('malformed_response', 'UNVERIFIED_GRADING_LABEL');
    }
    ...
    for (const candidate of selected) {
        ...
        if (!candidate.source_eligible || !candidate.image || comparison.classification !== 'matched' || !comparison.identity_match || !comparison.variant_match || !comparison.visual_match || !comparison.condition_match || !titleMatches(description, candidate, catalogIdentity) || identity.status === 'unresolved' && !photoSupported || analysis.target_condition.status === 'unresolved')
            throw new StaffInventoryResearchError('malformed_response');
        if (!conditionMatches(analysis.target_condition, candidate))
            throw new StaffInventoryResearchError('malformed_response');
    }
...
function comparisonAssessment(candidate, description, condition, comparison) {
...
function updateEstimate(result, duplicateImageSelections) {
...
function applyAnalysis(result, analysis, input, photos, referenceImages, duplicateImageSelections) {
    ...
    if (analysis.selected_candidate_ids.some(id => assessments.find(assessment => assessment.candidate_id === id)?.classification !== 'matched'))
        throw new StaffInventoryResearchError('malformed_response');
    result.identity = analysis.identity;
    result.target_condition = analysis.target_condition;
    ...
    const images = new Set(), uniqueSelections = [];
    for (const id of [...analysis.selected_candidate_ids].sort()) {
        ...
        if (images.has(image))
            duplicateImageSelections.add(id);
        else {
            images.add(image);
            uniqueSelections.push(id);
        }
    }
    ...
    for (const rejection of result.rejections)
        if (insufficient.includes(rejection.candidate_id) && !duplicateImageSelections.has(rejection.candidate_id))
            rejection.reason = 'Fewer than two independent verified matching sales support an estimate.';
    return { insufficient, description };
}
function mergeCandidates(result, incoming, description, duplicateImageSelections, details) {
    ...
    const sourceEvidence = (candidate) => {
        if (details)
            return details.originals.get(candidate)?.facts;
        const { retrieved_at: _retrievedAt, source_response_sha256: _responseHash, image: _image, image_url: _selectedImage, source_eligible: _eligible, exclusion_reason: _reason, ...evidence } = candidate;
        return evidence;
    };
    for (const candidate of incoming) {
        ...
        if (prior) {
            if (details?.conflicts.has(candidate.id) || JSON.stringify(sourceEvidence(prior)) !== JSON.stringify(sourceEvidence(candidate)) || !candidate.source_eligible && candidate.exclusion_reason?.includes('conflicting')) {
                prior.source_eligible = false;
                prior.exclusion_reason = 'The provider returned conflicting evidence for the same listing.';
            }
            else if (details) {
                ...
                if (!prior.source_eligible && candidate.source_eligible) {
                    ...
                    if (prior.image && [candidate.image_options?.thumbnail_url, candidate.image_options?.full_resolution_url].includes(prior.image.source_url)) {
                        candidate.image = prior.image;
                        candidate.image_url = prior.image.source_url;
                    }
    ...
    const retainedPriority = (candidate) => result.selected_candidate_ids.includes(candidate.id) ? 4 : priorAssessments.get(candidate.id)?.classification === 'matched' ? 3 : candidate.image ? 2 : incomingIds.has(candidate.id) ? 1 : 0;
    ...
    if (result.selected_candidate_ids.length < STAFF_INVENTORY_RESEARCH_LIMITS.minimumComps)
...
export async function researchCardSubject(input, deps = {}, signal) {
    if (deps.requestCount !== undefined && (!Number.isInteger(deps.requestCount) || deps.requestCount < 1 || deps.requestCount > EBAY_SOLD_COMPS_V2_REQUEST_COUNT)) throw new StaffInventoryResearchError('invalid_input');
    const parsed = StaffInventoryResearchInputSchema.safeParse(input);
    if (!parsed.success)
        throw new StaffInventoryResearchError('invalid_input');
    let data = parsed.data;
    const env = deps.env ?? {}, started = Date.now();
    const details = env.STAFF_INVENTORY_RESEARCH_SALE_DETAILS === 'true'
    ...
    return bounded(async (innerSignal) => {
        const timings = { photos: 0, sources: 0, images: 0, model: 0, total: 0 }, warnings = [];
        const query = deps.initialQuery === undefined ? buildStaffInventoryResearchQuery(data.description) : validateRefinedStaffInventoryResearchQuery(data.description, deps.initialQuery);
        if (deps.initialQuery !== undefined && !query) throw new StaffInventoryResearchError('invalid_input');
        const result = {
            schema_version: 1, subject: data.subject,
            ...
            ...(details ? { sale_details: { schema_version: 1, base_engine_version: baseEngineVersion, requests: details.requests } } : {}),
            ...(deps.loadCatalog ? { catalog_context: { schema_version: 1, status: 'not_consulted', publications: [], scope_evidence: [], scope_receipt: null } } : {}),
        ...
        if (!soldKey || !modelKey)
            throw new StaffInventoryResearchError('unavailable');
        const photos = {};
        await Promise.all([
            (async () => {
                const start = Date.now();
                await Promise.all(['front', 'back'].map(async (side) => {
                    const descriptor = data.photos[side];
                    const key = descriptor?.ref;
                    if (!key)
                        return;
                    try {
                        photos[side] = await bounded(async (photoSignal) => {
                            if (!deps.loadPhoto)
                                throw new StaffInventoryResearchError('unavailable');
                            const photo = await deps.loadPhoto(descriptor, photoSignal);
                            if (photo.ref !== descriptor.ref || photo.sha256 !== descriptor.sha256 || photo.sourceSha256 !== descriptor.sourceSha256 || descriptor.byteCount !== null && photo.bytes.length !== descriptor.byteCount || photo.mimeType !== descriptor.mimeType || sha256(photo.bytes) !== photo.sha256)
                                throw new StaffInventoryResearchError('unverified_photo');
                            await verifyImage(photo.bytes, descriptor.mimeType, photo.sha256);
                            return photo;
                        }, deadline(deps, 'photos'), innerSignal);
                        result.photos[side] = { ...descriptor, byteCount: photos[side].bytes.length };
                    }
                    catch {
                        if (innerSignal.aborted)
                            throw new StaffInventoryResearchError('cancelled');
                        warnings.push(`The ${side} photo could not be verified; identity remains unresolved.`);
                    }
                }));
            ...
            (async () => {
                ...
                try {
                    ...
                    if (!Array.isArray(loaded) || loaded.length > STAFF_INVENTORY_RESEARCH_LIMITS.references)
                        throw new StaffInventoryResearchError('malformed_response');
                    const records = loaded.map(reference => StaffInventoryResearchReferenceSchema.parse(reference));
                    if (new Set(records.map(reference => reference.id)).size !== records.length || records.some(reference => !isExactStaffInventoryResearchReference(data.description, reference)))
                        throw new StaffInventoryResearchError('malformed_response');
                    ...
                    if (innerSignal.aborted)
                        throw new StaffInventoryResearchError('cancelled');
                    warnings.push('Published checklist or reference evidence could not be verified.');
                }
            })(),
        ]);
        let catalogSnapshot;
        if (deps.loadCatalog) {
            ...
            try {
                ...
                if (result.references.length > STAFF_INVENTORY_RESEARCH_LIMITS.references || result.references.some(reference => !reference.catalog_binding))
                    throw new StaffInventoryResearchError('malformed_response');
            }
            catch {
                if (innerSignal.aborted)
                    throw new StaffInventoryResearchError('cancelled');
                result.references = [];
                result.catalog_context.status = 'unavailable';
        ...
        if (!result.references.length)
            warnings.push('No exact published checklist evidence is available. Private photo-based comparisons remain possible; catalog identity is unresolved.');
        ...
        if (photos.front && photos.back && photos.front.sha256 === photos.back.sha256)
            warnings.push('Front and back contain the same image; distinct card views are needed.');
        const candidateImages = new Map(), referenceImages = new Map();
        const imageReads = new Map();
        const imageAttempts = new Set();
        const imageOutcomes = new Map();
        const modelAssessments = new Map();
        const comparisonFailures = new Set();
        let initialComparisonFailed = false;
        let insufficientSelection = [];
        let comparisonDescription = data.description;
        const duplicateImageSelections = new Set();
        const rememberAnalysis = (analysis) => {
            for (const comparison of analysis.comparisons)
                modelAssessments.set(comparison.candidate_id, {
                    comparison: { ...comparison }, imageHash: result.candidates.find(candidate => candidate.id === comparison.candidate_id)?.image?.sha256 ?? null,
                });
        };
        const warn = (warning) => { if (!warnings.includes(warning) && warnings.length < 12)
            warnings.push(warning); };
        if (hasPhotos) {
            const downloadEvidence = async (pass, parent) => {
                ...
                try {
                    await bounded(async (imageRoundSignal) => {
                        ...
                        const tasks = candidates.map(candidate => async () => {
                            ...
                            try {
                                const urls = [...new Set([candidate.image_url, candidate.image_options?.thumbnail_url].filter((url) => !!url))];
                                let image;
                                for (const url of urls) {
                                    if (imageRoundSignal.aborted)
                                        break;
                                    let pending = imageReads.get(url);
                                    if (!pending && imageReads.size >= STAFF_INVENTORY_RESEARCH_LIMITS.candidateImages)
                                        break;
                                    const outcomes = imageOutcomes.get(candidate.id) ?? [];
                                    imageOutcomes.set(candidate.id, outcomes);
                                    try {
                                        if (!pending) {
                                            pending = bounded(imageSignal => fetchImage(url, deps, imageSignal), deadline(deps, 'images'), imageRoundSignal);
                                            imageReads.set(url, pending);
                                        }
                                        image = await pending;
                                        if (imageRoundSignal.aborted)
                                            throw new StaffInventoryResearchError('cancelled');
                                ...
                                if (deps.archiveCandidateImage) {
                                    try {
                                        ...
                                        if (stored.storage_key !== `research-evidence/${image.sha256}.${extension}`)
                                            throw new StaffInventoryResearchError('malformed_response');
                                        ...
                                        if (imageRoundSignal.aborted)
                                            throw new StaffInventoryResearchError('cancelled');
                                        warn('Some comparable images could not be retained in private storage.');
                                    }
                                }
                                if (imageRoundSignal.aborted)
                                    throw new StaffInventoryResearchError('cancelled');
                                if (candidate.image)
                                    candidate.image.storage_key = storageKey;
                            }
                            catch {
                                if (imageRoundSignal.aborted)
                                    throw new StaffInventoryResearchError('cancelled');
                        ...
                        if (pass === 0)
                            for (const reference of result.references.filter(reference => reference.kind === 'reference' && reference.image).slice(0, STAFF_INVENTORY_RESEARCH_LIMITS.referenceImages))
                                tasks.push(async () => {
                                    try {
                                        const image = await bounded(async (imageSignal) => {
                                            if (!deps.loadReferenceImage) {
                                                if (!reference.image.source_url)
                                                    throw new StaffInventoryResearchError('unavailable');
                                        ...
                                        if (imageRoundSignal.aborted)
                                            throw new StaffInventoryResearchError('cancelled');
                                        warn('An approved reference image could not be verified.');
                                    }
                                });
                    ...
                    if (parent.aborted)
                        throw new StaffInventoryResearchError('cancelled');
                    warn('The image time limit was reached; comparisons use only images already verified.');
                }
                finally {
                    timings.images += Date.now() - imageStart;
                }
            };
            const assess = async (parent) => {
                ...
                try {
                    ...
                    if (remaining <= 0)
                        throw new StaffInventoryResearchError('timeout');
                    return await bounded(async (modelSignal) => {
                        const payload = (await jsonRequest(OPENAI_ENDPOINT, {
                            ...
                            body: JSON.stringify(modelRequest(data, photos, result.references, result.candidates, candidateImages, referenceImages, result.research_queries)),
            ...
            try {
                ...
                const applied = applyAnalysis(result, firstAnalysis, data, photos, referenceImages, duplicateImageSelections);
                insufficientSelection = applied.insufficient;
                comparisonDescription = applied.description;
                ...
                for (const candidate of result.candidates)
                    if (candidate.image)
                        comparisonFailures.add(candidate.id);
            ...
            for (let pass = 1; pass < STAFF_INVENTORY_RESEARCH_LIMITS.searches && pendingRefinement && result.estimate.status !== 'estimated'; pass++) {
                ...
                if (remaining < reserve) {
                    warn('Further refinement was skipped to preserve the completed research within the time limit.');
                    break;
                }
                ...
                try {
                    await bounded(async (refinementSignal) => {
                        ...
                        try {
                            ...
                            timings.sources += Date.now() - sourceStart;
                        }
                        ...
                        const applied = applyAnalysis(result, nextAnalysis, data, photos, referenceImages, duplicateImageSelections);
                        insufficientSelection = applied.insufficient;
                        comparisonDescription = applied.description;
                    ...
                    if (sourceCompleted)
                        for (const candidate of result.candidates) {
                            if (candidate.image && modelAssessments.get(candidate.id)?.imageHash !== candidate.image.sha256)
                                comparisonFailures.add(candidate.id);
                        }
                    if (!sourceCompleted)
                        result.research_queries.push({ sequence: pass + 1, query: nextQuery, reason: refinement.reason, status: 'failed', source_response_sha256: null, candidate_ids: [], error_code: error instanceof StaffInventoryResearchError ? error.code : 'provider_error' });
                    ...
                    if (!sourceCompleted || !pendingRefinement)
                        break;
                }
            }
        }
        ...
        result.comparison_assessments = result.candidates.map(candidate => {
            ...
            if (candidate.sale_evidence?.status !== 'sold')
                codes.push(candidate.sale_evidence?.status === 'active' ? 'ACTIVE_LISTING' : 'SOLD_EVENT_UNVERIFIED');
            if (!candidate.source_eligible)
                codes.push('SALE_PRICE_INELIGIBLE');
            if (comparisonFailures.has(candidate.id))
                codes.push(initialComparisonFailed ? 'INITIAL_COMPARISON_FAILED' : 'OPTIONAL_COMPARISON_FAILED');
        ...
        if (!result.references.length)
            result.diagnostics.reason_codes.push('CATALOG_COVERAGE_MISSING');
        if (!hasPhotos)
            result.diagnostics.reason_codes.push('PHOTO_PAIR_UNVERIFIED');
        if (result.identity.status === 'unresolved')
            result.diagnostics.reason_codes.push('IDENTITY_UNRESOLVED');
        if (result.photo_identity?.status !== 'supported')
            result.diagnostics.reason_codes.push('PHOTO_IDENTITY_UNRESOLVED');
        if (result.candidates.length === 0)
            result.diagnostics.reason_codes.push('NO_SOURCE_CANDIDATES');
        if (comparisonFailures.size && !initialComparisonFailed)
            result.diagnostics.reason_codes.push('OPTIONAL_COMPARISON_FAILED');
        if (catalogSnapshot && result.catalog_context?.status === 'current') {
            let current = false;
            try {
                current = Boolean(deps.isCatalogCurrent && await bounded(checkSignal => deps.isCatalogCurrent(catalogSnapshot, checkSignal), deadline(deps, 'photos'), innerSignal));
            }
            catch { /* Authority cannot survive an unavailable final check. */ }
            if (!current) {
                result.catalog_context.status = 'unavailable';
                result.references = [];
                result.selected_candidate_ids = [];
        ...
        if (insufficientSelection.length && !result.selected_candidate_ids.length) {
            ...
            for (const rejection of result.rejections)
                if (insufficientSelection.includes(rejection.candidate_id) && !duplicateImageSelections.has(rejection.candidate_id)
        ...
        if (error instanceof StaffInventoryResearchError)
            throw error;
        throw new StaffInventoryResearchError('provider_error');
    });
}
