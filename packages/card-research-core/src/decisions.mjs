...
const span = (value, expected) => expected.length > 0 && ` ${value} `.includes(` ${expected} `);
const unique = (values) => [...new Set(values)];
const escape = (value) => value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
const dashes = (value) => value.replace(/[\u2010-\u2015\u2212]/g, '-');
const numberIdentity = (value) => dashes(value).replace(/^#\s*/, '').replace(/\s*([/-])\s*/g, '$1').trim().toLowerCase();
...
function gradeMatches(value) { return [...gradeText(value).matchAll(new RegExp(gradePattern))]; }
export function readStaffInventoryResearchGradeEvidence(title, condition) {
    const text = gradeText(`${title} ${condition ?? ''}`), matches = gradeMatches(text);
    const unresolved = () => ({ status: 'unresolved', grader: null, numeric_grade: null, reason_codes: ['listing_grade_unresolved'] });
    if (!graderMarker.test(text))
        return { status: 'raw', grader: null, numeric_grade: null, reason_codes: ['listing_grade_unmarked'] };
    if (!matches.length)
        return unresolved();
    ...
    if (markers.length !== matches.length || matches.some(match => {
        ...
        return unresolved();
    const grades = matches.map(match => ({ grader: match[1].toUpperCase(), numeric_grade: Number(`${match[2]}${match[3] ? `.${match[3]}` : ''}`) }));
    if (grades.some(grade => grade.numeric_grade > 10 || grade.grader !== grades[0].grader || grade.numeric_grade !== grades[0].numeric_grade))
        return unresolved();
...
export function staffInventoryResearchYearForms(year) {
    if (!year)
        return [];
    ...
    if (season) {
        ...
        if (season[2].length === 2 && end < start)
            end += 100;
        if (end === start + 1)
            forms.push(String(start), String(end), `${start}-${end}`, `${start}-${String(end).slice(-2)}`, `${start}/${end}`, `${start}/${String(end).slice(-2)}`);
    }
    return unique(forms);
}
function yearEvidence(description, title) {
    ...
    const evidence = { status: 'missing', matched: [], design: [], conflicting: [], ambiguous: [] };
    if (!forms.length)
        return evidence;
    ...
    for (const match of years.filter(match => !forms.includes(normalized(match[0])))) {
        ...
        if (/^\d{4}$/.test(match[0]) && matched.length && (explicitDesign && releaseYears.some(year => year > Number(match[0])) || anniversary && releaseYears.some(year => year - Number(match[0]) === Number(anniversary[1])))) {
            ...
            if (!explicitDesign && !anniversary && (!matched.length || beginsListing || precedesMaker))
                evidence.conflicting.push(match[0]);
            else
                evidence.ambiguous.push(match[0]);
        }
    }
...
function productAnchor(description) {
    ...
    for (const prefix of [normalized(description.year), normalized(description.manufacturer)]) {
        if (prefix && (product === prefix || product.startsWith(`${prefix} `)))
            product = product.slice(prefix.length).trim();
    }
    ...
    if (description.category === 'Sports cards')
        product = product.replace(new RegExp(`(?:^|\\s)(?:${sportWords.join('|')})$`), '').trim();
    return { product, original, removed: original !== product };
}
function cardNumberEvidence(description, title) {
    const expected = description.card_number && numberIdentity(description.card_number), text = gradeText(title);
    if (!expected)
        return { matched: false, conflicting: [] };
    const isCert = (index) => /\b(?:cert|certificate|certification)(?:\s+(?:number|no\.?))?\s*$/i.test(text.slice(Math.max(0, index - 30), index));
...
export function inspectStaffInventoryResearchSale(candidate) {
    const title = dashes(`${candidate.title} ${candidate.condition ?? ''}`), reasons = [];
    if (unsupportedProduct.test(title))
        reasons.push('unsupported_card_product');
    if (choiceListing.test(title.replace(/[-/]/g, ' ')))
        reasons.push('unsupported_choice_listing');
    if (titlePriceRange.test(title) || sourcePriceRange.test(dashes(candidate.sold_price ?? '')))
        reasons.push('unsupported_price_range');
    return { supported: reasons.length === 0, reason_codes: reasons };
}
function variantTitleConflict(description, title) {
    ...
    if (/^(?:base|base card|regular|standard)$/.test(expected) && phrases.length)
        return true;
    return !!expectedPhrase && phrases.some(match => match[0] !== expectedPhrase[0]);
}
function productTitleConflict(description, product, title) {
...
export function inspectStaffInventoryResearchTitle(description, candidate, reference) {
    const title = normalized(candidate.title), reasons = [];
    ...
    const hasAllTokens = (value) => value.length > 0 && value.split(' ').every(token => titleTokens.has(token));
    ...
    if (!nameAnchored)
        reasons.push('name_anchor_missing');
    if (!productAnchored)
        reasons.push('product_anchor_missing');
    if (!manufacturerAnchored)
        reasons.push('manufacturer_anchor_missing');
    if (product.removed && productAnchored && !span(title, product.original))
        reasons.push('descriptive_suffix_omitted');
    if (sportConflict)
        reasons.push('sport_conflict');
    if (productConflict)
        reasons.push('product_title_conflict');
    if (variantConflict)
        reasons.push('variant_title_conflict');
    if (year.status === 'conflicting')
        reasons.push('release_year_conflict');
    if (year.status === 'ambiguous')
        reasons.push('release_year_ambiguous');
    if (year.status === 'missing')
        reasons.push('release_year_missing');
    if (year.design.length)
        reasons.push('design_year_supported');
    if (year.matched.length && !year.matched.some(value => normalized(value) === normalized(description.year)))
        reasons.push('season_alias_supported');
    if (number.conflicting.length)
        reasons.push('card_number_conflict');
    if (!number.matched)
        reasons.push('card_number_missing');
    ...
    const referenceCompatible = !reference || (span(normalized(description.name), normalized(reference.identity.name)) &&
    ...
    if (!referenceCompatible)
        reasons.push('reference_anchor_conflict');
...
export function compareStaffInventoryResearchCondition(condition, grade) {
    if (condition.status === 'unresolved' || grade.status === 'unresolved')
        return 'unresolved';
    if (condition.status === 'raw')
        return grade.status === 'raw' ? 'matched' : 'conflicting';
    return grade.status === 'graded' && grade.grader === condition.grader && grade.numeric_grade === condition.numeric_grade ? 'matched' : 'conflicting';
}
export function assessStaffInventoryResearchComparison(candidate, description, condition, comparison, options = {}) {
    const assessment = comparison ? { ...comparison } : {
    ...
    const finish = (changes = {}, extra = []) => ({
        assessment: { ...assessment, ...changes }, reason_codes: unique([...reasons, ...extra]), evidence: { title, grade, condition: conditionEvidence, image },
    });
    if (!sale.supported)
        return finish({ classification: 'rejected', identity_match: false, reason: sale.reason_codes.includes('unsupported_choice_listing') ? 'The listing offers a choice of cards and does not establish one exact sold card.' : sale.reason_codes.includes('unsupported_price_range') ? 'The listing reports a price range rather than one exact card sale.' : 'The listing describes a lot or unsupported card product.' });
    if (title.reason_codes.includes('release_year_conflict'))
        return finish({ classification: 'rejected', identity_match: false, reason: 'The listing identifies a release year outside the saved card year or season.' });
    if (title.reason_codes.includes('card_number_conflict'))
        return finish({ classification: 'rejected', identity_match: false, reason: 'The listing identifies a different card number.' });
    if (title.reason_codes.includes('sport_conflict'))
        return finish({ classification: 'rejected', identity_match: false, reason: 'The listing identifies a different sport from the saved product.' });
    if (title.reason_codes.includes('product_title_conflict'))
        return finish({ classification: 'rejected', identity_match: false, reason: 'The listing explicitly names a different product or product edition.' });
    if (title.reason_codes.includes('variant_title_conflict'))
        return finish({ classification: 'rejected', variant_match: false, reason: 'The listing explicitly names a different parallel from the saved variant.' });
    if (conditionEvidence === 'conflicting')
        return finish({ classification: 'rejected', condition_match: false, reason: 'The listing raw/graded condition or exact grader and grade differs from the uploaded card.' }, ['condition_mismatch']);
    if (conditionEvidence === 'unresolved')
        return finish({ classification: 'possible', condition_match: false, reason: condition.status === 'unresolved' ? 'The uploaded card condition remains unresolved.' : 'The listing mentions grading but does not establish one unambiguous grader and numeric grade.' }, condition.status === 'unresolved' ? ['target_condition_unresolved'] : []);
    if (image === 'missing')
        return finish({ classification: 'possible', visual_match: false, reason: 'The listing image is unavailable for a verified visual comparison.' }, ['listing_image_missing']);
    if (image === 'acquired_unassessed')
        return finish({ classification: 'possible', visual_match: false, reason: 'The listing image was acquired but has not received a complete visual comparison.' }, ['comparison_not_completed']);
    if (!title.research_anchored && assessment.classification !== 'rejected')
        return finish({ classification: 'possible', reason: title.reason_codes.includes('release_year_ambiguous') ? 'An additional title year is ambiguous; it is not established as the release year or a design year.' : 'The listing title does not establish the saved name and product identity; visual evidence remains tentative.' });
    const modelReasons = [];
    if (!assessment.identity_match)
        modelReasons.push('model_identity_mismatch');
    if (!assessment.variant_match)
        modelReasons.push('model_variant_mismatch');
    if (!assessment.visual_match)
        modelReasons.push('model_visual_mismatch');
    if (!assessment.condition_match)
        modelReasons.push('model_condition_mismatch');
    if (assessment.classification === 'matched' && modelReasons.length)
        return finish({ classification: 'possible' }, modelReasons);
    if (assessment.classification === 'rejected' && !modelReasons.length)
        return finish({ classification: 'possible' }, ['model_decision_inconsistent']);
