...
import { resolveStaffInventoryResearchSaleDetails } from '../src/sale-details.mjs';
...
function input(search = sports, detail = sportsDetail) {
...
function denied(value, reason) {
    ...
    assert.equal(result.sold_price_cents, null);
    assert.equal(result.evidence, null);
    if (reason && result.status === 'unconfirmed')
        assert.equal(result.reason, reason);
}
test('the two observed ordinary-sale shapes confirm equal integer cents and preserve exact source spellings', () => {
    for (const [search, detail, expected] of [[sports, sportsDetail, 8900], [pokemon, pokemonDetail, 108]]) {
        const original = input(search, detail), result = resolveStaffInventoryResearchSaleDetails(original);
        assert.equal(result.status, 'confirmed');
        assert.equal(result.sold_price_cents, expected);
        if (result.status !== 'confirmed')
            assert.fail('ordinary evidence was not confirmed');
        ...
        assert.equal(Object.hasOwn(original.search.item, 'bestOfferAccepted'), false);
    }
});
...
test('the observed active anomaly and even a sold-looking active detail cannot establish sold authority', () => {
    ...
    for (const listingType of [undefined, null, 'unknown', 'Sold', false])
        denied(input({ ...sports, listingType }, sportsDetail), 'SOLD_STATUS_UNCONFIRMED');
...
test('only an absent or explicitly null search flag can be filled by an explicit detail false', () => {
    ...
    assert.equal(result.status, 'confirmed');
    if (result.status === 'confirmed')
        assert.equal(result.evidence.search.offer_field, 'null');
    denied(input({ ...sports, bestOfferAccepted: false }, sportsDetail), 'SEARCH_OFFER_FLAG_NOT_MISSING');
    for (const bestOfferAccepted of ['false', 0, {}, undefined])
        denied(input({ ...sports, bestOfferAccepted }, sportsDetail), 'MALFORMED_OFFER_FLAG');
    for (const bestOfferAccepted of [undefined, null, 'false', 0])
        denied(input(sports, { ...sportsDetail, bestOfferAccepted }), 'OFFER_STATUS_UNCONFIRMED');
    const withoutFlag = { ...sportsDetail };
    delete withoutFlag.bestOfferAccepted;
...
test('a matching item ID cannot erase title, currency or amount contradictions', () => {
    ...
    for (const title of ['', sports.title + '\n', '<b>sale</b>', 'x'.repeat(501)])
        denied(input(sports, { ...sportsDetail, title }), 'INVALID_TITLE');
...
test('relative, yearless, impossible or conflicting dates remain unconfirmed', () => {
    ...
    for (const ended of [false, null, 'true'])
        denied(input(sports, { ...sportsDetail, ended }), 'ENDING_UNCONFIRMED');
    ...
    const future = input();
    future.detail.retrieved_at = '2026-09-08T11:00:00.000Z';
    denied(future, 'DATE_MISMATCH');
});
...
test('calendar agreement uses the explicit source timezone without requiring the UTC date to match', () => {
    ...
    assert.equal(result.status, 'confirmed');
    if (result.status === 'confirmed')
        assert.equal(result.evidence.detail.sold_date, '2026-09-08');
});
test('malformed observations, stripped invalid fields and invalid receipts fail closed', () => {
    for (const value of [null, [], {}, { ...input(), search: null }, { ...input(), detail: { ...input().detail, item: [] } }])
        denied(value, 'MALFORMED_OBSERVATION');
    for (const invalid_fields of [['bestOfferAccepted'], 'none', null])
        denied(input({ ...sports, invalid_fields }, sportsDetail), 'MALFORMED_OBSERVATION');
    denied(input(sports, { ...sportsDetail, invalid_fields: ['price'] }), 'MALFORMED_OBSERVATION');
    for (const field of ['search', 'detail']) {
        const missingHash = input();
        missingHash[field].response_sha256 = '';
        denied(missingHash, 'INVALID_SOURCE_RECEIPT');
        const malformedHash = input();
        malformedHash[field].response_sha256 += '\n';
        denied(malformedHash, 'INVALID_SOURCE_RECEIPT');
        const badTime = input();
        badTime[field].retrieved_at = '2026-02-30T00:00:00.000Z';
        denied(badTime, 'INVALID_SOURCE_RECEIPT');
    }
});
test('repeated resolution is pure, preserves input hashes/observations, and does not return mutable input aliases', () => {
    ...
    for (const receipt of [original.search, original.detail]) {
        Object.freeze(receipt.item);
    ...
    if (first.status === 'confirmed')
        first.evidence.search.title = 'Changed returned evidence';
    assert.equal(JSON.stringify(original), before);
