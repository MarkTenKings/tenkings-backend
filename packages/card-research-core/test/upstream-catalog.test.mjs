...
import { catalogResearchReferences, createResearchCatalogAdapter, researchCatalogQuery } from '../src/catalog.mjs';
...
function scopeReceipt(photos) {
    return { attempt_id: 'fixture:attempt', invocation_id: 'fixture:scope', receipt_ref: 'fixture:receipt', request_sha256: 'd'.repeat(64), response_sha256: 'e'.repeat(64), http_status: 200, acknowledgement: 'adapter',
        images: ['front', 'back'].map(side => ({ side, mime_type: 'image/jpeg', transmitted_sha256: photos[side].sha256, source_sha256: null })) };
}
function harness(category = 'SPORTS') {
    ...
    const queries = [];
    let current = true;
    const photos = { front: { key: 'fixture:front', sha256: 'a'.repeat(64), bytes: Buffer.from('front') }, back: { key: 'fixture:back', sha256: 'b'.repeat(64), bytes: Buffer.from('back') } };
    const description = { category: category === 'SPORTS' ? 'Sports cards' : 'Pokémon', name: manifest.cards[0].name,
...
test('sports and Pokémon use exact category/product/number queries and do not infer printing scope', () => {
    for (const family of ['SPORTS', 'POKEMON']) {
        const f = harness(family), q = researchCatalogQuery(f.description);
        assert.equal(q.category, family);
        assert.equal(q.cardNumber, f.manifest.cards[0].number);
        assert.equal(q.language, undefined);
        assert.equal(q.printingLabel, undefined);
        assert.equal(q.edition, undefined);
        assert.equal(q.channel, undefined);
...
test('a target card photo, crop ancestor, or physical-card origin cannot become its own visual reference', async () => {
    for (const family of ['SPORTS', 'POKEMON']) {
        ...
        const crop = JSON.parse(JSON.stringify(lookup));
...
test('actual pure reader yields no authority without explicit scope, even when applicability was recorded supported', async () => {
    ...
    assert.equal(snapshot.context.status, 'current');
...
test('observed scope binds exact reviewed publication and representative image without claiming the depicted card is this card', async () => {
    for (const family of ['SPORTS', 'POKEMON']) {
        ...
        assert.equal(image.image?.source_url, null);
        assert.equal(image.image?.storage_key, null);
        assert.equal(image.catalog_binding?.card_id, f.manifest.cards[0].cardId);
        ...
        assert.equal(await adapter.current(snapshot, f.signal), true);
        f.revoke();
        assert.equal(await adapter.current(snapshot, f.signal), false);
    }
});
test('reviewed representative association retains different depicted printing and all diagnostic IDs while exposing only target features', async () => {
    for (const family of ['SPORTS', 'POKEMON']) {
        ...
        const depictedApplicability = manifest.applicability.find((a) => a.cardId === manifest.cards[1].cardId && a.printingId === depicted.printingId);
        depictedApplicability.status = 'supported';
        depictedApplicability.sourceIds = [manifest.sources[0].sourceId];
        ...
        for (const targetVisible of [true, false]) {
            if (!targetVisible)
                manifest.images[0].visibleDiagnosticIds = [depicted.diagnostics[0].id];
...
test('invented scope values, wrong photo hashes and duplicate field assertions fail before a scoped lookup', async () => {
    ...
    for (const evidence of [
        [{ field: 'language', value: 'invented', side: 'front', photo_sha256: f.photos.front.sha256, observation: 'Fixture' }],
        [{ field: 'language', value: 'en', side: 'front', photo_sha256: 'c'.repeat(64), observation: 'Fixture' }],
        [1, 2].map(() => ({ field: 'language', value: 'en', side: 'front', photo_sha256: f.photos.front.sha256, observation: 'Fixture' })),
    ])
...
test('a language-scoped reviewed number alias discovers bounded positive scope before retrying the original exact query', async () => {
    ...
    const alias = f.manifest.aliases.find((entry) => entry.kind === 'card_number').value;
    let calls = 0;
    const adapter = createResearchCatalogAdapter({ host: f.host, resolveScope: async ({ choices }) => {
            calls++;
            assert.deepEqual(choices.language, ['fr']);
    ...
    assert.equal(f.queries[0].cardNumber, alias);
    assert.equal(f.queries[1].cardNumber, undefined);
    assert.equal(f.queries[1].cardName, undefined);
    assert.equal(f.queries[1].setLabel, f.description.set_name);
    assert.equal(f.queries[2].cardNumber, alias);
    assert.equal(f.queries[2].language, 'fr');
...
test('successful exact-photo scope receipts are reused, including empty observations, without another paid resolver call', async () => {
    for (const empty of [false, true]) {
        ...
        const previousScope = { scope_receipt: scopeReceipt(f.photos), scope_evidence: empty ? [] : [
                { field: 'language', value: 'fr', side: 'front', photo_sha256: f.photos.front.sha256, observation: 'Synthetic visible French text.' },
                { field: 'edition', value: 'standard', side: 'back', photo_sha256: f.photos.back.sha256, observation: 'Synthetic printed edition.' },
            ] };
...
test('foreign cached photos and truncated broader discovery cannot authorize a hidden paid scope retry', async () => {
    ...
    assert.equal(snapshot.references.length, 0);
    assert.equal(snapshot.context.scope_receipt, null);
    const read = f.host.lookupPublishedSetCatalogEvidence;
    const truncatedHost = { ...f.host, lookupPublishedSetCatalogEvidence: async (args) => {
            const lookup = await read(args);
            return args.query.cardNumber ? lookup : { ...lookup, truncated: true };
        } };
    const alias = f.manifest.aliases.find((entry) => entry.kind === 'card_number').value;
...
test('a temporary absence of publication retains only exact-photo scope for a later cheap recheck', async () => {
    ...
    const previousScope = { scope_receipt: scopeReceipt(f.photos), scope_evidence: [
            { field: 'language', value: 'fr', side: 'front', photo_sha256: f.photos.front.sha256, observation: 'Synthetic printed language.' },
            { field: 'edition', value: 'standard', side: 'back', photo_sha256: f.photos.back.sha256, observation: 'Synthetic visible edition.' },
        ] };
    ...
    assert.deepEqual(absent.context.publications, []);
    assert.equal(absent.context.status, 'no_publication');
    ...
    assert.ok(restored.references.length > 0);
    assert.deepEqual(restored.context.scope_receipt, previousScope.scope_receipt);
});
test('unreviewed, truncated and excluded candidate rows cannot become catalog references', async () => {
    ...
    const excluded = { ...lookup, candidates: lookup.candidates.filter(c => c.applicability === 'excluded') };
...
test('truncated coverage and inconsistent counts provide no references while supported partial coverage remains useful', async () => {
    ...
    const lookup = await createPublishedCatalogReader({ loadAuthorizedPublication: async () => f.stored.loaded }).lookup({ publication: f.stored.pin,
        query: { category: 'SPORTS', setId: f.manifest.set.setId, cardId: f.manifest.cards[0].cardId, language: 'en', edition: 'standard' } });
    assert.equal(lookup.coverage.text.status, 'partial');
    assert.equal(lookup.coverage.applicability.status, 'partial');
    assert.ok(catalogResearchReferences([lookup]).length > 0, 'positive supported evidence survives normal partial coverage');
    for (const field of ['text', 'applicability']) {
        const truncated = { ...lookup, coverage: { ...lookup.coverage, [field]: { ...lookup.coverage[field], status: 'truncated' } } };
