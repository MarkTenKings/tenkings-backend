...
import { retainedEddyBeckettAnalysis, retainedEddyCorrectedAnalysis1, retainedEddyCorrectedAnalysis2 } from './fixtures/staffInventoryResearchBeckett.mjs';
import { retainedSnivyAnalysis1, retainedSnivyAnalysis2 } from './fixtures/staffInventoryResearchSnivy.mjs';
import { canonical, inventoryHash } from './test-hash.mjs';
import { StaffInventoryResearchInputSchema, StaffInventoryResearchResultSchema, } from '../src/inventory.mjs';
import { researchStaffInventoryCard, researchCatalogScopeResolver, StaffInventoryResearchError, parseStaffInventoryResearchOutput, isExactStaffInventoryResearchReference, buildStaffInventoryResearchQuery, validateRefinedStaffInventoryResearchQuery, } from '../src/inventory.mjs';
const NOW = '2026-09-11T10:00:00.000Z';
const sha = (bytes) => createHash('sha256').update(bytes).digest('hex');
const key = (bytes) => `inventory-photos/11111111-1111-4111-8111-111111111111/${sha(bytes)}.jpg`;
const output = (data) => ({ model: 'gpt-6-astra', status: 'completed', error: null, incomplete_details: null, output: [
        { type: 'reasoning', summary: [] }, { type: 'message', role: 'assistant', status: 'completed', content: [{ type: 'output_text', text: JSON.stringify(data) }] },
    ] });
const code = (expected) => (error) => error instanceof StaffInventoryResearchError && error.code === expected;
async function fixture() {
    ...
    const deps = {
        ...
        fetchImpl: (async (url, init) => {
            ...
            calls.push({ url: uri, init: init ?? {}, body: init?.body ? JSON.parse(String(init.body)) : null });
            if (uri.startsWith('https://api.sold-comps.com/v1/scrape?'))
                return Response.json({ keyword: new URL(uri).searchParams.get('keyword'), page: 1, totalItems: items.length, hasNextPage: false, items });
            if (uri === 'https://api.openai.com/v1/responses')
                return Response.json(output(modelValue));
    ...
    return { input, reference, items, bytes, deps, calls, get model() { return modelValue; }, set model(value) { modelValue = value; } };
}
async function assertAssessmentFailed(f) {
    const result = await researchStaffInventoryCard(f.input, f.deps);
    assert.equal(result.estimate.status, 'unknown');
    assert.deepEqual(result.selected_candidate_ids, []);
...
test('private research selects only exact supplied sales and calculates source cents without model prices', async () => {
    ...
    assert.equal(result.estimate.low_cents, 1001);
    assert.equal(result.estimate.high_cents, 1002);
    assert.equal(result.estimate.count, 2);
    ...
    assert.equal(source.init.method, 'GET');
    assert.equal(source.init.headers.Authorization, 'Bearer fixture-sold-key');
    ...
    const images = model.body.input[0].content.filter((part) => part.type === 'input_image');
    assert.deepEqual(images.slice(0, 2).map((image) => Buffer.from(image.image_url.split(',')[1], 'base64')), f.bytes.slice(0, 2));
...
test('blank variant never becomes base without exact published evidence; useful fetched comps remain', async () => {
    ...
    assert.equal(result.estimate.status, 'unknown');
    assert.equal(result.estimate.value_cents, null);
...
test('missing and corrupted photos retain source candidates and never enter image/model assessment', async () => {
    for (const corrupt of [false, true]) {
        const f = await fixture();
        if (corrupt)
            f.deps.loadPhoto = async (photoKey) => ({ key: photoKey, sha256: sha(f.bytes[0]), bytes: f.bytes[1] });
        else
        ...
        assert.ok(f.calls[0].url.startsWith('https://api.sold-comps.com/'));
    }
});
test('source price eligibility preserves Best Offer, currency, malformed price and date uncertainty', async () => {
    for (const change of [
        ...
        const f = await fixture();
        Object.assign(f.items[0], change);
        f.input.front_photo_key = null;
        const result = await researchStaffInventoryCard(f.input, f.deps);
        const candidate = result.candidates.find(candidate => candidate.id === 'ebay:111111111110');
        assert.equal(candidate.source_eligible, false);
        assert.ok(candidate.exclusion_reason);
        assert.equal(result.estimate.value_cents, null);
        if (change.bestOfferAccepted === true) {
            assert.equal(candidate.best_offer_accepted, true);
            assert.equal(candidate.sold_price, '10.01');
            assert.equal(candidate.sold_price_cents, null);
        }
    }
});
test('lots, proxy cards and other explicitly unsupported sale products stay unselectable', async () => {
    for (const suffix of ['lot of 2', 'custom proxy', 'sealed booster box', 'complete set']) {
        const f = await fixture();
        f.items[0].title = `${f.items[0].title} ${suffix}`;
...
test('foreign IDs, forged photo/reference facts, wrong identities and grade mismatches fail closed', async () => {
    const changes = [
        (f) => { f.model.selected_candidate_ids[0] = 'ebay:999999999999'; },
        (f) => { f.model.comparisons[0].candidate_id = 'ebay:999999999999'; },
        (f) => { f.model.identity.photo_features[0].photo_sha256 = 'f'.repeat(64); },
        (f) => { f.model.identity.reference_ids = ['catalog:invented']; },
        (f) => { f.model.identity.photo_features[0].reference_feature = 'Imaginary red pattern'; },
        (f) => { f.model.identity.variant_name = 'Invented Rainbow'; },
        (f) => { f.model.identity.suggestion = 'Invented Rainbow'; },
        (f) => { f.model.comparisons[0].visual_match = false; },
        (f) => { f.items[0].title = String(f.items[0].title).replace('2024', '2023'); },
        (f) => { f.items[0].title = `${f.items[0].title} PSA 10`; },
        (f) => { f.items[0].title = `${f.items[0].title} PSA`; },
        (f) => { f.items[0].bestOfferAccepted = true; },
        (f) => { f.model.target_condition = { status: 'graded', grader: 'PSA', numeric_grade: 10, photo_evidence: 'Front: PSA 10.' }; },
    ];
...
test('exact same visible grader and grade may match; no grade mapping occurs', async () => {
    ...
    for (const item of f.items)
        item.title = String(item.title).replace('Raw', 'PSA 9');
    ...
    assert.equal(result.target_condition.numeric_grade, 9);
    f.model.target_condition.numeric_grade = 9.5;
    f.model.target_condition.photo_evidence = 'Front label: PSA 9.5.';
    await assertAssessmentFailed(f);
});
test('retained Beckett 8.5 answer passes visual validation and executes its zero-result refinement offline', async () => {
    ...
    const refinedItems = f.items.splice(0).map(item => ({ ...item, title: '2001-02 Upper Deck Hardcourt Eddy Curry #116 Rookie On Court BGS 8.5' }));
    f.deps.loadReferences = async () => [];
    f.model = structuredClone(retainedEddyBeckettAnalysis);
    for (const observation of f.model.photo_identity.observations)
        observation.photo_sha256 = sha(f.bytes[observation.side === 'front' ? 0 : 1]);
    ...
    f.deps.fetchImpl = ((url, init) => {
        if (String(url).startsWith('https://api.sold-comps.com/') && f.calls.some(call => call.url === 'https://api.openai.com/v1/responses')) {
            f.items.push(...refinedItems);
            f.model.refinement = null;
    ...
    assert.equal(result.photo_identity?.status, 'supported');
    assert.equal(result.identity.status, 'unresolved');
    assert.deepEqual(result.target_condition, retainedEddyBeckettAnalysis.target_condition);
    assert.equal(result.candidates.length, 2);
    assert.ok(result.comparison_assessments?.every(assessment => assessment.classification === 'possible'));
    assert.deepEqual(result.selected_candidate_ids, []);
    assert.equal(result.estimate.status, 'unknown');
});
test('retained all-back photo assertion stays rejected while its independently valid third query retrieves evidence', async () => {
    ...
    const responses = [retainedEddyCorrectedAnalysis1, retainedEddyCorrectedAnalysis2].map(value => {
        ...
        for (const observation of answer.photo_identity.observations)
            observation.photo_sha256 = sha(f.bytes[observation.side === 'front' ? 0 : 1]);
        return answer;
    });
    ...
    f.deps.fetchImpl = ((url, init) => {
        ...
        if (String(url).startsWith('https://api.sold-comps.com/') && modelCalls === 2)
            f.items.push(...refinedItems);
        if (String(url) === 'https://api.openai.com/v1/responses') {
            ...
            if (modelCalls === 2)
                f.model.refinement = { query: '2002 Hardcourt Eddy Curry 116', reason: 'Synthetic fourth proposal must never run.' };
        }
        return fetch(url, init);
    });
    const result = await researchStaffInventoryCard(f.input, f.deps);
    ...
    assert.deepEqual(result.photo_identity, responses[0].photo_identity);
    assert.deepEqual(result.target_condition, responses[0].target_condition);
    assert.deepEqual(result.identity, responses[0].identity);
    assert.equal(result.candidates.length, 2);
...
test('retained duplicate-field Snivy response preserves six comparisons; a uniquely cited response accepts nine without a value', async () => {
    for (const scenario of ['duplicate', 'conflicting_duplicate', 'foreign_photo', 'compliant']) {
        ...
        const responses = [retainedSnivyAnalysis1, retainedSnivyAnalysis2].map(value => {
            ...
            for (const observation of answer.photo_identity.observations)
                observation.photo_sha256 = sha(f.bytes[observation.side === 'front' ? 0 : 1]);
            return { ...answer, refinement: answer.refinement };
        });
        ...
        if (scenario === 'foreign_photo')
            responses[1].photo_identity.observations[3].photo_sha256 = 'f'.repeat(64);
        ...
        f.deps.fetchImpl = ((url, init) => {
            ...
            if (index >= 0)
                return Promise.resolve(new Response(new Uint8Array(candidateBytes[index]), { headers: { 'content-type': 'image/jpeg' } }));
            if (String(url) === 'https://api.openai.com/v1/responses') {
        ...
        assert.equal(result.candidates.filter(candidate => candidate.image).length, 9);
        assert.equal(StaffInventoryResearchResultSchema.safeParse(result).success, true);
...
test('Beckett label alias cannot change grader or grade and rejects conflicting or absent visible evidence', async () => {
    for (const evidence of ['Front: BECKETT BCCG 8.5.', 'Front: BECKETT BVG 8.5.', 'Front: Beckett Collectors Club Grading 8.5.', 'Front: Beckett Vintage Grading 8.5.',
        ...
        const f = await fixture();
        for (const item of f.items)
            item.title = String(item.title).replace('Raw', 'BGS 8.5');
        f.model.target_condition = { status: 'graded', grader: 'BGS', numeric_grade: 8.5, photo_evidence: evidence };
        const result = await researchStaffInventoryCard(f.input, f.deps);
        assert.equal(result.estimate.status, 'unknown', evidence);
        assert.deepEqual(result.selected_candidate_ids, []);
    ...
    for (const evidence of [retainedEddyBeckettAnalysis.target_condition.photo_evidence, 'Front: Beckett tag reads 8.5.']) {
        ...
        const result = await researchStaffInventoryCard(f.input, f.deps);
        assert.equal(result.estimate.status, 'estimated');
        assert.equal(result.target_condition.grader, 'BGS');
        assert.equal(result.target_condition.numeric_grade, 8.5);
    }
...
test('original grading labels preserve exact decimal boundaries and accept surrounding punctuation', async () => {
    for (const [grader, grade, evidence] of [
        ...
        const f = await fixture();
        for (const item of f.items)
            item.title = String(item.title).replace('Raw', `${grader} ${grade}`);
    ...
    for (const [grade, evidence] of [[8, 'Front: BECKETT “8”.'], [8, 'Front: BECKETT “8.0”.'], [8.5, 'Front: BECKETT (8.5).'], [8.5, 'Front: BECKETT 8.50, NM-MT+.'], [9.5, 'Front: BECKETT: 9.5; NM-MT+.'], [10, 'Front: BECKETT [10].']]) {
        const f = await fixture();
        for (const item of f.items)
            item.title = String(item.title).replace('Raw', `BGS ${grade}`);
        f.model.target_condition = { status: 'graded', grader: 'BGS', numeric_grade: grade, photo_evidence: evidence };
        const result = await researchStaffInventoryCard(f.input, f.deps);
        assert.equal(result.estimate.status, 'estimated', evidence);
        assert.equal(result.target_condition.numeric_grade, grade);
    }
});
test('image downloads are source-allowlisted, bounded, redirect-free and hash-bound', async () => {
    ...
    for (const response of [
        ...
        f.deps.fetchImpl = ((url, init) => String(url).startsWith('https://i.ebayimg.com/') ? Promise.resolve(response()) : original(url, init));
        await assertAssessmentFailed(f);
    }
});
test('duplicate images across distinct listing IDs do not provide two independent visual comparisons', async () => {
    const f = await fixture(), original = f.deps.fetchImpl;
    f.deps.fetchImpl = ((url, init) => String(url).startsWith('https://i.ebayimg.com/') ? Promise.resolve(new Response(new Uint8Array(f.bytes[2]), { headers: { 'content-type': 'image/jpeg' } })) : original(url, init));
    const result = await researchStaffInventoryCard(f.input, f.deps);
    assert.equal(result.estimate.status, 'unknown');
    assert.equal(result.candidates.length, 2);
    assert.equal(result.rejections.length, 2);
    ...
    one.model.selected_candidate_ids.pop();
    const insufficient = await researchStaffInventoryCard(one.input, one.deps);
    assert.equal(insufficient.estimate.status, 'unknown');
    assert.equal(insufficient.estimate.value_cents, null);
    assert.equal(insufficient.candidates.length, 2);
});
async function duplicateSelectionFixture() {
    const f = await fixture();
    f.items[0].soldPrice = '100.01';
    f.items[1].soldPrice = '1.01';
...
test('v6 with and without sale details counts an exact duplicate image once without price or model-order bias', async () => {
    for (const detail of [false, true])
        for (const reverse of [false, true]) {
            ...
            if (detail)
                f.deps.env.STAFF_INVENTORY_RESEARCH_SALE_DETAILS = 'true';
            if (reverse) {
                f.items.reverse();
                f.model.selected_candidate_ids.reverse();
                f.model.comparisons.reverse();
            }
            ...
            assert.equal(result.estimate.count, 2);
            assert.equal(result.estimate.low_cents, 3002);
            assert.equal(result.estimate.high_cents, 10001);
            assert.equal(result.candidates.length, 3);
            assert.equal(result.comparison_assessments.length, 3);
            ...
            assert.deepEqual(result.diagnostics.candidates.map(candidate => candidate.model_assessment).sort((a, b) => a.candidate_id.localeCompare(b.candidate_id)), [...f.model.comparisons].sort((a, b) => a.candidate_id.localeCompare(b.candidate_id)));
            assert.equal(JSON.stringify(f.items), sourceBefore);
            assert.equal(JSON.stringify(f.model), modelBefore);
            assert.equal(StaffInventoryResearchResultSchema.safeParse(result).success, true);
...
test('immutable historical duplicate-weighted estimates still parse without rewriting their bytes', async () => {
    ...
    for (const engine of ['staff-inventory-research-v1', 'staff-inventory-research-v2', 'staff-inventory-research-v3', 'staff-inventory-research-v5']) {
        const prior = structuredClone(historical);
        prior.engine_version = engine;
        delete prior.photo_identity;
        if (engine === 'staff-inventory-research-v1') {
            delete prior.research_queries;
            delete prior.comparison_assessments;
            delete prior.diagnostics;
        }
        if (engine === 'staff-inventory-research-v5')
            prior.sale_details = { schema_version: 1, base_engine_version: 'staff-inventory-research-v3', requests: [] };
        const bytes = canonical(prior);
        const parsed = StaffInventoryResearchResultSchema.parse(JSON.parse(bytes));
        assert.equal(canonical(parsed), bytes);
        assert.equal(inventoryHash(parsed), inventoryHash(prior));
        assert.equal(parsed.estimate.value_cents, 4368);
        assert.equal(parsed.estimate.count, 3);
    }
});
...
test('approved reference image evidence requires the exact retained source hash', async () => {
    const f = await fixture();
    const reference = { ...f.reference, id: 'reference:fixture', kind: 'reference', trust: 'approved_reference', image: { source_url: 'https://i.ebayimg.com/images/g/reference/s-l400.jpg', sha256: sha(f.bytes[4]), storage_key: 'approved-reference/source.jpg', content_type: 'image/jpeg' } };
...
test('response and persistence validation reject extra model prices, wrong models and tampered math', async () => {
    const f = await fixture();
    for (const payload of [output({ ...f.model, value_cents: 99999 }), { ...output(f.model), model: 'gpt-5.5' }, { ...output(f.model), status: 'incomplete' }, output({ ...f.model, identity: { ...f.model.identity, reason: 'https://secret.example/key' } })])
        assert.throws(() => parseStaffInventoryResearchOutput(payload), code('malformed_response'));
    const result = await researchStaffInventoryCard(f.input, f.deps);
    for (const mutate of [
        (r) => { r.estimate.value_cents = 1000; }, (r) => { r.candidates[0].sold_price = '99.00'; },
        (r) => { r.candidates[0].best_offer_accepted = true; }, (r) => { r.candidates[0].image.sha256 = '0'.repeat(64); r.candidates[0].image.storage_key = 'research-evidence/' + '1'.repeat(64) + '.jpg'; },
        (r) => { r.identity.reference_ids = ['foreign']; }, (r) => { r.identity.status = 'unresolved'; },
    ]) {
...
test('late source and image setup shorten the model deadline and preserve ten seconds for engine finalization', async (t) => {
    ...
    t.mock.timers.enable({ apis: ['Date', 'setTimeout'], now: Date.parse(NOW) });
    let sourceBegan, imagesBegan, modelBegan;
    let releaseSource, releaseImages, models = 0;
    const sourceActive = new Promise(resolve => { sourceBegan = resolve; });
    const imagesActive = new Promise(resolve => { imagesBegan = resolve; });
    const modelActive = new Promise(resolve => { modelBegan = resolve; });
    const sourceGate = new Promise(resolve => { releaseSource = resolve; });
    const imageGate = new Promise(resolve => { releaseImages = resolve; });
    let photosDone, photoCompletions = 0;
    const photosFinished = new Promise(resolve => { photosDone = resolve; });
    f.deps.loadPhoto = async (key, signal) => {
        ...
        signal.addEventListener('abort', () => { if (++photoCompletions === 2)
            photosDone(); }, { once: true });
    ...
    f.deps.fetchImpl = (async (url, init) => {
        ...
        else if (uri.startsWith('https://i.ebayimg.com/')) {
            imagesBegan();
            await imageGate;
        }
        else if (uri === 'https://api.openai.com/v1/responses') {
            models++;
            modelBegan(init.signal);
            return new Promise(() => { });
        }
        return original(url, init);
    });
    ...
    t.mock.timers.tick(115999);
    assert.equal(signal.aborted, false);
    t.mock.timers.tick(1);
    const result = await running;
    assert.equal(signal.aborted, true);
    assert.equal(models, 1);
    assert.equal(result.timings_ms.model, 116000);
    assert.equal(result.timings_ms.total, 140000);
    assert.equal(result.estimate.status, 'unknown');
    assert.deepEqual(result.selected_candidate_ids, []);
    assert.equal(result.candidates.filter(candidate => candidate.image).length, 2);
    assert.ok(result.diagnostics.reason_codes.includes('TIMEOUT'));
...
test('the existing eighty-four-second optional round still cancels a longer model and preserves prior facts', async (t) => {
    ...
    assert.equal(result.research_queries?.length, 2);
    assert.equal(result.timings_ms.total, 84000);
    assert.equal(result.identity.status, 'base');
    assert.equal(result.estimate.status, 'unknown');
    assert.equal(result.candidates.filter(candidate => candidate.image).length, 2);
...
test('provider rows, image reads and concurrency stay bounded even with a large source response', async () => {
    ...
    const original = f.deps.fetchImpl;
    let active = 0, highest = 0, imageCount = 0;
    f.deps.fetchImpl = (async (url, init) => {
        if (!String(url).startsWith('https://i.ebayimg.com/'))
            return original(url, init);
        imageCount++;
        active++;
...
test('decoded compressed responses use decoded byte bounds without confusing the encoded length', async () => {
    ...
    f.deps.fetchImpl = (async () => new Response('x'.repeat(5 * 1024 * 1024 + 1), { headers: { 'content-type': 'application/json', 'content-encoding': 'gzip', 'content-length': '1' } }));
...
test('worker cancellation aborts one bounded source request without alternate providers or retries', async () => {
    ...
    f.deps.fetchImpl = (async (_url, init) => { calls++; observed = init?.signal; started(); return new Promise(() => { }); });
    const pending = researchStaffInventoryCard(f.input, f.deps, controller.signal);
    await ready;
    controller.abort();
...
async function adaptiveFixture(count = 2) {
    ...
    const sources = [], modelBodies = [];
    const rows = (round) => Array.from({ length: count }, (_, index) => ({
    ...
    let nextRows = rows, nextModel = null;
    f.deps.fetchImpl = (async (url, init) => {
        const uri = String(url);
        if (uri.startsWith('https://api.sold-comps.com/')) {
            const keyword = new URL(uri).searchParams.get('keyword');
            const round = sources.length;
        ...
        if (uri.startsWith('https://i.ebayimg.com/'))
            return new Response(new Uint8Array(f.bytes[2 + sources.length % 2]), { headers: { 'content-type': 'image/jpeg' } });
        assert.equal(uri, 'https://api.openai.com/v1/responses');
        const body = JSON.parse(String(init?.body));
        modelBodies.push(body);
        const candidates = body.input[0].content.filter((part) => part.type === 'input_text' && part.text.startsWith('Candidate data: ')).map((part) => JSON.parse(part.text.slice(16)));
...
test('Astra refines poor searches, retains query provenance and separates visual matches from unknown-price estimates', async () => {
    ...
    assert.equal(result.research_queries?.length, 3);
    ...
    assert.equal(result.selected_candidate_ids.length, 0);
    assert.equal(result.estimate.status, 'unknown');
...
test('query history attributes thirteen initial results and an empty refinement before an anchor-preserving broader search', async () => {
    ...
    f.setModel((value, round) => {
        ...
        const historyText = body.input[0].content.find((part) => part.type === 'input_text' && part.text.startsWith('Research queries already attempted: ')).text;
        const history = JSON.parse(historyText.slice('Research queries already attempted: '.length).split('. This is search ')[0]);
        histories.push(history);
        if (round === 1) {
            assert.deepEqual(history.map((query) => query.result_count), [13, 0]);
...
test('rejected first identity or condition assertions may propose retrieval but never establish facts or value', async () => {
    for (const failure of ['photo', 'condition']) {
        const f = await fixture();
        f.model.refinement = { query: '2024 Fixture Chrome Fixture Runner #007', reason: 'Try saved anchors without the manufacturer.' };
        if (failure === 'photo')
            f.model.photo_identity = { schema_version: 1, status: 'supported', observations: [], reason: 'This unsupported assertion must remain rejected.' };
        else
            f.model.target_condition = { status: 'raw', grader: 'PSA', numeric_grade: 10, photo_evidence: 'Contradictory condition assertion.' };
        const result = await researchStaffInventoryCard(f.input, f.deps);
        assert.equal(result.research_queries?.length, 2);
        assert.equal(result.candidates.length, 2);
        assert.equal(result.identity.status, 'unresolved');
        assert.equal(result.photo_identity?.status, 'unresolved');
        assert.equal(result.target_condition.status, 'unresolved');
        assert.deepEqual(result.selected_candidate_ids, []);
        assert.equal(result.estimate.status, 'unknown');
...
test('refinements from rejected assertions use only the last trusted anchors and condition', async () => {
    for (const query of ['2024 Fixture Chrome Different Player #007 silver', '2024 Fixture Chrome Fixture Runner #7 silver',
        ...
        f.setModel((value, round) => round !== 1 ? value : { ...value,
            ...
            selected_candidate_ids: value.comparisons.map((comparison) => comparison.candidate_id), refinement: { query, reason: 'Untrusted query proposal.' } });
        const result = await researchStaffInventoryCard(f.input, f.deps);
        assert.equal(f.sources.length, 2, query);
        assert.equal(result.target_condition.status, 'raw');
        assert.equal(result.target_condition.grader, null);
        assert.equal(result.identity.status, 'unresolved');
        assert.deepEqual(result.selected_candidate_ids, []);
        assert.equal(result.estimate.status, 'unknown');
    }
...
test('semantic rejection preserves prior facts across a valid refinement; malformed continuation cannot reuse its proposal', async () => {
    for (const failure of ['photo', 'condition', 'selection']) {
        const f = await adaptiveFixture();
        f.setModel((value, round) => {
            if (round === 2)
                return { ...value, invented_price: 5000 };
            if (round !== 1)
                return value;
            if (failure === 'photo')
                return { ...value, photo_identity: { schema_version: 1, status: 'supported', observations: [], reason: 'Rejected identity.' } };
            if (failure === 'condition')
                return { ...value, target_condition: { status: 'raw', grader: 'PSA', numeric_grade: 10, photo_evidence: 'Rejected inconsistent condition.' } };
        ...
        assert.equal(f.modelBodies.length, 3);
        assert.equal(result.candidates.length, 6);
        assert.equal(result.identity.reason, 'The exact published catalog is unavailable.');
        assert.equal(result.target_condition.status, 'raw');
        assert.equal(result.photo_identity?.status, 'unresolved');
        assert.deepEqual(result.selected_candidate_ids, []);
        assert.equal(result.estimate.status, 'unknown');
...
test('a valid-looking proposal cannot bypass malformed response envelopes or field shapes', async () => {
    for (const failure of ['model', 'incomplete', 'extra', 'photo', 'condition', 'refinement']) {
        const f = await adaptiveFixture(), fetch = f.deps.fetchImpl;
        let models = 0;
        f.deps.fetchImpl = (async (url, init) => {
            ...
            if (failure === 'model')
                ...
                if (failure === 'extra')
                    value.invented_price = 5000;
                if (failure === 'photo')
                    value.photo_identity = { schema_version: 1, status: 'supported', observations: 'invalid', reason: 'Invalid wire type.' };
                if (failure === 'condition')
                    value.target_condition.numeric_grade = '10';
                if (failure === 'refinement')
                    value.refinement.extra = 'not allowed';
                part.text = JSON.stringify(value);
            }
            return Response.json(payload);
        });
        const result = await researchStaffInventoryCard(f.input, f.deps);
        assert.equal(f.sources.length, 2, failure);
        assert.equal(models, 2);
...
test('three-query work deduplicates listings, bounds retained candidates and spends at most twelve image downloads', async () => {
    ...
    const original = f.deps.fetchImpl;
    let imageReads = 0, active = 0, maximum = 0;
    const imageUrls = [];
    f.deps.fetchImpl = (async (url, init) => {
        if (!String(url).startsWith('https://i.ebayimg.com/'))
            return original(url, init);
...
test('conflicting prices across queries preserve original source provenance and remove sale eligibility', async () => {
    ...
    assert.ok(prior);
    assert.equal(prior.source_eligible, false);
    assert.equal(prior.sold_price, '10.01');
...
test('unknown-price visual matches stay matched and deterministic wrong card or grade evidence cannot become a match', async () => {
    ...
    for (const suffix of ['#999', 'PSA 10']) {
        ...
        altered.model.selected_candidate_ids = [];
...
test('equivalent consecutive-season listing wording remains a possible visual match without rewriting saved identity', async () => {
    for (const titleYear of ['2024-25', '2024-2025', '2024/25', '2024/2025']) {
        const f = await fixture();
        f.input.description.year = '2024-25';
        f.deps.loadReferences = async () => [];
        f.model.identity = { status: 'unresolved', variant_name: null, suggestion: null, reason: 'The exact published catalog is unavailable.', reference_ids: [], photo_features: [] };
        f.model.selected_candidate_ids = [];
        for (const item of f.items)
            item.title = String(item.title).replace('2024', titleYear);
        const result = await researchStaffInventoryCard(f.input, f.deps);
        assert.equal(result.comparison_assessments?.every(assessment => assessment.classification === 'matched'), true, titleYear);
        assert.equal(f.input.description.year, '2024-25');
        assert.equal(result.estimate.status, 'unknown');
    }
});
test('legacy immutable results parse with identical JSON and hashes without inserting optional research or price fields', async () => {
    ...
    const legacy = { ...current, engine_version: 'staff-inventory-research-v1' };
    delete legacy.photo_identity;
    delete legacy.research_queries;
    delete legacy.comparison_assessments;
    delete legacy.diagnostics;
    for (const candidate of legacy.candidates) {
        delete candidate.sale_evidence;
        delete candidate.image_options;
        delete candidate.multiple_price_options;
    }
    ...
    for (const mutate of [
        (value) => { value.research_queries[0].source_response_sha256 = 'f'.repeat(64); },
        (value) => { value.comparison_assessments[0].candidate_id = 'ebay:999999999999'; },
        (value) => { value.comparison_assessments[0].visual_match = false; },
    ]) {
...
test('only explicit hydrated accepted-offer evidence can supply an estimate, retaining original source amounts', async () => {
    for (const crossCurrency of [false, true]) {
        const f = await fixture();
        for (const item of f.items)
            Object.assign(item, { bestOfferAccepted: true, boaHydrated: true, ...(crossCurrency ? { soldPrice: '30.00', soldCurrency: 'CAD', boaAcceptedPrice: '9.00', boaAcceptedCurrency: 'USD' } : {}) });
        const result = await researchStaffInventoryCard(f.input, f.deps);
        ...
        for (const flag of ['includeCompleteListing', 'exactMatch', 'hydrateBoa'])
            assert.equal(new URL(sourceCall.url).searchParams.get(flag), 'true');
        const bad = structuredClone(result);
        bad.candidates[0].accepted_offer.amount = '99.00';
        assert.equal(StaffInventoryResearchResultSchema.safeParse(bad).success, false);
    }
});
test('provider sale status is independent from visual matching and never inferred from an ended date', async () => {
    for (const listingType of ['active', undefined]) {
        ...
        assert.equal(result.estimate.status, 'unknown');
        const bad = structuredClone(result);
        bad.candidates[0].source_eligible = true;
        assert.equal(StaffInventoryResearchResultSchema.safeParse(bad).success, false);
    }
});
    ...
    for (const highResolution of [false, true])
        for (const failPrimary of [false, true]) {
            ...
            for (const [index, item] of f.items.entries())
                item.fullResThumbnailUrl = `https://i.ebayimg.com/images/g/primary${index}/s-l1600.jpg`;
            f.deps.fetchImpl = (async (url, init) => {
                const uri = String(url);
                if (!uri.startsWith('https://i.ebayimg.com/'))
                    return original(url, init);
            ...
            for (const [index, candidate] of result.candidates.entries()) {
                ...
                assert.equal(diagnostic.image_width, 100);
                assert.equal(diagnostic.image_height, 140);
...
test('a successful image download followed by optional model failure is recorded as unassessed, preserving original model evidence', async () => {
    ...
    f.deps.fetchImpl = (async (url, init) => String(url) === 'https://api.openai.com/v1/responses' && ++calls === 2
        ? new Response('failed', { status: 500 }) : original(url, init));
    const result = await researchStaffInventoryCard(f.input, f.deps);
    ...
    for (const candidate of later) {
        ...
        assert.equal(diagnostic.comparison_status, 'failed');
        assert.equal(diagnostic.model_assessment, null);
        ...
        assert.equal(assessment.classification, 'possible');
        assert.doesNotMatch(assessment.reason, /image.*unavailable|image.*missing/i);
    }
...
test('documented anniversary, NM-MT and sport-suffix failure mechanisms pass the final estimate gate without relaxing wrong-print controls', async () => {
    ...
    for (const example of cases) {
        ...
        if (grade)
            f.model.target_condition = { status: 'graded', grader: 'PSA', numeric_grade: grade, photo_evidence: `Front: PSA ${grade}.` };
        for (const item of f.items) {
            item.title = title;
            item.condition = grade ? 'Graded' : 'Ungraded';
        }
        const saved = JSON.stringify(f.input), result = await researchStaffInventoryCard(f.input, f.deps);
...
test('v3 diagnostics reject removed or mismatched model-image, source and price-option attribution', async () => {
    ...
    for (const mutate of [
        ...
        (value) => { delete value.candidates[0].multiple_price_options; },
        (value) => { value.diagnostics.candidates[0].model_image_sha256 = 'f'.repeat(64); },
        (value) => { value.diagnostics.sources[0].retained_count = 1; },
        (value) => { value.diagnostics.candidates[0].image_width = 8000; value.diagnostics.candidates[0].image_height = 8000; },
    ]) {
    ...
    legacy.engine_version = 'staff-inventory-research-v2';
    delete legacy.photo_identity;
    delete legacy.diagnostics;
    for (const candidate of legacy.candidates) {
        delete candidate.sale_evidence;
...
test('optional archival timeout retains an already verified image and consistent acquired diagnostics', async (t) => {
    ...
    t.mock.method(globalThis, 'setTimeout', ((callback, delay, ...args) => originalTimer(callback, delay === 18000 ? 30 : delay, ...args)));
    ...
    for (const current of [true, false]) {
        ...
        f.deps.loadCatalog = async (_description, photos) => ({ references: [f.reference], context: { schema_version: 1, status: 'current',
                ...
                scope_receipt: { attempt_id: 'fixture:attempt', invocation_id: 'fixture:scope', receipt_ref: 'fixture:receipt', request_sha256: 'd'.repeat(64), response_sha256: 'e'.repeat(64), http_status: 200, acknowledgement: 'adapter',
                    images: ['front', 'back'].map(side => ({ side, mime_type: 'image/jpeg', transmitted_sha256: photos[side].sha256, source_sha256: null })) },
        ...
        let checked = false;
        f.deps.isCatalogCurrent = async () => { checked = true; return current; };
        const result = await researchStaffInventoryCard(f.input, f.deps);
        assert.equal(result.engine_version, 'staff-inventory-research-v6');
        assert.equal(checked, true);
        assert.equal(result.identity.status, current ? 'base' : 'unresolved');
        assert.equal(result.estimate.status, current ? 'estimated' : 'unknown');
        assert.equal(result.catalog_context?.status, current ? 'current' : 'unavailable');
        if (!current) {
            assert.equal(result.references.length, 0);
            assert.equal(result.selected_candidate_ids.length, 0);
            assert.ok(result.comparison_assessments?.every(a => a.classification !== 'matched'));
        }
...
test('catalog printing-scope model cannot assert absent-stamp defaults, wrong photos or unavailable enum choices', async () => {
    ...
    assert.deepEqual(scoped.evidence, [evidence]);
    assert.equal(scoped.receipt?.acknowledgement, 'process');
    assert.equal(scoped.receipt?.images[0].transmitted_sha256, photos.front.sha256);
    assert.equal(scoped.receipt?.images[0].source_sha256, null);
...
test('v6 selects exact photographed sales without a catalog and preserves saved facts and catalog uncertainty', async () => {
    for (const catalog of ['disabled', 'no_publication', 'unavailable']) {
        const f = await photoIdentityFixture();
        f.input.description.name = '2024 Fixture Cards Fixture Runner #007';
        f.input.description.year = null;
        f.input.description.set_name = null;
        f.input.description.card_number = null;
        if (catalog !== 'disabled')
            f.deps.loadCatalog = async () => {
                if (catalog === 'unavailable')
                    throw new Error('private-catalog-error');
        ...
        assert.deepEqual(result.identity.reference_ids, []);
        assert.equal(result.estimate.value_cents, 1002);
        assert.equal(result.estimate.count, 2);
        assert.equal(canonical(f.input), saved);
        assert.equal(result.description_hash, f.input.description_hash);
...
test('v6 rejects incomplete, generic, absent-feature, foreign-photo and saved-fact-conflicting identity while retaining fetched listings', async () => {
    const changes = [
        f => { f.model.photo_identity.observations = f.model.photo_identity.observations.filter((row) => row.field !== 'year'); },
        f => { f.model.photo_identity.observations = f.model.photo_identity.observations.filter((row) => row.field !== 'manufacturer'); },
        f => { f.model.photo_identity.observations[0].photo_sha256 = 'e'.repeat(64); },
        f => { f.model.photo_identity.observations.forEach((row) => { row.side = 'front'; row.photo_sha256 = sha(f.bytes[0]); }); },
...
test('v6 unresolved photo observations retain candidates without inventing printing or a price', async () => {
    ...
    f.model.photo_identity.observations = f.model.photo_identity.observations.filter((row) => row.field !== 'treatment');
    f.model.selected_candidate_ids = [];
    const result = await researchStaffInventoryCard(f.input, f.deps);
    assert.equal(result.photo_identity?.status, 'unresolved');
    assert.equal(result.estimate.status, 'unknown');
...
test('first malformed or unavailable model response preserves source evidence with explicit failure and no model authority', async () => {
    for (const failure of ['malformed', 'http', 'throw']) {
...
test('v6 persistence requires its photo block and exact provenance; legacy versions cannot receive photo authority', async () => {
    ...
    for (const mutate of [
        (row) => { delete row.photo_identity; },
        (row) => { row.photo_identity.observations[0].photo_sha256 = 'f'.repeat(64); },
        (row) => { row.photo_identity.observations[5].value = 'Base'; },
        (row) => { row.photo_identity.status = 'unresolved'; },
        (row) => { row.photos.front = null; },
        (row) => { row.candidates[0].sale_evidence.status = 'unknown'; row.candidates[0].sale_evidence.basis = 'not_supplied'; },
        (row) => { row.candidates[0].best_offer_accepted = true; },
    ]) {
    ...
    for (const engine of ['staff-inventory-research-v1', 'staff-inventory-research-v2', 'staff-inventory-research-v3', 'staff-inventory-research-v4', 'staff-inventory-research-v5']) {
        ...
        const changed = { ...valid, engine_version: engine };
        delete changed.photo_identity;
