...
export function createConnectedHandler({connected,boundary,origin,assertRequest}) {
  const workflow=createWorkflowHandler({workflow:connected.workflow,boundary,origin,assertRequest,imageDescriptors:connected.imageDescriptors});
  ...
  return async(req,res)=>{
    ...
    try{
      ...
      if(req.method==='POST')requireThat(Buffer.byteLength(JSON.stringify(req.body??{})) <= (url.pathname.endsWith('/trace')?1048576:url.pathname.startsWith('/api/staff/manual-intake/')?8192:65536),413,'REQUEST_TOO_LARGE');
