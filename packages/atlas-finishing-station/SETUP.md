...
Before distribution, sign the exact final native binary with the selected Developer ID and hardened runtime, verify its actual signature, notarize the final distribution with the corresponding Apple account, and retain its accepted ticket and Gatekeeper result. Then regenerate the bundle file manifest and native executable hash from those final signed bytes. Key access must be tested using that exact installed identity; signing a successor after key qualification is a distinct acceptance step. No signing command or install is run by these build scripts.
