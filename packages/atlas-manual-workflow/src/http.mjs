 ...
 */
export function createWorkflowHandler({ workflow, boundary, origin, assertRequest, imageDescriptors }) {
  const ordinary = createManualHandler({ service: workflow.service, boundary, origin, assertRequest });
  const uuid = '[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}';
  const pattern = new RegExp(`^/api/staff/manual/cards/(${uuid})/(view|trace)$`);
  return async (req, res) => {
    ...
    try {
      ...
      if (write) res.status(200).json(await workflow.stageTrace(staff, found[1], req.body));
      else {
        ...
        res.status(200).json({ card, ...state, images, approval });
      }
    } catch (error) {
