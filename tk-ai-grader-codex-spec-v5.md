...
Explicit manual geometry is a distinct operator-confirmed path. It is worker-free, invokes no automatic detector, is never selected automatically, and cannot replace detector failure; its existing manual capture/override audit remains mandatory. Front captured-evidence processing may overlap the flip/back-positioning window only while this isolation keeps preview heartbeat, two-second freshness, six-second first-frame grace, watchdog, EOF/reconnect, disconnect cleanup, and verified safe-off timers responsive. Bridge protocol `ai-grader-local-station-bridge-v0.8` and report-producer contract `ai-grader-report-producer-v0.2` remain unchanged. This policy preserves the accepted-profile lighting, maximum duty/allowlist, truthful physical state, exact preview/frame binding, and atomic Back Capture contracts together with captured-evidence authority, pairwise consensus rejection, raw hashes, lossless `1200x1680` normalization, fail-closed processing, and public-response sanitization.
