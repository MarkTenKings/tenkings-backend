...
## July 2026 Final Capture-Path Policy

The production capture path is singular: Start New Card, Capture Front, Capture Back, and Approve & Publish. There is no production cold/debug path, automatic warm-to-cold recovery, manual geometry/capture fallback, automatic Rapid Capture shutter path, browser capture-intent system, fixture/ruler confirmation, per-card lighting acceptance, browser-owned Safe Off, or recovery ladder. The durable Rapid Capture background-report queue is retained separately as throughput infrastructure.

The bridge is the only hardware owner. It permits only allowlisted bounded Leimac commands, validates exact controller acknowledgements, owns one lighting watchdog, and performs automatic Safe Off only for fatal bridge failure, cancellation, or completed session. It retains the capture lock, single Basler/Pylon owner, scoped orphan cleanup, and one serialized hardware operation. The former fixed 5% ceiling is removed.

Each capture remains bound to the exact session, report/card, side, preview epoch, and fresh frame. Duplicate or stale frames fail without an alternate capture implementation. Raw evidence remains immutable, derived normalization never replaces it, and publication is one human Approve & Publish authority backed by one atomic database transaction with durable report/card/label/inventory linkage.
...
The retained physical controls are allowlisted/bounded Leimac commands with exact acknowledgements, one bridge-owned watchdog, automatic bridge Safe Off only for fatal bridge failure/cancellation/completed session, one capture lock, one Basler/Pylon owner, scoped orphan cleanup, and one serialized hardware operation. The former fixed 5% limit is removed; duty remains bounded to the controller range. A duty-requiring operation must receive an explicit duty or use the already-selected production profile. Omission never defaults to `99.9%`.

The retained capture authority binds exact session, report/card, side, preview epoch, frame, freshness, and uniqueness. Publication has one human authority and one atomic durable card/report/label/inventory transaction. There is no silent capture or publication fallback.
