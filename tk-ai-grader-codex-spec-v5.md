...
An operator Front command may establish the fixture/capture confirmation in that atomic audit. An automatic Front command requires a pre-existing explicit operator fixture/ruler confirmation recorded when Auto Capture is armed; it must preserve `auto` as the trigger identity and must not synthesize or relabel operator evidence. Automatic capture may use only detected-Ready geometry. Explicit manual geometry remains operator-only.
