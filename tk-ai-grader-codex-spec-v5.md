...
## July 2026 Superseding Capture-Path Policy

This policy supersedes any earlier allowance for automatic warm-to-cold capture recovery or automatic manual card-boundary substitution:

- A warm_full_forensic_runner failure is terminal for that capture attempt. The bridge must safe-off, release camera/lighting locks, persist and display the exact failure, and require an operator retry. It must not start cold_command_fallback automatically.
- cold_command_fallback is permitted only when an explicit developer/debug configuration disabled the warm runner before the session. It cannot run production_fast, cannot claim production speed acceptance, and cannot contribute to five-seconds-per-side proof.
- full_forensic and production_fast are separate explicit selectable profiles. full_forensic is the previous stable profile, not an automatic fallback. production_fast remains opt-in until a later Mark-approved Dell A/B run proves evidence quality and timing.
- Failed or low-confidence automatic card geometry remains Not Detected or Adjust Card. A configured fixture rectangle must not silently become geometry. Any manual capture or manual geometry override requires an explicit operator action and durable manual_capture/manual_override metadata with detectionUsed=false.
- Raw evidence is immutable. Normalized crop/deskew artifacts are derived additions and must never replace or destroy the raw front/back forensic images.
- Rapid capture must still feed human Confirm Card and Publish plus the existing label-sheet, comps, slab-photo, and inventory gates. OCR remains draft prefill and always requires human confirmation.
- Dell hardware timing and five seconds per side remain unproven until Mark runs and accepts the documented A/B procedure.

### Atomic Side-Capture Lifecycle

Bridge protocol `ai-grader-local-station-bridge-v0.9` makes Front Capture and Back Capture use one side-generic, authenticated, idempotency-keyed lifecycle instead of browser-composed preview-stop, lighting-safe-off, and capture calls. The report-producer contract remains `ai-grader-report-producer-v0.2`; the handoff change does not alter report packages, evidence roles, grading, OCR, Confirm, Publish, or public output.

For either side, the bridge must first reserve serialized lifecycle/capture ownership, acquire the capture lock, and validate the idempotency key. It then validates the exact active session, report, workflow step, side, side epoch, fresh frame, and either current detected-Ready geometry or deliberate current manual mode; snapshots its authoritative frame, geometry or manual decision, and durable accepted profile; and records the honest capture/manual audit. Browser IDs and mode are match assertions only. Browser geometry, duty, channels, profile values, controller address, paths, image bodies, or arbitrary hardware values are never capture authority. Only after validation, snapshot, and audit may the bridge mark an intentional capture transition, stop and drain preview, enter `safe_off_pending`, and require complete controller acknowledgement establishing `safe_off_verified`. It must revalidate and consume the authorization exactly once immediately before camera ownership and the first grab, then perform one forensic capture.

The Front capture command may not establish prerequisite authority. Fixture/ruler confirmation is a separate explicit, idempotent bridge mutation bound to the current session, report, Front side, and side epoch. Automatic and operator Front commands require that same pre-existing evidence from the separate `Confirm Fixture & Rulers` action; Auto Capture and Start Grading must never record, synthesize, or relabel it. The automatic command preserves `auto` as the trigger identity and may use only detected-Ready geometry. Explicit manual geometry remains operator-only.

Before `capture_front`, the bridge persists one Front workflow authority record under schema `ten-kings-ai-grader-front-workflow-authority-v1`. Controller-acknowledged initial light-idle, operator fixture/ruler confirmation, accepted profile, and the resulting transition must all match the exact session, report, Front side, and side epoch. Accepted-profile evidence additionally binds the bridge-observed full profile SHA-256 digest, acceptance timestamp, and a binding-specific identity. Each verified profile application receives a fresh opaque bridge candidate identity bound to that exact session/report/front epoch, full digest, acknowledgement time, and nonce; acceptance must return that identity as a match assertion. Acceptance requests contain only bounded idempotency, session/report/side/epoch, and opaque candidate assertions; browser profile, hardware, confirmation, geometry, or path values are rejected. Delayed prior-candidate requests, consumed keys, default, stale, browser-only, prior-session, prior-profile, or prior-epoch evidence can never satisfy this authority. Consumed profile-acceptance history must not be evicted inside a session; exhausting its bounded limit fails closed and requires a new session.

Every profile or fixture mutation reserves a bridge-owned Front-workflow transaction. Exact retries coalesce, conflicts reject, readiness stays pending, and Atomic Front remains blocked until the atomic manifest write succeeds. A failed write rolls capture-authorizing memory back. Heartbeats must reject while the transaction is pending, and every terminal manifest writer—including watchdog, preview-loss safe-off, operator preview stop, and shutdown—must reserve its lifecycle lane and await commit or rollback before writing cleanup state.

The bridge advances to `capture_front` exactly once when all required exact-bound evidence exists, and repeated identical or lost-response retries return the same state without duplicating the transition. The Station must refresh bridge status after each mutation and derive Start readiness entirely from `frontCaptureReadiness` plus the current preview binding. Start remains disabled during a transition, capture, cleanup, safe-off, or ambiguous request and while any prerequisite is absent; the UI displays the bridge's exact unmet prerequisite. Refresh or reconnect derives the same result without browser reconstruction. Immediately before every operator, manual, or automatic Front submission, the Station must fetch and revalidate that authoritative readiness.

The same idempotency key returns the same bounded result and never captures twice. A conflicting, crossed, stale, wrong-step, wrong-side, wrong-epoch, wrong-frame, Not Detected/Adjust Card, or unauthorized-manual request fails closed before capture. During an exact outstanding submitted intent, React abort/effect cleanup, browser EOF, disconnect, reader error, and status polling cannot clear the bridge snapshot, start a second safe-off, or reconnect. Intentional transition EOF never reconnects; unexpected EOF outside an intent retains the existing bounded recovery. A transport-ambiguous submitted intent is preserved for 15 seconds. Exact active bridge authority then permits at most 20 sequential two-second status checks with no overlap or competing mutation; exact completion/failure reconciles full token-gated Station status. A hard deadline while the bridge still owns capture surfaces do-not-retry and leaves the bridge authoritative. Only the absence of exact bridge authority after the bounded window permits intent expiry followed by the ordinary safe-off and bounded reconnect recovery.

Front Capture may release ownership and advance to back positioning only after immutable front raw evidence is safely persisted and the capture lock/preview hold are released. Any back-positioning restore then uses only the durable accepted station profile, accepted allowlisted channels, and accepted duty capped at `5%`; no browser-supplied hardware value is accepted. Back Capture retains the existing atomic ordering and verified-safe-off requirement. Every failure and terminal path keeps the pending/verified/unknown physical-state contract: attempted safe-off is never reported as off, and `physical_state_unknown` blocks every further camera, lighting, capture, restore, and automated-transition operation except the one explicit bounded recovery that can establish `safe_off_verified`.

The first actionable workflow or capture rejection remains the primary Station error while preview drain and safe-off cleanup still run. A later cleanup failure must not replace it. An incomplete safe-off acknowledgement is exposed separately through the physical-state safety channel, leaves final-light-off false, and blocks continuation until recovery; browser confirmation flags can never assert physical off.

This v0.9 change must preserve PR #89 exact session/side/epoch/frame binding, coherent image/geometry snapshots, manual audit, finite reconnect, stale-result rejection, accepted-profile lighting, Back Capture idempotency, and ACK-truthful cleanup. It must also preserve PR #92 detector policies, captured-evidence worker isolation and identity/path/hash validation, all-on authority and consensus rejection, immutable hashes, lossless `1200x1680` normalization, terminal no-fallback behavior, and final public-response sanitization.

### Detector Execution Policy

Every geometry detector entry point requires one explicit typed use-case policy; an omitted, unknown, or implicit policy fails before detection. `live_preview_fast` belongs only to live MJPEG positioning. It may execute only the existing bounded fast detector and must never invoke `perimeter_gradient_rectangle_v3`. Live work retains latest-frame queue semantics. Its result is advisory and may authorize nothing unless the exact station session, side, side epoch, and frame remain current and fresh when it completes; stale or crossed results are discarded.

`captured_evidence_full` belongs only to immutable full-resolution captured roles and may execute `perimeter_gradient_rectangle_v3`. That full authority calculation must run in a dedicated compiled Node worker thread loaded from built `dist`, never on the bridge event loop and never through a TypeScript runtime. The worker is scoped to geometry authority: camera, preview, Leimac, safe-off, atomic capture, and browser state remain bridge-owned. There may be at most one active full-resolution geometry job and one bounded queued job.

The versioned request binds request ID, package ID, station session, side, every source role, complete source-set digest, source image/frame identity, SHA-256, byte size, dimensions, MIME type, and timestamp. Inputs are bounded metadata plus local relative references from the bridge-created immutable side package. Canonical real-path containment must be established beneath a trusted constructor-configured output root at request creation, inside the worker, during source revalidation, and immediately before the exact main-process inputs consume the authority. A success response contains bounded geometry/evidence metadata only and no local paths, image bytes, base64, data URL, or hosted/public image body.

Acceptance is two-phase: validate the exact response identity/schema, source bindings, inspected roles, and consensus, then require the same worker to revalidate source hashes, sizes, dimensions, side, roles, and request identity and return the matching acknowledgement. Worker timeout, crash, malformed or extra output, wrong identity/hash/role/dimensions, path-containment failure, failed acknowledgement, cancellation, terminal session failure, or shutdown is terminal for that processing attempt. No in-process full detector, cold capture, synthetic geometry, hidden retry, or weakened authority, agreement, evidence, normalization, finding, or public-sanitization fallback is permitted.

Explicit manual geometry is a distinct operator-confirmed path. It is worker-free, invokes no automatic detector, is never selected automatically, and cannot replace detector failure; its existing manual capture/override audit remains mandatory. Front captured-evidence processing may overlap the flip/back-positioning window only while this isolation keeps preview heartbeat, two-second freshness, six-second first-frame grace, watchdog, EOF/reconnect, disconnect cleanup, and verified safe-off timers responsive. Bridge protocol `ai-grader-local-station-bridge-v0.9` governs the atomic side-capture handoff, while report-producer contract `ai-grader-report-producer-v0.2` remains unchanged. This policy preserves accepted-profile lighting, maximum duty/allowlist, truthful physical state, exact preview/frame binding, and atomic Front/Back Capture together with captured-evidence authority, pairwise consensus rejection, raw hashes, lossless `1200x1680` normalization, fail-closed processing, worker isolation, and public-response sanitization.
...
PR #39 adds fixed-rig calibration/preview foundation only. Local manifests may include a `FixedRigCalibrationProfile` with selected exposure/gain/duty/polarity, default card dimensions, optional uncalibrated pixel/mm estimates, and statuses such as `preview_assisted`, `focus_assisted`, `framing_assisted`, or `channel_characterized`, but `isCalibrated=false` remains mandatory until real calibration/repeatability is implemented. The Basler operator preview/focus path requires a visible Windows pylon live-stream camera window for manual focus/alignment and is not autofocus; saved PNG/HTML diagnostics alone do not satisfy preview acceptance. The operator preview uses pylon continuous acquisition with latest-frame display and stale-frame dropping from a compiled .NET frame-pump thread, and may display a portrait-rotated view plus overlay/sidebar controls for ergonomics while raw capture orientation remains unchanged. Preview Leimac lighting controls are explicit, low-duty, coalesced through a compiled .NET worker, safe-off on exit, and never save persistent Leimac/Basler User Sets; when the operator accepts preview, the selected duty/channels may become a local software active fixed-rig lighting profile for subsequent fixed-rig commands until CLI override or Reset Default. Preview PWM duty is rounded to the Leimac 0000-0999 step scale, setup light uses low-duty `W11` plus `W86` output-enable rather than the grading trigger profile, and duty-only updates on unchanged channels send only `W11` so the hardware does not flash through safe-off/re-enable. Basler remains fixed overhead and does not zoom automatically; later corner/edge/surface screening uses full-resolution image ROIs/crops. Raw Basler evidence stays in `basler_sensor_pixels`; derived report/display images and ROI crops may use a portrait display transform with explicit metadata. Pixel/mm estimates must account for raw/display orientation, warn on X/Y inconsistency, and remain uncalibrated. Overlay preview PNGs are debug artifacts only and must not be baked into raw evidence images, and overlay alignment must report template/detected boundary metrics. Leimac 8-channel characterization labels channels numerically until measured/reviewed; physical channel mapping must not be invented. A fixed-rig V1 evidence package may capture front/back dark, all-on, accepted-profile, and channels 1-8 as `macro_fixed_rig_v1_uncalibrated` evidence with no final/certified claim. Mark's operator-built fixed-position V1 fixture now supports `fixed_metric_rulers` as the preferred local reference: the operator records horizontal and vertical ruler-span pixel coordinates plus known millimeter distances, and the helper computes `pixelsPerMmX/Y` and `mmPerPixelX/Y` from the rulers. Card dimensions may be used as a cross-check/fallback only, not as production-candidate scale. Non-certified references must be marked `ruler_reference_unvalidated` or `rough_reference_unvalidated`, lens distortion and homography remain not computed, and `isCalibrated=false` remains mandatory. Production-candidate diagnostic status requires ruler calibration, strict framing/margin gate, overlay alignment, remove/re-place repeatability, locked lighting profile metadata, and final light-off confirmation; a card touching the frame edge is a framing failure. Repeatability tests may report no-touch and remove/re-place diagnostic variation, but repeatability alone does not create production calibration. The 2026-06-30 Dell rough-fixture smoke accepted live preview at about 20.5 FPS, captured rough fixture calibration, five no-touch repeatability images, and front/back uncalibrated evidence packages at 1.4% duty with dark/all-on/accepted-profile/channel 1-8 images; it still reported near-frame-edge and pixel/mm consistency warnings, so it remains rough diagnostic evidence only. A later 2026-06-30 fixed-ruler smoke used 50.8mm horizontal/vertical ruler spans, an operator-entered card boundary, and five remove/re-seat captures; Mark accepted the fixture positioning/re-seat geometry as passed for diagnostic production-candidate fixture use, with scale consistency and overlay/framing passing, while lighting clipping remains a per-card tuning warning and `isCalibrated=false` still applies. PR #40 provisional diagnostic outputs may compute centering only when fixed-ruler scale, framing, and overlay gates pass; corner/edge metrics and 8-channel surface candidates remain proxy evidence labeled `provisional_diagnostic`, with `finalGradeComputed=false` and no certificate or certified claim. The 2026-06-30 PR #40 supervised smoke accepted a 20.49 FPS live preview and 1.2% Leimac duty profile, captured front/back fixed-rig evidence packages with dark/all-on/accepted-profile/channel 1-8 images, computed provisional centering/corner/edge/surface diagnostics, and confirmed final light-off; clipping remained high, especially on the back, so the run passes the provisional evidence workflow but not lighting-quality acceptance for reliable scoring. PR #40 now also includes `ai-grader-fixed-rig-v1-card-report`, an offline front+back report combiner that reads existing front/back evidence-package folders and writes a single premium card-level provisional diagnostic report. The report must include Ten Kings branding, session metadata, `Diagnostic Grade Pending`, front/back portrait evidence, overlays, ROI crops, per-side centering/corner/edge/surface diagnostics, 8-channel evidence, clipping warnings, and a technical appendix while preserving `isCalibrated=false`, `finalGradeComputed=false`, and `certifiedClaim=false`. Multi-light surface screening and Dino-Lite target recommendations remain future work after mapping and quality review. A circular ring reflection observed during PR #39 preview review is an unresolved specular-glare limitation; candidate mitigations include lower exposure/duty, selective channels, multi-light profiles, dark hood/ambient control, cross-polarization, diffusion, and geometry changes, but no code or report may claim the reflection is solved until controlled optical testing proves it. PR #41 starts the internal AI Grader Station operator workflow on top of PR #39/#40. The station workflow models Start New Card, Verify Fixture/Rulers, Live Preview / Focus / Framing, Lighting / Exposure Tune, Accept Capture Profile, Capture Front, Prompt Flip Card, Capture Back, Run Provisional Diagnostics, View Unified Report, Rerun If Warnings, Export/Open Report, and Safe Off / End Session. PR #41 keeps mock mode for automated testing, but `ai-grader-station-operator-workflow --apply` is now a supervised hardware-capable CLI station orchestrator: it launches the visible Basler live preview, captures front evidence, prompts for fixture/light/flip/final-light confirmations at the actual supervised workflow points, captures back evidence only after flip confirmation, generates the unified front/back provisional diagnostic report, and runs Leimac safe-off through the existing guarded commands. It must not run hardware unattended. The station layer includes a guided clipping-aware lighting/exposure tune recommendation, explicit provisional diagnostic scoring rules V0 for Centering/Corners/Edges/Surface, premium station report structure, report open/export metadata, command-plan/result recording, operator confirmation records, and a future Ten Kings integration contract with reserved report/grade/label fields. The 2026-07-01 Dell PR #41 supervised station smoke functionally proved the real station workflow with a visible live preview, accepted `1.3%` duty / PWM `13` / channels `1-8` profile, front/back evidence packages, unified provisional diagnostic report, and safe-off. Its station manifest remains `blocked` because final physical light-off was confirmed after a separate explicit safe-off command and chat confirmation rather than inside the station prompt; Mark explicitly accepted that external closeout before merge. It does not add migrations or DB writes, does not save persistent Basler/Leimac User Sets, does not generate a final grade, and does not create a label, QR report, certificate, or certified grading claim. PR #42 adds Ten Kings Vision Lab V0 to the unified report as a local static interactive inspection widget. Vision Lab supports True View, Surface Vision V0, Heatmap, Light Sweep Wheel, Measurement Overlay, Confidence Lens, Evidence Replay, front/back toggle, zoom/pan, severity filters, and Collector/Expert mode. It reuses existing front/back evidence packages and embedded report refs, labels channel directions numerically until physical mapping is confirmed, uses fixed-ruler metadata only when available, and records missing-data states instead of guessing. Vision Lab is premium evidence inspection only: `Surface Vision V0` is directional light evidence visualization, not certified photometric stereo; evidence remains `macro_fixed_rig_v1_uncalibrated`, `isCalibrated=false`, `finalGradeComputed=false`, and `certifiedClaim=false`.

PR #43 adds `preliminary_surface_intelligence_v0`, the first real multi-light surface analysis layer for Vision Lab. It is software-only by default and uses existing front/back fixed-rig evidence packages unless Mark explicitly approves new hardware. The pipeline reads portrait Leimac channel `1-8` images, applies dark-aware directional response normalization when compatible inputs exist, creates clipping/glare and underexposure confidence masks, generates a Surface Vision false-color contrast image, generates a cool-to-red anomaly heatmap, and emits conservative surface candidate boxes with evidence refs. Candidate source attribution remains numeric Channel `1-8` while physical Leimac direction mapping is pending; do not invent north/east/etc. Confidence must decrease for high clipping/glare, underexposure, missing channels, focus/framing warnings, or calibration/ruler warnings. Vision Lab Heatmap, Surface Vision, and Evidence Replay use these artifacts when present and display clean insufficient-data states when absent. This is not a final surface grade, not certified photometric stereo, and not production certification; `isCalibrated=false`, `finalGradeComputed=false`, and `certifiedClaim=false` remain mandatory.

PR #44 adds light-direction calibration and photometric-stereo preparation artifacts only. A local `LeimacLightDirectionCalibrationProfile` may record channel metadata, intensity scale factors, flat-field status, normal-map status, source evidence refs, warnings, and `isCertifiedPhotometricStereo=false`. Until a controlled physical light-vector calibration target is captured and reviewed, channel directions are `unknown` or `approximate_directional_model`; the helper may use an approximate even-ring model only for preliminary normal/relief proxy visualization, and channel labels remain numeric. The fixed-rig report may generate normalized channel images, a preliminary normal proxy, gradient magnitude proxy, relief proxy, confidence map, and channel-balance panel from existing channel evidence. These artifacts must be labeled `Preliminary normal/relief proxy - approximate directional model`, must not be described as certified photometric stereo, must not produce a final surface grade, and must preserve `finalGradeComputed=false`, `certifiedClaim=false`, and `isCertifiedPhotometricStereo=false`. If no flat-field/reference capture exists, fallback normalization is allowed only with an explicit warning and reduced confidence.

PR #45 adds the first controlled `provisional_diagnostic_grade` rules layer and Grade Story Engine for the fixed-rig report. A provisional overall grade and element scores may be shown only when ruler calibration, repeatability, framing/overlay, front evidence, back evidence, Surface Intelligence, clipping, and focus/sharpness gates pass or are explicitly accepted as diagnostic warnings. Hard gate failures must return `insufficient_evidence` with the failed gate reasons and no provisional score. The story layer must emit evidence-linked claims, confidence explanations, `Why Not 10?` reasons, grade-impact candidates, and formula/cap-rule metadata for Centering, Corners, Edges, and Surface. It may use Surface Intelligence V0 and PR #44 normal/relief/confidence artifacts, but every score remains provisional diagnostic only. Reports and JSON must preserve `certificationStatus=not_certified`, `finalGradeComputed=false`, `certifiedClaim=false`, `labelGenerated=false`, `qrGenerated=false`, and `certificateGenerated=false`; no label, QR report, certificate, or certified/final Ten Kings grade is generated.

The local station/web viewer pass separates the Dell operator experience from the future public customer report viewer. The Dell operator station may run locally at `http://127.0.0.1:3020/ai-grader/station` with no Ten Kings login; the production operator station is `https://collect.tenkings.co/ai-grader/station` and requires the approved Ten Kings operator/admin login for production publish/search/slab/comps actions. PR #46 keeps a mock/contract fallback for development but adds a real local loopback capture-station bridge command, `ai-grader-station-bridge`, that lets the browser call a token-gated `127.0.0.1` capture-helper service on the Dell PC. Real bridge mode is disabled by default and requires explicit local station enablement, a local station token, loopback host binding, origin allowlisting, `--apply`, Mark-present/wiring/status flags, and staged operator confirmations before invoking the existing fixed-rig station command plan. PR #51 productizes this bridge for production operator use: `scripts/ai-grader/install-local-station-bridge.ps1 -StartNow -CreateShortcut` installs a Windows Scheduled Task at user logon, stores local bridge config at `C:\TenKings\config\ai-grader-local-bridge.json`, locks the config ACL where possible, passes the local station token to the child bridge process through environment rather than task/shortcut command-line arguments, and creates a `Ten Kings AI Grader Station` launcher. The production station auto-checks `http://127.0.0.1:47652/health`, auto-connects when the Dell browser has a saved local token, and pairs first-time browsers through a short local pairing code exchanged at `POST /pair`; the raw station token is stored only in this browser's `localStorage` and is never the production service-account token. PR #57 adds a browser-embedded local preview stream: `GET /preview/status` and `GET /preview/stream` are loopback-only, token-gated, origin-limited, and never public report routes. Real mode uses a Basler pylon continuous-grab MJPEG stream in portrait orientation and does not command Leimac lighting, save persistent hardware settings, or use production service-account auth. Capture actions must explicitly pause/release preview ownership before running the existing capture commands, then the station reconnects preview when the workflow returns to idle. The shareable report viewer route `/ai-grader/reports/[reportId]` remains read-only and has no hardware controls; it consumes a web-ready report bundle with provisional grade/story, Vision Lab artifacts, evidence refs, calibration/lighting metadata, checksums, and reserved future storage/label/QR/certificate fields. Generated local report IDs must resolve through the token-gated local bridge, not through fixture data: the bridge exposes read-only `GET /reports/<reportId>/bundle`, `GET /reports/<reportId>/html`, and `GET /report-history` endpoints, and the browser report route must show an explicit local-bridge-needed state if the generated report cannot be resolved. After a bridge restart, `/status.latestReport` must fall back to the newest existing generated local report found in station history so the station `View Report` control remains usable for the last completed card. The 2026-07-02 PR #46 retest verified `ai-grader-browser-station-session-2026-07-02T035658313Z-report` rendered through the browser report route as the generated bundle with provisional grade `6.69`, not the `sample-pr45` fallback, and Card History showed the generated local report path/grade. The station page is a cockpit-style local workflow with a large camera workspace, embedded preview, alignment/grid overlay, connection scrim, right sidebar, Start New Card, Start Grading, flip-card scrim, View Report, Card History Reports, Safe Off, path summary, and timing summary. PR #57 records detailed cold-path timing fields for preview start/first frame, Basler open/configure/grab/save/hash/close-dispose, Leimac write/ack/safe-off, front/back package totals, report generation, local report open, and publish/upload placeholders. It does not reduce the forensic workflow: dark/all-on/accepted-profile/channel `1-8`, front/back, ROI/display artifacts, Surface Intelligence, Vision Lab, and unified report remain required. Reducing inter-image delay toward about one second requires a future warm-session capture runner that avoids per-image process startup and repeated hardware setup while preserving synchronized lighting, evidence completeness, and safe-off behavior.

PR #58 establishes the actual Dell warm-runner execution path as the default for the local station. The normal path is `warm_full_forensic_runner`; `cold_command_fallback` may remain only for explicit developer/operator debug disable selected before the session. A warm-runner failure must stop, safe-off, release ownership, expose the exact failure, and require retry; it must never switch to the cold path automatically. The warm path opens/configures Basler once per side batch, keeps per-side persistent Leimac ownership where safe, applies tight channel changes, preserves dark control, all-on, accepted profile, Leimac channels `1-8`, ROI/display crops, Surface Intelligence, Vision Lab, unified report generation, and safe-off cleanup. Current session manifests and timing summaries record `executionPath`, `explicitColdDebugModeUsed`, and `explicitColdDebugReason`; the historical PR #58 evidence below used the earlier `fallbackUsed` / `fallbackReason` field names. The 2026-07-06 supervised Dell PR #58 smoke used report id `pr58-warm-smoke-20260706T051208Z` and confirmed `executionPath=warm_full_forensic_runner`, `fallbackUsed=false`, full front/back evidence completeness, token-gated report HTML `200`, local report open, and final physical light off. Measured command timings were front capture `9442 ms`, back capture `9243 ms`, front processing queue `4229 ms` during flip, back processing queue `3611 ms`, unified report `14867 ms`, first safe-off `1441 ms`, and preview first frame `1764 ms`. The bridge report-ready timing summary before cleanup was `74944 ms`, materially better than the previous about `461 s` baseline and inside the 60-150 second target. The same smoke's chat-mediated wall clock was `191786 ms` from first capture start to report ready because operator flip and report-trigger waits were handled through chat; that wall-clock number should not be used as the warm runner's machine-speed baseline. Public report routes remain read-only and hardware-control-free, and local preview/bridge auth uses only the browser-local station token.
...
The production publication API must use `publish-init`, direct presigned storage uploads, and `publish-finalize`: the station sends only small manifest JSON to Vercel, uploads sanitized report bundles, production-release data, label data, publication manifests, integration contracts, label previews, asset manifests, checksums, and evidence assets directly to storage, then sends a small upload manifest for DB persistence. Image/base64 bodies must not pass through Vercel request or response bodies. Public report reads must use the read-only persisted report endpoint gated by `AI_GRADER_PUBLIC_REPORT_DB_ENABLED=true`, and public bundles must not leak Dell local paths or loopback bridge URLs.
...
Credential exposure checkpoint: if the production database connection string is exposed during rollout, the rollout must stop before env gate changes, restarts, storage uploads, publication smokes, or public report validation. The affected DigitalOcean managed PostgreSQL user's password must be rotated through the approved DigitalOcean cluster user reset flow or an approved authenticated API path, and all approved runtime envs must receive the rotated `DATABASE_URL` without printing the value. Verification may use only redacted evidence such as connection success, migration status, host/database match, length, or a non-secret fingerprint.

2026-07-03 production rollout exception: Mark explicitly accepted the temporary risk of continuing with the exposed production Postgres credential and deferred DigitalOcean credential rotation until after AI Grader rollout. This exception allowed redacted production verification and smoke checks without printing secrets; it does not remove the requirement to rotate the exposed credential after rollout and update every approved runtime `DATABASE_URL` securely.
...
Current production rollout state: the migration was applied during the approved 2026-07-02/2026-07-03 rollout and Prisma now reports the production schema up to date with the seven `AiGrader*` tables present. Hosted persisted public report reads remain pending on confirmed Vercel production env and storage configuration.

2026-07-03 Vercel env follow-up: hosted production runtime behavior confirmed AI Grader publish, public-read, and live-comps gates active after Mark added Vercel Production env vars. The previous direct-DB smoke report is not a valid public report proof because it was not created through the storage-upload API path. Production acceptance requires a fresh authenticated publish that calls `publish-init`, uploads the report bundle/release/label/publication/integration/asset manifest/checksum artifacts and evidence assets directly to production storage, calls `publish-finalize` to persist the DB rows, and verifies the public viewer resolves stored data. Unknown generated report IDs must render missing-data/local-bridge-needed states rather than substituting fixture/sample data.
...
2026-07-03 production-live result: PR #49 permanent auth and PR #50 public-output security fix were merged and deployed to Vercel Production. The final hosted production smoke used permanent service-account auth and produced `reportId=ai-grader-prod-smoke-20260703T212555`, `certId=TK-AIG-6C033F43`, public report URL `https://collect.tenkings.co/ai-grader/reports/ai-grader-prod-smoke-20260703T212555`, and label preview URL `https://collect.tenkings.co/ai-grader/labels/ai-grader-prod-smoke-20260703T212555`. Production status showed publish, public DB reads, live comps, human operator/admin auth, service account auth, all five service scopes, and `noHardwareControls=true`. Publish/storage readback verified the seven expected artifacts; public report/label/QR loaded persisted data; slabbed front/back photo upload persisted; manual draft identity avoided production inventory changes; one controlled live comps request completed and persisted with zero comp refs; history/latest found the fresh report; wrong-token/no-auth/public-POST security checks returned `401`/`401`/`405`; public output scans found no secret markers, Dell local paths, bridge URLs/tokens, or hardware controls. AI Grader is production-live under the permanent auth model. Deferred follow-up remains DigitalOcean Postgres credential rotation after rollout.

## Appendix D. Normalization-Safe Solid-Plate Geometry Contract

For the fixed overhead Basler workflow, operator placement and card grading coordinates are separate concerns:

- `Ready` requires fresh, confident, fully visible, card-shaped geometry with safe frame clearance and the fixed-rig `30%-85%` expected coverage envelope, which includes the existing 97%-height production guide. Camera-frame center offset is guidance rather than a direct gate. Rotation through the `35°` automatic-normalization envelope is allowed even when it exceeds the preferred `10°` placement guide, provided the complete card remains safe to normalize.
- `Adjust Card` is limited to detected geometry that is clipped, low-confidence, wrong-aspect, outside safe size/coverage, or beyond the automatic-normalization rotation envelope. `Not Detected` means no credible card candidate. Neither state may silently substitute fixture/manual geometry.
- The live detector models a solid black or white plate from border color, closes small segmentation gaps, fills enclosed artwork holes, rejects weak non-rectangular candidates, estimates an oriented outer-card rectangle, and processes only the newest preview frame. It does not claim independent four-line/projective corner fitting.
- A captured automatic side must independently reproduce coherent full-resolution detected geometry. If it cannot, processing stops and requires reposition/retry. Explicit manual capture remains separately authorized and labeled.
- All-on supplies the authoritative side transform. Accepted-profile and channels `1-8` reuse that transform into a fixed `1200x1680` portrait coordinate frame. A successful full-resolution transform requires at least `1000x1400` source pixels and no more than `1.2x` upscaling. Corner, edge, surface, and report assets use that normalized frame; camera-frame placement is acquisition-only metadata and cannot become a grade signal. The current fixed-overhead transform corrects translation and in-plane rotation but does not claim lens undistortion or projective homography.
- Directional-light calculations must rotate their source sensor vectors by that same authoritative deskew before interpreting normalized channel images. Missing or mixed coordinate provenance suppresses directional normal/relief results. Raw dark controls cannot be subtracted from normalized-card images without coordinate-frame and dimension registration, and V0 must not claim pixelwise flat-field correction when it only performs channel-intensity balancing.
- When Surface Intelligence runs on a canonical normalized artifact, defect candidate rectangles are already in `normalized_card_portrait_pixels`; projection to public `normalized_card` fractions divides by the exact `1200x1680` artifact dimensions once. It must validate normalized image/artifact and raw-source fingerprints and must not apply the raw/display inverse transform or deskew again. Front/back joins stay side-scoped, incoherent geometry fails closed, and a candidate-free report emits no markers.
- Geometry cannot distinguish printed top from bottom. The operator must keep the printed top generally toward the preview top; metadata records that semantic orientation is operator-assumed rather than machine-verified.
- Printed-design centering remains uncomputed. For normalized evidence, Grade Story V0.2 may compute a capped, confidence-penalized provisional result from complete corner/edge/surface diagnostics with the missing centering weight explicitly redistributed; it must persist that policy and must never substitute acquisition placement. Both side geometry-provenance records are required, with explicit manual capture remaining an accepted warning and incoherent provenance failing closed.
- PNG encoding is lossless, but normalization metadata must separately disclose crop dimensions, scaling, interpolation/resampling, and any upscaling. Raw evidence is immutable and all 11 side roles are hash/size verified after derived processing.
- Printed-design centering must remain `not_computed` until a normalized artwork/border centering algorithm exists. Camera-frame margins must never be presented as printed card centering.
- A stale or failed detector result must clear capture readiness. Manual capture stays an explicit operator confirmation, never an automatic fallback.
- Production and the Dell helper must negotiate the same bridge contract. An incompatible bridge must show update/restart-required and fail closed; it must not send unknown actions or silently change paths.

This software contract does not prove perfect detection or Dell latency. Identical card-edge/base-plate color, glare, clipping, blur, and optical perspective can still remove observable boundary information. A matte, contrasting plate and a Mark-approved Dell validation set are required before claiming production accuracy or real-time timing.
