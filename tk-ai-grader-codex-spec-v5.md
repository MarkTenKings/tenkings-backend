...
The local station/web viewer pass separates the Dell operator experience from the future public customer report viewer. The Dell operator station may run locally at `/ai-grader/station` with no Ten Kings login and must eventually orchestrate the fixed-rig preview/capture/report workflow, but the first browser bridge is contract/mock only until an approved local process bridge connects it to `ai-grader-station-operator-workflow --apply`. The shareable report viewer route `/ai-grader/reports/[reportId]` is read-only and has no hardware controls; it consumes a web-ready report bundle with provisional grade/story, Vision Lab artifacts, evidence refs, calibration/lighting metadata, checksums, and reserved future storage/label/QR/certificate fields. This pass must not add DB migrations or DB writes, must not upload artifacts, must not run hardware unattended, and must preserve `finalGradeComputed=false`, `certifiedClaim=false`, `labelGenerated=false`, `qrGenerated=false`, and `certificateGenerated=false`.
