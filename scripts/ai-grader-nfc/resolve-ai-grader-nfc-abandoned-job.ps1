...
$requiredConfirmation = "I removed and quarantined the exact F8215 tag for this attempt."
...
$goToTags = @(Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Where-Object {
  [int]$_.ProcessId -ne $PID -and (
    ([string]$_.Name -match '(?i)gototags|desktopapp') -or
    ([string]$_.CommandLine -match '(?i)gototags')
  )
})
...
try {
  ...
  [Environment]::SetEnvironmentVariable($variables[2], $Confirmation, "Process")
  ...
  if ($result.resolution -cne "quarantined_abandoned_job_resolved" -or
      ...
      [string]$result.attemptFingerprintSha256 -cnotmatch '^[a-f0-9]{64}$') {
    throw "The abandoned-job resolver returned an unsafe result."
  }
  $auditPath = Join-Path $jobRoot "abandoned-job-audit.jsonl"
  if (Test-Path -LiteralPath $auditPath) { Protect-NfcPath -Path $auditPath -AllowedRoot $script:NfcConfigRoot }
  [pscustomobject]@{
    ok = $true
