...
param(
  ...
  [int]$Port = 47653,
  [string]$ChromeUserDataDir = "C:\TenKings\chrome-ai-grader-mathematical-calibration-v1"
)
...
if ($Port -ne 47653) {
  throw "Mathematical Calibration V1.0.1 is restricted to protected loopback port 47653."
}
...
$pageUrl = "$($bridgeUri.AbsoluteUri.TrimEnd('/'))/calibration/mathematical-v1?sessionId=$([Uri]::EscapeDataString($SessionId))#aiGraderBridgePair=$([Uri]::EscapeDataString([string]$config.pairingCode))"
$chromeArgs = @(
...
[pscustomobject]@{
  ok = $true
  pagePath = "/calibration/mathematical-v1"
