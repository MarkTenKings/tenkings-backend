...
import {
  assessStaffInventoryResearchComparison, compareStaffInventoryResearchCondition, inspectStaffInventoryResearchSale,
  inspectStaffInventoryResearchTitle, readStaffInventoryResearchGradeEvidence, staffInventoryResearchYearForms,
} from '../lib/server/staffInventoryResearchDecisions';

const description = (changes: Partial<StaffInventoryResearchDescription> = {}): StaffInventoryResearchDescription => ({
...
const candidate = (title = '2023-24 Panini Contenders #77 Jayson Tatum Boston Celtics', changes: Partial<StaffInventoryResearchCandidate> = {}): StaffInventoryResearchCandidate => ({
...
const comparison = (changes: Partial<StaffInventoryResearchComparison> = {}): StaffInventoryResearchComparison => ({
...
const raw: StaffInventoryResearchResult['target_condition'] = { status: 'raw', grader: null, numeric_grade: null, photo_evidence: 'The uploaded card is ungraded.' };
const graded = (grader: 'PSA' | 'BGS' | 'SGC' | 'CGC', numeric_grade: number): StaffInventoryResearchResult['target_condition'] => ({ status: 'graded', grader, numeric_grade, photo_evidence: `The uploaded label reads ${grader} ${numeric_grade}.` });
...
const reference = (saved: StaffInventoryResearchDescription): StaffInventoryResearchReference => ({
...
test('season aliases do not weaken an actual wrong season or a saved individual-year identity', () => {
  for (const [saved, year] of [[description(), '2024-25'], [description(), '2022'], [description({ year: '2023' }), '2024']] as const) {
...
test('private grade parsing handles known descriptors and preserves exact grading companies and fractions', () => {
  for (const [text, grader, numeric] of [['PSA NM–MT 8', 'PSA', 8], ['PSA GEM-MINT 10', 'PSA', 10], ['BGS GEM MINT 9.5', 'BGS', 9.5], ['SGC 9', 'SGC', 9], ['CGC 8.5', 'CGC', 8.5]] as const) {
    const result = readStaffInventoryResearchGradeEvidence(text, null);
    assert.equal(result.status, 'graded', text); assert.equal(result.grader, grader); assert.equal(result.numeric_grade, numeric);
  }
});
...
test('true wrong exact grade, grader and raw-versus-graded controls stay rejected', () => {
  for (const [label, condition] of [['PSA 9', graded('PSA', 8)], ['BGS 8', graded('PSA', 8)], ['Raw', graded('PSA', 8)], ['PSA 8', raw]] as const) {
...
test('missing images and downloaded-but-unassessed images are separate from completed visual evidence', () => {
  ...
  assert.equal(missing.assessment.classification, 'possible'); assert.equal(missing.evidence.image, 'missing');
  ...
  assert.equal(pending.assessment.classification, 'possible'); assert.equal(pending.evidence.image, 'acquired_unassessed');
...
test('a research match does not supply missing final title, sold-price or catalog evidence', () => {
  ...
  assert.equal(listing.source_eligible, false); assert.equal(listing.sold_price_cents, null);
});
...
test('a supplied catalog reference cannot substitute a different saved product or number', () => {
  for (const changes of [{ set_name: 'Phoenix' }, { set_name: 'Contenders Football' }, { set_name: null }, { card_number: '383' }, { year: '2024-25' }, { name: 'Jaylen Brown' }]) {
    const other = reference(description()); Object.assign(other.identity, changes);
...
test('model mismatch and uncertainty never become a match just because deterministic anchors pass', () => {
  for (const flag of ['identity_match', 'variant_match', 'visual_match', 'condition_match'] as const) {
