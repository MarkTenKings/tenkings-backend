// Synthetic worker response shaped from the independently verified Release 26
// image configuration, build log, and signed source. No provider is invoked.
export function currentSpeedsterIdentityFixture() {
  return {
  version: "speedster-detector-identity-v1",
  detectorVersion: "sam3-local-box-inspection-2mm@96914d2425f90a64f45ca977c2b5165418099543",
  source: {
    repository: "https://github.com/MarkTenKings/tenkings-backend",
    commitSha: "7bde06f65959e376e12725186d605cc91c67852b",
    treeSha: "2a323e3889c94ad638b62a81166275f3a098efd9",
  },
  runtime: {
    ociDigest: "sha256:9339614adc8183a87d0df4051c09fc88350f76fe477925d231f718edf393cfd3",
    ociDigestProvenance: "DEPLOYMENT_INJECTED",
    ociImageReference: "ghcr.io/marktenkings/tenkings-backend-speedster-v2:7bde06f65959e376e12725186d605cc91c67852b-32511140945-1",
    buildId: "32511140945-1",
    buildIdentityProvenance: "OCI_IMAGE_ENV",
    platform: "linux/x86_64",
    pythonVersion: "3.12.14",
    frameworkVersion: "sam3@96914d2425f90a64f45ca977c2b5165418099543",
    torchVersion: "2.7.1+cu126",
    cudaVersion: "12.6",
    cudnnVersion: "90501",
    accelerator: "NVIDIA GeForce RTX 4090",
    gpuName: "NVIDIA GeForce RTX 4090",
    gpuCapability: "8.9",
    gpuCount: 1,
    gpuPolicy: {
      policyVersion: "speedster-rtx4090-only-v1",
      required: { gpuName: "NVIDIA GeForce RTX 4090", gpuCapability: "8.9", gpuCount: 1 },
      observed: { gpuName: "NVIDIA GeForce RTX 4090", gpuCapability: "8.9", gpuCount: 1, currentDevice: 0 },
      validation: "OBSERVED_CUDA_RUNTIME_BEFORE_MODEL_LOAD",
    },
    compiler: {
      contractVersion: "speedster-host-compiler-v1",
      ccPath: "/usr/bin/gcc-14",
      packageName: "gcc-14",
      packageVersion: "14.2.0-19",
      libcDevPackageName: "libc6-dev",
      libcDevPackageVersion: "2.41-12+deb13u3",
      ccSha256: "f".repeat(64), // Synthetic observed hash; no hardware execution claimed.
      version: "14.2.0",
      target: "x86_64-linux-gnu",
      validation: "IMMUTABLE_IMAGE_STARTUP_AND_PRE_TORCH",
    },
  },
  model: {
    name: "sam3-speedster",
    repository: "facebook/sam3",
    revision: "3c879f39826c281e95690f02c7821c4de09afae7",
    checkpointSha256: "9999e2341ceef5e136daa386eecb55cb414446a00ac2b55eb2dfd2f7c3cf8c9e",
    sourceCommitSha: "96914d2425f90a64f45ca977c2b5165418099543",
  },
  policy: {
    detectorVersion: "sam3-local-box-inspection-2mm@96914d2425f90a64f45ca977c2b5165418099543",
    promptVersion: "sam3-box-and-smart-mark-point-v1",
    fusionVersion: "speedster-side-wide-memory-cap-v2",
    measurementVersion: "speedster-exact-canonical-mask-v1",
    memoryVersion: "sam-memory-v2-lesson-verdict-v1",
  },
  determinism: {
    deterministicAlgorithms: true,
    cudnnDeterministic: true,
    cudnnBenchmark: false,
    allowTf32: false,
    evalMode: true,
    compile: false,
    autocastDtype: "bfloat16",
    cublasWorkspaceConfig: ":4096:8",
  },
} as const;
}

export function emptySpeedsterMemoryBankFixture() {
  return {
    version: 2,
    fingerprintVersion: "sam3-fpn32-inspection-2mm@96914d2425f90a64f45ca977c2b5165418099543",
    capacityPerTypePolarity: 50,
    calibration: { status: "CALIBRATED", tau: 0.8, margin: 0.1 },
    replayCursor: null,
    exemplars: [],
  } as const;
}

export function emptySpeedsterLessonVerdictsFixture(side: "FRONT" | "BACK") {
  return {
    version: "speedster-memory-lesson-side-verdicts-v1",
    side,
    loadedLessonCount: 0,
    verdicts: [],
  } as const;
}

export function alteredSpeedsterIdentityFixture(path: string, value: unknown): unknown {
  const identity = structuredClone(currentSpeedsterIdentityFixture());
  const keys = path.split(".");
  let object = identity as unknown as Record<string, unknown>;
  for (const key of keys.slice(0, -1)) object = object[key] as Record<string, unknown>;
  object[keys.at(-1)!] = value;
  return identity;
}

export const rejectedSpeedsterIdentityFields = [
  ["version", "speedster-detector-identity-v0"],
  ["detectorVersion", "older-detector"],
  ["source.repository", "https://github.com/example/other"],
  ["source.commitSha", "a".repeat(40)],
  ["source.treeSha", "a".repeat(40)],
  ["runtime.ociDigest", `sha256:${"a".repeat(64)}`],
  ["runtime.ociDigestProvenance", "WORKER_REPORTED"],
  ["runtime.ociImageReference", "ghcr.io/example/worker:latest"],
  ["runtime.buildId", "32511140945-2"],
  ["runtime.buildIdentityProvenance", "ENV_OVERRIDE"],
  ["runtime.platform", "linux/arm64"],
  ["runtime.pythonVersion", "3.12.4"],
  ["runtime.frameworkVersion", "sam3@older"],
  ["runtime.torchVersion", "2.7.1+cpu"],
  ["runtime.cudaVersion", "12.8"],
  ["runtime.cudnnVersion", "91002"],
  ["runtime.accelerator", "CPU"],
  ["runtime.gpuName", "NVIDIA L4"],
  ["runtime.gpuCapability", "8.0"],
  ["runtime.gpuCount", 2],
  ["runtime.gpuPolicy", undefined],
  ["runtime.gpuPolicy.observed.gpuName", "NVIDIA L4"],
  ["runtime.gpuPolicy.observed.currentDevice", 1],
  ["runtime.gpuPolicy.validation", "UNOBSERVED"],
  ["runtime.compiler", undefined],
  ["runtime.compiler.packageVersion", "14.2.0-18"],
  ["runtime.compiler.ccSha256", undefined],
  ["model.name", "other-model"],
  ["model.repository", "other/sam3"],
  ["model.revision", "a".repeat(40)],
  ["model.checkpointSha256", "a".repeat(64)],
  ["model.sourceCommitSha", "a".repeat(40)],
  ["policy.detectorVersion", "old-policy"],
  ["policy.promptVersion", "old-policy"],
  ["policy.fusionVersion", "old-policy"],
  ["policy.measurementVersion", "old-policy"],
  ["policy.memoryVersion", "sam-memory-v2"],
  ["determinism.deterministicAlgorithms", false],
  ["determinism.cudnnDeterministic", false],
  ["determinism.cudnnBenchmark", true],
  ["determinism.allowTf32", true],
  ["determinism.evalMode", false],
  ["determinism.compile", true],
  ["determinism.autocastDtype", "float32"],
  ["determinism.cublasWorkspaceConfig", ":16:8"],
] as const;
