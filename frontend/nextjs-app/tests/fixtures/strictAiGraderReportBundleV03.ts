...
function axis(axisName: "horizontal" | "vertical") {
  return {
    ...
    grade10ToleranceMm:
      MATHEMATICAL_GRADING_V1_THRESHOLD_MANIFEST.centering.grade10Tolerance
        .marginDifferenceMm,
    balanceRatio: 100,
    score: 10,
  };
...
function centeringSide(side: "front" | "back") {
  ...
  const observedArtifact = {
    ...
    intendedBoundaryProfileId:
      "standard_sports_card_63_50x88_90_r3_18_v1",
    intendedBoundaryProfileVersion: "1.0.0",
    contourAuthority: "canonical_pixel_derived_dense_contour",
    canonicalRawContourSha256: SHA,
    canonicalNormalizedContourSha256: SHA,
    rawContour: contour,
  ...
  return {
    ...
    outerCutGeometryEvidence: {
      ...
      intendedBoundaryProfileVersion:
        observedArtifact.intendedBoundaryProfileVersion,
...
function elementScore(
  formula: string,
  locationScores: ReturnType<typeof location>[] = [],
) {
  return {
    score: 10,
    startingScore: 10,
    ...
    formula,
    explanation:
      "No condition defect measured beyond U95 and the published Grade-10 tolerance.",
  };
}

function observationAssetId(
...
function conditionObservation(
  ...
  return {
    ...
    segmentationMaskAssetId: observationAssetId(
      element,
      side,
      locationName,
      "segmentation",
    ),
    confidenceMaskAssetId: observationAssetId(
      element,
      side,
      locationName,
      "confidence",
    ),
    illuminationMaskAssetId: observationAssetId(
      element,
      side,
      locationName,
      "illumination",
...
export function buildStrictAiGraderReportBundleV03Fixture(): AiGraderReportBundleV03 {
  const corners = ["top_left", "top_right", "bottom_right", "bottom_left"];
  const edges = ["top", "right", "bottom", "left"];
  const observations = {
    corners: (["front", "back"] as const).flatMap((side) =>
      corners.map((name) => conditionObservation("corners", side, name))),
    edges: (["front", "back"] as const).flatMap((side) =>
      edges.map((name) => conditionObservation("edges", side, name))),
  };
  const publicAssets = (["front", "back"] as const).flatMap((side) => [
    ...
    ...[
      ...
      asset(
        observationAssetId(element, side, name, "roi"),
        side,
        "roi_crop",
      ),
      asset(
        observationAssetId(element, side, name, "segmentation"),
        side,
        "segmentation_mask",
      ),
      asset(
        observationAssetId(element, side, name, "confidence"),
        side,
        "confidence_mask",
      ),
      asset(
        observationAssetId(element, side, name, "illumination"),
        side,
        "illumination_mask",
      ),
    ]),
  ...
  return {
    ...
    gradingStandard: {
      ...
      thresholdSetHash: MATHEMATICAL_GRADING_V1_THRESHOLD_SET_HASH,
      algorithmVersion:
        MATHEMATICAL_GRADING_V1_THRESHOLD_MANIFEST.algorithmVersion,
      defectFindingSchemaVersion: AI_GRADER_DEFECT_FINDING_V2_VERSION,
      designReferenceSchemaVersion:
        MATHEMATICAL_DESIGN_REFERENCE_V1_SCHEMA_VERSION,
    },
    productionRelease: {
      finalGrade: {
        ...
        elements: {
          centering: elementScore(
            MATHEMATICAL_GRADING_V1_THRESHOLD_MANIFEST.centering
              .frontBackFusion.formula,
          ),
          corners: elementScore(
            MATHEMATICAL_GRADING_V1_THRESHOLD_MANIFEST.corners.formula,
            (["front", "back"] as const).flatMap((side) =>
              corners.map((name) => location(side, name))),
          ),
          edges: elementScore(
            MATHEMATICAL_GRADING_V1_THRESHOLD_MANIFEST.edges.formula,
            (["front", "back"] as const).flatMap((side) =>
              edges.map((name) => location(side, name))),
          ),
          surface: elementScore(
      ...
      label: {
        certId: "CERT-MATH-V1",
        labelGradeText: "10.0",
        publicReportUrl: "/ai-grader/reports/math-v1-release-test",
        qrPayloadUrl: "/ai-grader/reports/math-v1-release-test",
      },
      publication: {
        publicReportUrl: "/ai-grader/reports/math-v1-release-test",
      },
    ...
    centeringEvidence: {
      ...
      deduction: 0,
      formula:
        MATHEMATICAL_GRADING_V1_THRESHOLD_MANIFEST.centering.frontBackFusion
          .formula,
      balanceCurve:
        MATHEMATICAL_GRADING_V1_THRESHOLD_MANIFEST.centering.balanceCurve,
    },
