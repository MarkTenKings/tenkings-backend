...
test("dedicated programming and public tap pages keep hardware controls out of Finish/public surfaces", async () => {
  ...
  assert.doesNotMatch(nfcPage, /Retry Current Attempt/);
  assert.match(nfcPage, /quarantine that exact tag and have an administrator revoke the current NFC link/i);
  ...
  assert.doesNotMatch(nfcPage, /Revoke and program replacement/);
  const replacementHandler = nfcPage.slice(nfcPage.indexOf("const replace = async"), nfcPage.indexOf("const revoke = async"));
  assert.match(replacementHandler, /if \(hosted\?\.status !== "revoked"\)/);
  const revocationHandler = nfcPage.slice(nfcPage.indexOf("const revoke = async"), nfcPage.indexOf("const confirmOverwrite"));
  assert.match(revocationHandler, /setReplacementRequest\(null\)/);
  const adminActions = nfcPage.slice(nfcPage.indexOf("hosted\?\.canAdmin"), nfcPage.indexOf("<footer>"));
  assert.match(adminActions, /programmingReady && hosted\.status === "revoked"/);
