...
type HandlerFactory = typeof import('../pages/api/admin/set-ops/catalog/sports-preparation').createSportsPreparationHandler;

test('sports preparation API requires enabled human review and explicit stage; it preserves the actor and request', async context => {
  const userId = 'offline-sports-api-reviewer';
  const environment = {
  ...
  const { createSportsPreparationHandler } = await import('../pages/api/admin/set-ops/catalog/sports-preparation');
  const handler = createSportsPreparationHandler(service);
  ...
  try {
    ...
    const { StaleSportsPreparationSnapshotError } = await import('../lib/server/setCatalogSportsPreparation');
    service.stage = async () => { throw new StaleSportsPreparationSnapshotError('fixture-exact-key', 'd'.repeat(64)); };
    const stale = await invoke('POST', { action: 'stage', request });
    assert.equal(stale.code, 409);
    assert.deepEqual(stale.result, { message: 'Preparation snapshot changed; preview again.', code: 'SPORTS_PREPARATION_SNAPSHOT_STALE',
