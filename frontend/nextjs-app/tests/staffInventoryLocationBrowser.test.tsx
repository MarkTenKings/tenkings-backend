...
async function mount(fetcher: typeof fetch) {
  ...
  const render = async (locations = [location]) => { await act(async () => root.render(<Browser locations={locations} items={[]} token="fixture-admin" onViewInventory={(key: string) => viewed.push(key)} />)); };
  await render();
