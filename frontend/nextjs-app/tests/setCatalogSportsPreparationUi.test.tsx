...
import Preparation from '../components/admin/SetCatalogSportsPreparation';
const { JSDOM } = require('jsdom');
(globalThis as typeof globalThis & { React: typeof React }).React = React;

const setId = '2023_Bowman_University_Chrome_Football';
const storageKey = 'tenkings:pending-sports-catalog-preparation:v1';
const preview = { schemaVersion: 'setops-sports-additive-preparation-preview/v1', snapshotSha256: 'a'.repeat(64),
  proposalSha256: 'b'.repeat(64), creates: { pendingJobs: 2, sources: 2, parallels: 3, scopes: 3 } };
const confirmed = (outcome = 'recorded') => ({ outcome, receiptSha256: 'c'.repeat(64),
  receipt: { schemaVersion: 'setops-sports-additive-preparation-receipt/v1', baselineSha256: preview.snapshotSha256, proposalSha256: preview.proposalSha256 } });
async function mount() {
  const dom = new JSDOM('<div id="root"></div>', { url: 'https://collect.tenkings.co/admin/set-ops-review' });
  const previous = new Map<string, PropertyDescriptor | undefined>();
  const calls: Array<{ url: string; init: RequestInit }> = [];
  let post = async (_body: string): Promise<Response> => Response.json(confirmed());
  let get = async (): Promise<Response> => Response.json(preview);
  const fetcher: typeof fetch = async (url, init) => {
    assert.equal(url, '/api/admin/set-ops/catalog/sports-preparation');
...
test('pilot UI is scoped, never auto-runs and separates preview from explicit preparation', async () => {
  ...
  try {
    ...
    assert.match(ui.host.textContent!, /All nine card-to-printing relationships remain unknown/);
...
test('uncertain response survives remount and retries the exact original bytes without a new key', async () => {
  ...
  try {
    ...
    ui.setPost(async () => Response.json(confirmed('replay')));
...
test('double clicks dispatch once and a late response cannot clear the request after context changes', async () => {
  ...
  try {
    ...
    await act(async () => { finish(Response.json(confirmed())); });
...
test('storage failure prevents dispatch and malformed confirmation keeps the retry request', async () => {
  ...
  try {
    ...
    ui.setPost(async () => Response.json({ ...confirmed(), receipt: { ...confirmed().receipt, baselineSha256: 'd'.repeat(64) } }));
    await ui.click('Prepare pending mappings');
...
test('only an exact authoritative stale rejection permits a new deliberate preview', async () => {
  ...
  try {
    ui.setPost(async bytes => { const request = JSON.parse(bytes).request; return Response.json({
      message: 'Stale snapshot', code: 'SPORTS_PREPARATION_SNAPSHOT_STALE', idempotencyKey: 'wrong-key',
    ...
    assert.equal(ui.button('Preview missing mappings').disabled, true);
    ui.setPost(async bytes => { const request = JSON.parse(bytes).request; return Response.json({
      message: 'Preparation snapshot changed; preview again.', code: 'SPORTS_PREPARATION_SNAPSHOT_STALE',
    ...
    await ui.click('Preview missing mappings');
    ui.setPost(async () => Response.json(confirmed()));
    await ui.click('Prepare pending mappings');
