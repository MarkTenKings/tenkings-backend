...
import LiveRipPreview from "../LiveRipPreview";
...
type LiveRipDirectoryMode = "public" | "admin";

interface LiveRipDirectoryPageProps {
  mode: LiveRipDirectoryMode;
}
...
export default function LiveRipDirectoryPage({ mode }: LiveRipDirectoryPageProps) {
  ...
  const uploadMediaFile = useCallback(
    async (file: File, kind: "video" | "thumbnail") => {
      ...
      if (!activeSession) {
        try {
          ...
          if (!(authError instanceof Error && authError.message === "Authentication cancelled")) {
      ...
      const response = await fetch(`${ADMIN_API_BASE}/upload?${params.toString()}`, {
        method: "PUT",
        headers,
        body: file,
      });
    ...
    [ensureSession, isManagementMode, session]
  );
  ...
  const refreshLiveRips = useCallback(async () => {
    try {
      ...
      const message = refreshError instanceof Error ? refreshError.message : "Unable to refresh live rips";
      setFlash(message);
    }
  }, []);

  const handleSave = useCallback(async () => {
    if (!isManagementMode || !editMode) {
      return;
    }
    setSaving(true);
    ...
    try {
      const endpoint = editMode === "create" ? ADMIN_API_BASE : `${ADMIN_API_BASE}/${formState.id}`;
      ...
      const message = saveError instanceof Error ? saveError.message : "Unable to save live rip";
      setFlash(message);
    } finally {
      setSaving(false);
    }
  }, [closeEditor, editMode, ensureSession, formState, isManagementMode, logout, refreshLiveRips, session]);
  ...
  const pageTitle = isManagementMode ? "Ten Kings Admin · Live Rips" : "Ten Kings · Live Rips";
  const pageDescription = isManagementMode
    ? "Manage Ten Kings live rip entries, uploads, and public links."
    : "Watch Ten Kings live rips from our collectible vending machines and share the biggest hits.";
  const eyebrow = isManagementMode ? "Admin Portal" : "Ten Kings Live";
  const heading = isManagementMode ? "Live Rip Manager" : "Recent Live Rips";
  const summary = isManagementMode
    ? "Manage public live rip entries, upload media, and copy direct links for live reveal pages."
    : "Relive the best pulls from Ten Kings locations. Share the hype or copy a link for your location page.";
  const emptyStateMessage = isManagementMode
    ? "No live rips yet. Add one to kick things off!"
    : "No live rips yet. Check back soon.";
    ...
    <AppShell>
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-6 py-12">
        <header className="space-y-4">
          ...
          <h1 className="font-heading text-4xl uppercase tracking-[0.2em] text-white md:text-5xl">{heading}</h1>
          <p className="max-w-3xl text-sm text-slate-400">{summary}</p>
          {isManagementMode ? (
            <div className="flex flex-wrap gap-3">
              <Link
                href="/admin"
                className="rounded-full border border-white/15 px-4 py-2 text-[10px] uppercase tracking-[0.3em] text-slate-200 transition hover:border-gold-300 hover:text-gold-200"
              >
        ...
        {isManagementMode && !isAdmin && !sessionLoading ? (
          <div className="rounded-3xl border border-amber-500/30 bg-amber-500/10 px-6 py-5 text-sm text-amber-100">
            Admin sign-in is required to create, edit, or upload live rips.
          </div>
        ) : null}
      ...
      {isManagementMode && editMode && isAdmin ? (
