...
import { z } from "zod";
import { speedsterMapAuthorityEvidenceFromCapture, type SpeedsterMapAuthorityEvent, type SpeedsterMapAuthorityFailure } from "../ai-grader-v2/map-authority";
import { canonicalizeSpeedsterSessionIdentity, type SpeedsterSessionIdentity } from "../ai-grader-v2/identity";
import type { SpeedsterCardProfile } from "../ai-grader-v2/contracts";
import { SpeedsterMapIntegrityError, type SpeedsterAppliedMapRevision } from "./speedsterCardTypeMaps";
...
export type SpeedsterMapAuthoritySession = Readonly<{
  id: string;
...
export type SpeedsterMapAuthorityDependencies = Readonly<{
  findSession: (sessionId: string, adminId: string) => Promise<SpeedsterMapAuthoritySession | null>;
  ...
  persistEvidence: (
    session: SpeedsterMapAuthoritySession,
    adminId: string,
    event: SpeedsterMapAuthorityEvent,
  ) => Promise<SpeedsterMapAuthoritySession | null>;
  now?: () => Date;
  randomId?: () => string;
}>;
...
/** Original Card Map authority actions with authenticated scope supplied by
 * the host. The original durable failure/decision and CAS rules are retained. */
export async function resolveSpeedsterMapAuthority(deps: SpeedsterMapAuthorityDependencies,
  input: Readonly<{ sessionId: string; createdByUserId: string; body: unknown }>): Promise<{ status: number; body: Record<string, unknown> }> {
  const { sessionId, createdByUserId } = input;
  if (!/^[a-z0-9-]{20,80}$/i.test(sessionId)) return { status: 400, body: { message: "Invalid Speedster session ID" } };
  const parsed = requestSchema.safeParse(input.body ?? {});
  if (!parsed.success) return { status: 400, body: { message: "Invalid Card Map authority request" } };
  const session = await deps.findSession(sessionId, createdByUserId);
  if (!session) return { status: 404, body: { message: "Speedster session not found" } };
  if (session.workflowState !== "DRAFT") {
    return { status: 409, body: { message: "Card Map authority can only be resolved for a DRAFT session" } };
  }
  ...
  if (parsed.data.action === "CONTINUE_WITHOUT_MAP") {
    if (priorAuthority?.status === "HUMAN_REVIEW_WITHOUT_MAP") {
      if (priorAuthority.operatorDecisionId === parsed.data.decisionId) {
        return { status: 200, body: { authority: priorAuthority, map: clientMap(null) } };
      }
    ...
    const persisted = await deps.persistEvidence(session, createdByUserId, event);
    if (!persisted) return { status: 409, body: { message: "Card Map authority changed before the human-review decision could be recorded" } };
    return { status: 200, body: { authority: event, map: clientMap(null) } };
  }
  ...
  try {
    ...
    const persisted = await deps.persistEvidence(session, createdByUserId, event);
    if (!persisted) return { status: 409, body: { message: "Card Map authority state changed before the blocker could be recorded" } };
    return { status: integrity ? 409 : 503, body: { authority: event, message: event.message } };
  }
  ...
  if (parsed.data.action === "BLOCK_REGISTRATION") {
    ...
    if (!exactBinding) {
      ...
      const persisted = await deps.persistEvidence(session, createdByUserId, event);
      if (!persisted) return { status: 409, body: { message: "Card Map authority state changed before the blocker could be recorded" } };
      return { status: 409, body: { authority: event, message: event.message } };
    }
    const event = authorityEvent({
      now,
      attemptId,
    ...
    const persisted = await deps.persistEvidence(session, createdByUserId, event);
    if (!persisted) return { status: 409, body: { message: "Card Map authority state changed before the blocker could be recorded" } };
    return { status: 200, body: { authority: event } };
  }
  ...
  if (priorAuthority?.status === "REGISTRATION_BLOCKED"
    ...
    return { status: 200, body: { authority: priorAuthority, map: clientMap(selected) } };
  }
  ...
  const persisted = await deps.persistEvidence(session, createdByUserId, event);
  if (!persisted) return { status: 409, body: { message: "Card Map authority state changed before it could be recorded" } };
  return { status: 200, body: { authority: event, map: clientMap(selected) } };
}
