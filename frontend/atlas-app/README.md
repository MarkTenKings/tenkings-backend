...
The persistent fixture can seed deterministic synthetic findings and a fictional trained reviewer. Its grade summary uses the shared original ATLAS calculation, and the screen explicitly says no optical detector/Astra run occurred. Reviewers save the identity/Front/Back checklist, then approve the exact analysis, evidence and saved-review hashes with a recent sign-in. Approval and immutable public packet commit together; failed audits roll back both. Later notes preserve the prior approved version and require another approval. An approved queue state describes report approval only, separate from label/NFC/physical finishing. The separate `frontend/atlas-public` now delivers approved versions. Live capture/grading adapters and public evidence-photo/trace inspection remain subsequent integrations.

The actual web fixture now checks 21 behaviors across both apps including approval, queue state, web restart and exact approval retry. The extended disposable database validation checks 31 scenarios. Production approval also requires the separately activated grading policy hash and current reviewer certification training; staff credentials cannot change either. No live release or grading-quality claim is implied.
