...
The persistent fixture can seed deterministic synthetic findings and a fictional trained reviewer. Its grade summary uses the shared original ATLAS calculation, and the screen explicitly says no optical detector/Astra run occurred. Reviewers save the identity/Front/Back checklist, then approve the exact analysis, evidence and saved-review hashes with a recent sign-in. Approval and immutable public packet commit together; failed audits roll back both. Later notes preserve the prior approved version and require another approval. An approved queue state describes report approval only, separate from label/NFC/physical finishing. The minimal public HTTP reader and live capture/grading adapters are subsequent integrations.

The actual web fixture now checks 16 behaviors including approval, queue state, web restart and exact approval retry. The extended disposable database validation checks 28 scenarios. Production approval also requires the separately activated grading policy hash and current reviewer certification training; staff credentials cannot change either. No live release or grading-quality claim is implied.
