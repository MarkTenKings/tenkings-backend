...
function Queue({ staff }) {
    const router = useRouter(), resource = useStaffResource('workspace'), assigned = useStaffResource('cards');
    ...
    return <Shell staff={staff} title="Grading workspace"><main className="main-content">
        ...
        <div className={styles.queueTabs} aria-label="Grading queues">{Object.entries(stateNames).map(([state, title]) => <Link key={state} href={`/grading?queue=${state}`} className={`${styles.queueTab} ${filter === state ? styles.selected : ''}`} aria-current={filter === state ? 'page' : undefined}><span>{title}</span><strong>{resource.loading || assigned.loading ? '—' : all.filter(card => card.state === state).length}</strong></Link>)}</div>
