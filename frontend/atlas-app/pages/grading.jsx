...
import { queueCards, stageNames, stateNames, verifiedSide, workspaceCardPath, workspaceMessage } from '../lib/workspace-client.mjs';
...
export default function Grading({ unavailable, staff, manualEnabled }) { return unavailable ? <Unavailable /> : manualEnabled ? <ManualCards staff={staff}/> : <Queue staff={staff} />; }
function Queue({ staff }) {
    ...
    async function claim(card, operator, mode = 'CONTINUOUS') {
        if (disabled || card.legacyReview || card.state !== 'WAITING') return;
        suspend?.();
        try {
            const claimed = await mutation.mutate(`${workspaceCardPath(card.id)}/claim`, { expectedRevision: card.revision, operator, ...(operator === 'ASTRA' ? { mode } : {}) }, card.id);
            if (claimed) await router.push(`/workspace/${claimed.id}`);
        } finally { resume?.(); }
    }
    const liveHeld = resource.error || resource.signedOut || resource.refreshError || assigned.error || assigned.signedOut || assigned.refreshError;
    const liveLabel = resource.loading ? 'Connecting' : resource.signedOut || assigned.signedOut ? 'Sign in required'
        : resource.error && !resource.data ? 'Connection interrupted' : liveHeld ? 'Showing saved state' : assigned.loading ? 'Loading reports' : 'Live workspace';
    return <Shell staff={staff} title="Grading workspace"><main className="main-content">
        <div className={styles.queueHeading}><div><div className={styles.queueKicker}>ATLAS GRADING FLOOR <span className={`${styles.queueLive} ${liveHeld ? styles.queueLivePaused : ''}`}><i aria-hidden="true" />{liveLabel}</span></div><h1>Great cards. Great teamwork.</h1><p className="muted">Keep the hopper full, watch Astra work, and bring each draft through human review.</p></div>{staff.role !== 'OBSERVER' && <Link className="primary" href="/add-cards">+ Add cards</Link>}</div>
        ...
        {(resource.refreshError || assigned.refreshError) && <Notice>Live updates were interrupted. The last saved cards remain visible while the connection recovers. {resource.refreshError || assigned.refreshError}</Notice>}
