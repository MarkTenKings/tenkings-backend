...
function Workspace({ initial, csrf, mode }) {
    ...
    {card.grading?.report && <GradedReport report={card.grading.report} approved={card.grading.published?.matchesCurrent} synthetic={synthetic}>
      {card.grading.published && <div className="publication-note"><strong>{card.grading.published.reportNumber} · Approved version {card.grading.published.version}</strong><p>{card.grading.published.matchesCurrent ? 'The current analysis and saved review match this approval.' : 'The draft has changed. The prior approved report remains available; these changes need a new approval.'}</p></div>}
    </GradedReport>}
