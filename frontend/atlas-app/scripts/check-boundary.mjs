...
for (const trace of traces) {
    for (const file of JSON.parse(readFileSync(trace, 'utf8')).files) {
        ...
        assert.ok(rel.startsWith('frontend/atlas-app/') || rel.startsWith('node_modules/') || rel === 'package.json' || rel === 'packages/atlas-grading-core/package.json' || rel.startsWith('packages/atlas-grading-core/dist/') || rel.startsWith('packages/atlas-report-view/') || rel.startsWith('packages/atlas-service-bridge/'), `Non-app server dependency: ${rel}`);
