...
try {
    const scenario = async (name, work) => {
        ...
        const login = async () => {
            const boot = await auth.bootstrap(undefined, 'sms-test-limits-fixture'), cookie = `${config.cookies.browser}=${boot.browserToken}`;
            const challenge = await auth.send(cookie, boot.csrf, { phone: '+12025550141', requestId: randomUUID() }, 'sms-test-limits-fixture');
            const result = await auth.verify(cookie, boot.csrf, { challengeId: challenge.challengeId, code: '424242' }, 'sms-test-limits-fixture');
