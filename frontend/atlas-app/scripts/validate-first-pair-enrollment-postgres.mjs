...
try {
    const scenario = async (name, work) => {
        ...
        const login = async () => {
            const boot = await auth.bootstrap(undefined, 'first-pair-enrollment-fixture'), cookie = `${config.cookies.browser}=${boot.browserToken}`;
            const challenge = await auth.send(cookie, boot.csrf, { phone: '+12025550141', requestId: randomUUID() }, 'first-pair-enrollment-fixture');
            const result = await auth.verify(cookie, boot.csrf, { challengeId: challenge.challengeId, code: '424242' }, 'first-pair-enrollment-fixture');
            return { ...result, cookie: `${cookie}; ${config.cookies.session}=${result.token}` };
        };
        try {
            await work({ db, admin, client, config, identities, auth, login, sql: (query, values) => fixture.sql(query, values, db.name) });
    ...
    console.log(JSON.stringify({ result: 'FIRST_PAIR_ENROLLMENT_POSTGRES_PASS', passed, directory: fixture.directory,
