...
try {
    const scenario = async (name, work) => {
        ...
        const login = async () => {
            const boot = await auth.bootstrap(undefined, 'identification-fixture'), cookie = `${config.cookies.browser}=${boot.browserToken}`;
            const challenge = await auth.send(cookie, boot.csrf, { phone: '+12025550141', requestId: randomUUID() }, 'identification-fixture');
            const result = await auth.verify(cookie, boot.csrf, { challengeId: challenge.challengeId, code: '424242' }, 'identification-fixture');
    ...
    console.log(JSON.stringify({ result: 'IDENTIFICATION_POSTGRES_PASS', passed, source: fixture.source, directory: fixture.directory }));
