...
async function pair(h) { await h.select(0, photo('front')); await h.select(1, photo('back')); }
const queued = (f, h, state = 'WAITING') => h.waitFor(() => f.saved[0]?.card?.state === state
    && f.saved[0].pending === null && f.saved[0].autoQueue === false
    && !h.nodes(node => node.type === 'fieldset')[0]?.props.disabled, `${state} card with queue admission saved and editing released`);
...
test('Front and Back alone verify, identify and queue; next capture stays ready', async t => {
    ...
    await h.select(1, photo('back')); await queued(f, h);
...
test('preparations overlap and a late retake cannot replace newer evidence', async t => {
    ...
    await h.waitFor(() => work.size === 2, 'both original preparations to start');
    assert.equal(work.size, 2); assert.equal(f.requests.length, 0); assert.equal(h.files()[0].props.disabled, false);
    await h.select(0, photo('front-new')); work.get('front-old.png')();
    await h.waitFor(() => work.has('front-new.png'), 'retaken Front preparation to start');
    work.get('back.png')(); work.get('front-new.png')(); await queued(f, h);
...
test('retakes preserve the category context of human names and blanks but clear machine-only identity', async t => {
    for (const name of ['Staff corrected name', '', null]) {
        ...
        await h.waitFor(() => f.saved[0].files.FRONT?.name === 'retaken-front.png', 'retaken Front to be saved');
...
test('an explicit staff label survives discovered identity and automatic queueing', async t => {
    ...
    h.close(); h = await mount(f); await pair(h); await queued(f, h);
...
test('reload during HEIC preparation resumes saved original bytes without selecting again', async t => {
    ...
    await h.select(0, original); await h.waitFor(() => Boolean(release), 'HEIC preparation to start');
    assert.deepEqual(await f.saved[0].selections.FRONT.file.arrayBuffer(), await original.arrayBuffer());
    ...
    h = await mount(f); await h.waitFor(() => f.saved[0].files.FRONT?.name === 'front.heic.png', 'reloaded HEIC conversion to be saved');
...
test('failed local selection keeps the previous photo and permits retry', async t => {
    const f = fixture(), h = await mount(f); t.after(() => h.close());
    await h.select(0, photo('old-front')); await h.waitFor(() => f.saved[0].files.FRONT?.name === 'old-front.png', 'first Front preparation to be saved');
    const prior = clone(f.saved[0]); f.failWrite = true;
    await h.select(0, photo('replacement')); await h.waitFor(() => h.text().includes('Your previous photo is kept'), 'failed local save to be shown');
    assert.deepEqual(f.saved[0], prior);
    await h.select(0, photo('replacement')); await h.waitFor(() => f.saved[0].files.FRONT?.name === 'replacement.png', 'replacement Front to be saved');
    assert.equal(f.requests.length, 0);
});
...
test('staff edits and deliberate blanks during identification win before automatic queueing', async t => {
    ...
    await pair(h); await h.waitFor(() => Boolean(f.releaseIdentity), 'identification request to reach its held response');
    await h.change('Category', 'POKEMON'); await h.change('Card name', 'Human correction'); await h.change('Card number', '');
    f.releaseIdentity(); await queued(f, h);
...
test('identity editing is locked before the queue manifest commits, including stale rendered handlers', async t => {
    ...
    await pair(h); await h.waitFor(() => Boolean(releaseQueueWrite), 'queue manifest to reach its held write');
    ...
    releaseQueueWrite(); await queued(f, h);
...
test('an edit accepted before the final handoff is committed and synchronized before queue admission', async t => {
    ...
    await h.change('Category', 'POKEMON'); await pair(h);
    await h.waitFor(() => Boolean(releaseIdentificationWrite), 'identification adoption to reach its held write');
    ...
    releaseIdentificationWrite(); await edit; await queued(f, h);
...
test('lost verification and queue responses resume exactly without replacing or reuploading photos', async t => {
    for (const action of ['upload-complete', 'queue']) {
        ...
        await h.waitFor(() => h.text().includes('Reply lost after commit'), `${action} lost reply to be shown`);
        ...
        h.close(); h = await mount(f); await queued(f, h, action === 'queue' ? 'IN_PROGRESS' : 'WAITING');
...
test('exact duplicate queue recovery opens the original card and retains HEIC originals', async t => {
    ...
    await h.waitFor(() => h.text().includes('Lost duplicate reply'), 'lost duplicate queue reply to be shown');
    ...
    h.close(); h = await mount(f);
    await h.waitFor(() => f.saved[0].resolvedCard?.cardId === f.duplicate.cardId && f.saved[0].pending === null
        && h.text().includes('Open existing card'), 'saved duplicate pair to resolve to its original card');
...
test('rejected or mismatched verification preserves the pair and cannot identify or queue', async t => {
    for (const malformed of [false, true]) {
        ...
        await h.waitFor(() => h.text().includes('Continue saving'), 'verification refusal to release the active attempt');
...
test('identity response for another pair stays pending and cannot queue', async t => {
    ...
    await h.waitFor(() => h.text().includes('The saved card details could not be verified.'), 'mismatched identity result to be rejected');
...
test('queue completion waits for delayed photo hashes and PUTs instead of event-loop turns', async t => {
    for (const phase of ['describe', 'put']) {
        const f = fixture(); f.unknownIdentity = true;
        const original = f[phase]; let release, entered = 0;
        const held = new Promise(resolve => { release = resolve; });
        f[phase] = async (...args) => { entered++; await held; return original(...args); };
        const h = await mount(f); t.after(() => { release(); h.close(); });
        await pair(h); await h.waitFor(() => entered === 2, `both photo ${phase} operations to reach the barrier`);
        assert.equal(f.saved[0].card.state, 'DRAFT');
        let finished = false;
        const completion = queued(f, h).then(() => { finished = true; });
        // A rendered event-loop turn is not completion while the real async
        // work is held. Releasing it needs no new capture, retry or model call.
        await h.flush(); assert.equal(finished, false);
        assert.equal(f.requests.filter(value => value.path.endsWith('/queue')).length, 0);
        release(); await completion;
        assert.equal(f.saved[0].card.state, 'WAITING'); assert.match(h.text(), /Choose the card category/);
        assert.equal(f.requests.filter(value => value.path.endsWith('/identify')).length, 1);
        assert.equal(f.requests.filter(value => value.path.endsWith('/queue')).length, 1);
    }
});
...
test('sign-in recovery continues the original queue operation on focus', async t => {
    ...
    const h = await mount(f); t.after(() => h.close()); await pair(h);
    await h.waitFor(() => h.text().includes('Sign in to continue'), 'sign-in recovery to be shown for the original queue request');
    const pending = clone(f.saved[0].pending); assert.match(h.text(), /Sign in to continue/);
    await h.event('focus'); await queued(f, h);
    assert.equal(f.saved[0].pending, null); assert.equal(f.saved[0].card.state, 'WAITING'); assert.deepEqual(f.requests.at(-1).body, pending.body); assert.equal(f.puts.length, 2);
});
