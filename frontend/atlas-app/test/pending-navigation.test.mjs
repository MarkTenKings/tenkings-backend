...
const origin='https://app.atlasgrading.com';
...
function fixture({older=null,prior=null}={}){
    ...
    const entries=older?[{href:origin+older.path,state:older.state},{href:origin+'/grading',state:state('/grading','queue')}]:[{href:origin+'/grading',state:state('/grading','queue')}];
...
test('multiple panels share one central guard and latest predicates, with separate-tab sign-in allowed',()=>{
    ...
    assert.equal(f.click('/operations').prevented,true);assert.equal(noticesA,1);assert.equal(noticesB,0);
    assert.equal(f.click('/',{target:'_blank'}).prevented,undefined);assert.equal(f.click('#section').prevented,undefined);
    a=false;b=true;assert.equal(f.click('/operations').prevented,true);assert.equal(noticesA,1);assert.equal(noticesB,1);
    assert.doesNotThrow(()=>f.router.events.emit('routeChangeStart','/operations'));
    assert.throws(()=>f.router.events.emit('beforeHistoryChange','/operations'),e=>e.cancelled===true);
...
test('push and replace preserve Next state and keys with correctly ordered metadata',()=>{
    ...
    f.navigate('/cards/card');const card=structuredClone(f.window.history.state);
    ...
    assert.deepEqual({...card,[HISTORY_MARK]:undefined},{...state('/cards/card',card.key),[HISTORY_MARK]:undefined});
    f.navigate('/cards/card?view=back',{replace:true});assert.equal(f.window.history.state.key,card.key);
    assert.deepEqual(f.window.history.state[HISTORY_MARK],{...card[HISTORY_MARK],href:origin+'/cards/card?view=back'});assert.equal(f.entries.length,2);f.owner();
});

test('blocked Back compensates traversal without overwriting destinations or duplicating Next keys',()=>{
    const f=fixture();f.navigate('/cards/card');const before=structuredClone(f.entries);let pending=true;
    ...
    assert.equal(f.index(),0);assert.deepEqual(f.transitions,['/grading']);assert.deepEqual(f.entries,before);off();f.owner();
});

test('blocked Forward restores the original entry with no guessed key ordering',()=>{
    const f=fixture();f.navigate('/cards/card');f.navigate('/operations');f.window.history.go(-1);f.flush();f.complete();
...
test('pending created during asynchronous route loading blocks a push commit inside Next cancellation catch',async()=>{
    ...
    const transition=(async()=>{f.router.events.emit('routeChangeStart','/cards/card');await loaded;
        try{f.router.events.emit('beforeHistoryChange','/cards/card');commits++;return true;}catch(error){if(error.cancelled)return false;throw error;}})();
...
test('pending created during a pop load restores the original browser entry and Next internal key',()=>{
    const f=fixture();f.navigate('/cards/card');const before=structuredClone(f.entries);let pending=false;const off=f.subscribe(()=>pending);
    f.window.history.go(-1);f.flush();assert.equal(f.index(),0);assert.equal(f.router._key,before[0].state.key);
    pending=true;assert.throws(()=>f.router.events.emit('beforeHistoryChange','/grading'),e=>e.cancelled);f.flush();
...
test('Next hash completion tracks the correct entry and replacement keeps its index',()=>{
    const f=fixture();f.navigate('/cards/card');const off=f.subscribe(()=>true);
    f.navigate('/cards/card#label',{hash:true});const saved=structuredClone(f.entries);
    f.window.history.go(-2);f.flush();assert.equal(f.index(),2);assert.equal(f.window.location.href,origin+'/cards/card#label');assert.deepEqual(f.entries,saved);
    f.navigate('/cards/card#print',{hash:true,replace:true});assert.equal(f.entries.length,3);assert.deepEqual(f.window.history.state[HISTORY_MARK],{...saved[2].state[HISTORY_MARK],href:origin+'/cards/card#print'});off();f.owner();
});
...
for(const nextOwned of [true,false])test(`unknown pre-app ${nextOwned?'Next':'native'} entry is preserved with explicit address mismatch until manual return`,()=>{
    ...
    assert.ok(f.notices.some(n=>n.includes('browser address currently differs')&&n.includes(origin+'/grading')));
    assert.throws(()=>f.router.events.emit('beforeHistoryChange','/operations'),e=>e.cancelled);
...
test('owner replacement/HMR retains subscribers without duplicate wrappers and cleanup restores prior callback',()=>{
    ...
    assert.equal(f.click('/operations').prevented,true);otherOwner();assert.equal(f.count('popstate'),1);
...
test('native same-page hash entry with no marker gets a fresh tracking origin without guessing direction',()=>{
    const f=fixture();f.navigate('/cards/card');const prior=structuredClone(f.entries);
...
for(const copied of [false,true])test(`native fragment with ${copied?'copied':'null'} state cannot cause an off-by-one compensation across a later wrapped push`,()=>{
    const f=fixture();f.navigate('/cards/card');const beforeFragment=structuredClone(f.entries);
    ...
    assert.equal(f.window.history.state[HISTORY_MARK].href,origin+'/cards/card#native');
    assert.deepEqual(f.entries.slice(0,2),beforeFragment);
    f.navigate('/operations');const all=structuredClone(f.entries),off=f.subscribe(()=>true);
...
test('copied native fragment state is rebased before Next can rewrite it prior to hash completion',()=>{
    const f=fixture();f.navigate('/cards/card');const original=structuredClone(f.window.history.state);
    ...
    f.window.history.replaceState({...original,as:'/cards/card#native'},'','#native');f.dispatch('hashchange');
...
test('Next initial-SSR shortcut still completes compensation when beforePopState is skipped',async()=>{
    const f=fixture();f.navigate('/cards/card');f.router.skipBeforePopAt='/cards/card';const off=f.subscribe(()=>true);
    ...
    assert.doesNotThrow(()=>f.router.events.emit('beforeHistoryChange','/cards/card#label'));off();f.owner();
});

test('compensation timeout is bounded and does not rewrite the unresolved destination',()=>{
    const f=fixture();f.navigate('/cards/card');const before=structuredClone(f.entries),off=f.subscribe(()=>true);
