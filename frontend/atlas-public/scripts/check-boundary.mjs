...
assert.deepEqual(Object.keys(pages).sort(), ['/', '/404', '/_app', '/_document', '/_error', '/reports/[token]'].sort());
...
for (const trace of traces) for (const file of JSON.parse(readFileSync(trace, 'utf8')).files) {
    ...
    assert(rel.startsWith('frontend/atlas-public/') || rel.startsWith('packages/atlas-report-view/') || rel.startsWith('node_modules/') || rel === 'package.json', `Unreviewed public server dependency: ${rel}`);
...
console.log(JSON.stringify({ status: 'PUBLIC_BUILD_BOUNDARY_PASS', pages: Object.keys(pages), browserChunks: chunks.length,
    serverTraces: traces.length, tracedFilesChecked: checked, staffRoutes: 0, writeRoutes: 0, databasePort: 'read_approved_report only' }));
